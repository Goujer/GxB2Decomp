-- chunkname: @../../../Product/Bundles/Android/src/lib/battle/battle_buffs/reduceSpdHealBuff.lua
...
function reduceSpdHealBuff:getReduceSpdTimes()
	...
	for _, v in ipairs(self.target.sideTeam_) do
		local buffs = v:getBuffsByNameAndFighter(xyd.BUFF_REDUCE_SPD_HEAL_RECORD)

		if next(buffs) and buffs[1].fighter == self.fighter then
			local queyangBuffs = v:getBuffsByNameAndFighter(xyd.BUFF_REDUCE_SPD)
