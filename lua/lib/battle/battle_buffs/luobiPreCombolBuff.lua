-- chunkname: @../../../Product/Bundles/Android/src/lib/battle/battle_buffs/luobiPreCombolBuff.lua
...
function luobiPreCombolBuff:addExtraJumpSkill(unit)
	...
	local marks = self.target:getBuffsByNameAndFighter(xyd.BUFF_LUOBI_MARK)

	for i = 1, #marks do
		local ifNoBlock = false
		local curUnit = self.target:useOneJumpPasSkill(nil, extraSkillID, unit, false, ifNoBlock, true)
