-- chunkname: @../../../Product/Bundles/Android/src/lib/battle/battle_buffs/dianaDieBuff.lua
...
function dianaDieBuff:setIsHit()
	...
	if self:isHit() and xyd.Battle.god and self.fighter == xyd.Battle.god then
...
function dianaDieBuff:excuteAfterRound(unit)
	...
	self.fighter:recordData(-displayBuff1:getFinalNum(), 0, true)
...
local function dieEffect(unit, dieTarget)
	local dieTargetBuffs = dieTarget:getBuffsByNameAndFighter(xyd.BUFF_DIANA_DIE)
