-- chunkname: @../../../Product/Bundles/Android/src/lib/battle/battle_buffs/feisinaExplodeBuff.lua
...
function feisinaExplodeBuff:setIsHit()
	...
	if self.target:getOneBuffByName(xyd.BUFF_FEISINA_EXPLODE) then
		hasBuff = true
	end
...
function feisinaExplodeBuff:excuteAfterRound(unit)
	if not self.hasSameBuff and not self.target:isDeath() then
		...
		local shieldResult = self.target:getShieldHarm(finalNum, unit, nil, nil, nil, nil, self)

		finalNum = shieldResult.result
