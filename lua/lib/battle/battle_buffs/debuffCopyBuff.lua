-- chunkname: @../../../Product/Bundles/Android/src/lib/battle/battle_buffs/debuffCopyBuff.lua
...
function debuffCopyBuff:excuteBuff(unit, recordBuffs)
	...
	exceptFighters[self.target:getPos()] = true
	...
	for _, dName in ipairs(copyNameList) do
		...
		for i = 1, #debuffs do
			...
			for _, copyTarget in ipairs(newTarget) do
				local newBuff = xyd.copyBuff(debuff, copyTarget, debuff.fighter, 1)
				...
				if newBuff:isHit() then
					...
					copyTarget:delSpecialBuffCount(newBuff)
