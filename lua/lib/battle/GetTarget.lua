-- chunkname: @../../../Product/Bundles/Android/src/lib/battle/GetTarget.lua
...
function GetTarget.S4(fighter)
	local alive = GetTarget.S2(fighter)
	local fighter_, maxRate_

	for k, v in ipairs(alive) do
		local vRate = v:getHp() / v:getHpLimit()

		if not fighter_ or maxRate_ < vRate or vRate == maxRate_ and v:getHp() > fighter_:getHp() then
			fighter_ = v
			maxRate_ = vRate
		end
	end
...
function GetTarget.CheckIsRandom(type)
	return xyd.tables.battleTargetTable:isRandomEnemy(type)
end
...
function GetTarget.findMistletoesAll(alive, num)
	local mistletoesTarget, notChoosen = GetTarget.findMistletoes(alive, num)
	...
	local tmpRes = xyd.randomSelects(notChoosen, num - #mistletoesTarget)
...
function GetTarget.S64(fighter, num)
	...
	for _, v in ipairs(alive) do
		if v:getAckSpeed() > fighter:getAckSpeed() then
...
function GetTarget.S96(fighter)
	...
	if xyd.Battle.dressAttrsA[1] > xyd.Battle.dressAttrsB[1] then
...
function GetTarget.S97(fighter)
	...
	if xyd.Battle.dressAttrsA[2] > xyd.Battle.dressAttrsB[2] then
		return xyd.Battle.teamA
...
function GetTarget.S98(fighter)
	...
	if xyd.Battle.dressAttrsA[3] > xyd.Battle.dressAttrsB[3] then
		return xyd.Battle.teamA
...
function GetTarget.S100(fighter)
	...
	for k, v in ipairs(result_) do
		if next(v:getBuffsByNameAndFighter(xyd.BUFF_LUBAN_HURT_B)) then
			table.insert(result, v)
		end
	end
...
function GetTarget.S101(fighter)
	...
	for k, v in ipairs(result_) do
		if next(v:getBuffsByNameAndFighter(xyd.BUFF_LUBAN_HURT_C)) then
			table.insert(result, v)
		end
	end
...
function GetTarget.S105(fighter)
	...
	for _, v in ipairs(xyd.Battle.puGongNormalSkillTargets) do
		if not v:isDeath() then
			local crystallizeBuffs = v:getBuffsByNameAndFighter(xyd.BUFF_CRYSTALLIZE)
			local crystallBuffs = v:getBuffsByNameAndFighter(xyd.BUFF_CRYSTALL)
...
function GetTarget.S108(fighter, num)
	...
	for i = 1, num do
		if lives[i] then
			table.insert(result, lives[i].partner)
		end
	end
...
function GetTarget.S110(fighter, num)
	local alive = GetTarget.S2(fighter)
	local num_ = num or 1
	local targets = {}
	...
	return xyd.randomSelects(targets, num_)
end

function GetTarget.S111(fighter)
	local result = {}

	for _, v in ipairs(xyd.Battle.preHitTargets) do
		if v:getTeamType() ~= fighter:getTeamType() and not v:isDeath() and v:getHp() / v:getHpLimit() >= 0.5 then
			table.insert(result, v)
		end
	end
...
function GetTarget.S112(fighter)
	local result = {}
...
function GetTarget.S113(fighter)
	local result = {}
	local alives = GetTarget.S13(fighter)

	for _, v in ipairs(alives) do
		if next(v:getBuffsByNameAndFighter(xyd.BUFF_DEGULA_DEBUFF, fighter)) then
			table.insert(result, v)
		end
	end
...
function GetTarget.S114(fighter)
	local result = {}

	for _, v in ipairs(xyd.Battle.team) do
		if v.firstDieHit and v.firstDieHit == 1 then
			table.insert(result, v)
		end
	end
...
function GetTarget.S115(fighter)
	local result = {}

	for k, v in ipairs(xyd.Battle.teamPet) do
		if fighter:getTeamType() == xyd.TeamType.A then
			if v:getPos() == 13 then
				table.insert(result, v)
			end
		elseif v:getPos() == 14 then
			table.insert(result, v)
		end
	end
...
function GetTarget.S116(fighter, num)
	local alive = GetTarget.S2(fighter)
	local num_ = num or 1
	local targets = {}
	...
	for _, v in ipairs(alive) do
		...
		if pos > xyd.DEFAULT_FRONT_NUM and pos <= 6 then
			table.insert(targets, v)
		end
	end
...
function GetTarget.S117(fighter)
	local result = {}

	for _, v in ipairs(xyd.Battle.teamA) do
		if xyd.Battle.firstDieFighter == v then
			table.insert(result, v)
...
function GetTarget.S118(fighter)
	local result = {}

	for k, v in ipairs(xyd.Battle.teamPet) do
		if fighter:getTeamType() == xyd.TeamType.B then
			if v:getPos() == 13 then
				table.insert(result, v)
			end
		elseif v:getPos() == 14 then
...
function GetTarget.S119(fighter)
	local result = {}

	if xyd.Battle.lastKiller then
		table.insert(result, xyd.Battle.lastKiller)
	end

	return result
end

function GetTarget.S120(fighter)
	local result = {}

	if xyd.Battle.lastDotHurtTarget then
		table.insert(result, xyd.Battle.lastDotHurtTarget)
	end

	return result
end

function GetTarget.S121(fighter)
	local alive = GetTarget.S13(fighter)
	local target = alive[1]

	for i = 2, #alive do
		if target:getCrit() < alive[i]:getCrit() then
			target = alive[i]
		end
	end
...
function GetTarget.S122(fighter)
	local alive = GetTarget.S13(fighter)
	local res = {}
...
function GetTarget.S123(fighter, num)
	local alive = GetTarget.S13(fighter)
	local hasYifuTarget = {}
	local hasNoYifuTarget = {}

	for k, v in ipairs(alive) do
		if next(v:getBuffsByNameAndFighter(xyd.BUFF_YIFU_MARK)) then
			table.insert(hasYifuTarget, v)
		else
			table.insert(hasNoYifuTarget, v)
		end
	end

	if num <= #hasYifuTarget then
		return xyd.randomSelects(hasYifuTarget, num)
	else
		local leftNum = num - #hasYifuTarget
		local tmpRes = GetTarget.findMistletoesAll(hasNoYifuTarget, leftNum)

		if tmpRes and next(tmpRes) then
			hasYifuTarget = xyd.arrayMerge(hasYifuTarget, tmpRes)
		end

		return hasYifuTarget
	end
end

function GetTarget.S124(fighter, num)
	local alive = GetTarget.S2(fighter)
	local controlPartner = {}

	for k, v in ipairs(alive) do
		if v:isControl() then
			table.insert(controlPartner, v)
		end
	end

	return xyd.randomSelects(controlPartner, num)
end

function GetTarget.S125(fighter, num)
	local alive = GetTarget.S13(fighter)
	local hasYifuTarget = {}
	local hasNoYifuTarget = {}

	for k, v in ipairs(alive) do
		if next(v:getBuffsByNameAndFighter(xyd.BUFF_FEISINA_EXPLODE)) then
			table.insert(hasYifuTarget, v)
		else
			table.insert(hasNoYifuTarget, v)
		end
	...
	if num <= #hasYifuTarget then
		...
		local leftNum = num - #hasYifuTarget
		local tmpRes = xyd.randomSelects(hasNoYifuTarget, leftNum)
...
function GetTarget.S126(fighter)
	local res = {}
	local rPos = {}

	for _, v in ipairs(xyd.Battle.specialNormalSkillTargets) do
		if not v:isDeath() then
			local pos = v:getPos()
...
function GetTarget.S127(fighter)
	local result = {}
	local alives = GetTarget.S2(fighter)

	for _, v in ipairs(alives) do
		if v:getHp() / v:getHpLimit() <= 0.5 then
			table.insert(result, v)
		end
	end
...
function GetTarget.S128(fighter)
	local result = GetTarget.S116(fighter, 4)
...
function GetTarget.S129(fighter)
	local result = {}
	local alive = GetTarget.S13(fighter)
...
function GetTarget.S130(fighter)
	local result = {}
	local alive = GetTarget.S13(fighter)

	for k, v in ipairs(alive) do
		if v:isDotPoison() then
			table.insert(result, v)
		end
	end
...
function GetTarget.S131(fighter, num)
	local alive = GetTarget.S13(fighter)
	local hasLongnvTarget = {}
	local hasNoLongnvTarget = {}

	for k, v in ipairs(alive) do
		if next(v:getBuffsByNameAndFighter(xyd.BUFF_LONGNV_DEBUFF)) then
			table.insert(hasLongnvTarget, v)
		else
			table.insert(hasNoLongnvTarget, v)
		end
	end

	if num <= #hasNoLongnvTarget then
		return xyd.randomSelects(hasNoLongnvTarget, num)
	else
		local leftNum = num - #hasNoLongnvTarget
		local tmpRes = GetTarget.findMistletoesAll(hasLongnvTarget, leftNum)

		if tmpRes and next(tmpRes) then
			hasNoLongnvTarget = xyd.arrayMerge(hasNoLongnvTarget, tmpRes)
		end

		return hasNoLongnvTarget
	end
end

function GetTarget.S132(fighter, num)
	local result = {}
	...
	local alives = GetTarget.S11(fighter)
...
function GetTarget.S133(fighter, num)
	local result = {}
...
function GetTarget.S134(fighter)
	local result = {}
	local alives = GetTarget.S13(fighter)

	for _, v in ipairs(alives) do
		if next(v:getBuffsByNameAndFighter(xyd.BUFF_ANSUNA_TO_SHIELD, fighter)) then
			table.insert(result, v)
		end
	end
...
function GetTarget.S135(fighter, num)
	local result = {}
	...
	local alives = GetTarget.S7(fighter)

	for k, v in ipairs(alives) do
		if v:isHasDebuff() then
			table.insert(result, v)
		end
	end

	return xyd.randomSelects(result, num)
end

function GetTarget.S136(fighter)
	local result = {}
	local alives = GetTarget.S13(fighter)
...
function GetTarget.S137(fighter)
	local result = {}
	local alives = GetTarget.S13(fighter)

	for k, v in ipairs(alives) do
		if v:getOneBuffByName(xyd.BUFF_BLIND) then
			table.insert(result, v)
		end
	end
...
function GetTarget.S138(fighter, num)
	local result = {}
	...
	local hasNoBuffTarget = {}

	for k, v in ipairs(notChoosen) do
		if not v:getOneBuffByName(xyd.BUFF_SOUL_SEAL) then
			table.insert(hasNoBuffTarget, v)
		end
	end

	local tmpRes = xyd.randomSelects(hasNoBuffTarget, num - #mistletoesTarget)
...
function GetTarget.S139(fighter)
	local result = {}
	local alives = GetTarget.S13(fighter)
	local noSoul = {}
	...
	for k, v in ipairs(alives) do
		if v:getOneBuffByName(xyd.BUFF_SOUL_SEAL) then
			...
			table.insert(noSoul, v)
		end
	end

	local noSoulOne = GetTarget.S138(fighter, 1)

	if next(noSoulOne) then
		table.insert(result, noSoulOne[1])
	end

	return result
end

function GetTarget.S140(fighter)
	local result = {}
	local alives = GetTarget.S13(fighter)
...
function GetTarget.S141(fighter)
	local result = {}
	local alives = GetTarget.S13(fighter)
