-- chunkname: @../../../Product/Bundles/Android/src/lib/battle/BaseFighter.lua
...
function HeadView:initUI()
	...
	self.shieldHpNode = self.progress:NodeByName("shieldHpNode").gameObject
	self.shieldEffectNode = self.shieldHpNode:NodeByName("shieldEffectNode").gameObject
	...
	self.shieldHpBar = self.shieldHpNode:ComponentByName("shieldHpBar", typeof(UISlider))
	self.shieldThumb = self.shieldHpBar:ComponentByName("hpThumb2", typeof(UISprite))
	self.depthObjs_ = {
		...
		self.hpThumb2,
		self.shieldThumb,
		self.shieldEffectNode:GetComponent(typeof(UIWidget)),
	...
	UIEventListener.Get(self.buffGroup).onClick = handler(self, self.touchBuff)
end

function HeadView:touchBuff()
	local wnd = xyd.WindowManager.get():getWindow("battle_window")

	if not wnd or wnd.battleEnds == true then
		return
	end

	if not next(self.showBuffs) then
		return
	end
	...
	xyd.WindowManager.get():openWindow("battle_buff_list_window", {
		...
		showIcons = self.showBuffs,
		callback = wnd.resumeBattleCallback
	})
end
...
function HeadView:updateShieldHpBar(value)
	if not self.shieldEffect then
		local effectName = "hudun_buff"

		self.shieldEffect = xyd.Spine.new(self.shieldEffectNode)

		self.shieldEffect:setInfo(effectName, function()
			self.shieldEffect:play("texiao01", 0, 1)
		end)
	end

	local scale = 0.85

	self.shieldEffect:SetLocalScale(scale, scale, 1)

	local length = 123
	local startPosX = -58

	self.shieldEffectNode:X(startPosX + length * value / xyd.PERCENT_BASE)

	self.shieldHpBar.value = value / xyd.PERCENT_BASE

	self.shieldHpNode:SetActive(value > 0)
end
...
function HeadView:refreshBuffIcons(buffs)
	...
	local function checkHave(path)
		...
		for i = 1, #self.showBuffs do
			local val = self.showBuffs[i]
	...
	for i = count, 1, -1 do
		local path = self.showBuffs[i].path
		local num = self.showBuffs[i].num

		if not self.buffItems_[i] then
			self.buffItems_[i] = NGUITools.AddChild(self.buffGroup, self.buffItem)

			self.buffItems_[i]:SetLocalPosition(31 * i - 16, 0, 0)
		end
...
function BaseFighter:checkIsAttack(skillID)
	...
	return self:checkSkillIsPugong(skillID) or self:getEnergyID() == skillID or self:getAddHurtID(skillID) or self:checkSkillHasAddHurtFreeArm(skillID) == 1
end
...
function BaseFighter:checkSkillHasAddHurtFreeArm(skillID)
	...
	for i = 1, #effects do
		for _, effect in ipairs(effects[i]) do
			...
			if EffectTable:fxHurt(effect) ~= 0 then
				return EffectTable:fxHurt(effect)
			end
		end
	end
...
function BaseFighter:updateShieldHpBar(value)
	if self.headView ~= nil then
		self.headView:updateShieldHpBar(value)
	end
end
...
function BaseFighter:playAttack(skillID, targets, callback)
	...
	if self.playingName == "dead" and name == "attack" then
		local event = {}
...
function BaseFighter:playSound(skillID)
	...
	if id > 0 then
		xyd.SoundManager.get():playSound(id, nil, self.timeScale_)
	end
end
...
function BaseFighter:playOne(fx, target, skillID, unitPos, renderDelta)
	...
	effect.name = "" .. tostring(fx) .. "_" .. tostring(self.frameIndex_)
	...
	elseif fxType == xyd.BattleFxType.HURT then
		...
		ResCache.AddChild(uiLayer, effect)
		self:changeEffectOrderByParent(uiLayer, effectAnim)
		...
		self:checkAilisheColorEffect(uiLayer)
...
function BaseFighter:checkAilisheColorEffect(uiLayer)
	local count = uiLayer.transform.childCount
	local trans = uiLayer.transform
	local colorItems = {}

	for i = 1, count do
		local child = trans:GetChild(i - 1)
		local nameArr = xyd.split(child.name, "_", true)

		if nameArr[1] and nameArr[1] >= 2048 and nameArr[1] <= 2051 then
			table.insert(colorItems, child)
		end
	end

	for k, v in ipairs(colorItems) do
		local x_ = v.transform.localPosition.x
		local y_ = 60 + (k - 1) * 30

		v.transform.localPosition = Vector3(x_, y_, 0)
	end
end
...
function BaseFighter:die2()
	...
	local effectName = ""
	local modelName = self:getModelName()
	local reviveName = ModelTable:revive(self:getModelID())
	local modelScale = 0.6

	if reviveName ~= "" then
		effectName = reviveName
		modelScale = ModelTable:getScale(self:getModelID())
	end
	...
	if dieEffect then
		NGUITools.DestroyChildren(layer.transform)
	end
	...
	local diretion = self:getTeamType() == xyd.TeamType.A and 1 or -1
	local effect = self:getEffect(effectName, modelScale, modelScale)

	ResCache.AddChild(layer, effect)

	effect.transform.localScale = Vector3(diretion * modelScale, modelScale, 1)
	effect.name = "fuhuo_fx"
...
function BaseFighter:die3()
	...
	for i = 1, count do
		...
		if child.name ~= "805_" .. self.frameIndex and child.name ~= "806_" .. self.frameIndex then
	...
	for i = 1, count do
		...
		if child.name ~= "805_" .. self.frameIndex_ and child.name ~= "806_" .. self.frameIndex_ then
			child:SetActive(false)
		end
	end
end
...
function BaseFighter:revive()
	...
	self.headView:SetActive(true)

	if effect then
		local modelNode = self.modelRender_
		...
		modelNode:GetComponent(typeof(UITexture)).color = Color.New2(4294967040)

		effectAnim:addListener("Complete", function()
			self:returnToObjs(obj)

			modelNode:GetComponent(typeof(UITexture)).color = Color.New2(4294967295)
		end)
	end
...
function BaseFighter:getNumView(num, isCrit, isRestarint, isShield)
...
function BaseFighter:playHPDeltas(hpDeltas, callback)
	...
	for _, hpDelta in ipairs(hpDeltas) do
		...
		local label = self:getNumView(value, isCrit, isRestarint, hpDelta.isShield)
...
function BaseFighter:playAllDamageNumbers()
	...
	for index = 1, #self.damageNumbers do
		...
		if result.shieldCost and result.shieldCost < 0 then
			table.insert(hpDeltas, {
				isShield = true,
				value = result.shieldCost
			})
		end
		...
		elseif value < 0 and (value ~= -1 or not result.shieldCost or not (result.shieldCost > 0)) then
			table.insert(hpDeltas, {
				value = value,
				isCrit = isCrit,
				isRestarint = isRestarint
		...
		for _, fx in ipairs(fxs) do
			if self:checkFxCanPlayInPos(fx) and not xyd.Battle.aoeBuffEffect_[self:getAoeEffectKey(fx)] then
				...
				if not uiLayer:NodeByName("" .. tostring(fx) .. tostring(self.frameIndex_)) then
					self:playOne(fx, self, nil, nil, curLayerNum)
				end
...
function BaseFighter:changeEffectOrderByParent(uiLayer, effect)
	local count = uiLayer.transform.childCount
	local texture = uiLayer.gameObject:GetComponent(typeof(UITexture))

	if texture then
		XYDCo.WaitForFrame(1, function()
			effect.RenderTarget = texture
			effect.targetDelta = count
		end, xyd.getTimeKey())
	end
end
