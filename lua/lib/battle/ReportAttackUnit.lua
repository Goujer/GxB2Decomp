-- chunkname: @../../../Product/Bundles/Android/src/lib/battle/ReportAttackUnit.lua
...
function ReportAttackUnit:init()
	...
	for i = 1, #selectTypes do
		...
		if selectType > 0 then
			...
			local targets, hasRandom = self:getBuffTargets(selectType, randNum, self.specialUnit_, i)
			local isRandom = GetTarget_.CheckIsRandom(selectType)
			...
			if targets and next(targets) then
				...
				for j = 1, #targets do
					...
					if target then
						...
						if isRandom and next(target:getBuffsByNameAndFighter(xyd.BUFF_XIERKEDI_POWER)) and target:getTeamType() ~= self.fighter:getTeamType() then
							isMiss = true
						end
						...
						if isMiss then
							...
							self:createBuff(target, trueEffects, skillIndex, targets)
...
function ReportAttackUnit:getBuffTargets(selectType, randNum, unit, index)
	...
	if (self.skillID == self.fighter:getEnergySkillID() and self.fighter:getOneBuffByName(xyd.BUFF_TARGET_CHANGE) or next(allTargetChangeBuffs)) and targetType == xyd.TargetType.TARGET_ENEMY and BattleTargetTable:isCanChange(selectType) then
		selectType = 18
		randNum = 1
		isRandom = true
	end
...
function ReportAttackUnit:isSkillMiss(target, index, fighter)
	...
	if fighter and not fighter:isPet() and not fighter:isGod() and target:getTeamType() ~= self.fighter:getTeamType() and fighter.hero_ ~= nil and (self.skillID == fighter:getEnergySkillID() or fighter:isPugongSkill(self.skillID)) then
		local blindRate = fighter:getBuffsTotalFinalNumArray(xyd.BUFF_BLIND, 4)

		if blindRate > 0 and xyd.weightedChoise({
			blindRate,
			1 - blindRate
		}) == 1 then
			return true
...
function ReportAttackUnit:createBuff(target, effects, index, targets)
	local tmpBuffs = {}
	...
	local function newBuff(target, effectID, index)
		...
		buff:setIsHit(self.preAddBuffs)

		return buff
	end

	local noTriggerPas = false
	local isMiss, isCrit, hitBuffs, missBuffs, isFree, isBlock = false, false, {}, {}, false, false
	...
	local function getHurt()
		...
		for _, buff_ in ipairs(tmpBuffs) do
			if buff_:getName() == xyd.BUFF_HURT_MAX_H_LIMIT15 or buff_:getName() == xyd.BUFF_HURT_DMG_H_LIMIT15 or buff_:getName() == xyd.BUFF_HURT_MAX_H_LIMIT25 or buff_:getName() == xyd.BUFF_ADD_HURT or buff_:getName() == xyd.BUFF_HURT or buff_:getName() == xyd.BUFF_HURT_FREE_SHIELD or buff_:getName() == xyd.BUFF_PHURT_PAS_L_B or buff_:getName() == xyd.BUFF_PHURT_SKILL_L_B or buff_:getName() == xyd.BUFF_SUCK_REAL_BLOOD then
	...
	for i = 1, #effects do
		...
		if effectID <= 0 then
			...
			if name_ == xyd.BUFF_HURT or name_ == xyd.BUFF_HURT_NOW_H_LIMIT15 or name_ == xyd.BUFF_HURT_DMG_H_LIMIT15 or name_ == xyd.BUFF_HURT_FREE_SHIELD then
			...
			elseif buff:isHit() then
				if buff.target:getLowestCountBuffByName(xyd.BUFF_MAYA_BUFF_RESIST) and buff:isAttrBuff() then
					local mayaResist = buff.target:getLowestCountBuffByName(xyd.BUFF_MAYA_BUFF_RESIST)

					buff = mayaResist:excuteResist(hitBuffs, self)
				end

				table.insert(self.preAddBuffs, buff)
				...
				elseif buff:getName() == xyd.BUFF_DEBUFF_SAME then
					if xyd.Battle.lastDebuff then
						...
						if newDebuff:isHit() and lastDebuff:isCanDebuffSame() then
				...
				elseif buff:getName() == xyd.BUFF_ENERGY_STEAL then
					...
					table.insert(hitBuffs, buff)
				elseif buff:getName() == xyd.BUFF_IMMEDIATELY then
					buff:excuteBuff(self, hitBuffs)
				else
					buff:calculate()
					table.insert(hitBuffs, buff)
			...
			if buff.needCoverCheck_ and buff.noCheckLimitHit_ then
				buff:checkCoverAddBuff(hitBuffs)
			end
