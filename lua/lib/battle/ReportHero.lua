-- chunkname: @../../../Product/Bundles/Android/src/lib/battle/ReportHero.lua
...
local SoulEquip2Table = xyd.tables.soulEquip2Table
local SoulEquip2GroupTable = xyd.tables.soulEquip2GroupTable

function ReportHero:ctor()
	self.heroID_ = INVALID_HERO_ID
	self.playerID_ = 0
	self.isMonster_ = false
	self.isReportHero = true
	self.souls = {}
end
...
function ReportHero:initUnCollected(tableID, partnerID, params)
	...
	self.star_origin = params.star_origin or {}
	self.souls = params.soul_infos or {}
end
...
function ReportHero:setSkinBonus(info)
	self.skinBonus = info
end

function ReportHero:setChimeBonus(info)
	self.chimeBonus = info
end

function ReportHero:setSouls(info)
	self.souls = info
end

function ReportHero:setShrine(id)
	if id then
		self.shrineData = xyd.tables.shrineDifficultyTable:getBuffsById(id)
	end
end

function ReportHero:setGalaxy(galaxy_info)
	if galaxy_info then
		self.galaxyData = galaxy_info
	end
end
...
function ReportHero:getSkinID()
	return self.skinID_ or 0
end
...
function ReportHero:getStarOrigin()
	return self.star_origin
end

function ReportHero:getTotalExLev()
	local result = 0
...
function ReportHero:getPasSkill()
	...
	for i = 2, #self.souls do
		local soul = self.souls[i]

		if soul and soul.table_id then
			local group = SoulEquip2Table:getGroup(soul.table_id)

			if sameGroup == 0 then
				sameGroup = group
			elseif group ~= sameGroup then
				break
			end
	...
	if soulIndex == xyd.TOTAL_SOULS_NUM and sameGroup ~= 0 then
		local skillIDs = SoulEquip2GroupTable:getSkill(sameGroup)
	...
	if self.fightPos and xyd.Battle.godPosSkill[self.fightPos] then
		for _, skill in ipairs(xyd.Battle.godPosSkill[self.fightPos]) do
			table.insert(skills, skill)
		end
	end
