-- chunkname: @../../../Product/Bundles/Android/src/lib/battle/BattleCreateReport.lua
...
function BattleCreateReport:ctor(params)
	...
	xyd.Battle.battleType = self.battleType
	...
	self.chimeBonusA = params.chimeBonusA
	self.chimeBonusB = params.chimeBonusB
	self.godSkills = params.god_skills or {}
	self.insertPosSkills = params.insertPosSkills or {}
	self.dressGodSkills = {}
	self.chimeGodSkills = {}
	self.preUseGodSkill = {}
	...
	self.shrineId = params.shrineId
	self.galaxy_info = params.galaxy_info
	self.skinBonusA = params.skinBonusA or 0
	self.skinBonusB = params.skinBonusB or 0
	...
	xyd.Battle.firstDieFighter = nil

	battleEvent:reg_evt()
	self:getChimeSkill()

	self.preUseGodSkill = self:addGodSubSkill()

	self:checkInsertPosSkill()
end
...
function BattleCreateReport:checkInsertPosSkill()
	for i = 1, math.floor(#self.insertPosSkills / 2) do
		local index = i * 2 - 1
		local key = self.insertPosSkills[index]
		local value = self.insertPosSkills[index + 1]

		if not xyd.Battle.godPosSkill[key] then
			xyd.Battle.godPosSkill[key] = {}
		end

		table.insert(xyd.Battle.godPosSkill[key], value)

		local subSkills = SkillTable:getSubSkills(value)

		if #subSkills > 0 then
			for _, subSkill in ipairs(subSkills) do
				table.insert(xyd.Battle.godPosSkill[key], subSkill)
			end
		end
	end
end
...
function BattleCreateReport:checkGodInitFighterSkills(skillId)
	if SkillTable:trigger(skillId) == xyd.TriggerType.GOD_INIT_FIGHTER_SKILL then
		...
		return true
	elseif SkillTable:trigger(skillId) == xyd.TriggerType.GOD_ADD_PET_EXSKILL and self.petA_ then
		local effects = SkillTable:getEffect(skillId, 1)

		self.petA_.godExLevel = effects[1]

		if self.petA_.exLevel_ + self.petA_.godExLevel > 100 then
			self.petA_.godExLevel = 100 - self.petA_.exLevel_
		end
	end
end
...
function BattleCreateReport:setupConfig()
	...
	xyd.Battle.round = 0
	xyd.Battle.buffs = {}
	xyd.Battle.buffsIndex = 0
	xyd.Battle.roundBuffs = {}
	xyd.Battle.lastKiller = nil
	xyd.Battle.lastDotHurtTarget = nil
	...
	xyd.Battle.purposes = {}
	xyd.Battle.roundTriggerTimes = {}
	xyd.Battle.overRoundKey = {}
...
function BattleCreateReport:setFormation()
	...
	for k, hero in ipairs(self.herosA) do
		...
		local canAddHero = true

		if hero.status then
			if tonumber(hero.status.true_hp) and tonumber(hero.status.true_hp) == 0 then
				canAddHero = false
			elseif tonumber(hero.status.hp) and tonumber(hero.status.hp) == 0 then
				canAddHero = false
			end
		end
		...
		if canAddHero then
			...
			hero:setChimeBonus(self.chimeBonusA)
			hero:setSkinBonus(self.skinBonusA)
	...
	for k, hero in ipairs(self.herosB) do
		...
		if canAddHero then
			hero:setShrine(self.shrineId)
			hero:setGalaxy(self.galaxy_info)
			hero:setGuildInfo(self.guildSkillsB)
			hero:setChimeBonus(self.chimeBonusB)
			hero:setSkinBonus(self.skinBonusB)
...
function BattleCreateReport:newFighter(hero, teamType, index)
	...
	if hero.status then
		...
		if not fighter.status.hp and not fighter.status.true_hp then
			fighter.status = nil
		end
	end
...
function BattleCreateReport:setupGlobalBuffs()
	...
	if self.battleType ~= xyd.ReportBattleType.SPFARM then
		self:addGroupBuffs()
	end
...
function BattleCreateReport:getGroupBuffs(team)
	...
	local effectPos = GroupBuffTable:getEffectAndPos(groups, team)

	return effectPos.effect, effectPos.pos
end
...
function BattleCreateReport:getChimeSkill()
	self:getChimeSkillByInfo(self.chimeBonusA, 2)
	self:getChimeSkillByInfo(self.chimeBonusB, 3)
end

function BattleCreateReport:getChimeSkillByInfo(chimeInfo, index)
	if not chimeInfo then
		return
	end
	...
	for k, v in pairs(chimeInfo) do
		if v.buffs and v.buffs[4] and v.buffs[4] == 1 then
			local buffs = chimeTable:getBuff(v.chime_id, 4)

			for j, buff in ipairs(buffs) do
				if buff[1] == "skill" then
					table.insert(self.chimeGodSkills, tonumber(buff[index]))
				end
			end
		end
	end
...
function BattleCreateReport:initGod()
	...
	for k, v in ipairs(self.preUseGodSkill) do
	...
	for k, v in ipairs(self.chimeGodSkills) do
		table.insert(skills, v)
	end
...
function BattleCreateReport:mainLoop()
	...
	while fighter do
		...
		if not isEnd and not isReborn then
...
function BattleCreateReport:teamCheckTrigger(trigger_, unit)
	for _, v in ipairs(xyd.Battle.team) do
		if not v:isDeath() then
			local skillIDs = v:getPasSkillByType(trigger_)

			if next(skillIDs) then
				unit = unit or self:createAttackUnit(false)

				v:usePasSkill(trigger_, unit)
			end
		end
	end
	...
	if xyd.Battle.god then
		...
		xyd.Battle.god:usePasSkill(trigger_, unit)
	end

	return unit
end

function BattleCreateReport:createAttackUnit(noPasSkill, fighter)
	local params = {}

	params.skillID = 0
	params.notTrigPasSkill = noPasSkill
...
function BattleCreateReport:useBattleStartSkill()
	local unit, unit2, lastBaseFighter

	if xyd.Battle.round == 1 then
		unit = self:teamCheckTrigger(xyd.TriggerType.BEFORE_BATTLE_START, unit)

		for _, v in ipairs(xyd.Battle.team) do
			if not v:isDeath() then
				lastBaseFighter = v

				local skillIDs = v:getPasSkillByType(xyd.TriggerType.ROUND_BEGIN_ALL)

				if next(skillIDs) then
					unit = unit or self:createAttackUnit(false)

					v:usePasSkill(xyd.TriggerType.ROUND_BEGIN_ALL, unit)
				end

				skillIDs = v:getPasSkillByType(xyd.TriggerType.BATTLE_START)
				...
				if next(skillIDs) then
					unit2 = unit2 or self:createAttackUnit(true)
		...
		if xyd.Battle.god then
			local skillIDs = xyd.Battle.god:getPasSkillByType(xyd.TriggerType.BATTLE_START)

			if next(skillIDs) then
				unit = unit or self:createAttackUnit(false)

				xyd.Battle.god:usePasSkill(xyd.TriggerType.BATTLE_START, unit)
			end
		end

		unit = self:teamCheckTrigger(xyd.TriggerType.AFTER_BATTLE_START, unit)
	end
end
...
function BattleCreateReport:useGodPasSkill(triggerTypes)
	if xyd.Battle.god then
		...
		for _, triggerType in ipairs(triggerTypes) do
			...
			if next(skillIDs) then
				unit = unit or self:createAttackUnit(false, xyd.Battle.god)
...
function BattleCreateReport:updateAllBuffs()
	local unit = self:createAttackUnit(false)
	...
	if xyd.Battle.god then
		xyd.Battle.god:updateBuffCount(unit)
	...
	for i, v in ipairs(xyd.Battle.teamA) do
		if (v:canReborn(true) or v:apateSpecialReborn()) and v:getDieRound() ~= xyd.Battle.round then
	...
	local unit2 = self:createAttackUnit(false)
...
function BattleCreateReport:createFighterQueue()
	...
	table.sort(fighters, function(a, b)
		if a:getAckSpeed() ~= b:getAckSpeed() then
			return a:getAckSpeed() > b:getAckSpeed()
...
function BattleCreateReport:checkHpWin()
	...
	elseif self.battleType == xyd.ReportBattleType.DUNGEON or self.battleType == xyd.ReportBattleType.TEST then
		local AliveB = getAliveNum(xyd.Battle.teamB)
...
function BattleCreateReport:writeReport()
	...
	local function getFighterData(fighter)
		...
		local params = {
			...
			ver = hero:getVer(),
			star_origin = hero:getStarOrigin(),
			soul_infos = hero.souls
		}
	...
	self.report_.insertPosSkills = self.insertPosSkills
	...
	self.report_.over_round_key = xyd.Battle.overRoundKey
	self.report_.chimeBonusA = self.chimeBonusA
	self.report_.chimeBonusB = self.chimeBonusB
	self.report_.shrineId = self.shrineId
	self.report_.galaxy_info = self.galaxy_info
	self.report_.skinBonusA = self.skinBonusA
	self.report_.skinBonusB = self.skinBonusB
	...
	self.report_.groupBuffA = {}
	self.report_.groupBuffB = {}

	for i, v in ipairs(xyd.Battle.groupBuffA) do
		table.insert(self.report_.groupBuffA, v.effectID)
	end

	for i, v in ipairs(xyd.Battle.groupBuffB) do
		table.insert(self.report_.groupBuffB, v.effectID)
	end
...
function BattleCreateReport:getTotalHarm()
	if self.battleType == xyd.ReportBattleType.SKY_ISLAND then
		return self:getEnemyHpHarm()
	end
	...
	if self.battleType == xyd.ReportBattleType.TRIAL_NEW or self.battleType == xyd.ReportBattleType.ICE_SECRET_BOSS or self.battleType == xyd.ReportBattleType.LIMIT_GACHA_BOSS or self.battleType == xyd.ReportBattleType.SHRINE then
		if self.petA_ then
	...
	totalHarm = math.floor(totalHarm)

	return totalHarm
end

function BattleCreateReport:getEnemyHpHarm()
	local enemyHpChange = 0

	for k, v in ipairs(xyd.Battle.teamB) do
		enemyHpChange = enemyHpChange + (v.initSetHp - v:getHp())
	end

	return enemyHpChange
end

function BattleCreateReport:getEnemyTotalHP()
	local totalHp = 0

	for i, hero in ipairs(self.herosB) do
		local fighter = self:newFighter(hero, xyd.TeamType.B, hero.pos + xyd.TEAM_B_POS_BASIC)

		fighter:setupHpLimit()

		totalHp = totalHp + fighter:getHpLimit(true)
	end
...
function BattleCreateReport:getSaveData()
	...
	params.hurts = self.report_ and self.report_.hurts or {}
	params.die_info = self.report_ and self.report_.die_info or {}
	params.isWin = self.isWin_
	params.battle_type = self.battleType
	params.teamA = {}
	params.guildSkillsA = self.guildSkillsA
	params.guildSkillsB = self.guildSkillsB
	params.dressAttrsA = xyd.Battle.dressAttrsA
	params.dressAttrsB = xyd.Battle.dressAttrsB
	params.chimeBonusA = self.chimeBonusA
	params.chimeBonusB = self.chimeBonusB
	params.shrineId = self.shrineId
	params.galaxy_info = self.galaxy_info
	params.skinBonusA = self.skinBonusA
	params.skinBonusB = self.skinBonusB
	params.random_seed = self.randomSeed
	params.god_skills = self.godSkills
	params.insertPosSkills = self.insertPosSkills
	params.total_round = xyd.Battle.round
	params.battle_version = xyd.tables.miscTable:getNumber("battle_version", "value") or 0
	...
	for i, v in ipairs(xyd.Battle.groupBuffA) do
		table.insert(params.groupBuffA, v.effectID)
	end

	for i, v in ipairs(xyd.Battle.groupBuffB) do
		table.insert(params.groupBuffB, v.effectID)
	end
