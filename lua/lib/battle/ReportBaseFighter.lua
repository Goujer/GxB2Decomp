-- chunkname: @../../../Product/Bundles/Android/src/lib/battle/ReportBaseFighter.lua
...
function ReportBaseFighter:init()
	...
	self.roundHarm = {}
	self.fakeHpLimit_ = 0
	self.roundAction = xyd.FighterAction.NONE
end
...
function ReportBaseFighter:isBackTarget()
	...
	if pos > xyd.DEFAULT_FRONT_NUM and pos <= xyd.DEFAULT_FRONT_NUM + xyd.DEFAULT_BACK_NUM then
		return true
	end
...
function ReportBaseFighter:addEnergy(value, unit, isClearEnergy)
	...
	if addNum > 0 and self:getOneBuffByName(xyd.BUFF_GOD_CONTROL_ENERGY) then
		addNum = addNum * 0.5
	end

	if addNum > 0 then
		local getRate = self:getEnemyGetRate()

		if getRate < 0 then
			getRate = 0
		end

		addNum = addNum * getRate
	end
...
function ReportBaseFighter:getEnemyGetRate()
	if self:getOneBuffByName(xyd.BUFF_AILISHE_COLOR123) then
		return 0
	end

	local rate = 1 + self:getBuffsTotalFinalNumArray(xyd.BUFF_ENERGY_GET_RATE, 1)

	rate = rate - self:getBuffsTotalFinalNumArray(xyd.BUFF_HYPOXIA, 2)
	rate = rate - self:getBuffsTotalFinalNumArray(xyd.BUFF_PLMXS_DEBUFF, 2)
...
function ReportBaseFighter:canControlBuffAdd(buff)
	...
	if firstControlBuffs and next(firstControlBuffs) then
		if self.firstControlName then
			if xyd.arrayIndexOf(sameCrystals, self.firstControlName) >= 0 and xyd.arrayIndexOf(sameCrystals, name) >= 0 then
				return false
			end
...
function ReportBaseFighter:updateEnergyByHarm(isCrit, unit, isClearEnergy)
	...
	local addRate = 1
	local snowWindMixs = self:getBuffsByNameAndFighter(xyd.BUFF_SNOW_WIND_MIX)

	if next(snowWindMixs) then
		addRate = addRate + snowWindMixs[1]:getHurtEnergyRate()
	end

	if next(self.hurtForEnergy_) then
		local buff = self:getOneBuffByName(xyd.BUFF_HURT_FOR_ENERGY)

		addRate = addRate + buff:getFinalNum()
	end
...
function ReportBaseFighter:setEnergy(value, unit, isClearEnergy)
	...
	if self.energy_ >= 100 then
		if not self.hasFullEnergy and unit then
			self.hasFullEnergy = true

			self:usePasSkill(xyd.TriggerType.ENERGY_FULL, unit)
		end
	else
		self.hasFullEnergy = false
	end
...
function ReportBaseFighter:updateShield(num)
	self.shieldHp_ = self.shieldHp_ + num

	if self.shieldHp_ > self:getHpLimit() then
		self.shieldHp_ = self:getHpLimit()
	elseif self.shieldHp_ < 0 then
		self.shieldHp_ = 0
	end
end
	...
	if self.energy_ >= 100 and self:isFullEnergyHurt() then
		...
		for _, buff in ipairs(self.isFullEnergyHurt_) do
			...
			if canChangeEnergy then
				...
				num = self:getShieldHarm(num, unit, false, true).result
			end
...
function ReportBaseFighter:populateWithHero(hero)
	...
	self.hero_ = hero
	self.hero_.fightPos = self:getPos()
...
function ReportBaseFighter:updateInitHp()
	local percent = self:getHpPercent()

	self:setupHpLimit()

	self.hp_ = percent * self:getHpLimit()
end
...
function ReportBaseFighter:changeHpLimit(hpLimit, isTrueChange)
	if isTrueChange then
		self.hpLimit_ = hpLimit
	else
		self.fakeHpLimit_ = self.fakeHpLimit_ + hpLimit
	end

	if self:getHp() > self:getHpLimit() then
		self:setHp(self:getHpLimit())
	end
end

function ReportBaseFighter:getHpLimit(isOrigin)
	if isOrigin then
		return self.hpLimit_
	end

	return self.hpLimit_ + self.fakeHpLimit_
end
...
function ReportBaseFighter:getAckSpeed()
	if not self.___ackSpeed then
		...
		self.___ackSpeed = self.___ackSpeed + self:getBuffsTotalFinalNumArray(xyd.BUFF_DEATH_SPD, 1)
		...
		self.___ackSpeed = self.___ackSpeed - self:getBuffsTotalFinalNumArray(xyd.BUFF_AILISHE_COLOR1, 1)
	end
...
function ReportBaseFighter:recordData(harm, cure, isNew)
	if isNew then
		-- block empty
	end
...
function ReportBaseFighter:beginAttack()
	...
	if self.shieldHp_ > 0 then
		self:usePasSkill(xyd.TriggerType.SELF_SKILL_HPSHIELD, unit)
	end
	...
	if skillID == self:getEnergySkillID() then
		...
		if self:getTeamType() == xyd.TeamType.A then
			if not xyd.Battle.purposes.skillTimes then
				xyd.Battle.purposes.skillTimes = 0
			end

			xyd.Battle.purposes.skillTimes = xyd.Battle.purposes.skillTimes + 1
		end

		local hittedBuff = self:getOneBuffByName(xyd.BUFF_HITTED_HARDER)

		if hittedBuff then
			hittedBuff:clearHitted()
		end
	end
...
function ReportBaseFighter:getTeamACanSkillBuff()
	if xyd.Battle.god and self:getPos() <= 6 then
		local xCanSkillBuffs = xyd.Battle.god:getBuffsByNameAndFighter(xyd.BUFF_X_CAN_SKILL)
...
function ReportBaseFighter:createAttackUnit(skillID, isPas, unit, isNeedRecord, ifNoPas, ifNoBlock)
	...
	params.isPas = isPas
	params.unit = unit
	params.rebornTimes = self.infRebornTimes_
	params.totalHarm = self.curRoundTotalHarm_
	params.totalCritHarm = self.curRoundTotalCritHarm_
	params.isNeedRecord = isNeedRecord
	params.notTrigPasSkill = ifNoPas
	params.ifNoBlock = ifNoBlock
...
function ReportBaseFighter:applyUnit(unit)
	...
	for _, data in ipairs(datas) do
		...
		if not target:isDeath() then
			...
			local tmpHarm, tmpCure, tmpIsCrit, recordBuffs, tmpIsHarm, inHarmTargetList, shield = target:applyBuffHarmsByTarget(data, unit)
			...
			if not data.noTriggerPas and ((tmpHarm or 0) > 0 or shield > 0 or tmpIsHarm) and not hasSelect[target] then
				if target:hasAttackedHurtSkill() then
				...
				local spzglBuff = target:getOneBuffByName(xyd.BUFF_SPZGL_HITTED)

				if spzglBuff then
					spzglBuff:hittedExcute(unit)
				end
	...
	if totalHarm > 0 then
		...
		if isCrit then
			...
			if unit.skillID == self:getEnergySkillID() then
				self:usePasSkill(xyd.TriggerType.SELF_SKILL_WITH_CRIT, unit)
			end
		end
	end
	...
	local soulTearBuff = self:getOneBuffByName(xyd.BUFF_SOUL_TEAR)

	if soulTearBuff and totalHarm > 0 and (unit.skillID == self:getEnergySkillID() or self:isPugongSkill(unit.skillID)) then
		soulTearBuff:afterAttack(unit, totalHarm)
	end
...
function ReportBaseFighter:checkCrystalPas(unit)
	if next(xyd.Battle.isJustCrystal) then
		local teamTypes = {}

		for k, v in ipairs(xyd.Battle.isJustCrystal) do
			teamTypes[v:getTeamType()] = true
		end

		for _, v in ipairs(xyd.Battle.team) do
			if not v:isDeath() then
				local enemyType = xyd.TeamType.B

				if v:getTeamType() == xyd.TeamType.B then
					enemyType = xyd.TeamType.A
				end

				if teamTypes[enemyType] then
...
function ReportBaseFighter:harmShareCalculate(unit, buffs)
	...
	for _, buff in ipairs(buffs) do
		...
		if target and not target:isDeath() then
			local addHarmRate = 1 + buff.target:getExtraHarmRate(unit)

			addHarmRate = addHarmRate + buff.target:getExtraRoundHarm(buff)
			num = num * addHarmRate
			num = num * buff.target:getSuperDecHurtRate(buff)
			num = target:checkHpShield(num, unit, buff.fighter, buff)
...
function ReportBaseFighter:addExtraJumpSkill(unit, tmpFragranceGetBuffs)
	...
	if unit.skillID == self:getEnergySkillID() then
		...
		local nuolisiAddBuff = self:getOneBuffByName(xyd.BUFF_NUOLISI_ADD_RATE)

		if nuolisiAddBuff then
			nuolisiAddBuff:addExtraJumpSkill(unit)
		end

		local luobiPreCombolBuff = self:getOneBuffByName(xyd.BUFF_LUOBI_PRE_COMBOL)
		...
		for _, v in ipairs(xyd.Battle.team) do
			if not v:isDeath() and v:canAttack() and v:getTeamType() == self:getTeamType() and #v.isMarkAddHurt_ > 0 then
				for _, markSkill in ipairs(v.isMarkAddHurt_) do
					if v:canAttack() then
						local curUnit = v:useOneJumpPasSkill(nil, markSkill:getFinalNum(), unit)
					end
	...
	if unit.skillID == self:getEnergySkillID() or self:isPugongSkill(unit.skillID) then
		local longnvDeadlBuff = self:getOneBuffByName(xyd.BUFF_LONGNV_DEAD)

		if longnvDeadlBuff then
			longnvDeadlBuff:addExtraJumpSkill(unit)
		end
	end
...
function ReportBaseFighter:checkExEnergyHurt(unit)
	local buff = self:getOneBuffByName(xyd.BUFF_FREE_SKILL)
...
function ReportBaseFighter:checkExHurt(unit)
	local buff = self:getOneBuffByName(xyd.BUFF_EX_HURT)
	...
	if buff then
		...
		if self.isPassiveQuene_ ~= xyd.Battle.round then
			table.insert(xyd.Battle.passiveQueue, self)

			self.isPassiveQuene_ = xyd.Battle.round
		else
			self.isPassiveQuene_ = 0
		end
	else
...
function ReportBaseFighter:checkNewRound(unit)
	if self:getOneBuffByName(xyd.BUFF_NEW_ROUND) then
		local hasDieA = false
		local hasDieB = false
	...
	if enemy then
		...
		local buff = enemy:getOneBuffByName(xyd.BUFF_ATK_D)
		local num = buff:getFinalNum()
...
function ReportBaseFighter:enemyHasBuff(unit, buffName)
	...
	for _, data in ipairs(datas) do
		if data.target:getOneBuffByName(buffName) then
			return data.target
		end
	end
end
...
function ReportBaseFighter:checkCanAddHurt(effectID, target, fighter)
	...
	if EffectTable:buff(effectID) == "addHurtd" and target:getHujia() > fighter:getHujia() or EffectTable:buff(effectID) == xyd.BUFF_ADD_HURT then
		flag = true
	end
...
function ReportBaseFighter:applyBuffHarmsByTarget(data, unit)
	...
	local harm, cure, recordBuffs, status, inHarmTargetList, shield = self:applyBuffHarm(unit, hitBuffs)
	...
	if self:isDeath() then
		...
		if harm > 0 or shield > 0 or status.isHarm then
			...
			if fighter:isRidicule() then
				self:usePasSkill(xyd.TriggerType.SELF_ATTACKED_BY_RIDICULE, unit)
			end

			if fighter and (unit.skillID == fighter:getEnergySkillID() or fighter:isPugongSkill(unit.skillID)) then
				self:usePasSkill(xyd.TriggerType.SELF_ATTACKED_SKILL_PUGONG, unit)
				self:usePasSkill(xyd.TriggerType.SELF_ATTACKED_ONCE, unit)

				if fighter:getOneBuffByName(xyd.BUFF_ANSUNA_TO_SHIELD) then
					self:usePasSkill(xyd.TriggerType.SELF_ATTACKED_BY_ANSUNA_TO_SHIELD, unit)
				end

				local soulSealBuff = self:getOneBuffByName(xyd.BUFF_SOUL_SEAL)

				if soulSealBuff then
					soulSealBuff:hitted(unit)
				end
			end
		...
		if fighter and (unit.skillID == fighter:getEnergySkillID() or fighter:isPugongSkill(unit.skillID)) and fighter:getTeamType() ~= self:getTeamType() then
			local lowestFighter = GetTarget_.S3(fighter)[1]

			if lowestFighter == unit.fighter then
				self:usePasSkill(xyd.TriggerType.SELF_ATTACKED_BY_HP_LOWEST, unit)
			end
		end

		if status.isControl then
			self:usePasSkill(xyd.TriggerType.SELF_CONTROLED, unit)
			self:usePasSkill(xyd.TriggerType.SELF_CONTROLED_FIRST, unit)
		...
		if status.isBlock then
			...
			if fighter and (unit.skillID == fighter:getEnergySkillID() or fighter:isPugongSkill(unit.skillID)) then
				self:usePasSkill(xyd.TriggerType.SELF_BLOCK_ATTACK, unit)
			end
	...
	return harm, cure, status.isCrit, recordBuffs, status.isHarm, inHarmTargetList, shield
end
...
function ReportBaseFighter:checkStatusSkill(unit, status)
		...
		if not v:isDeath() then
			...
			if status.isStun then
				...
				if not vIsFriend then
					if v == unit.fighter then
						v:usePasSkill(xyd.TriggerType.SELF_STUN, unit)
					end

					v:usePasSkill(xyd.TriggerType.ENEMY_STUN, unit)
				end
			end
	...
	if status.isCrystall then
		table.insert(xyd.Battle.isJustCrystal, self)
	end
...
function ReportBaseFighter:checkHpPasSkill(unit)
	if self:isPet() or self:isDeath() then
		return
	end
	...
	if percent < 0.3 and self.maxHpP_ >= 0.3 * xyd.PERCENT_BASE then
		...
		self:usePasSkill(xyd.TriggerType.SELF_HP_LESS_30_OR_ROUND_5, unit)
...
function ReportBaseFighter:checkHarmAfterRound(unit)
	...
	for _, data in ipairs(datas) do
		...
		if target:getTeamType() ~= self:getTeamType() then
			if xyd.Battle.roundHarm[target:getPos()] and not target:isDeath() then
			...
			if xyd.Battle.roundHarm[target:getPos()] or xyd.Battle.roundShieldHarm[target:getPos()] and xyd.Battle.roundShieldHarm[target:getPos()] < 0 then
...
function ReportBaseFighter:die(unit, harmBuff)
	...
	xyd.Battle.lastKiller = self.killer
	...
	if self:hasRebornInf() then
		...
		if self.infRebornTimes_ == 6 then
			self:usePasSkill(xyd.TriggerType.AFTER_REBORN_6, unit)
		end

		if self.infRebornMaxTimes_ ~= -1 and self.infRebornTimes_ > self.infRebornMaxTimes_ then
		...
		if v ~= self and not v:isDeath() then
			if v:getTeamType() == self:getTeamType() then
				...
				if xyd.Battle.round <= 3 then
					v:usePasSkill(xyd.TriggerType.FRIEND_DIE_ROUND_3, unit)
				end

				if next(v:getBuffsByNameAndFighter(xyd.BUFF_SHARE_BY_HURT, self)) then
					v:getBuffsByNameAndFighter(xyd.BUFF_SHARE_BY_HURT, self)[1]:removeSelf(unit)
				end
	...
	if xyd.Battle.god then
		...
		elseif self:getTeamType() == xyd.TeamType.A then
			if xyd.Battle.round <= 3 then
				xyd.Battle.god:usePasSkill(xyd.TriggerType.FRIEND_DIE_ROUND_3, unit)
			end

			if not xyd.Battle.firstDieFighter or xyd.Battle.firstDieFighter == self then
				xyd.Battle.firstDieFighter = self

				xyd.Battle.god:usePasSkill(xyd.TriggerType.FIRST_FRIEND_DIE, unit)
			end
		end

		xyd.Battle.god:usePasSkill(xyd.TriggerType.DIE, unit)
	end
	...
	local spdExchangeBuff = self:getOneBuffByName(xyd.BUFF_EXCHANGE_SPD)
...
function ReportBaseFighter:getLoseSealHp()
	...
	if next(mayaHpLoseSeals) then
		...
		if mayaHpPercent > mayaHpLoseSeals[1].finalNumArray_[2] then
...
function ReportBaseFighter:getLostSealLimit(num)
...
function ReportBaseFighter:blockHeal(num, unit, buff)
	...
	if soulSealBuff then
		soulSealBuff:addRecordHeal(result)

		result = result * (1 - soulSealBuff.finalNumArray_[1])
	end

	result = self:getLostSealLimit(result)

	if buff and not buff.noBlockHeal and buff.fighter and not buff.fighter:isPet() and result > 0 then
		if self.blockHealUnit == nil or self.blockHealUnit ~= unit then
			if self:isGetHealCurse() then
				...
				local addHarmRate = 1 + self:getExtraHarmRate(unit, true, delBuff.fighter)
				...
				newBuff.finalNum_ = self:getShieldHarm(newBuff.finalNum_, unit, false, false, delBuff.fighter, true).result
				...
				local cleanBuffs = self:getOneBuffByName(xyd.BUFF_HEAL_CURSE_CLEAN_HURT)
			...
			newBuff.finalNum_ = -newBuff.finalNum_ * num * addHarmRate
			newBuff.finalNum_ = self:getShieldHarm(newBuff.finalNum_, unit, false, false, self.blockHealBuff.target, true).result
...
function ReportBaseFighter:getShieldHarm(num, unit, isRate, isImpress, buffFighter, isHealCurse, buff)
	...
	if buff and buff:isImpress() then
		isImpress = true
	end
	...
	if result < 0 and next(hurtMaxHpLimitBBuffs) then
		result = hurtMaxHpLimitBBuffs[1]:calculateResult(result, self)
	end
	...
	if not isHealCurse then
		if not isImpress and unit and not unit:isPasSkill() and unit.fighter and not unit.fighter:isPet() and result < 0 and self:isGetAbsorbShield() and attacker:getTeamType() == unit.fighter:getTeamType() and (not buff or buff:getName() ~= xyd.BUFF_SUN_ARROW_SHOOT) then
			if self:isGetThorns() then
				self:usePasSkill(xyd.TriggerType.SELF_ATTACKED_WITH_THORNS, unit)
			end
	...
	if buff and buff:getName() == xyd.BUFF_HURT_FREE_SHIELD then
		resultTmp = result
	else
		resultTmp = self:checkHpShield(result, unit, attacker, buff)
	end

	local shieldHp = result - resultTmp
	...
	xyd.Battle.roundShieldHarm[self:getPos()] = (xyd.Battle.roundShieldHarm[self:getPos()] or 0) + shieldHp
...
function ReportBaseFighter:checkHpShield(result, unit, attacker, buff)
	local costShield = 0
	...
	local addRate = self:getBuffsTotalFinalNumArray(xyd.BUFF_HP_SHIELD_RECOVER, 2)
	local preUseShield = -result / (1 + addRate)

	if preUseShield < self.shieldHp_ then
		result = 0
		costShield = -preUseShield
	else
		result = -(preUseShield - self.shieldHp_) * (1 + addRate)
		costShield = -self.shieldHp_
	end

	local attacker_ = attacker or unit.fighter

	if attacker_ and xyd.Battle.battleType ~= xyd.ReportBattleType.SKY_ISLAND and not xyd.Battle.god:getOneBuffByName(xyd.BUFF_HP_SHIELD_NO_RECORD_HARM) then
		attacker_.hurt_ = attacker_.hurt_ - math.ceil(costShield)
		self.tank_ = self.tank_ - math.ceil(costShield)
	end

	if buff then
		buff:updateRecordshield(costShield)
	elseif costShield < 0 then
		self:updateShield(costShield)
		self:recordHpShield(unit, costShield, unit.skillID, attacker)
	end

	if costShield < 0 then
		self:checkRemoveHpShield(unit)
	end

	return result
end

function ReportBaseFighter:checkRemoveHpShield(unit)
	if self.shieldHp_ == 0 then
		if self.hasHpShieldAnimFlag then
			local flagBuff = self:getOneBuffByName(xyd.BUFF_HP_SHIELD_ANIM_FLAG)

			if flagBuff then
				flagBuff:clearAnim(unit)
			end
		end

		local healBuffs = self:getBuffsByNameAndFighter(xyd.BUFF_HP_SHIELD_END_TO_HEAL)

		if next(healBuffs) then
			for k, v in ipairs(healBuffs) do
				local params1 = {
					effectID = v.finalNumArray_[1],
					fighter = v.fighter,
					target = self,
					skillID = v.skillID
				}
			...
			self:removeBuffs(healBuffs, nil, unit)
		end
	end
end

function ReportBaseFighter:addHpShield(unit, value, skillID, fighter)
	value = value * self:getShieldRate()

	if value <= 0 then
		return
	end

	if self.hasHpShieldAnimFlag and self.shieldHp_ <= 0 then
		local flagBuff = self:getOneBuffByName(xyd.BUFF_HP_SHIELD_ANIM_FLAG)

		if flagBuff then
			flagBuff:addAnim(unit)
		end
	end

	self:updateShield(value)
	self:recordHpShield(unit, value, skillID, fighter)
end

function ReportBaseFighter:recordHpShield(unit, value, skillID, fighter)
	if xyd.Battle.recordUnit then
		local index = 2

		if unit == xyd.Battle.recordUnit.mainUnit then
			index = 1
		end
		...
		local record = {
			...
			pos = self:getPos(),
			f_pos = f_pos,
			name = xyd.BUFF_HP_SHIELD,
			skill_id = skillID,
			effect_index = index,
			shieldCost = math.floor(value),
			shield = math.ceil(self.shieldHp_ / self:getHpLimit() * 100)
		}

		table.insert(xyd.Battle.recordUnit.buffs_, record)
		xyd.Battle.recordUnit:saveTarget(self, index)
	end
end

function ReportBaseFighter:getShieldRate()
	if self:getOneBuffByName(xyd.BUFF_AILISHE_COLOR123) then
		return 0
	end
	...
	result = result - self:getBuffsTotalFinalNumArray(xyd.BUFF_AILISHE_COLOR13, 2)
	result = result - self:getBuffsTotalFinalNumArray(xyd.BUFF_HADISI_DEBUFF, 2)
	result = result - self:getBuffsTotalFinalNumArray(xyd.BUFF_SHUAIRUO, 2)
	result = result + self:getBuffsTotalFinalNumArray(xyd.BUFF_HP_SHIELD_RATE, 1)
...
function ReportBaseFighter:getSuperDecHurtRate(buff)
	...
	if buff.fighter and buff.fighter:getOneBuffByName(xyd.BUFF_GOD_CONTROL_ENERGY) and buff.fighter:getEnergy() == 0 then
		return 0
	end
	...
	local lubanBBuffs = self:getBuffsByNameAndFighter(xyd.BUFF_LUBAN_HURT_B)

	if next(lubanBBuffs) then
		rate = rate * (1 + lubanBBuffs[1].finalNumArray_[1])
	end

	local lubanCBuffs = self:getBuffsByNameAndFighter(xyd.BUFF_LUBAN_HURT_C)
...
function ReportBaseFighter:getHarmRateDump(buff, isGroup, fighter)
	if not UNITY_EDITOR and not UNITY_STANDALONE then
		return
	end

	local fighter_ = fighter

	fighter_ = fighter_ or self

	local buffs = {}

	if type(buff) == "string" then
		local bffs_ = fighter_:getBuffsByNameAndFighter(buff)

		if isGroup then
			buffs = bffs_
		elseif next(bffs_) then
			table.insert(buffs, bffs_[1])
		end
	elseif isGroup then
		buffs = buff
	else
	...
	if not self.allHarmRateBuffs then
		return
	end

	for k, v in ipairs(buffs) do
		table.insert(self.allHarmRateBuffs, v:getName() .. " " .. v.effectID .. " " .. fighter_:getPos() .. " " .. v.skillID)
	end
end

function ReportBaseFighter:getExtraHarmRate(unit, isRate, fighter_)
	self.allHarmRateBuffs = {}

	local result = 0
	local fighter = fighter_

	if unit and unit.fighter and not fighter then
		fighter = unit.fighter

		if next(fighter:getBuffsByNameAndFighter(xyd.BUFF_TARGET_DMG_UP)) then
			local targetDmg = fighter:getBuffsByNameAndFighter(xyd.BUFF_TARGET_DMG_UP)[1]:getHarmRate(unit)

			result = result + targetDmg

			self:getHarmRateDump(xyd.BUFF_TARGET_DMG_UP, false, fighter)
		end
	end
	...
	if self:isGetLeaf() then
		...
		self:getHarmRateDump(self.isGetLeaf_, true)
	end

	if fighter then
		local cBuffs = fighter:getBuffsByNameAndFighter(xyd.BUFF_ALL_DMG_C)
		local tmpRate = 0

		if next(cBuffs) then
			tmpRate = fighter:getBuffTotalNumByGroup(cBuffs)

			self:getHarmRateDump(cBuffs, true, fighter)
		end

		local qiubitePowerBuff = fighter:getBuffsByNameAndFighter(xyd.BUFF_QIUBITE_POWER)

		if next(qiubitePowerBuff) and fighter.shieldHp_ > 0 then
			tmpRate = tmpRate + qiubitePowerBuff[1].finalNumArray_[1]

			self:getHarmRateDump(qiubitePowerBuff[1], false, fighter)
		end

		local xierkediBuff = fighter:getBuffsByNameAndFighter(xyd.BUFF_XIERKEDI_DOT, self)

		if next(xierkediBuff) then
			tmpRate = tmpRate - xierkediBuff[1].finalNumArray_[1]

			self:getHarmRateDump(xierkediBuff[1], false, fighter)
		end

		local toBossAllDmg = fighter:getOneBuffByName(xyd.BUFF_TO_BOSS_ALL_DMG)

		if toBossAllDmg and self:isMonster() and MonsterTable:isRealBoss(self:getTableID()) then
			tmpRate = tmpRate + fighter:getBuffsTotalFinalNumArray(xyd.BUFF_TO_BOSS_ALL_DMG, 1)

			self:getHarmRateDump(toBossAllDmg, false, fighter)
		end

		local hurt3AllDmg = fighter:getOneBuffByName(xyd.BUFF_HURT_3_ALL_DMG)

		if hurt3AllDmg then
			local hurt3AllDmgAdd = hurt3AllDmg:getDmgAdd()

			tmpRate = tmpRate + hurt3AllDmgAdd

			if hurt3AllDmgAdd > 0 then
				self:getHarmRateDump(hurt3AllDmg, false, fighter)
			end
		end

		local upHpAllDmgs = fighter:getBuffsByNameAndFighter(xyd.BUFF_UP_HP_ALL_DMG)

		if next(upHpAllDmgs) then
			for k, v in ipairs(upHpAllDmgs) do
				if self:getHpPercent() > v.finalNumArray_[1] then
					tmpRate = tmpRate + v.finalNumArray_[2]

					self:getHarmRateDump(v, false, fighter)
				end
			end
		end

		local ansunaToShieldBuff = fighter:getBuffsByNameAndFighter(xyd.BUFF_ANSUNA_TO_SHIELD, self)

		if next(ansunaToShieldBuff) then
			tmpRate = tmpRate - ansunaToShieldBuff[1].finalNumArray_[1]

			self:getHarmRateDump(xierkediBuff[1], false, fighter)
		end

		local spzglRate = fighter:getBuffsTotalFinalNumArray(xyd.BUFF_SPZGL_ALL_HARM, 1)

		if spzglRate > 0 then
			local preAddRate = math_floor(fighter:getAD() / 1000) * spzglRate

			if preAddRate > 0.5 then
				preAddRate = 0.5
			end

			self:getHarmRateDump(xyd.BUFF_SPZGL_ALL_HARM, true, fighter)

			tmpRate = tmpRate + preAddRate
		end

		local energyAllHarm = fighter:getBuffsTotalFinalNumArray(xyd.BUFF_ENERGY_ALL_HARM, 1)

		if energyAllHarm > 0 then
			local preAddRate = fighter:getEnergy() * energyAllHarm

			self:getHarmRateDump(xyd.BUFF_ENERGY_ALL_HARM, true, fighter)

			tmpRate = tmpRate + preAddRate
		end

		local mingzhong2AllHarm = fighter:getBuffsTotalFinalNumArray(xyd.BUFF_MINGZHONG_2_ALL_HARM, 1)

		tmpRate = tmpRate + fighter:getMingZhong(fighter, self) * mingzhong2AllHarm

		self:getHarmRateDump(xyd.BUFF_MINGZHONG_2_ALL_HARM, true, fighter)

		local dmgRate = fighter:getAttrByType(xyd.BUFF_ALL_DMG_RATE)

		if dmgRate ~= 0 then
			self:getHarmRateDump(xyd.BUFF_ALL_DMG_RATE, true, fighter)

			tmpRate = tmpRate + dmgRate
		end
		...
		if next(cBuffs40) then
			...
			self:getHarmRateDump(cBuffs40, true, fighter)
		end
		...
		local luobiBuffs = fighter:getBuffsByNameAndFighter(xyd.BUFF_LUOBI_HP)

		if next(luobiBuffs) then
			tmpRate = tmpRate + luobiBuffs[1]:getHarmRate()

			self:getHarmRateDump(luobiBuffs[1], false, fighter)
		end

		if fighter:isGetLeaf() then
			tmpRate = tmpRate - fighter:getBuffTotalNumByGroup(fighter.isGetLeaf_, 2)

			self:getHarmRateDump(fighter.isGetLeaf_, true, fighter)
		end

		tmpRate = tmpRate - fighter:getBuffsTotalFinalNumArray(xyd.BUFF_AILISHE_COLOR23, 1)

		self:getHarmRateDump(xyd.BUFF_AILISHE_COLOR23, true, fighter)

		local party5Dmg = fighter:getBuffsTotalFinalNumArray(xyd.BUFF_GROUP5_DMG_UP, 1)

		if party5Dmg > 0 then
			for k, v in ipairs(fighter.selfTeam_) do
				if v:getGroup() == 5 then
					result = result + party5Dmg

					self:getHarmRateDump(xyd.BUFF_GROUP5_DMG_UP, true, fighter)
				end
			end
		end

		local controlPower = fighter:getBuffsTotalFinalNumArray(xyd.BUFF_TO_CONTROL_DMG_UP, 1)

		if controlPower ~= 0 and self:isControl() then
			result = result + controlPower
		end

		result = result + tmpRate
	end

	local party6Dec = self:getBuffsTotalFinalNumArray(xyd.BUFF_GROUP6_HARM_DEC, 1)

	if party6Dec > 0 then
		for k, v in ipairs(self.selfTeam_) do
			if v:getGroup() == 6 then
				result = result - party6Dec

				self:getHarmRateDump(xyd.BUFF_GROUP6_HARM_DEC, true)
			end
		end
	end

	result = result + self:getBuffsTotalFinalNumArray(xyd.BUFF_QIANHEZI_HEAL, 1)

	self:getHarmRateDump(xyd.BUFF_QIANHEZI_HEAL, true)

	result = result + self:getBuffsTotalFinalNumArray(xyd.BUFF_DMG_RATE_SPREAD, 1)

	self:getHarmRateDump(xyd.BUFF_DMG_RATE_SPREAD, true)

	result = result + self:getBuffsTotalFinalNumArray(xyd.BUFF_AILISHE_COLOR12, 1)

	self:getHarmRateDump(xyd.BUFF_AILISHE_COLOR12, true)

	result = result + self:getBuffsTotalFinalNumArray(xyd.BUFF_AILISHE_COLOR123, 1)

	self:getHarmRateDump(xyd.BUFF_AILISHE_COLOR123, true)

	local plmxsThorns = self:getOneBuffByName(xyd.BUFF_PLMXS_THORNS)

	if plmxsThorns then
		result = result - plmxsThorns:getJianshang()

		self:getHarmRateDump(plmxsThorns)
	end
	...
	if isRate then
		...
		if self:isDecHurt() then
			...
			self:getHarmRateDump(self.isDecHurt_, true)
		...
		if self:isHalfHpDmg() then
			...
			self:getHarmRateDump(self.isHalfHpDmg_, true)
		...
		if fighter and fighter:isAllDmgB() then
			...
			self:getHarmRateDump(fighter.isAllDmgB_, true, fighter)
		...
		if self:isDecDmgShieldLimit8() then
			...
			self:getHarmRateDump(self.isDecDmgShieldLimit8_, true)
		...
		if self:isControl() then
			result = result + self:getBuffsTotalFinalNumArray(xyd.BUFF_CONTROL_DEC_HARM, 1)

			self:getHarmRateDump(xyd.BUFF_CONTROL_DEC_HARM, true)
		end

		if fighter then
			local tmpRate = fighter:getBuffsTotalFinalNumArray(xyd.BUFF_DOT_MARK_ALL_DMG, 1)

			self:getHarmRateDump(xyd.BUFF_DOT_MARK_ALL_DMG, true, fighter)

			result = result + tmpRate

			if fighter.shieldHp_ > 0 then
				local shieldRate = fighter:getBuffsTotalFinalNumArray(xyd.BUFF_HP_SHIELD_ALL_DMG, 1)

				self:getHarmRateDump(xyd.BUFF_HP_SHIELD_ALL_DMG, true, fighter)

				result = result + shieldRate
			end

			if next(fighter:getBuffsByNameAndFighter(xyd.BUFF_FS_ALL_DMG)) then
				local tmpRate1 = fighter:getBuffsTotalFinalNumArray(xyd.BUFF_FS_ALL_DMG, 1)

				for k, v in ipairs(fighter.selfTeam_) do
					if v:getJob() == xyd.HeroJob.FS then
						result = result + tmpRate1

						self:getHarmRateDump(xyd.BUFF_FS_ALL_DMG, true, fighter)
					end
				end
			end

			if next(fighter:getBuffsByNameAndFighter(xyd.BUFF_MS_ALIVE_ALL_DMG)) then
				local tmpRate1 = fighter:getBuffsTotalFinalNumArray(xyd.BUFF_MS_ALIVE_ALL_DMG, 1)
				local hasMs = false

				for k, v in ipairs(fighter.selfTeam_) do
					if v:getJob() == xyd.HeroJob.MS and not v:isDeath() then
						hasMs = true
				...
				if hasMs then
					...
					self:getHarmRateDump(xyd.BUFF_MS_ALIVE_ALL_DMG, true, fighter)
				end
			end

			if next(fighter:getBuffsByNameAndFighter(xyd.BUFF_RESTRAINTED_DMG_UP)) and GroupTable:isRestraint(self, fighter) then
				local tmpRate1 = fighter:getBuffsTotalFinalNumArray(xyd.BUFF_RESTRAINTED_DMG_UP, 1)

				result = result + tmpRate1

				self:getHarmRateDump(xyd.BUFF_RESTRAINTED_DMG_UP, true, fighter)
			end

			local groupDmgUps = fighter:getBuffsByNameAndFighter(xyd.BUFF_GROUP_DMG_UP)

			if next(groupDmgUps) then
				result = result + groupDmgUps[1]:calculateUpRate()

				self:getHarmRateDump(groupDmgUps[1], false, fighter)
			end
		end
		...
		if self:isAbsorbDamage() then
			...
			self:getHarmRateDump(self.isAbsorbDamage_, true)
		end
		...
		if rImpressesTreatBlockBuffs and next(rImpressesTreatBlockBuffs) and (rImpressesHpLimitBuffs and next(rImpressesHpLimitBuffs) or self.rImpresses_ and next(self.rImpresses_)) then
			...
			self:getHarmRateDump(rImpressesTreatBlockBuffs, true)
		end
...
function ReportBaseFighter:applyLeafBuffHarm(unit)
	local getLeafBuffs = self:getBuffsByNameAndFighter(xyd.BUFF_GET_LEAF)
	...
	local params1 = {
		...
		target = self,
		skillID = getLeafBuffs[1].skillID
	}
...
function ReportBaseFighter:triggerSelfInAction(unit, isCrit, isMiss, isBlock)
	...
	if self:isPugongSkill(unit.skillID) then
		self.roundAction = xyd.FighterAction.PUGONG

		if not self:getOneBuffByName(xyd.BUFF_PLMXS_TRANS) then
			self:usePasSkill(xyd.TriggerType.PUGONG_WITHOUT_PLMXSTRANS, unit)
		end

		self:usePasSkill(xyd.TriggerType.SELF_PUGONG, unit)

		local nextPugongColor3Bff = self:getOneBuffByName(xyd.BUFF_NEXT_PUGONG_COLOR3)

		if nextPugongColor3Bff then
			nextPugongColor3Bff:pugongTrigger(unit)
		end

		isPugongOrSkill = true
	elseif unit.skillID == self:getEnergySkillID() then
		self.roundAction = xyd.FighterAction.SKILL

		if self:getPos() == 13 and xyd.Battle.god then
			xyd.Battle.god:usePasSkill(xyd.TriggerType.AFTER_SELF_PET_SKILL, unit)
		end

		if not self:getOneBuffByName(xyd.BUFF_PLMXS_TRANS) then
			self:usePasSkill(xyd.TriggerType.SKILL_WITHOUT_PLMXSTRANS, unit)
		else
			self:usePasSkill(xyd.TriggerType.SKILL_WITH_PLMXSTRANS, unit)
		end

		self:usePasSkill(xyd.TriggerType.SELF_SKILL, unit)

		if next(self:getBuffsByNameAndFighter(xyd.BUFF_NEXT_HURT_CRIT)) then
			self:usePasSkill(xyd.TriggerType.SELF_SKILL_NEXT_HURT_CRIT, unit)
		end
	...
	if isPugongOrSkill then
		if self.shieldHp_ > 0 then
			self:usePasSkill(xyd.TriggerType.SKILL_PUGONG_WITH_SHIELD, unit)
		end
	...
	if next(self.bImpresses_) and not self:isPugongSkill(unit.skillID) and not self:isDeath() then
		...
		num = num * addHarmRate
		num = self:getShieldHarm(num, unit, true, true, bBuff.fighter).result
...
function ReportBaseFighter:triggerOthersInAction(unit, harm, cure, isCrit, isMiss, isBlock)
		...
		if fighterIsFriend then
			if not fighterIsPet then
				...
				if fighter ~= self then
					...
					self:usePasSkill(xyd.TriggerType.FRIEND_SKILL_WITHOUT_SELF_H_2, unit)
				end
			end
	...
	if targetIsFriend then
		if not isBlock then
			self:usePasSkill(xyd.TriggerType.FRIEND_NO_BLOCK, unit)
		else
			self:usePasSkill(xyd.TriggerType.FRIEND_BLOCK, unit)
		end
	end
	...
	if targetIsFriend and (fighter:isPugongSkill(unit.skillID) or unit.skillID == fighter:getEnergySkillID()) and fighter:getOneBuffByName(xyd.BUFF_BLIND) then
		self:usePasSkill(xyd.TriggerType.FRIEND_ATTACKED_BY_BLIND, unit)
	end
	...
	if self:canAttack() and fighterIsFriend and not fighterIsPet and next(fighter:getBuffsByNameAndFighter(xyd.BUFF_SPZGL_ALL_HARM, self)) then
		self:usePasSkill(xyd.TriggerType.SPZGL_ALL_HARM_ATTACK, unit)
	end
...
function ReportBaseFighter:usePasSkill(pasType, unit)
	...
	if self:isSeal() and pasType ~= xyd.TriggerType.FOREVER and pasType ~= xyd.TriggerType.SELF_DIE and pasType ~= xyd.TriggerType.ENERGY_SKILL_HOLD then
		local tmpSkills = {}
	...
	if self:isForceSeal() and pasType ~= xyd.TriggerType.SELF_DIE then
		return
	end
	...
	local limitRoundTimes = BattleTriggerTable:getRoundLimitTimes(pasType)

	if limitRoundTimes == 0 then
		limitRoundTimes = 99
	end

	local key = self:getPos() .. "_" .. pasType

	if xyd.Battle.roundTriggerTimes[key] then
		xyd.Battle.roundTriggerTimes[key] = xyd.Battle.roundTriggerTimes[key] + 1
	else
		xyd.Battle.roundTriggerTimes[key] = 1
	end

	if limitRoundTimes < xyd.Battle.roundTriggerTimes[key] then
		if limitRoundTimes > 99 then
			table.insert(xyd.Battle.overRoundKey, xyd.Battle.round .. "_" .. key)
		end
	...
	for _, skillID in ipairs(skillIDs) do
		...
		elseif pasType == xyd.TriggerType.MASTER_ATTACK or pasType == xyd.TriggerType.SELF_ATTACKED or pasType == xyd.TriggerType.SELF_CRITED then
			local isJump = SkillTable:jump(skillID)
...
function ReportBaseFighter:useOnePasSkill(pasType, skillID, unit)
	...
	if isJump >= 1 and not self:isDeath() then
		-- block empty
	else
		...
		for _, data in ipairs(datas) do
			...
			if not target:isDeath() then
				local harm, cure, isCrit, recordBuffs, isHarm = target:applyBuffHarmsByTarget(data, specialUnit)
...
function ReportBaseFighter:useOneJumpPasSkill(pasType, skillID, unit, ifNoPas, ifNoBlock, needMiss)
...
function ReportBaseFighter:useDieSkill(skillID, unit)
	...
	for _, data in ipairs(datas) do
		local target = data.target
		local hitBuffs = data.hitBuffs
		local harm, cure, recordBuffs, status = target:applyBuffHarm(specialUnit, hitBuffs)
...
function ReportBaseFighter:reborn(unit, isApateReborn)
	...
	local reviveBuff = self.isRevive_[self.hasReviveTimes_]

	reviveBuff = reviveBuff or self.isRevive_[#self.isRevive_]

	if reviveBuff and reviveBuff.effectID and EffectTable:round(reviveBuff.effectID) > 1 then
		self.hasReviveTimes_ = 0
		self.noReviveDie_ = false
	end
...
function ReportBaseFighter:applyBuffHarm(unit, hitBuffs, noRecord, noTrigger)
	local harm = 0
	local shield = 0
	local cure = 0
	local isCrit = false
	...
	for i = 1, #hitBuffs do
		...
		local originHarm = harm
		local originShield = shield
		...
		if (buff:isDamage() or buff:getName() == xyd.BUFF_OVER_FLOW or buff:getName() == xyd.BUFF_SUCK_REAL_BLOOD) and not self:isDeath() then
			...
			if buff:getName() ~= xyd.BUFF_REAL_HURT and buff:getName() ~= xyd.BUFF_DIRECT_HURT and buff:getName() ~= xyd.BUFF_PLMXS_THORNS_HURT then
				addHarmRate = addHarmRate - baseAddHarmRate + 1 + self:getExtraHarmRate(unit, false, buff.fighter)
				num = num * addHarmRate
			end

			local shareBuff = self:getOneBuffByName(xyd.BUFF_HURT_SHARE)
			...
			num = self:targetShareHarm(unit, recordBuffs, buff, num)

			local shieldResult = self:getShieldHarm(num, unit, nil, nil, nil, nil, buff)

			num = shieldResult.result
			...
			if not buff.noAddHarm then
				harm = harm - num
				shield = shield - shieldResult.shield
			end
			...
			if buff:isCrit() then
				...
				local critAddHurtMark = buff.fighter:getOneBuffByName(xyd.BUFF_CRIT_ADD_HURT_MARK)

				if critAddHurtMark and buff:getName() ~= xyd.BUFF_DIRECT_HURT then
					critAddHurtMark:addHurt(unit, num, buff.target, recordBuffs)
				end
			end
		...
		elseif buff:getName() == xyd.BUFF_ROUND_REFLECT_HURT then
			...
			local shieldResult = self:getShieldHarm(num, unit)
		...
		elseif buff:getName() == xyd.BUFF_INJURED then
			...
			status.isHarm = true
		elseif (buff:getName() == xyd.BUFF_HURT_MAX_H or buff:getName() == xyd.BUFF_HURT_MAX_M or buff:getName() == xyd.BUFF_HURT_MAX_H_LIMIT15 or buff:getName() == xyd.BUFF_HURT_MAX_H_LIMIT or buff:getName() == xyd.BUFF_HURT_MAX_H_LIMIT25 or buff:getName() == xyd.BUFF_HURT_DMG_H_LIMIT15 or buff:getName() == xyd.BUFF_HURT_NOW_H_LIMIT15 or buff:getName() == xyd.BUFF_HURT_MAX_H_LIMIT15_BLOOD or buff:getName() == xyd.BUFF_HURT_MAX_H_LIMIT15_TEAR or buff:getName() == xyd.BUFF_HURT_SELF_HP_LOSE or buff:getName() == xyd.BUFF_REAL_HURT_HP_LIMIT or buff:getName() == xyd.BUFF_HURT_MAX_HP) and not self:isDeath() then
			local specialAddHarmRate = 1 + self:getExtraHarmRate(unit, true, buff.fighter)
			...
			if buff:getName() == xyd.BUFF_REAL_HURT_HP_LIMIT then
				specialAddHarmRate = 1
			end

			num = num * specialAddHarmRate
			num = self:targetShareHarm(unit, recordBuffs, buff, num)

			local shieldResult = self:getShieldHarm(num, unit, true, nil, nil, nil, buff)
		...
		elseif buff:getName() == xyd.BUFF_ENERGY_HURT then
			...
			local shieldResult = self:getShieldHarm(num, unit, false)
		...
		elseif buff:getName() == xyd.BUFF_FIRE_EXPLOADED then
			...
			self:updateHpByHarm(allHarm, unit, false, false, false, buff)
			saveAndDelBuff(buff, xyd.BUFF_WORK)
		...
		elseif buff:isDot() and not self:isDeath() then
			num = num * self:getDotChangeRate(buff.fighter)
			...
			if buff.fighter:isDeath() or buff:getName() == xyd.BUFF_DOT_TWINS then
				buff:delCount()
			end
		...
		elseif buff:getName() == xyd.BUFF_DEBUFF_CLEAN then
			self:cleanMultiDebuff(unit, recordBuffs, buff.finalNumArray_[1])
			saveAndDelBuff(buff, xyd.BUFF_ON_WORK)
		...
		elseif buff:getName() == xyd.BUFF_CONTROL_REMOVE then
			saveAndDelBuff(buff, xyd.BUFF_ON_WORK)
		elseif buff:isControlNew() and not self:isDeath() and buff:getName() ~= xyd.BUFF_RIDICULE and buff:getName() ~= xyd.BUFF_TELEISHA_SEAL and buff:getName() ~= xyd.BUFF_AILISHE_COLOR123 and buff:getName() ~= xyd.BUFF_ALL_TARGET_CHANGE then
		...
		elseif buff:getName() == xyd.BUFF_REDUCE_SPD then
			if tmpReduceSpd < 5 then
				...
				for _, buff_ in ipairs(self.isReduceSpd_) do
					if minNum > buff_:getFinalNum() then
		...
		elseif not self:isDeath() or buff:getName() == xyd.BUFF_REVIVE and not self.noReviveDie_ then
			...
			local buffType = BuffTable:getBuffType(buff:getName())

			if buffType == 6 then
				saveAndDelBuff(buff, xyd.BUFF_WORK)
			elseif buff:hasFx() then
				saveAndDelBuff(buff, xyd.BUFF_ON_WORK)
			else
				saveAndDelBuff(buff, xyd.BUFF_ON)
			end
		...
		if buff.realHarm_ and buff.realHarm_ > 0 then
			harm = harm + buff.realHarm_
		end
	...
	local originHitBuffs = {}

	for i = #hitBuffs, 1, -1 do
		local buff = hitBuffs[i]

		table.insert(originHitBuffs, buff)
	...
	if not self:isDeath() and not unit:isPasSkill() and unit.fighter and not unit.fighter:isPet() and not noRecord and self:isGetAbsorbShield() and unit.totalAttackHarm_[self:getPos()] < 0 then
		...
		newBuff.finalNum_ = self:getLostSealLimit(newBuff.finalNum_)
		unit.totalAttackHarm_[self:getPos()] = 0

		self:updateHpByHeal(newBuff:getFinalNum(), unit, noTrigger)
	...
	if status.isCrit and next(self.cImpresses_) and not self:isDeath() then
		for i = #self.cImpresses_, 1, -1 do
			...
			local shieldResult = self:getShieldHarm(num, unit, false, true, buff.fighter)
	...
	for k, v in ipairs(originHitBuffs) do
		v:excuteBuffAfterRecord(unit)
	end
	...
	return harm, cure, recordBuffs, status, inHarmTargetList, shield
end

function ReportBaseFighter:targetShareHarm(unit, recordBuffs, buff, num)
	local shareOnceBuff = self:getBuffsByNameAndFighter(xyd.BUFF_HARM_SHARE_ONCE)
	local shareHurtBuff = self:getOneBuffByName(xyd.BUFF_SHARE_BY_HURT)

	if next(shareOnceBuff) then
		num = shareOnceBuff[1]:shareHarm(unit, recordBuffs, num, buff)

		buff:setRecordNum(num)
	elseif shareHurtBuff then
		num = shareHurtBuff:shareHarm(unit, recordBuffs, num, buff)
...
function ReportBaseFighter:getResistEnergy(num)
	local resists = self:getBuffsByNameAndFighter(xyd.BUFF_ENERGY_RESIST)
	...
	local luobiMarks = self:getBuffsByNameAndFighter(xyd.BUFF_LUOBI_MARK)
...
function ReportBaseFighter:checkFreeHarm(unit, value)
	local roundFreeHarmBuff = self:getBuffsByNameAndFighter(xyd.BUFF_ROUND_FREE_HARM)

	if next(roundFreeHarmBuff) or self.pre_round_free_round == xyd.Battle.round then
		self:checkLeft1(unit)
...
function ReportBaseFighter:checkLeft1(unit)
	local left1Buff = self:getOneBuffByName(xyd.BUFF_FREE_HARM_LEFT1)

	if self:getHp() > 1 and left1Buff then
		self:setHp(1)
		left1Buff:writeRecord(self, xyd.BUFF_REMOVE)
		unit:recordBuffs(self, {
			left1Buff
		})
		self:removeBuffs({
...
function ReportBaseFighter:delFreeHarmBuffCount(recordBuffs, unit, noCheckUnit)
	if self:checkFreeHarm(unit) then
		return
	end

	local shieldBuff = self:getOneBuffByName(xyd.BUFF_SHIELD)

	if shieldBuff then
		shieldBuff:checkLeftTimes(unit)
...
function ReportBaseFighter:isControlBuff(name)
...
function ReportBaseFighter:clearControlBuff(unit)
	...
	for i = #buffRecords, 1, -1 do
		...
		if record.pos == self:getPos() and self:isControlBuff(record.name) then
...
function ReportBaseFighter:updateHpByHarm(harm, unit, isImpress, isDot, cannotReflect, harmBuff)
	...
	if harm < 0 then
		...
		if isDot and harmFighter then
			xyd.Battle.lastDotHurtTarget = self

			harmFighter:usePasSkill(xyd.TriggerType.SELF_DOT_HURT, unit)
		end
	end

	if harmFighter then
		local hurt3AllDmg = harmFighter:getOneBuffByName(xyd.BUFF_HURT_3_ALL_DMG)

		if hurt3AllDmg then
			hurt3AllDmg:addTimes()
		end
	end
	...
	if self:checkFreeHarm(unit, harm) then
		return
	end

	local qianheziDecBuff = self:getBuffsByNameAndFighter(xyd.BUFF_QIANHEZI_DEC)

	if next(qianheziDecBuff) then
		qianheziDecBuff[1]:recordHarmNum(unit, harm)
	...
	if nextHp <= 0 and not self.hasDieHit_ then
		...
		if not self.firstDieHit and xyd.Battle.god then
			self.firstDieHit = 1

			xyd.Battle.god:usePasSkill(xyd.TriggerType.FIRST_DIE_HIT, unit)

			self.firstDieHit = 2
		end
	end

	qianheziDecBuff = self:getBuffsByNameAndFighter(xyd.BUFF_QIANHEZI_DEC)

	if self:checkFreeHarm(unit, harm) or next(qianheziDecBuff) then
		return
	end
	...
	if not cannotReflect and (not harmBuff or not harmBuff.isCopy) then
	...
	if f then
		if -harm > self:getHp() then
			f.hpHurt = f.hpHurt - self:getHp()
		else
			f.hpHurt = f.hpHurt + harm
		end
	end
	...
	if not self:isDeath() and harm < 0 then
		if xyd.arrayIndexOf(self.hurtedUnits_, unit) <= 0 then
			...
			if not harmBuff or harmBuff:getName() ~= xyd.BUFF_HEAL_CURSE then
				...
				if unit.fighter and unit.fighter:getPos() <= 12 and self:getHpPercent() > unit.fighter:getHpPercent() then
		...
		if not harmBuff or harmBuff:getName() ~= xyd.BUFF_HEAL_CURSE then
			if next(self:getBuffsByNameAndFighter(xyd.BUFF_PLMXS_THORNS)) then
				self:getOneBuffByName(xyd.BUFF_PLMXS_THORNS):hitted(unit, harm, harmBuff, f)
			end

			if next(self:getBuffsByNameAndFighter(xyd.BUFF_HARM_TO_HEAL)) then
				self:getOneBuffByName(xyd.BUFF_HARM_TO_HEAL):hitted(unit, harm, harmBuff, f)
			end

			if next(self:getBuffsByNameAndFighter(xyd.BUFF_LIANHUA_HITTED_HEAL)) then
				self:getOneBuffByName(xyd.BUFF_LIANHUA_HITTED_HEAL):hitted(unit, harm, harmBuff, f)
			end

			if f and next(f:getBuffsByNameAndFighter(xyd.BUFF_ANSUNA_TO_SHIELD)) then
				f:getOneBuffByName(xyd.BUFF_ANSUNA_TO_SHIELD):hitted(unit, harm, harmBuff, f)
			end
		end
	end
end
...
function ReportBaseFighter:initHurtByReceive(harm, unit)
	...
	if target then
		num = target:getShieldHarm(num * harm, unit, false, false, self).result
...
function ReportBaseFighter:updateHpByHeal(value, unit, noTrigger)
	local overflowHp = self:getHp() + value - self:getHpLimit()

	self:updateHpDirect(value, unit)

	local shieldFromHealBuffs = self:getBuffsByNameAndFighter(xyd.BUFF_HP_SHIELD_FROM_HEAL)

	for k, v in ipairs(shieldFromHealBuffs) do
		v:excuteBuffByHeal(unit, value)
	end

	local qiubitePowerBuffs = self:getBuffsByNameAndFighter(xyd.BUFF_QIUBITE_POWER)

	for k, v in ipairs(qiubitePowerBuffs) do
		v:excuteBuffByHeal(unit, overflowHp)
	end

	if self:getTeamType() == xyd.TeamType.A then
		if not xyd.Battle.purposes.healTimes then
			xyd.Battle.purposes.healTimes = 0
		end

		xyd.Battle.purposes.healTimes = xyd.Battle.purposes.healTimes + 1
	end

	if not noTrigger then
		local qianheziHealBuffs = self:getBuffsByNameAndFighter(xyd.BUFF_QIANHEZI_HEAL)

		for k, v in ipairs(qianheziHealBuffs) do
			v:cureByBuff(unit, value)
		end
		...
		if value > 0 then
			self:usePasSkill(xyd.TriggerType.SELF_HEAL_ONCE, unit)
		end
	end
end
...
function ReportBaseFighter:getPugongID()
	...
	local buff = self:getOneBuffByName("changeCombat", true)
...
function ReportBaseFighter:isFreeHarm()
	if next(self:getBuffsByNameAndFighter(xyd.BUFF_SHIELD)) then
		return true
	end
...
function ReportBaseFighter:getAD(params)
	...
	extraAdd = extraAdd + self:getBuffsTotalFinalNumArray(xyd.BUFF_ATK_PMAX_TIMES, 1)
	extraAdd = extraAdd + self:getBuffsTotalFinalNumArray(xyd.BUFF_XIERKEDI_POWER, 1)
	extraAdd = extraAdd + self:getBuffsTotalFinalNumArray(xyd.BUFF_XINGMANG, 1)
	extraAdd = extraAdd - self:getBuffsTotalFinalNumArray(xyd.BUFF_AILISHE_COLOR3, 1)

	if self.shieldHp_ > 0 then
		extraAdd = extraAdd + self:getBuffsTotalFinalNumArray(xyd.BUFF_QIUBITE_POWER, 3)
	end
	...
	local godControlEnergyBuffs = self:getBuffsByNameAndFighter(xyd.BUFF_GOD_CONTROL_ENERGY)

	if next(godControlEnergyBuffs) and self:getEnergy() > 50 then
		final = final * 0.8
	end
	...
	local atkPHpMaxBuffsNum = self:getBuffsTotalFinalNumArray(xyd.BUFF_ATKP_BY_HP_MAX, 1)

	if atkPHpMaxBuffsNum > 0 then
		final = final + atkPHpMaxBuffsNum * self:getHpLimit()
	end
	...
	local deathLayerBuff = self:getOneBuffByName(xyd.BUFF_DEATH_LAYER)

	if deathLayerBuff then
		final = final * (1 + deathLayerBuff:calculateAD())
	end

	if xyd.Battle.god and (not params or not params.exceptEnemySteal) then
		local enemyStealAtkBuff = xyd.Battle.god:getBuffsByNameAndFighter(xyd.BUFF_ENEMY_STEAL_ATK)
	...
	final = final * (1 - self:getBuffsTotalFinalNumArray(xyd.BUFF_QIANHEZI_HEAL, 2))
...
function ReportBaseFighter:getAllHarmDec()
	...
	self:getHarmRateDump(xyd.BUFF_ALL_HARM_DEC, true)
	...
	if next(feisinaWeak) then
		...
		self:getHarmRateDump(feisinaWeak, true)
	...
	if next(magicShoot) then
		...
		self:getHarmRateDump(magicShoot, true)
	...
	if next(allHarmDecDec) then
		...
		self:getHarmRateDump(allHarmDecDec, true)
	...
	if next(luobiHp) then
		self:getHarmRateDump(luobiHp[1])
	...
	if next(snowWindMixs) then
		num = num + snowWindMixs[1]:getAllHarmFreeRate()

		self:getHarmRateDump(snowWindMixs[1])
	end

	if self.shieldHp_ > 0 then
		num = num + self:getBuffsTotalFinalNumArray(xyd.BUFF_QIUBITE_POWER, 2)

		self:getHarmRateDump(xyd.BUFF_QIUBITE_POWER, true)
	end

	local hittedBuff = self:getOneBuffByName(xyd.BUFF_HITTED_HARDER)

	if hittedBuff then
		num = num + hittedBuff.addFreeHarm
	end

	num = num + self:getBuffsTotalFinalNumArray(xyd.BUFF_LONGNV_DEAD, 2)
	num = math_max(0, num)
...
function ReportBaseFighter:getHujia()
	...
	rate = rate - self:getBuffsTotalFinalNumArray(xyd.BUFF_AILISHE_COLOR2, 1)
...
function ReportBaseFighter:getShanBi()
	...
	shanbi = shanbi - self:getBuffsTotalFinalNumArray(xyd.BUFF_PLMXS_DEBUFF, 3)
	shanbi = shanbi - self:getBuffsTotalFinalNumArray(xyd.BUFF_LONGNV_DEBUFF, 2)
	shanbi = shanbi - self:getBuffsTotalFinalNumArray(xyd.BUFF_BLIND, 2)
...
function ReportBaseFighter:getMingZhong(attacker, defender)
	...
	extraAdd = extraAdd + self:getBuffsTotalFinalNumArray(xyd.BUFF_XIGEMA_POWER, 2)
	extraAdd = extraAdd + self:getBuffsTotalFinalNumArray(xyd.BUFF_XIERKEDI_POWER, 3)
	extraAdd = extraAdd + self:getBuffsTotalFinalNumArray(xyd.BUFF_XINGMANG, 2)
	extraAdd = extraAdd - self:getBuffsTotalFinalNumArray(xyd.BUFF_BLIND, 1)
	num = num + extraAdd
	num = math_max(num, 0)
...
function ReportBaseFighter:getCrit(defender, theBuff)
	local extraAdd = 0
	local trueVampireBuffs = self:getBuffsByNameAndFighter(xyd.BUFF_TRUE_VAMPIRE)
	...
	extraAdd = extraAdd + self:getBuffsTotalFinalNumArray(xyd.BUFF_XIERKEDI_POWER, 2)
	extraAdd = extraAdd - self:getBuffsTotalFinalNumArray(xyd.BUFF_LONGNV_DEBUFF, 1)

	local crit = self:getAttrByType(xyd.BUFF_CRIT)

	crit = crit + extraAdd

	if theBuff and theBuff.skillID and theBuff.skillID > 0 and self:isPugongSkill(theBuff.skillID) then
		local pugongCrits = self:getBuffsByNameAndFighter(xyd.BUFF_HURT_B_CRIT)

		for k, v in ipairs(pugongCrits) do
...
function ReportBaseFighter:getCritTimeLimit()
	local result = xyd.CRIT_TIME_BASE

	result = result + self:getAttrByType(xyd.BUFF_CRIT_TIME_LIMIT)
...
function ReportBaseFighter:getCritLimit(defender)
	local result = 1

	result = result + self:getBuffTotalNumByGroup(self.isCritTimeLimit_)
	result = result - defender:getBuffsTotalFinalNumArray(xyd.BUFF_AILISHE_ROUND3, 2)
...
function ReportBaseFighter:getCritTime(defender)
	...
	local critAddTimes = self:getBuffsByNameAndFighter(xyd.BUFF_CRIT_ADD_CRIT_TIME)

	for k, v in ipairs(critAddTimes) do
		multiExRate = multiExRate + v.finalNumArray_[1] * self:getCrit(defender)
	end

	if next(self:getBuffsByNameAndFighter(xyd.BUFF_CRIT_OVERFLOW)) then
		local overflowNum = self:getCrit(defender) - 1

		if overflowNum > 0 then
			multiExRate = multiExRate + overflowNum * 100 * self:getBuffsTotalFinalNumArray(xyd.BUFF_CRIT_OVERFLOW, 1)
		end
	end
	...
	local critTimeLimit = self:getCritTimeLimit()

	crit = crit + multiExRate
	crit = critTimeLimit < crit and critTimeLimit or crit
...
function ReportBaseFighter:getHealB()
	...
	num = num - self:getBuffsTotalFinalNumArray(xyd.BUFF_HYPOXIA, 1)

	local snowWindMixs = self:getBuffsByNameAndFighter(xyd.BUFF_SNOW_WIND_MIX)

	if next(snowWindMixs) then
		num = num + snowWindMixs[1]:getHealBRate()
	end
	...
	if next(mayaHpSeals) then
		...
		if mayaHpSealNum > mayaHpSeals[1].finalNumArray_[4] then
	...
	local hpPercentHealBBuff = self:getBuffsByNameAndFighter(xyd.BUFF_HP_PERCENT_HEAL_B)

	if next(hpPercentHealBBuff) then
		local addNum = hpPercentHealBBuff[1]:getHealRate()

		num = num + addNum
	end

	num = num - self:getBuffsTotalFinalNumArray(xyd.BUFF_PLMXS_DEBUFF, 1)
	num = num - self:getBuffsTotalFinalNumArray(xyd.BUFF_AILISHE_COLOR13, 1)
	num = num - self:getBuffsTotalFinalNumArray(xyd.BUFF_HADISI_DEBUFF, 1)
	num = num - self:getBuffsTotalFinalNumArray(xyd.BUFF_SHUAIRUO, 1)
...
function ReportBaseFighter:getJianshang()
	...
	local critDefToDec = self:getOneBuffByName(xyd.BUFF_CRIT_DEF_TO_DEC_DMG)

	if critDefToDec then
		rate = rate + self:getUnCrit() * critDefToDec.finalNumArray_[1]
	end
...
function ReportBaseFighter:getOImpress()
	...
	totalNum = totalNum > 3 and 3 or totalNum
	totalNum = totalNum + self:getBuffsTotalFinalNumArray(xyd.BUFF_NINGSHI, 1)
...
function ReportBaseFighter:getTrueAtk()
	local extraAdd = self:getBuffsTotalFinalNumArray(xyd.BUFF_XINGMANG, 3)
	local extraAdd = self:getBuffsTotalFinalNumArray(xyd.BUFF_LONGNV_DEAD, 1)
	local extraAdd = self:getBuffsTotalFinalNumArray(xyd.BUFF_ZHOUSI_TRUE_ATK, 1)
	local result = self:getAttrByType(xyd.BUFF_TRUE_ATK) + extraAdd
...
function ReportBaseFighter:getFree()
	...
	exRate = exRate + self:getBuffsTotalFinalNumArray(xyd.BUFF_XIGEMA_POWER, 1)
	exRate = exRate - self:getBuffsTotalFinalNumArray(xyd.BUFF_BLIND, 3)
...
function ReportBaseFighter:getUnFree()
	...
	local unfreeRecieveBuffs = self:getBuffsByNameAndFighter(xyd.BUFF_UNFREE_RECIEVE)

	for k, v in ipairs(unfreeRecieveBuffs) do
		rate = rate + v.rate
	end

	rate = rate + self:getBuffsTotalFinalNumArray(xyd.BUFF_AILISHE_ROUND3, 1)
...
function ReportBaseFighter:getDotB()
	local rate = self:getBuffTotalNumByGroup(self.isDotB_)

	rate = rate + self:getBuffsTotalFinalNumArray(xyd.BUFF_HYPOXIA, 3)

	local ansunaDot = self:getOneBuffByName(xyd.BUFF_ANSUNA_DOT)

	if ansunaDot then
		rate = rate + ansunaDot.finalNumArray_[1]
	end

	return rate
end

function ReportBaseFighter:getDotChangeRate(fighter)
	local result = 1

	result = result * (1 + self:getTearRate() + self:getDotB()) * fighter:getDotRate()

	return result
end

function ReportBaseFighter:getDotRate()
	local num = 1 + self:getBuffsTotalFinalNumArray(xyd.BUFF_DOT_RATE, 1)
...
function ReportBaseFighter:canReborn(needCheckLeftCount)
	...
	if next(self.isRevive_) then
		if needCheckLeftCount then
			local rebornNum = 0

			for k, v in ipairs(self.isRevive_) do
				if v.leftCount_ <= 1 then
					rebornNum = rebornNum + 1
				end
			end

			if rebornNum > self.hasReviveTimes_ then
				return true
			end
		else
			if self.isLongnv then
				return false
			end
...
function ReportBaseFighter:getBuffsTotalFinalNumArray(name, index)
	local num = 0
	local buffs = self:getBuffListByName(name)
	local result = {}

	if next(buffs) then
		for _, buff in ipairs(buffs) do
			num = num + buff.finalNumArray_[index]
		end
	end
...
function ReportBaseFighter:getOneBuffByName(name)
	local buffs = self:getBuffsByNameAndFighter(name)
...
function ReportBaseFighter:addBuffs(buffs, unit)
	...
	for i = 1, #buffs do
		...
		elseif buff:isAddHpLimit() then
			local limit = self:getHpLimit(true)
			local num = buff:getFinalNum()

			if #buff.finalNumArray_ > 1 then
				self:changeHpLimit(limit * num, false)
			else
				self:changeHpLimit(limit * (1 + num), true)
			end
		...
		elseif type_ == xyd.BUFF_CRIT_TIME_MAX3 then
		...
		elseif type_ == xyd.BUFF_YX_CONTROL_REMOVE then
			...
			if #self.isYxControlRemove_ >= self.isYxControlRemove_[1]:getFinalNum() then
				local recordBuffs = {}
		...
		elseif type_ == xyd.BUFF_REVIVE or type_ == xyd.BUFF_REVIVE_INF then
			if not next(self.isApateRevive_) then
				if buff.leftCount_ > 1 then
					self.isRevive_ = {}
					self.isLongnv = true

					table.insert(self.isRevive_, buff)
				elseif not self.isLongnv then
					table.insert(self.isRevive_, buff)
				end
			end
		...
		elseif type_ == xyd.BUFF_CRIT_LIMIT then
		...
		elseif type_ == xyd.BUFF_GET_LIGHT or type_ == xyd.BUFF_ADD_GET_LIGHT then
			...
			if #self.isGetLight_ >= 2 and unit then
				...
				unit:recordPasBuffs(self, self.isGetLight_)
		...
		if unit and xyd.Battle.hasAllInited then
			...
			if buff:isDot() and buff:getName() ~= xyd.BUFF_FEISINA_EXPLODE then
				if not buff.isCopy then
					self:addCopyDot(buff, unit)
				end

				self:usePasSkill(xyd.TriggerType.SELF_DOT, unit)

				if buff.fighter and not buff.isCopy then
					lastAddDot = buff
				end
			end
			...
			if (buff:getName() == xyd.BUFF_M_DOT_BLOOD or buff:getName() == xyd.BUFF_DOT_BLOOD) and buff.target:getOneBuffByName(xyd.BUFF_LIANHUA_BLOOD_HURT) then
				buff.target:getOneBuffByName(xyd.BUFF_LIANHUA_BLOOD_HURT):blooded(unit, buff)
			end
		end
	end

	if lastAddDot and SkillTable:trigger(unit.skillID) ~= xyd.TriggerType.ADD_DOT then
		lastAddDot.fighter.lastAddDot = lastAddDot

		lastAddDot.fighter:usePasSkill(xyd.TriggerType.ADD_DOT, unit)
	end
end

function ReportBaseFighter:addCopyDot(buff_, unit)
	local dotDoubleBuffs = buff_.fighter:getBuffsByNameAndFighter(xyd.BUFF_DOT_DOUBLE)

	if not next(dotDoubleBuffs) then
		return
	end
	...
	local params = {
		...
		target = buff_.target,
		skillID = buff_.skillID
	}
	local buff = BuffManager:newBuff(params)

	buff.isCopy = true

	buff:setLeftCount(dotDoubleBuffs[1].finalNumArray_[1])
	buff:setIsHit()

	local harm, cure = buff.target:applyBuffHarm(unit, {
		buff
	})

	buff.fighter:recordData(harm, cure)
end
...
function ReportBaseFighter:clearAttrCache(name)
	...
	if name == xyd.BUFF_EXCHANGE_SPD or name == xyd.BUFF_SPD_STEAL or name == xyd.BUFF_REDUCE_SPD or name == xyd.BUFF_XIFENG_SPD or name == xyd.BUFF_AILISHE_COLOR1 or name == xyd.BUFF_DEATH_SPD then
		self.___ackSpeed = nil
		xyd.Battle.ackSpeedFlag = false
	end
end
				...
				if next(selectDBuffs) then
					for _, buff in ipairs(selectDBuffs) do
						...
						if target then
							...
							elseif not newBuff:isHit() and newBuff.isFree_ then
...
function ReportBaseFighter:removeBuffs(buffs, noRemoves, unit)
	...
	for i = #buffs, 1, -1 do
		...
		if buff and xyd.arrayIndexOf(noRemoves, buff:getName()) < 0 then
			...
			if unit then
				if type_ == xyd.BUFF_WULIEER_SEAL or type_ == xyd.BUFF_QIANHEZI_DEC or type_ == xyd.BUFF_SOUL_SEAL then
					table.insert(afterRemoves, buff)
				end

				if buff:getCount() > 0 and not self:isDeath() and buff:isDebuff() then
					self:usePasSkill(xyd.TriggerType.CLEAN_DEBUFF, unit)
				end
			end
	...
	for k, v in ipairs(afterRemoves) do
		v:excuteBuffByRemove(unit)
	end
end
...
function ReportBaseFighter:checkEnergySkill()
	...
	elseif self:isPugongOnly() then
		return false
	elseif self.isPassiveQuene_ == xyd.Battle.round then
		return false
...
function ReportBaseFighter:updateBuffCount(unit)
	...
	self:usePasSkill(xyd.TriggerType.ROUND_BEGIN_ALL, unit)
	...
	if self.roundAction ~= xyd.FighterAction.SKILL then
		self:usePasSkill(xyd.TriggerType.SELF_ROUND_END_NO_SKILL, unit)
	end

	if self.shieldHp_ > 0 then
		self:usePasSkill(xyd.TriggerType.ROUND_END_OWN_HPSHIELD, unit)
	end
	...
	if self:isDecHurtLess() then
		...
		for _, buff in ipairs(self.isDecHurt_) do
			if buff and minRate > buff:getFinalNum() then
	...
	for i = #self.isDot_, 1, -1 do
		...
		num = num * self:getDotChangeRate(buff.fighter)
		num = num * addHarmRate
		num = self:getShieldHarm(num, unit, false, false, buff.fighter, nil, buff).result
	...
	for i = #self.rImpresses_, 1, -1 do
		...
		if buff:getCount() <= 0 then
			...
			num = num * addHarmRate
			num = self:getShieldHarm(num, unit, false, true, buff.fighter).result
	...
	local disappearedMarkCrystalBuff
...
function ReportBaseFighter:checkRoundEndRemoveControlBuff(unit)
	local recordBuffs = {}
	local buffControlRemove = self:getOneBuffByName(xyd.BUFF_CONTROL_REMOVE)
	local buffOutBreak = self:getOneBuffByName(xyd.BUFF_CONTROL_REMOVE_YUNMU)
	local cleanBuffs = {}
...
function ReportBaseFighter:updateRebornBuff(unit)
	...
	if self.hasReviveTimes_ < #self.isRevive_ then
		...
		if buff.longnvBuff then
			buff.longnvBuff:afterReborn(unit)
		end
...
function ReportBaseFighter:initFakeHp()
	self.fakeHpLimit_ = 0
...
function ReportBaseFighter:getLowestCountBuffByName(buffName)
	local buffs = self:getBuffListByName(buffName)
	local resultBuff

	if next(buffs) then
		for _, buff in ipairs(buffs) do
			if not resultBuff then
				resultBuff = buff
			elseif buff:getCount() < resultBuff:getCount() then
				resultBuff = buff
			end
		end
	end
...
function ReportBaseFighter:getPetExBuffHarmRate(fighter)
	...
	if fighter:isPet() then
		rate = fighter:getBuffsTotalFinalNumArray(xyd.BUFF_PET_DMG_B, 1)
	end
