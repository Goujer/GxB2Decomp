-- chunkname: @../../../Product/Bundles/Android/src/app/common/tables/ActivityFreeRevertTable.lua

local ActivityFreeRevertTable = class("ActivityFreeRevertTable", import(".BaseTable"))
local slotModel = xyd.models.slot

function ActivityFreeRevertTable:ctor()
	ActivityFreeRevertTable.super.ctor(self, "activity_free_revert")
end

function ActivityFreeRevertTable:getReturnPartner(partnerID)
	local partner = slotModel:getPartner(partnerID)

	return self:getReturnPartnerByPartner(partner)
end

function ActivityFreeRevertTable:getReturnPartnerInfo(partnerID)
	local partner = slotModel:getPartner(partnerID)
...
function ActivityFreeRevertTable:getReturnPartnerByPartner(partner)
...
function ActivityFreeRevertTable:getCost(star)
	return self:split2Cost(star, "cost", "#")
end

function ActivityFreeRevertTable:getAllItems(partnerID)
	local partner = slotModel:getPartner(partnerID)

	return self:getAllItemsByPartner(partner)
end

function ActivityFreeRevertTable:getAllItemsByPartner(partner, returnAllItem)
	local items = {}
	local partner = partner
	local star = partner:getStar()
	local group = partner:getGroup()
...
function ActivityFreeRevertTable:getCrystalByPartnerID(partnerID)
	local partner = slotModel:getPartner(partnerID)
...
function ActivityFreeRevertTable:getCrystalByPartner(partner)
	local id = partner:getStar()

	return self:getNumber(id, "crystal")
end

return ActivityFreeRevertTable
