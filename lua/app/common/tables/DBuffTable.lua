-- chunkname: @../../../Product/Bundles/Android/src/app/common/tables/DBuffTable.lua
...
function DBuffTable:ctor()
	...
	self.dmgs = {
		...
		xyd.BUFF_REAL_HURT,
		xyd.BUFF_DIRECT_HURT,
		xyd.BUFF_TARGET_HURT,
		xyd.BUFF_HURT_FREE_SHIELD
	}
	self.dmgRestarints = {
		xyd.BUFF_HURT,
		xyd.BUFF_TRUE_HURT,
...
function DBuffTable:notShowAttr(name)
	return self:getNumber(name, "not_show_attr") == 1
end
...
function DBuffTable:translationDesc(str, needFloor, twoF)
	local name = str[1]
	local val = str[2]
	...
	if self:isShowPercent(name) then
		...
		if twoF then
			str = string.format("%.2f", val * 100 / factor)
		end

		desc = tostring(desc) .. str .. "%"
	elseif needFloor then
		desc = tostring(desc) .. tostring(math.floor(val))
	else
...
function DBuffTable:getUseOriginName(name)
	return self:getNumber(name, "use_origin_name") or 0
end
