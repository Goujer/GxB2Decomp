-- chunkname: @../../../Product/Bundles/Android/src/app/common/tables/PartnerTable.lua
...
function PartnerTable:ctor()
	...
	local forgeNineStarList = {}
	local forgeTenStarList = {}
	...
	for _, id in pairs(self.TableLua.ids) do
		...
		if showInForge >= 1 then
			...
			if not forgeNineStarList[tostring(group)] then
				forgeNineStarList[tostring(group)] = {}
			end

			if not forgeTenStarList[tostring(group)] then
				forgeTenStarList[tostring(group)] = {}
			end
			...
			elseif star == 6 then
				...
				table.insert(forgeNineStarList[tostring(group)], id)
			elseif star == 10 then
				table.insert(forgeTenStarList[tostring(group)], id)
			end
		end
	...
	for key, _ in pairs(forgeFiveStarList) do
		...
		for i, _ in pairs(forgeNineStarList[key]) do
			table.insert(self.forgeList[key], forgeNineStarList[key][i])
		end

		for i, _ in pairs(forgeTenStarList[key]) do
			table.insert(self.forgeList[key], forgeTenStarList[key][i])
		end
	end
...
function PartnerTable:getStarOrigin(id)
	return self:getNumber(id, "star_origin") or 0
end
...
function PartnerTable:getMaxStar(id)
	local star = self:getStar(id)
	local awake = #(xyd.split(self:getAwakeMaterial(id), "|") or {})

	__TRACE(id)

	local maxStar = star + awake
...
function PartnerTable:getClickSoundNum(id, skin_id)
	local list = self:getDialogList(id)
	local type2 = xyd.PartnerToSoundTableKey.click_sound
	local skinIndex = self:getSkinIndex(id, skin_id)
	local sound_id = xyd.tables.partnerDialogTable:getIdsInList(list, type2, skinIndex)
...
function PartnerTable:getClickDialogInfo(id, index, skin_id, lovePoint)
	if index == nil then
		index = 1
	end
	...
	local time = xyd.tables.soundTable:getLength(sound)
	local dialog = xyd.tables.partnerDialogTextTable:getText(partnerSoundId)
	local clickSoundNum = self:getClickSoundNum(id, skin_id)

	if lovePoint and lovePoint > 0 then
		...
		if unLockNum > 0 then
			local lovePointIndex = math.random(1, clickSoundNum + unLockNum) - clickSoundNum

			if lovePointIndex > 0 then
				local lovePointInfo = self:getLovePointDialogInfo(id, lovePointIndex, skin_id)

				if lovePointInfo.sound and lovePointInfo.sound > 0 and lovePointInfo.dialog ~= "-1" then
					return lovePointInfo
				end
			end
		end
	end
...
function PartnerTable:getShowIdsWithNum(id)
...
function PartnerTable:getGradeupDialogInfo(id, skin_id)
	...
	if sound == 0 and partnerSoundId == 0 then
		sound, partnerSoundId = self:getSoundByType(id, "power_up", nil, skin_id)
	end
...
function PartnerTable:getPowerUpDialogInfo(id, skin_id)
	local sound, partnerSoundId = self:getSoundByType(id, "power_up", nil, skin_id)
	local time = xyd.tables.soundTable:getLength(sound)
	local dialog = xyd.tables.partnerDialogTextTable:getText(partnerSoundId)
...
function PartnerTable:getSayHelloDialog(id, skin_id)
	local sound, partnerSoundId = self:getSoundByType(id, "say_hello", nil, skin_id)
	local time = xyd.tables.soundTable:getLength(sound)
	local dialog = xyd.tables.partnerDialogTextTable:getText(partnerSoundId)
...
function PartnerTable:getAlertDialog(id, skin_id)
	local sound, partnerSoundId = self:getSoundByType(id, "alert", nil, skin_id)
	local time = xyd.tables.soundTable:getLength(sound)
	local dialog = xyd.tables.partnerDialogTextTable:getText(partnerSoundId)
...
function PartnerTable:getHouseDialog(id, skin_id)
	...
	if sound == 0 and partnerSoundId == 0 then
		sound, partnerSoundId = self:getSoundByType(id, "say_hello", nil, skin_id)
	end
...
function PartnerTable:getFiveStarTableID(tableID)
	local list = self:getHeroList(tableID)

	for i = 1, #list do
		if self:getStar(list[i]) == 5 then
			return list[i]
		end
	end
...
function PartnerTable:getSixStarTableID(tableID)
	local list = self:getHeroList(tableID)

	for i = 1, #list do
		if self:getStar(list[i]) == 6 then
			return list[i]
		end
	end
...
function PartnerTable:getTenStarTableID(tableID)
	local list = self:getHeroList(tableID)

	if list then
		for i = 1, #list do
			if self:getStar(list[i]) == 10 then
				return list[i]
			end
		end
	end
...
function PartnerTable:get5PartnerNum()
	local num = 0
	local noNeedPartners = xyd.tables.miscTable:split2num("year_summary_star_5", "value", "|")
	local noNeedKey = {}

	for k, v in ipairs(noNeedPartners) do
		noNeedKey[v] = true
	end

	for _, id in ipairs(self.ids_) do
		if self:getStar(id) == 5 and self:getShowInGuide(id) >= 1 and not noNeedKey[id] then
			num = num + 1
		end
	end
...
function PartnerTable:get10PartnerNum()
	local num = 0
	local noNeedPartners = xyd.tables.miscTable:split2num("year_summary_star_10", "value", "|")
	local noNeedKey = {}
	...
	for _, id in ipairs(self.ids_) do
		if self:getStar(id) == 10 and self:getShowInGuide(id) >= 1 and not noNeedKey[id] then
			num = num + 1
		end
	end
