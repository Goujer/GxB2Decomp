-- chunkname: @../../../Product/Bundles/Android/src/app/common/utils.lua
...
function xyd.getItemIcon(params, iconType, offsetDepth)
	...
	if iconType then
		...
		elseif iconType == xyd.ItemIconType.ADVANCE_ICON then
			params.offsetDepth = offsetDepth
			icon = iconClass[xyd.ItemIconType.ADVANCE_ICON].new(params)
		else
		...
		if type_ == xyd.ItemType.AVATAR_FRAME then
			...
			params.avatarID = params.avatar_id or params.avatarID
			params.avatar_frame_id = itemID
...
function xyd.setUISpriteAsyncWithAtlas(uiSprite, atlasName, spriteName, callback, showLoading, isMakePixel)
	if not spriteName then
		return
	end
	...
	atlasName = atlasName or xyd.Sprites2Atlas.table_index[index]
...
function xyd.getReceiveTime2(receiveTime)
	local duration = math.max(xyd.getServerTime() - receiveTime, 0)
	local str
	...
	elseif duration > 86399 and duration <= 863999 then
		str = __("RECEIVE_TIME_4", math.floor(duration / 86400))
	elseif duration > 863999 and duration <= 2591999 then
		str = __("RECEIVE_TIME_4", math.floor(duration / 86400))
	else
		str = __("RECEIVE_TIME_4", 30)
	end
...
function xyd.getRoughDisplayNumber(num, limitNum)
	...
	if absNum < 10000000 then
		denoNum = 1000
		postfix = "K"
	elseif absNum < 10000000000 then
		denoNum = 1000000
		postfix = "M"
	elseif absNum < 10000000000000 then
		denoNum = 1000000000
		postfix = "B"
	elseif absNum < 1e+16 then
		denoNum = 1000000000000
		postfix = "T"
	else
		denoNum = 1e+15
		postfix = "P"
...
function xyd.secondsToString(seconds, strType)
	...
	local minute = math.floor(seconds % xyd.HOUR / xyd.MINUTE)
	local second = math.floor(seconds % xyd.MINUTE)
	local secondString = ""
	...
	elseif strType == xyd.SECOND2STR.ALL then
		return day, math.floor(seconds % xyd.DAY / xyd.HOUR), minute, second
	elseif strType == xyd.SECOND2STR.HOUR_MIN then
		secondString = string.format("%02d:%02d", hour, minute)
	elseif strType == xyd.SECOND2STR.MIN_SEC then
		secondString = string.format("%02d:%02d", minute, second)
	elseif strType == xyd.SECOND2STR.SEC then
...
function xyd.getLineID()
	...
	res = xyd.isTWPkg() and "@mafia" or res
...
function xyd.isTWPkg()
	...
	return pkgName == "com.matchrella.bellavilla"
end
...
function xyd.getIntersectionPoints(p1, p2, p3, p4)
	...
	if p1.x ~= p2.x and p3.x ~= p4.x then
		...
		if ix > p1.x and ix < p2.x and ix > p3.x and ix < p4.x then
			return ix, iy
		end
...
function xyd.closeWindow(windowName, callback, skipAnimation, isCloseAll)
	xyd.WindowManager.get():closeWindow(windowName, callback, skipAnimation, isCloseAll)
end
...
function xyd.stringFormat2(formatStr, matchNum, ...)
	local inargs = {
		...
	}
	local args = {}
...
function xyd.getQualityColor(quality)
	...
	elseif quality == 2 then
		color = Color.New2(4071227647)
	elseif quality == 3 then
		color = Color.New2(2452081919)
	elseif quality == 4 then
		color = Color.New2(915996927)
	elseif quality == 5 then
		color = Color.New2(3422556671)
	elseif quality == 6 then
		color = Color.New2(3613720831)
	elseif quality == 7 then
		color = Color.New2(3980891135)
	else
...
function xyd.getQualityColor2(quality)
	...
	color = quality == 1 and "0069cc" or quality == 2 and "f2aa00" or quality == 3 and "9227cc" or quality == 4 and "369900" or quality == 5 and "cc0011" or quality == 6 and "d76500" or "ffffff"
...
function xyd.setBgColorType(btn, bgType)
	...
	if bgType == xyd.ButtonBgColorType.shop_green_btn then
		label.color = Color.New2(4093310975)
...
function xyd.alertItems(items, callback, title, des, sortFunc)
	...
	for _, item in ipairs(items) do
		...
		equipInfoData[itemID] = item.soulEquipInfo
	end
...
function xyd.checkLast(partner)
	...
	for i = 1, #lock do
		...
		elseif lockState == 11 then
			local teamNum = xyd.models.quickFormation:getTeamNum()

			for i = 1, teamNum do
				local partners = xyd.models.quickFormation:getPartnerList(i)
				local num = 0
				local partnerPos = 0
				...
				for pos, partnerInfo in pairs(partners) do
					...
					if partnerInfo:getPartnerID() == partner:getPartnerID() then
						partnerPos = pos
					end
				end

				if partnerPos > 0 and num <= 1 then
					return true
				end
			end
		end
...
function xyd.checkQuickFormation(partner)
	return partner:getLockType() == 11
end

function xyd.checkGalaxyFormation(partner)
	return partner:getLockType() == xyd.PartnerFlag.GALAXY_TRIP_FORMATION
end
...
function xyd.partnerArenaAllServerScoreUnlock(partner)
	...
	local petIDs = info.pet_infos or {}
	local petIndex = 0
...
function xyd.isItemAbsence(id, need, toast)
	...
	if need > xyd.models.backpack:getItemNumByID(id) then
		if toast then
...
function xyd.setEnabled(obj, enable)
	...
	obj:GetComponent(typeof(UISprite)).color = xyd.checkCondition(enable, Color.New2(4294967295), Color.New2(255))
...
function xyd.getCopy(data)
	if type(data) ~= "table" then
		return data
	end

	if type(data) == "table" then
		return xyd.cloneTable(data)
	end

	return data
end

function xyd.cloneTable(data)
	local arry = {}
...
function xyd.cloneTable2(data, arry)
	for k, v in pairs(data) do
		if type(v) ~= "table" then
			arry[k] = v
		else
...
function xyd.fitFullScreen2(obj)
	...
	if common_scaling <= screen_scaling then
		...
		if real_scaling < screen_scaling then
...
function xyd.uploadGMImgURL()
	if XYDUtils.IsTest() then
		return "http://xuemeih5back.twwebgame.com/upload_gm_img"
	end

	return "https://xuemeih5upload.carolgames.com/upload_gm_img"
end

function xyd.downloadGMImgURL()
	if XYDUtils.IsTest() then
		return "http://xuemeih5back.twwebgame.com:84/gm-img/"
	end
...
function xyd.decodeProtoBufData(msg, name)
	if type(msg) == "number" or type(msg) == "string" then
		return msg
	end

	if not msg.ListFields then
		if not msg._listener or not msg._listener._parent_message or not msg._listener._parent_message.ListFields then
			return msg
		end

		if UNITY_EDITOR and not name then
			print("注意！！！检查decodeProtoBufData非原始数据，需要传入name参数")
			print("注意！！！检查decodeProtoBufData非原始数据，需要传入name参数")
			print("注意！！！检查decodeProtoBufData非原始数据，需要传入name参数")
	...
	return text_format.msg_decode_data(msg, name)
end
...
function xyd.getPartnerInfo(partner_info)
	...
	params.table_id = partner_info.table_id or partner_info.tableID
	params.partner_id = partner_info.partner_id
	params.level = partner_info.level or partner_info.lev
	params.awake = partner_info.awake
	...
	if partner_info.equipments and tostring(partner_info.equipments) ~= "" then
		params.equips = {}

		for _, equip in ipairs(partner_info.equipments) do
			table.insert(params.equips, equip)
		end
	end
	...
	if partner_info.status and tostring(partner_info.status) ~= "" then
		params.status = {}
		params.status.hp = partner_info.status.hp
		params.status.mp = partner_info.status.mp
		params.status.pos = partner_info.status.pos
		params.status.true_hp = partner_info.status.true_hp
	end
...
function xyd.getStoryLoadRes(storyID, storyType, bigPics_, effectNames_)
	...
	elseif storyType == xyd.StoryType.CRYSTAL_BALL then
		storyTable = xyd.tables.storyTable
	elseif storyType == xyd.StoryType.SHRINE_HURDLE then
		storyTable = xyd.tables.activityShrinePlotTable
	elseif storyType == xyd.StoryType.ACTIVITY_4BIRTHDAY_PARTY then
		storyTable = xyd.tables.activity4birthdayPlotTable
	elseif storyType == xyd.StoryType.ACTIVITY_PIRATE then
		storyTable = xyd.tables.activityPiratePlotTable
	end
...
function xyd.goWay(id, wndCallback, callback, closeCallBack)
	...
	for i = 1, #windows do
		...
		if windowName == "activity_window" then
			...
			if getWayID == 125 and not (xyd.models.guild.guildID > 0) then
				xyd.WindowManager.get():openWindow("guild_join_window")

				return
			end

			if id == 12 and xyd.models.trial:checkClose() then
				xyd.alertTips(__("NEW_TRIAL_RESET_TEXT01"))

				return
			end

			if id == 236 and not xyd.models.petTraining:isTrainOpen() then
				xyd.alertTips(__("GROWTH_DIARY_TEXT13"))

				return
			end

			if id == 235 and not xyd.models.shrineHurdleModel:checkIsCanOpen() then
				return
			end
...
function xyd.checkFunctionOpen(funcID, notToast)
	...
	if opened_funcs[funcID] and funcID ~= xyd.FunctionID.GALAXY_TRIP then
		return true
	else
		...
		if funcID == xyd.FunctionID.SKIN_COLLECTION then
			...
			if not ids or ids and #ids < 3 then
				return false
			elseif ids and #ids >= 3 then
				return true
			end
		end

		if funcID == xyd.FunctionID.STARRY_ALTAR then
			local actId = xyd.ActivityID.ACTIVITY_STAR_ALTAR_MISSION
			local isActOpen = xyd.models.activity:isOpen(actId)
		...
		if funcID == xyd.FunctionID.GALAXY_TRIP then
			if maxStage < openValue then
				return false
			end

			local openTime = tonumber(xyd.tables.miscTable:getVal("galaxy_trip_open_time"))
			local nowTime = xyd.getServerTime()
			...
			if achievements then
				...
				for i in pairs(achievements) do
					if achievements[i].achieve_type == achievement_id then
						...
						if achievements[i].value >= xyd.tables.achievementTable:getCompleteValue(achieve_id) then
							return true
						end
					end
				end
		...
		if funcID == xyd.FunctionID.SOUL_LAND then
			if not notToast and not xyd.models.soulLand:checkIsOpen() then
				xyd.alertTips(__("SOUL_LAND_UNLOCK_TIPS"))
			end

			return xyd.models.soulLand:checkIsOpen()
		end
...
function xyd.checkPartnerGroupImgStr(group, str, isMaxStar)
	if group == xyd.PartnerGroup.TIANYI then
		if isMaxStar then
			return str .. "_group_7_max"
		else
			return str .. "_group_7"
		end
	end
...
function xyd.gmSkinGuide()
	if UNITY_EDITOR then
		local msg = messages_pb.save_guide_info_req()

		msg.marks = table.concat({
			1,
			2,
			3,
			5,
		...
		if currentStage < 3 then
			xyd.gmTestCommon("#gm stage 3")
		end

		if xyd.models.backpack:getLev() < 3 then
			xyd.gmTestCommon("#gm item 3 150")
		end
...
function xyd.gmFunctionOpenMask(maskID)
	if UNITY_EDITOR then
		print("測試1：", maskID)

		xyd.GuideController.gmMaskFunID = maskID
	end
end

function xyd.gmSetShowSkinList(skin_list)
	skin_list = skin_list or ""

	local skin_ids = xyd.split(skin_list, "|", true)
...
function xyd.isMaxStarOrigin(partnerInfo, partner)
	local isMaxStar = false

	if partner and partner.isEntrance == true then
		isMaxStar = true
	elseif partnerInfo and partnerInfo.isEntrance == true then
		isMaxStar = true
	elseif partnerInfo and partnerInfo.star_origin and #partnerInfo.star_origin ~= 0 then
		local starOrigin = partnerInfo.star_origin
		local partnerTableID = partnerInfo.tableID or partnerInfo.table_id

		if not partnerTableID or not starOrigin then
			isMaxStar = false
		else
			...
			if listTableID then
				...
				if not beginIDs or #beginIDs == 0 then
					isMaxStar = false
				else
					for i, lv in ipairs(starOrigin) do
						if beginIDs[i] then
	...
	elseif partner or partnerInfo then
		local partnerID
		...
		if partner and partner.partnerID then
		...
		if partnerInfo then
			...
			if not starOrigin then
				...
				if not listTableID or not beginIDs or #beginIDs == 0 then
					isMaxStar = false
				else
...
function xyd.culSoulEquipAttr(equipIDs)
	...
	for i = 1, 5 do
		if equipIDs[i] and equipIDs[i] > 0 then
			local equip = xyd.models.slot:getSoulEquip(equipIDs[i])

			if equip then
				local baseAttr = equip:getBaseAttr()

				for index, buff in pairs(baseAttr) do
					...
					elseif not resultHelpArr[buff[1]] then
						table.insert(result, buff)

						resultHelpArr[buff[1]] = #result
					else
						result[resultHelpArr[buff[1]]][2] = result[resultHelpArr[buff[1]]][2] + buff[2]
					end
				end

				local exAttr = equip:getExAttr()

				for index, buff in pairs(exAttr) do
					local buffArr = xyd.split(buff[1], "P")
	...
	for key, value in pairs(buffP) do
		local buffArr = xyd.split(key, "P")
		local baseBuffKey = buffArr[1]

		if resultHelpArr[baseBuffKey] then
			result[resultHelpArr[baseBuffKey]][2] = result[resultHelpArr[baseBuffKey]][2] * (1 + value)
		end
	end
...
function xyd.getSoulEquipSkill(equipIDs)
	...
	return xyd.tables.soulEquip2GroupTable:getSuitSkill(curSkillID)
end

function xyd.getBuffValue(key1, value)
	local value1 = value
	local bt = xyd.tables.dBuffTable

	if bt:isShowPercent(key1) then
		local factor = tonumber(bt:getFactor(key1))

		value1 = value1 * 100 / tonumber(bt:getFactor(key1))
		value1 = string.format("%.2f", value1)
