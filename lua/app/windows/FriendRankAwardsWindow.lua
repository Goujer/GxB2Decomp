-- chunkname: @../../../Product/Bundles/Android/src/app/windows/FriendRankAwardsWindow.lua
...
function FriendRankAwardItem:update(index, realIndex, info)
	...
	if id then
		...
		if rank <= 3 then
			xyd.setUISpriteAsync(self.imgRankIcon_, nil, "rank_icon0" .. rank, nil, nil)
		...
		XYDCo.WaitForFrame(idx, function()
			if self.window_ ~= nil then
				...
				if not item then
					...
					item:update(nil, nil, tonumber(info))
