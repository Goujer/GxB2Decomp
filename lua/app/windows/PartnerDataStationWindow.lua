-- chunkname: @../../../Product/Bundles/Android/src/app/windows/PartnerDataStationWindow.lua
...
function PartnerDirectionComponent:initText()
	...
	for i = 1, self.artifact_num_ do
		if tables:getArtifactId(self.table_id_, i) then
			local comp = PartnerDirectionArtifactComponent.new(self.groupArtifacts, self, {
				partner_id = self.table_id_,
				id = i
			})
		end
...
function PartnerArrayComponent:initSprite()
	...
	xyd.setUISpriteAsync(self.imgBg2, nil, "station_bg", function()
		self.imgBg2.color = Color.New2(4042128639)
	end)
...
function PartnerArrayComponent:updateChoose(index)
	...
	for i = 0, self.label_num_ - 1 do
		...
		source = index == i and "station_small_label_click" or "station_small_label"
