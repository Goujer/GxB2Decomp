-- chunkname: @../../../Product/Bundles/Android/src/app/windows/OneClickClearanceWindow.lua
...
function OneClickClearanceWindow:setLayout()
	...
	if self.purchaseNum > self.canBuy1 then
...
function OneClickClearanceWindow:updateLayout()
	if self.onTouchMax == true then
		...
		if self.purchaseNum >= self.left_stage then
	...
	if self.purchaseNum > self.maxNum then
		print(self.maxNum)
		self.selectNum:setCurNum(self.maxNum)
