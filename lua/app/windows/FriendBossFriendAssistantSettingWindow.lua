-- chunkname: @../../../Product/Bundles/Android/src/app/windows/FriendBossFriendAssistantSettingWindow.lua
...
function FriendBossFriendAssistantSettingWindow:onClickheroIcon(heroIcon, needUpdate, isChoose, needAnimation, posId)
	...
	if heroPosId ~= nil then
		...
		return
	elseif self.selectedPositionNumber >= self.totalPositionNumber then
		return
	else
...
function FriendBossFriendAssistantSettingWindow:getEmptyIconPosition()
	if self.selectedPositionNumber >= self.totalPositionNumber then
		return nil
	end
...
function FriendBossFriendAssistantIconRenderer:onFriendAssistantInfoTouch()
	...
	tempPartnerInfo.equipments = partnerInfo.equips
	tempPartnerInfo.tableID = self.baseinfo.tableID
	tempPartnerInfo.awake = partnerInfo.awake
	tempPartnerInfo.lev = self.baseinfo.lev
	tempPartnerInfo.ex_skills = self.data.friendSharedPartnerInfo.shared_partner.ex_skills
	tempPartnerInfo.noWays = true
