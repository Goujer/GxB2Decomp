-- chunkname: @../../../Product/Bundles/Android/src/app/windows/TimeCloisterProbeWindow.lua
...
function TimeCloisterProbeWindow:initCardTable()
	...
	for i = 1, 3 do
		...
		for j = 1, 5 do
			...
			xyd.setUISpriteAsync(cardImgUISprite, nil, imgName, nil, nil, nil)
...
function TimeCloisterProbeWindow:drawCard(sequence)
	...
	sequence:AppendCallback(function()
		...
		self.curRow, self.nextRow = self.nextRow, 0
		...
		for tec_id, info in pairs(groupList) do
			if type(info) == "table" and info.curLv < info.maxLv then
				...
				if info.curLv > 0 or not next(preId) then
					canUp = xyd.models.backpack:getItemNumByID(cost[1]) >= cost[2]
				else
					local unlock = false
					...
					if next(preId) then
						...
						for i, id in ipairs(preId) do
							unlock = unlock or groupList[id].curLv >= preLv[i]
						end
					end
					...
					if unlock then
						...
						if unLockType == xyd.TimeCloisterUnLockType.SELF_BASE then
							...
							for i = 1, 3 do
								unlock = unlock and baseAttr[i] >= unLockNum[i]
							end
						elseif unLockType == xyd.TimeCloisterUnLockType.EVENT_NUM then
							local sum_events = timeCloister:getSumEvents()

							unlock = unlock and (sum_events[tostring(unLockNum[1])] or 0) >= unLockNum[2]
						elseif unLockType == xyd.TimeCloisterUnLockType.ACHIEVEMENT then
							...
							for _, data in ipairs(achInfo) do
								if data.achieve_type == unLockNum[1] then
									unlock = unlock and (data.achieve_id == 0 or data.achieve_id > unLockNum[2])

									break
								end
							end
						elseif unLockType == xyd.TimeCloisterUnLockType.PROGRESS then
							unlock = unlock and self.hangInfo.progress >= unLockNum[1]
						elseif unLockType == xyd.TimeCloisterUnLockType.ENCOUNTER_FIGHT then
							...
							unlock = sum_start_events and needNum >= tonumber(unLockNum[2])
						end
...
function TimeCloisterProbeWindow:checkGuide(type)
	...
	if type and type ~= xyd.GuideType.TIME_CLOISTER_1 or exskillGuideWd or xyd.db.misc:getValue("time_cloister_guide_" .. xyd.GuideType.TIME_CLOISTER_1) or self.hangInfo and self.hangInfo.stop_time and self.hangInfo.stop_time > 0 then
		-- block empty
...
function TimeCloisterProbeWindow:updateShowEventIcon()
	...
	local firstFightCardImgName, firstDressMissionImgName
