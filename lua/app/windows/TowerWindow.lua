-- chunkname: @../../../Product/Bundles/Android/src/app/windows/TowerWindow.lua
...
function TowerWindow:checkOldSchoolImg()
	...
	if xyd.models.oldSchool:getAllInfo() then
		if xyd.models.oldSchool:isCanOpen() == true then
			xyd.setUISpriteAsync(self.oldSchoolBtn_UISprite, nil, "old_school_enter_active_" .. xyd.Global.lang, nil, nil)
			self:initTime()
		else
...
function TowerItem:setChildren()
	UIEventListener.Get(self.go).onClick = function()
		if self.data > xyd.tables.miscTable:getNumber("tower_top", "value") then
			return
		end
...
function TowerItem:update()
	...
	elseif curStage > self.data then
...
function TowerItem:updateAwards()
	...
	if self.data % 5 ~= 0 or curStage > self.data then
		return
	end
