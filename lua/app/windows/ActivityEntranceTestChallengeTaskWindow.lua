-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityEntranceTestChallengeTaskWindow.lua
...
function ActivityEntranceTestChallengeTaskWindow:initData()
	...
	for i = 1, #ids do
		...
		table.insert(self.data2, {
			...
			isCompleted = self.activityData:getTotalEnergy() >= xyd.tables.activityWarmupArenaTaskAwardTable:getEnergy(id),
			hasStar = hasStar,
	...
	for i = 1, #ids do
		...
		elseif totalEnergy < needEnergy then
			if totalEnergy > xyd.tables.activityWarmupArenaTaskAwardTable:getEnergy(id - 1) then
...
function ActivityEntranceTestChallengeTaskWindow:initListPositon()
	self:waitForFrame(5, function()
		...
		for i = #ids, 1, -1 do
			...
			if self.activityData:getTotalEnergy() >= xyd.tables.activityWarmupArenaTaskAwardTable:getEnergy(id) and (self.activityData.detail.mission_info.awards[id] == 0 or self.activityData.detail.mission_info.ex_awards[id] == 0 and self.activityData:IfBuyGiftBag() == true) then
				moveIndex = id
				flag = true
			end
		end
...
function ActivityEntranceTestChallengeTaskWindow:Register()
	...
	for i = 1, 3 do
		UIEventListener.Get(self["charterBtn" .. i]).onClick = function()
			...
			elseif self.activityData:getCharterStar() >= i and self.activityData:getCharterState() < i and self.activityData:getCurrentPlotID() ~= nil then
				...
				stateinfo.stateNum = self.activityData:getCharterState() + 1
				stateinfo.timeStamp = xyd.getServerTime()
...
function ActivityEntranceTestChallengeTaskWindow:updateCharterBtns()
	for i = 1, 3 do
		if i > self.activityData:getCharterState() and i > self.activityData:getCharterStar() then
...
function ActivityEntranceTestChallengeTaskItem1:initUI()
	...
	UIEventListener.Get(self.gotoBtn).onClick = function()
		...
		elseif self.data.type == 6 or self.data.type == 7 then
			local bossID = 1
		...
		elseif self.data.type == 8 or self.data.type == 9 then
...
function ActivityEntranceTestChallengeTaskItem2:updateInfo()
	...
	if self.parent.activityData:getTotalEnergy() >= self.needEnergy then
		self.symbol.gameObject:SetActive(true)
		self.energyNum.gameObject:SetActive(false)
	else
		self.symbol.gameObject:SetActive(false)
	...
	for i = 1, #self.packAwards do
		...
		elseif self.packAwarded == 1 then
			...
			if self.isCompleted == false and (data[1] ~= 264 or true) then
				icon:setLock(false)
				icon:setChoose(false)
				icon:setMask(true)
			end
