-- chunkname: @../../../Product/Bundles/Android/src/app/windows/HouseDialog.lua
...
function HouseDialogAction:updateHerosFlip()
	...
	table.sort(data, function(a, b)
		...
		if info0.coord_y ~= info1.coord_y then
			return info0.coord_y > info1.coord_y and true or false
		else
			if info0.coord_x > info1.coord_x then
				-- block empty
			end
...
function HouseDialog:getMinPath(startP, endP)
	...
	data[startP.y * self.mapCol + startP.x] = 1
	...
	while #que > 0 do
		...
		if p.x >= endP.x and p.x < endP.x + endP.length and p.y >= endP.y and p.y < endP.y + endP.width then
			flag = true
			finalPoint = p
...
function HouseDialog:update()
	...
	local isSuccess = rate_ > math.random()
