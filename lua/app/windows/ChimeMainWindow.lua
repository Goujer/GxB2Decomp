-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ChimeMainWindow.lua
...
function ChimeMainWindow:initChime()
	...
	for i = 1, layerNum do
		datas[i] = {}
		datas[i][1] = infos[3 * (i - 1) + 1]
		datas[i][2] = infos[3 * (i - 1) + 2]
		datas[i][3] = infos[3 * (i - 1) + 3]
		datas[i].type = i % 2 + 1
	end
...
function ChimeMainItem:setInfo(params)
	...
	if self.debrisNum >= self.needDebrisNumCompose and self.lock then
		self.canUnlock = true
	else
