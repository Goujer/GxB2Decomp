-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityFairyTaleBattleFormationWindow.lua
	...
	while i <= self.npcPartnerNum do
		...
		do
			...
			if monsterId ~= nil then
				...
				partnerIcon:update(nil, nil, data)
...
function ActivityFairyTaleBattleFormationWindow:endDrag(copyIcon)
	...
	if cPosId > 0 and self:chackCanMove(posId, cPosId) then
		...
		cPosId = (cPosId > 0 and isNpc and not canMoveNpc or isTargetNpc and not canChangeNpc) and -1 or -1
	end
		...
		if type_ ~= xyd.GroupBuffIconType.HERO_CHALLENGE and type_ ~= xyd.GroupBuffIconType.NEW_TRIAL then
			...
			if tNum < 6 then
				...
				elseif tonumber(type) == 2 then
					...
					isNewAct = groupNum[5] + groupNum[6] == 3 and numCount[1] == 3 and true or false
				end
			end
