-- chunkname: @../../../Product/Bundles/Android/src/app/windows/LimitItemPurchaseWindow.lua
...
function LimitItemPurchaseWindow:setLayout()
	...
	if self.costType then
		xyd.setUISpriteAsync(self.ImgExchange, nil, tostring(xyd.tables.itemTable:getIcon(self.costType)), nil, nil)
	end
...
function LimitItemPurchaseWindow:updateLayout()
	if self.purchaseNum > self.limitNum then
	...
	elseif self.costType and self.calPriceCallback(self.purchaseNum) > self.backpack:getItemNumByID(self.costType) then
		local real_num = self:getRealNum()

		if self.showWindowCallback then
			self.labelTotal.color = Color.New2(3422556671)
		else
...
function LimitItemPurchaseWindow:getRealNum()
	...
	while l <= r do
		...
		if sum >= self.calPriceCallback(mid) then
			l = mid + 1
