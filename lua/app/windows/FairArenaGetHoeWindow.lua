-- chunkname: @../../../Product/Bundles/Android/src/app/windows/FairArenaGetHoeWindow.lua
...
function FairArenaGetHoeWindow:registerEvent()
	UIEventListener.Get(self.btnSure.gameObject).onClick = handler(self, function()
		...
		local function buyFun()
			...
			if xyd.models.backpack:getItemNumByID(cost_arr[1]) >= cost_arr[2] then
		...
		if not timeStamp or timeStamp < activityData:startTime() or timeStamp > activityData:getEndTime() then
			local params = {
