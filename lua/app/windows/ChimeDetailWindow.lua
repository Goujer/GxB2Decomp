-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ChimeDetailWindow.lua
...
local NameColor = {
	Color.New2(4294967295),
	Color.New2(3889376511),
	Color.New2(4098292479),
	Color.New2(685729023),
	Color.New2(4128270335),
	Color.New2(4253160703)
}
...
function ChimeDetailWindow:register()
	...
	UIEventListener.Get(self.btnLevelUp).onClick = function()
		...
		if self.lev == -1 then
			...
			return
		elseif self.lev >= self.maxLev then
	...
	UIEventListener.Get(self.btnAwake).onClick = function()
		...
		if self.lev == -1 then
			...
			return
		elseif limitNum > self.activeTime then
...
function ChimeDetailWindow:updateBtnGroup()
	...
	for i = 1, 4 do
		if self.curTabIndex == i then
			xyd.setUISpriteAsync(btns[i]:ComponentByName("bg", typeof(UISprite)), nil, "chime_btn_yq_2", nil, nil, false)
			xyd.setUISpriteAsync(btns[i]:ComponentByName("bg/img", typeof(UISprite)), nil, "chime_btn_hw_1", nil, nil, false)
		else
			xyd.setUISpriteAsync(btns[i]:ComponentByName("bg", typeof(UISprite)), nil, "chime_btn_yq_1", nil, nil, false)
			xyd.setUISpriteAsync(btns[i]:ComponentByName("bg/img", typeof(UISprite)), nil, "chime_btn_hw_2", nil, nil, false)
		end
	end

	if self.lev + self.fakeLev == -1 or self.lev + self.fakeLev >= self.maxLev then
	...
	if self.lev + self.fakeLev == -1 or not self.data.AwakeAtrr or limitNum > self.activeTime or self.lev + self.fakeLev < self.data.AwakeAtrr.limit[1] or self.awakeTime > 0 then
...
function ChimeDetailWindow:updateCommonGroup(donotUpdate)
	...
	if self.lev + self.fakeLev > -1 then
		...
		if xyd.models.backpack:getItemNumByID(self.debrisItemID) >= self.needDebrisNumCompose then
	...
	for i = 1, 3 do
		if AttrNum < i then
			...
			if i <= self.activeTime then
				...
				descLabel.color = Color.New2(2155905279)
			end
		end
	end
	...
	for i = 1, 3 do
		...
		if i <= costNum then
			...
			if xyd.models.backpack:getItemNumByID(cost[i][1]) - self:getFakeUseRes(cost[i][1]) >= cost[i][2] then
				...
				needLabel.color = Color.New2(3422556671)
			end
		else
...
function ChimeDetailWindow:onClickBtnConfirmAwake()
	if self.curTabIndex == 3 then
		...
		if needEnergyValue > self.lev then
	...
	xyd.alertYesNo(message_text, callback, __("YES"), false, nil, nil, nil, nil, nil, nil)
end
