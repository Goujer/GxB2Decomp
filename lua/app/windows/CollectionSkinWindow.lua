-- chunkname: @../../../Product/Bundles/Android/src/app/windows/CollectionSkinWindow.lua
...
function CollectionSkinWindow:updateRankGroup()
	...
	self.btnLevelUp:SetActive(self.point >= self.nextLevPoint)

	if self.point >= self.nextLevPoint and not self.levelUpEffect then
...
function CollectionSkinWindow:register()
	...
	UIEventListener.Get(self.btnCancel).onClick = function()
		...
		self.checkStr = nil
		self.textEdit.text = ""
		self.textEditBefore.text = __("COLLECTION_SKIN_TEXT05"), self.textEditBefore:SetActive(true)
...
function CollectionSkinWindow:onClickTab(index)
	...
	for i = 1, 4 do
		...
		if self.themeTabIndex == i then
			self["tabLabel" .. i].color = Color.New2(4278124287)
		else
	...
	if self.data.themeID then
		...
		for i = 1, #self.skinIDs do
			...
			if flag then
				...
				self.items[count]:update(nil, nil, params)
