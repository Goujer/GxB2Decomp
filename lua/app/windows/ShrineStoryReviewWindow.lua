-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ShrineStoryReviewWindow.lua
...
function ShrineTab1Item:setSelected(status)
	...
	local bgImg, fontColor, outlineColor

	if self.isSelected then
		bgImg = "btn_bq_1_xz"
		fontColor = 4193710847
		outlineColor = 2906151679
	else
		bgImg = "btn_bq_1_mr"
	...
	xyd.setUISpriteAsync(self.tabBg, nil, bgImg, nil, nil)
end
...
function ShrineTab2Item:setSelected(status)
	...
	bgImgVisible = self.isSelected and true or false
...
function ShrinePlotItem:setInfo(info)
	...
	if iconImg then
		xyd.setUISpriteAsync(self.desImg, nil, iconImg, nil, nil, true)
		self.desImg:SetActive(true)
	else
		self.desImg:SetActive(false)
	end

	xyd.setUISpriteAsync(self.bgImg, nil, bgImg, nil, nil)
end
