-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ArenaAllServerWindow.lua
...
function HonorHalls:layout()
	...
	xyd.setUISpriteAsync(self.imgTitle_, nil, "arena_as_title_" .. xyd.Global.lang, nil, nil, true)
...
function ArenaAllServerFight:onTouchBot(index)
	...
	elseif self.curPage_ > self.maxPage_ then
		self.curPage_ = 1
	end
...
function ArenaAllServerScore:initLayout()
	xyd.setUISpriteAsync(self.imgTitle, nil, "arena_as_final_" .. tostring(xyd.Global.lang), nil, nil, true)
...
function ArenaAllServerScore:updateSelfItemInfo(selfRank)
	...
	info.score = xyd.models.arenaAllServerScore:getScore() or 0
	info.player_name = xyd.Global.playerName
	info.avatar_id = xyd.models.selfPlayer:getAvatarID()
	info.avatar_frame_id = xyd.models.selfPlayer:getAvatarFrameID()
	info.lev = xyd.models.selfPlayer.level_
	info.player_id = xyd.models.selfPlayer.playerID_
	info.power = xyd.models.arenaAllServerScore:getPower() or 0
	info.server_id = xyd.models.selfPlayer.serverID_
	info.rank = selfRank or xyd.models.arenaAllServerScore:getRank()
	info.state = "self"
...
function ArenaAllServerScore:onGetRankList()
	...
	if ranklist then
		for i = 1, #ranklist do
			infos[i] = {}
			infos[i].score = ranklist[i].score
			infos[i].player_name = ranklist[i].player_name
			infos[i].avatar_id = ranklist[i].avatar_id
			infos[i].avatar_frame_id = ranklist[i].avatar_frame_id
			infos[i].lev = ranklist[i].lev
			infos[i].player_id = ranklist[i].player_id
			infos[i].power = ranklist[i].power
			infos[i].server_id = ranklist[i].server_id
			infos[i].rank = i
			infos[i].state = "arena_as"
			infos[i].reqType = reqType
		end
	end
...
function ArenaAllServerScore:registerEvent()
	...
	UIEventListener.Get(self.btnMission.gameObject).onClick = function()
		...
		for i in pairs(ids) do
			...
			data.id = i
			data.max_value = xyd.tables.arenaAllServerMissionTable:getComplete(i)
...
function ArenaAllServerScore:onBuyCard(event)
	...
	getItems.item_id = event.data.item_id
	getItems.item_num = event.data.item_num
...
function ArenaAllServerWindow:checkOpenIndex()
	...
	if scoreEndTime > xyd.getServerTime() then
		self.curIndex_ = 4
	else
...
function ArenaAllServerWindow:registerEvent()
	...
	for i = 1, 4 do
		UIEventListener.Get(self["nav" .. tostring(i)]).onClick = function()
			...
			if scoreEndTime > xyd.getServerTime() and i == 2 then
