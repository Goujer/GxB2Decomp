-- chunkname: @../../../Product/Bundles/Android/src/app/windows/GameAssistantResultWindow.lua
...
function GameAssistantResultWindow:onGetMarketMsg(event)
	...
	params.items = self.model:getTempBuyMarketItems()
...
function GameAssistantResultWindow:onGetPetBuyTicketMsg(event)
	...
	if self.hasBuyPetTicketNum + self.oldPetBuyTime + baseTime >= self.presetData.pet.fight then
...
function GameAssistantResultWindow:doGuildDinnerGetAward()
	...
	if not flag then
		if self.model.orderAwards then
			...
			params.items = self.model.orderAwards
...
function GameAssistantResultWindow:doGuildDinnerLevelUpOrder()
	...
	if self.ifCanDo.guild.level then
		...
		for _, order in pairs(orderList) do
			if order.start_time == 0 and desLev > order.order_lv then
...
function GameAssistantResultWindow:onGetGuildGymMsg(event)
	...
	params.battle_report = data.battle_report
	params.items = data.awards
	params.total_harm = data.total_harm
		...
		for i = 1, 2 do
			...
			if itemNum then
				...
				if mission_id == xyd.models.mission:getFinalMissionInfo().mission_id then
					...
					if endTime > xyd:getServerTime() and itemId == xyd.ItemID.CRYSTAL then
						num = num * 2
					end
				end
