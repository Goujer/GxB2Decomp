-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityIceSecretSelectWindow.lua
...
function ActivityIceSecretSelectWindow:initData()
	...
	for _, id in ipairs(self.bigRewardList_) do
		...
		data.id = id
		data.award = xyd.tables.activityIceSecretAwardsTable:getAwards(id)
		data.limit = xyd.tables.activityIceSecretAwardsTable:getLimit(id)
		data.level = xyd.tables.activityIceSecretAwardsTable:getLevel(id)
		data.round = self.round_
	...
	table.sort(self.selectAwardsList_, function(a, b)
		local canSelectA = a.limit > a.hasGotNum
		local canSelectB = b.limit > b.hasGotNum
...
function ActivityIceSecretSelectItem:update(_, _, info)
	...
	elseif info.limit <= info.hasGotNum then
		...
		self.labelNum_.color = Color.New2(3422556671)
	else
...
function ActivityIceSecretSelectItem:onClick()
	...
	if self.data_.level > self.data_.round then
