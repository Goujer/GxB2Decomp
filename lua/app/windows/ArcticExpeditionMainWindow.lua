-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ArcticExpeditionMainWindow.lua
...
function CellItem:refreshState()
	...
	if isSafe and isSafe >= 1 and xyd.getServerTime() < safe_time + xyd.tables.arcticExpeditionCellsTypeTable:getTruceTime(self.cellType_) then
		...
		xyd.setUISpriteAsync(self.stateImg_, nil, "arctic_expedition_cell_state_icon_1", nil, nil, true)
	...
	elseif fierceFighting then
		self.stateImg_.gameObject:SetActive(true)
		xyd.setUISpriteAsync(self.stateImg_, nil, "arctic_expedition_cell_state_icon_2", nil, nil, true)
	else
...
function ArcticExpeditionMainWindow:updateTimeGroup()
	...
	elseif endTime - xyd.getServerTime() > xyd.DAY_TIME then
...
function ArcticExpeditionMainWindow:initLayout()
	...
	xyd.setUISpriteAsync(self.btnMissionLabel, nil, "arctic_expedition_btn_mission_" .. xyd.Global.lang, nil, nil, true)
	xyd.setUISpriteAsync(self.btnAwardLabel, nil, "arctic_expedition_btn_award_" .. xyd.Global.lang, nil, nil, true)
	xyd.setUISpriteAsync(self.btnRankLabel, nil, "arctic_expedition_btn_rank_" .. xyd.Global.lang, nil, nil, true)
...
function ArcticExpeditionMainWindow:refreshMap()
	local GroupColor = {
		Color.New2(3903474175),
		Color.New2(4016532991),
		Color.New2(2464115199),
...
function ArcticExpeditionMainWindow:checkShowRallyList()
	...
	for i = 1, 3 do
		if rallyList[i] and rallyList[i].end_time >= xyd.getServerTime() then
