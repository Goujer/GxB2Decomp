-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivitySpfarmBattlepassWindow.lua
...
function SpfarmLevelItem:setInfo(id, is_awarded, is_paidAwarded, totalLev, hasBuy)
	...
	if is_awarded ~= 1 and totalLev >= self.needLevel_ then
		canBuyNormal = true
	end

	if is_paidAwarded ~= 1 and totalLev >= self.needLevel_ then
		canBuyPaid = true
	end
