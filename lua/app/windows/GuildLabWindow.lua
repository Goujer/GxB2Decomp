-- chunkname: @../../../Product/Bundles/Android/src/app/windows/GuildLabWindow.lua
...
function GuildLabWindow:showSkillUnLock()
	...
	if btnSkill then
		...
		sequence:Append(DG.Tweening.DOTween.To(DG.Tweening.Core.DOSetter_float(setter), 4294967040, 4294967295, 0.1))
	...
	for i = 1, #baseEffects do
		...
		if not self.isHideNewSkill then
			...
			if self["labelAttrName" .. i].width > 190 then
				...
				until self["labelAttrName" .. i].width <= 190
			end
...
function GuildLabWindow:updateCost(isShowCost)
	...
	if cost.isMax then
		...
		if self.selfMana_ < cost.mana then
			self.labelMana_.color = Color.New2(3422556671)
		else
			self.labelMana_.color = Color.New2(1432789759)
		end

		if self.selfCoin_ < cost.coin then
			self.labelGuildCoin_.color = Color.New2(3422556671)
		else
...
function GuildLabWindow:setPageCircle(page)
	...
	for i = 1, 2 do
		if i == page then
			if self["page" .. i] then
				xyd.setUISpriteAsync(self["page" .. i], nil, "page_icon_1", nil, nil)
			end
		elseif self["page" .. i] then
			xyd.setUISpriteAsync(self["page" .. i], nil, "page_icon_0", nil, nil)
		end
	end
end
