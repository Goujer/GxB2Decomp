-- chunkname: @../../../Product/Bundles/Android/src/app/windows/BuySeniorScrollWindow.lua
...
function BuySeniorScrollWindow:callbackOne(num)
	...
	if num == 0 or num > self.maxOneNum and self.limit ~= 0 then
		self.labelOneCost_.color = Color.New2(4278190335)
	else
...
function BuySeniorScrollWindow:callbackTen(num)
	...
	if num == 0 or num > self.maxTenNum and self.limit ~= 0 then
		self.labelTenCost_.color = Color.New2(4278190335)
	else
...
function BuySeniorScrollWindow:buySeniorScroll(rate)
	...
	if self.limit == 0 or xyd.models.backpack:getItemNumByID(xyd.ItemID.SENIOR_SUMMON_SCROLL) >= xyd.tables.itemTable:stackLimit(xyd.ItemID.SENIOR_SUMMON_SCROLL) then
