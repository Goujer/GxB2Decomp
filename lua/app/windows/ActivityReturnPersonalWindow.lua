-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityReturnPersonalWindow.lua
...
function ActivityReturnPersonalWindow:startCountDown(leftTime)
	...
	params.duration = self.activityData:getEndTime() - xyd.getServerTime()
...
function ActivityReturnPersonalWindow:refreshPMissionPart(keepPosition)
	...
	for _, id in ipairs(missionList) do
		...
		params.id = id
		params.compTime = tonumber(self.missionCompList_[id])
		params.count = tonumber(self.missionCountList_[id])
	...
	table.sort(tempList, function(a, b)
		...
		local wightA = xyd.checkCondition(a.compTime >= xyd.tables.activityReturnPMissionTable:getRepeatTimes(a.id), rankA + 100, rankA)
		local wightB = xyd.checkCondition(b.compTime >= xyd.tables.activityReturnPMissionTable:getRepeatTimes(b.id), rankB + 100, rankB)
...
function pMissionItem:refreshUI()
	...
	self.imgMask_:SetActive(self.compTime_ >= self.limitTimes_)
...
function ActivityReturnPersonalWindow:refreshShopPart(keepPosition)
	...
	for _, id in ipairs(shopIds) do
		...
		params.id = id
		params.buy_times = self.buyTimesList_[id]
		params.limit_times = xyd.tables.activityReturnShopTable:getLimit(id)
...
function returnShopItem:registerEvent()
	UIEventListener.Get(self.mainNode_).onClick = function()
		if self.limit_ and self.buyTime_ >= self.limit_ then
		...
		local params = {
			...
			limitNum = self.limit_ - self.buyTime_,
...
function ActivityReturnPersonalWindow:refreshTmissionList(keepPosition)
	...
	for idx, missionId in ipairs(tMissionList) do
		...
		params.id = missionId
		params.teamerCount = self.otherTMissionCount_[idx] or 0
		params.count = self.tMissionCount_[idx] or 0
		params.compTime = self.tMissionAwarded_[idx] or 0
		params.limitTimes = 1
	...
	table.sort(tempList, function(a, b)
		...
		local wightA = xyd.checkCondition(a.compTime >= a.limitTimes, rankA + 100, rankA)
		local wightB = xyd.checkCondition(b.compTime >= b.limitTimes, rankB + 100, rankB)
