-- chunkname: @../../../Product/Bundles/Android/src/app/windows/NewNoticeWindow.lua
		...
		if content.contents and #content.contents > 0 then
			for i = 1, #content.contents do
				...
				for _, str in ipairs(strs) do
					...
					xyd.setUISpriteAsync(goLine1Img, nil, "setting_up_help_icon_2", nil, nil)
...
function SettingUpInfoItem:loadImgByUrl(url, texture)
	xyd.setTextureByURL(url, texture, nil, nil, function()
...
function NewNoticeWindow:initList()
	...
	for index, data in ipairs(game_notice_data) do
		if xyd.getServerTime() >= data.start_time and xyd.getServerTime() <= data.end_time then
	...
	for _, data in ipairs(game_notice_data) do
		if xyd.getServerTime() >= data.start_time and xyd.getServerTime() <= data.end_time then
			...
			if index == 1 or index >= self.lineNum_ then
