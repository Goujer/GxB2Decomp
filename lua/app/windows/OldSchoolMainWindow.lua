-- chunkname: @../../../Product/Bundles/Android/src/app/windows/OldSchoolMainWindow.lua
...
local self_oldBuildingFloorTable
	...
	for i in pairs(infoItems) do
		if infoItems[i].data and tonumber(infoItems[i].data.floorId) == backFloorId then
			...
			for k in pairs(data.floor_info.cur_scores) do
				if tonumber(data.floor_info.cur_scores[k]) > tonumber(xyd.models.oldSchool:getAllInfo().max_score) then
...
function OldSchoolMainWindow:checkFloorLock(backFloorId)
	...
	if floorIndex > xyd.models.oldSchool:getAllInfo().max_index then
