-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityShelterMissionWindow.lua
...
function ActivityShelterMissionWindow:initWindow()
	...
	self.confirmBtnLabel_.color = Color.New2(4278124287)
...
function ActivityShelterMissionWindow:initItemStatus()
	for i = 1, #self.itemList_ do
		...
		if #self.itemList_[i].optionalList >= self.itemList_[i]:getNeedNum() then
...
function ActivityShelterGiftBagItem:confirmCallback(optionalList, materialList)
	...
	if #self.materialList >= self.needNum_ then
		self:setMask(false)
		self:setRedPoint(false)
	else
		self:setMask(true)

		if #self.optionalList + #self.materialList >= self.needNum_ then
