-- chunkname: @../../../Product/Bundles/Android/src/app/windows/DogMiniGameWindow.lua
...
function DogMiniGameWindow:update()
	...
	if UnityEngine.Input.GetMouseButtonDown(0) then
		if hit and hit.transform and hit.transform.name and (string.find(hit.transform.name, "border") or string.find(hit.transform.name, "Btn")) then
			-- block empty
		else
...
function DogMiniGameWindow:win()
	...
	xyd.WindowManager.get():openWindow("battle_win_window", {
		...
		battle_type = xyd.BattleType.DOG_MINI_GAME,
		dogMiniGameRightBtnFun = function()
			if self.level >= xyd.models.selfPlayer:getDogMiniPassLevel() then
			...
			if self.level > allLevelIds[#allLevelIds] then
		...
		dogLevel = self.level,
		dogIsShowItems = self.level > xyd.models.selfPlayer:getDogMiniPassLevel()
	})
