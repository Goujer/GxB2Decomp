-- chunkname: @../../../Product/Bundles/Android/src/app/windows/NewbieCampListWindow.lua
...
function NewbieCampListItem:onClickAward()
	if self.status_ == 0 then
		...
		if level_limit and #limit_params == 1 and level_limit > xyd.models.backpack:getLev() then
			xyd.alertTips(__("FUNC_OPEN_LEV", level_limit))

			return
		elseif level_limit and level_limit > xyd.models.backpack:getLev() and vip_limit > xyd.models.backpack:getVipLev() then
...
function NewbieCampListItem:updateBtn()
	...
	elseif self.status_ == 1 then
		...
		self.btnAwardLabel_.color = Color.New2(4294967295)
