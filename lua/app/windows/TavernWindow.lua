-- chunkname: @../../../Product/Bundles/Android/src/app/windows/TavernWindow.lua
...
function MissionItem:initUI()
	...
	if self.parent_.isShowUp ~= -1 then
		...
		xyd.setUISpriteAsync(self.upIcon.gameObject:GetComponent(typeof(UISprite)), nil, "common_tips_up_" .. self.parent_.isShowUp, nil, nil, nil)
	else
...
function MissionItem:update(_, _, missionData)
	...
	self.progressDesc_.color = Color.New2(4294967295)
	...
	if tonumber(cost[2]) == 0 then
		...
		xyd.setUISpriteAsync(self.btnSpeedIcon_, nil, "item_2", nil, nil)
	...
	if not self.star_ or self.star_ ~= star then
		...
		xyd.setUISpriteAsync(self.starBg_, nil, "pub_mission_star" .. self.star_, nil, nil)
...
function MissionItem:chargeLockStatus()
	if self.data_.is_lock == 1 then
		xyd.setUISpriteAsync(self.lockBtn_, nil, "partner_lock_btn", nil, nil)
	else
		xyd.setUISpriteAsync(self.lockBtn_, nil, "partner_unlock_btn", nil, nil)
	end
end
...
function MissionItem:changeBarValue(isRefresh)
	...
	if self.missionTime_ - duration <= 2 or self.data_.status == missionStatus.DONE then
		...
		if not isRefresh then
			...
			params.duration = self.missionTime_ - duration

			function params.doOnTime()
				self:onTime()
			end
...
function MissionItem:onClickLock(isRefresh)
	...
	if self.data_.status ~= 0 then
		...
		params.alertType = xyd.AlertType.TIPS
		params.message = __("PUB_LOCK_MISSION")
	...
	isLock = self.data_.is_lock == 1 and 0 or 1
...
function MissionItem:setProgressEffect(state)
	if state then
		...
		self.progressDesc_.color = Color.New2(4294908671)
		self.progressDesc_.effectStyle = UILabel.Effect.Outline
		self.progressDesc_.effectColor = Color.New2(255)
	else
		if self.progressEffect_ then
...
function MissionItem:onClickComplete()
	...
	if star >= 6 then
		...
		params.message = __("PUB_COMPLETE_MISSION")
...
function MissionItem:onClickCancel()
	...
	params.alertType = xyd.AlertType.YES_NO
	params.message = __("PUB_MISSION_CANCEL")
...
function TavernWindow:initScrollList(refreshMissionList, keepPosition)
	...
	for idx, missionID in ipairs(missionIDs) do
		...
		if self.currentSortType_ and self.currentSortType_ ~= star then
			-- block empty
		else
