-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ShrineHurdleChooseBuffWindow.lua
...
function ShrineHurdleChooseBuffWindow:close(callback, skipAnimation)
	...
	if self.wndType_ == 3 and floor_id > tonumber(close_floor) and not self.canClose_ then
...
function ShrineHurdleChooseBuffWindow:onClickChoose()
	...
	elseif self.wndType_ == 3 then
		...
		if self.costNum_ and self.costNum_ > xyd.models.shrineHurdleModel:getGold() then
...
function ShrineHurdleChooseBuffWindow:showGoldChange(changeNum)
	if changeNum > 0 then
		...
		self.resItemChangeLabel_.color = Color.New2(2751463679)
		self.resItemChangeLabel_.text = changeNum
	end
	...
	local function autoFunction()
		...
		elseif self.wndType_ == 3 then
			...
			for i = 1, 3 do
				...
				if (not skill_lev or skill_lev == 0) and xyd.models.shrineHurdleModel:getGold() >= cost[2] then
					buy_index = i
			...
			if buy_index == 0 then
				...
				for i = 1, 3 do
					...
					if skill_lev == 1 then
						...
						if xyd.models.shrineHurdleModel:getGold() >= cost[2] then
							upgrade_index = i
