-- chunkname: @../../../Product/Bundles/Android/src/app/windows/CampaignWindow.lua
...
function StageItem:initItem()
	...
	if self:checkStatus() then
		...
		if tonumber(self.stageId) + 1 > self.maxStageId then
			-- block empty
		end
	else
		self:lock()
...
function StageItem:checkStatus()
	...
	if tonumber(self.stageId) > tonumber(self.maxStageId) then
		res = false
	end
...
function CampaignWindow:initWindowTop()
	self.windowTop = WindowTop.new(self.window_, self.name_, nil, nil, function()
...
function CampaignWindow:showResAction()
	...
	local awardGold, awardPartnerExp
	local rate = 1
...
function CampaignWindow:iosTestChangeUI()
	...
	label.effectColor = Color.New2(3429853951)
