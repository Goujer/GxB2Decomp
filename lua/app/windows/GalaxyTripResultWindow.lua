-- chunkname: @../../../Product/Bundles/Android/src/app/windows/GalaxyTripResultWindow.lua
...
function GalaxyTripResultWindow:updateShow(index)
	...
	if index == 0 then
		...
		xyd.setUISpriteAsync(self.upGalaxyIcon, nil, imgIcon, nil, nil, true)
...
function GalaxyTripResultWindow:updateItemIconShow(i, isChoice)
	...
	elseif self.defaultStates[i] == xyd.GalaxyTripGridStateType.NOT_YET_SEARCH then
		iconStr = isChoice and "galaxy_trip_ts_ing1" or "galaxy_trip_ts_ing2"
	end

	xyd.setUISpriteAsync(self["itemIcon" .. i], nil, iconStr, nil, nil, true)
end
