-- chunkname: @../../../Product/Bundles/Android/src/app/windows/GalaxyTripMainWindow.lua
...
function GalaxyItemClass:update(index, realIndex, info)
	...
	if self.ballId ~= xyd.models.galaxyTrip:getBossMapId() and (allMaxId < self.ballId or mapSeason > xyd.models.galaxyTrip:getGalaxyTripGetMainCount()) then
		xyd.setUISpriteAsync(self.galaxyImg, nil, "galaxy_trip_ball_lock", nil, nil, true)
	else
		local imgIcon = xyd.tables.galaxyTripMapTable:getIconText(self.ballId)

		xyd.setUISpriteAsync(self.galaxyImg, nil, imgIcon, nil, nil, true)
	end
...
function GalaxyItemClass:onTouch()
	...
	if mapSeason > xyd.models.galaxyTrip:getGalaxyTripGetMainCount() then
