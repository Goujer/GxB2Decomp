-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityEntranceTestPveWindow.lua
...
function ActivityEntranceTestPveWindow:registerEvent()
	...
	UIEventListener.Get(self.arrowRight.gameObject).onClick = handler(self, function()
		if self.testType >= self.activityData:getPveMaxStage() then
			self.testType = 1
		else
			self.testType = self.testType + 1
		end
...
function ActivityEntranceTestPveWindow:updateFreeTimesShow()
	...
	if times <= 0 then
		self.overChallengeTimeText.color = Color.New2(3422556671)
	else
