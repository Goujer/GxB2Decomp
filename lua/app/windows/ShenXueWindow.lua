-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ShenXueWindow.lua
...
function ShenXueWindow:updateBatchShenXueBtn()
	...
	for key in pairs(partners) do
		if partners[key]:getStar() == 4 and not partners[key]:isLockFlag() then
			...
			selectedPartners[partners[key]:getPartnerID()] = true
...
function ShenXueWindow:updateSelectBtn(index)
	for i = 1, 6 do
		if index == i then
			...
			self["label" .. i].color = Color.New2(4294967295)
			...
			self["label" .. i].effectColor = Color.New2(4294967295)
		end
	end
end
...
function ShenXueWindow:onSelectContainer(id)
	local optionalList, materialList, mTableID
...
function ShenXueWindow:initResGroup(partnerInfo, desStar)
	...
	for i = 1, 3 do
		...
		if resNum < costNum then
			self["labelResNum" .. i].color = Color.New2(3422556671)
