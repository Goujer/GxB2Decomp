-- chunkname: @../../../Product/Bundles/Android/src/app/windows/GambleRewardsWindow.lua
...
function GambleRewardsWindow:initCost()
	if self.curWndType_ == self.WindowType.GAMBLE then
		...
		xyd.setUISpriteAsync(self.btnBuyIcon_, nil, xyd.tables.itemTable:getIcon(self.buyCost_[1]), nil, nil)
	...
	elseif self.curWndType_ == self.WindowType.DRESS then
		local cost, free_times, iconName
		...
		if self.curBuyType_ == self.buyType.ONE then
			if free_times and free_times > 0 then
				...
				if self.type_ == 1 then
					...
					xyd.setUISpriteAsync(self.btnBuyIcon_, nil, "icon_258", nil, nil)
					...
					xyd.setUISpriteAsync(self.btnBuyIcon_, nil, "icon_259", nil, nil)
				end
...
function GambleRewardsWindow:buyTouch(isCost2)
	if self.curWndType_ == self.WindowType.GAMBLE then
		if xyd.models.backpack:getItemNumByID(self.buyCost_[1]) >= self.buyCost_[2] then
			...
			if i < #actions - 1 then
				...
				local function callback()
					...
					if self.afterAnimationCallback then
						self.afterAnimationCallback(self)
					end
				end
