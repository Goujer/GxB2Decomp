-- chunkname: @../../../Product/Bundles/Android/src/app/windows/QuestionnaireWindow.lua
...
function QuestionnaireWindow:onNextTouch()
	...
	if hasLimit and #self.select_items_ > self.limit_ then
...
function QuestionnaireWindowItem:refreshSelect(flag)
	...
	if flag then
		xyd.setUISpriteAsync(self.selectImg_, "qustionnaire_web", "questionnaire_select_png", nil, nil)
	else
		xyd.setUISpriteAsync(self.selectImg_, "qustionnaire_web", "questionnaire_unselect_png", nil, nil)
	end
...
function QuestionnaireMatrixItem:init(parentGo)
	...
	for i = 1, #self.options_ do
		...
		xyd.setUISpriteAsync(sp, "CommonUI", "setting_up_lan1_png", nil, nil)
		...
		UIEventListener.Get(obj).onClick = handler(self, function()
			if self.cur_select_ ~= self.options_[i] then
				xyd.setUISpriteAsync(sp, "CommonUI", "setting_up_lan2_png", nil, nil)

				self.cur_select_ = self.options_[i]

				if self.cur_img_ then
					xyd.setUISpriteAsync(self.cur_img_, "CommonUI", "setting_up_lan1_png", nil, nil)
				end
