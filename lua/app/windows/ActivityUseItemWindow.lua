-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityUseItemWindow.lua
	...
	for i = 1, 2 do
		...
		UIEventListener.Get(self["BtnUse_" .. i]).onClick = function()
			if self["CurNum_" .. i] > 0 then
				...
				if self["CurNum_" .. i] >= self.onceUseMax then
				...
				params.message = __("NOT_ENOUGH", xyd.tables.itemTextTable:getName(self.itemList[i][1]))
...
function ActivityUseItemWindow:useCallback(event)
	if self.usedTotal < self["CurNum_" .. self.curUsedItemIndex] then
		...
		if self.usedTotal >= self.onceUseMax then
			if self.time < 1 then
