-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ItemTips.lua
...
function ItemTips:checkSpecilGetWayNotShow(getWayID)
	if getWayID == 24 then
		...
		if vip[1] > Backpack:getVipLev() and level > Backpack:getLev() then
...
function ItemTips:canSummon()
	...
	if self.showBagType_ and partnerCost and partnerCost[2] and self.itemNum and (self.showBagType_ == xyd.BackpackShowType.DEBRIS or self.showBagType_ == xyd.BackpackShowType.CONSUMABLES) and self.itemNum >= partnerCost[2] then
		return true
	end
...
function ItemTips:initDesc()
	...
	if self.wndType_ == xyd.ItemTipsWndType.BACKPACK and not canSell and self:canSummon() == false then
		...
		if tmpType >= xyd.ItemType.WEAPON and tmpType <= xyd.ItemType.SHOES then
			label.height = 27
	...
	do
		...
		while i < #self.descs_ do
			...
			if label.text == __("NOT_SELL") then
				label.color = Color.New2(4278190335)
			end
		...
		for k, act in ipairs(acts) do
			...
			if self.data.equipedPartner then
				...
				if isFit then
					...
					label.color = Color.New2(3613720831)
				end
			end
		...
		if #suitName > 0 then
			...
			if not self.data.equipedPartner then
				...
				if p_job ~= suitJob then
					...
					label2.color = Color.New2(2593823487)
					label3.color = Color.New2(2593823487)
...
function ItemTips:summonTouch()
	...
	if ItemTable:getType(self.itemID) == nil then
		...
		if self.itemNum >= treasureCost[1] then
		...
		if type == xyd.ItemType.HERO_RANDOM_DEBRIS and xyd.tables.itemTable:getGroup(self.itemID) == xyd.PartnerGroup.TIANYI and math.floor(self.itemNum / partnerCost[2]) >= 1 then
			...
			local tempParams = {
				selectMinNum = 1,
				sureCallback = function(itemID, num)
				...
				itemsInfo = items,
				itemNum = math.floor(self.itemNum / partnerCost[2])
			}
