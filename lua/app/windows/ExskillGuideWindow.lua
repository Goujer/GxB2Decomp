-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ExskillGuideWindow.lua

local BaseWindow = import("app.windows.BaseWindow")
local ExskillGuideWindow = class("ExskillGuideWindow", BaseWindow)

function ExskillGuideWindow:ctor(name, params)
	ExskillGuideWindow.super.ctor(self, name, params)

	self.wnd = params.wnd
	self.table_ = params.table or xyd.tables.partnerExskillGuideTable
...
function ExskillGuideWindow:initWindow()
	ExskillGuideWindow.super.initWindow(self)
	self:getUIComponent()
...
function ExskillGuideWindow:getUIComponent()
...
function ExskillGuideWindow:runGuide(index)
...
function ExskillGuideWindow:getCurIndex()
	return self.curIndex_
end

function ExskillGuideWindow:initDialog(index, obj)
	...
	if desc and desc ~= "" then
		...
		if self.labelDesc1.spacingY + size >= self.labelDesc1.height then
...
function ExskillGuideWindow:getWnd(index)
...
function ExskillGuideWindow:getWndName(index)
	if self.table_ and self.table_.getWindowName then
		local winName = self.table_:getWindowName(index)
...
function ExskillGuideWindow:checkCurWindowIsOpen(index)
	if self.table_ and self.table_.getWindowName then
		local winName = self.table_:getWindowName(index)
...
function ExskillGuideWindow:showHand(objPos, index)
	local handPos = {}
...
function ExskillGuideWindow:playOpenAnimation(callback)
	ExskillGuideWindow.super.playOpenAnimation(self, callback)
...
function ExskillGuideWindow:getFirsetIndex()
	local index = 1
	...
	elseif self:isTimeCloister() or self:isArcticExpedition() then
...
function ExskillGuideWindow:checkCurIndex(index)
	local flag = true
	...
	if self.curGuideType_ == xyd.GuideType.PARTNER_EXSKILL then
		local ids = xyd.tables.partnerExskillGuideTable:getIDs()

		if index > ids[#ids] then
			flag = false
		end
	end

	if self:isTimeCloister() and self.curGuideType_ ~= self.table_:getType(index) then
		flag = false
	end

	if self:isArcticExpedition() and self.curGuideType_ ~= self.table_:getType(index) then
		flag = false
	end
...
function ExskillGuideWindow:dispose()
	local wnd = xyd.WindowManager.get():getWindow("partner_detail_window")

	if wnd then
		wnd.windowTop:setCloseBtnState(true)
	end

	ExskillGuideWindow.super.dispose(self)
end
...
function ExskillGuideWindow:isArcticExpedition()
	if self.curGuideType_ == xyd.GuideType.ARCTIC_EXPEDITION_1 or self.curGuideType_ == xyd.GuideType.ARCTIC_EXPEDITION_2 or self.curGuideType_ == xyd.GuideType.ARCTIC_EXPEDITION_3 or self.curGuideType_ == xyd.GuideType.ARCTIC_EXPEDITION_4 then
		return true
	end
...
function ExskillGuideWindow:runIndexFunction()
	if self.curGuideType_ == xyd.GuideType.ARCTIC_EXPEDITION_1 and self.curIndex_ == 33 then
		xyd.WindowManager.get():closeWindow("arctic_expedition_cell_detail_window")
		xyd.WindowManager.get():closeWindow("arctic_expedition_cell_window")
	end
end

return ExskillGuideWindow
