-- chunkname: @../../../Product/Bundles/Android/src/app/windows/WindowManager.lua
...
function WindowManager:addWindow(window)
	...
	if (layerType == xyd.UILayerType.FULL_SCREEN_UI or layerType == xyd.UILayerType.FLOATING_UI) and backType > xyd.BacksType.UNKONOW then
...
function WindowManager:screenToWorldPoint(positon)
	if positon.x < 0 then
		positon.x = 0
	elseif positon.x > UnityEngine.Screen.width then
		positon.x = UnityEngine.Screen.width
	end

	if positon.y < 0 then
		positon.y = 0
	elseif positon.y > UnityEngine.Screen.height then
