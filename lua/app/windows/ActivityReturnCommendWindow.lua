-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityReturnCommendWindow.lua
...
function ActivityReturnCommendWindow:initNav()
	...
	local labelStates = {
		...
		unchosen = {
			...
			effectColor = Color.New2(4278124287)
		}
	}
...
function FriendSearchItem:update(index, realIndex, info, setType)
	...
	if targetPower <= self.data_.arena_power and labelNum < 3 then
		xyd.setUISpriteAsync(self["label" .. labelNum], nil, "station_label_pink", nil, nil)

		self["label" .. labelNum .. "_text"].text = __("ACTIVITY_RETURN_COMMEND_LABEL_POWER")
		self["label" .. labelNum .. "_text"].effectColor = Color.New2(3765474047)
		labelNum = labelNum + 1
	end
	...
	if targetLev <= self.data_.lev and labelNum < 3 then
		xyd.setUISpriteAsync(self["label" .. labelNum], nil, "station_label_orange", nil, nil)

		self["label" .. labelNum .. "_text"].text = __("ACTIVITY_RETURN_COMMEND_LABEL_LEV")
		self["label" .. labelNum .. "_text"].effectColor = Color.New2(3515703039)
		labelNum = labelNum + 1
	end
	...
	if targetActiveTime > xyd.getServerTime() - self.data_.last_time and labelNum < 3 then
		xyd.setUISpriteAsync(self["label" .. labelNum], nil, "station_label_green", nil, nil)
