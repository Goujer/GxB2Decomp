-- chunkname: @../../../Product/Bundles/Android/src/app/windows/StarryAltarWindow.lua
...
function StarryAltarWindow:register()
	...
	UIEventListener.Get(self.mainActNode).onClick = handler(self, function()
		xyd.goWay(214, nil, nil, function()
...
function StarryAltarWindow:updateActivityLayout()
	...
	if self.actData and self.actData.detail and limit > self.actData.detail.is_completeds[missionId] then
...
function StarryAltarWindow:initActivityLayout()
	if self.isActOpen then
		...
		if limit > self.actData.detail.is_completeds[missionId] then
...
function StarryAltarWindow:checkSummonId()
	local normalID, actCost1ID, actCost2ID
