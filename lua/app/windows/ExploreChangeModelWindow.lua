-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ExploreChangeModelWindow.lua

local BaseWindow = import(".BaseWindow")
local ExploreChangeModelWindow = class("ExploreChangeModelWindow", BaseWindow)
...
function ExploreChangeModelWindow:ctor(name, params)
...
function ExploreChangeModelWindow:getUIComponent()
...
function ExploreChangeModelWindow:loadFilterFromStorage()
	local value = xyd.db.misc:getValue("xwtx_filter")
...
function ExploreChangeModelWindow:layout()
	self.labelTitle_.text = __("TRAVEL_MAIN_TEXT61")
	self.buttomLabel.text = __("TRAVEL_MAIN_TEXT63")
	self.buttomBtnLabel.text = __("FOR_SURE")
	self.buttomTitle.text = __("TRAVEL_MAIN_TEXT62")
	...
	self.showRandom = tonumber(xyd.db.misc:getValue("xwtx_show_random")) == 1 and true or false
...
function ExploreChangeModelWindow:willCloseFunc()
...
function ExploreChangeModelWindow:initData(type)
	local pictures = self.backpack:getExplorePictures()
	...
	for i = 0, xyd.GROUP_NUM do
		table.sort(self.items_[i], function(a, b)
			...
			if a_id == b_id and xyd.tables.itemTable:getType(a_id) == xyd.ItemType.HERO and xyd.tables.partnerTable:getGroup(a_id) == xyd.PartnerGroup.TIANYI then
				...
				return xyd.arrayIndexOf(showIDs, skin_id_a) > xyd.arrayIndexOf(showIDs, skin_id_b)
			end
...
function ExploreChangeModelWindow:changeDataGroup(keepPosition)
	if self.selectJobIndex ~= 0 then
...
function ExploreChangeModelWindow:jobFilter()
	local data = {}
...
function ExploreChangeModelWindow:initChosenPartnerGroup()
	NGUITools.DestroyChildren(self.itemGroup.transform)

	local initChosenArray
	local value = xyd.db.misc:getValue("xwtx_chosen_models")

	if value then
		initChosenArray = cjson.decode(value)
	else
		initChosenArray = {
			(tonumber(xyd.split(xyd.tables.miscTable:getVal("travel_event_model_u3"), "|")[1]))
		}
	end
...
function ExploreChangeModelWindow:sortPictures(pictures, type_)
	...
	table.sort(pictures, function(a, b)
		...
		local aStar, bStar, aGroup, bGroup
...
function ExploreChangeModelWindow:unSelectForMainGroup(itemID)
...
function ExploreChangeModelWindow:onClickbuttomBtn()
	...
	xyd.db.misc:setValue({
		key = "xwtx_chosen_models",
		value = cjson.encode(chosenArray)
	})
	...
	xyd.db.misc:setValue({
		key = "xwtx_show_random",
		value = val
	})

	if self.confirmCallback_ then
		self.confirmCallback_(chosenArray[1])
	end

	xyd.WindowManager.get():closeWindow("explore_change_model_window")
end

function ExploreChangeModelWindow:onItemClick(id, isSelect)
	if isSelect then
...
function ExploreChangeModelWindow:unSelect(id)
	if id == nil then
		return false
	end
...
function ExploreChangeModelWindow:changeJobGroup(index)
...
function ExploreChangeModelWindow:onClickSortBtn()
...
function ExploreChangeModelWindow:movegroupSort_()
...
function ExploreChangeModelWindow:changeSortType(sortType)
...
return ExploreChangeModelWindow
