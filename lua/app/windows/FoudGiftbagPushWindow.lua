-- chunkname: @../../../Product/Bundles/Android/src/app/windows/FoudGiftbagPushWindow.lua
...
function FoudGiftbagPushWindow:setLevelFoudUIComponent()
	...
	if activityData.detail.charges[1].buy_times >= activityData.detail.charges[1].limit_times then
		xyd.setUISpriteAsync(self.gainText_, nil, "level_fund_gain_" .. xyd.Global.lang, nil, nil, true)
	else
		xyd.setUISpriteAsync(self.gainText_, nil, "level_fund_addup_" .. xyd.Global.lang, nil, nil, true)
	end

	xyd.setUISpriteAsync(self.totalText_, nil, "level_fund_total_" .. xyd.Global.lang, nil, nil, true)

	local gainNum, totalNum = 0, 0
	...
	for i = 1, string.len(gainNum) do
		...
		xyd.setUISpriteAsync(go:GetComponent(typeof(UISprite)), nil, "level_fund_num" .. num, nil, nil, true)
	end
...
function FoudGiftbagPushWindow:setTowerFoudUIComponent(giftbag_id)
	...
	if level_ == 1 then
		...
		xyd.setUISpriteAsync(self.imgTitle_, nil, "tower_fund_giftbag_logo_" .. xyd.Global.lang, nil, nil, true)
	else
		self.imgPageNum_.gameObject:SetActive(true)
		xyd.setUISpriteAsync(self.imgTitle_, nil, "tower_fund_giftbag_logo_" .. xyd.Global.lang .. "2", nil, nil, true)
	...
	self.label1_.color = Color.New2(2234264831)
	self.label2_.text = __("NEW_RECHARGE_TEXT10")
	self.label2_.fontSize = 22
	self.label2_.color = Color.New2(2739775743)
