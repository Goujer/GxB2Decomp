-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityFishingCollectWindow.lua
...
function ActivityFishingCollectWindow:showInfo(index)
	...
	elseif self.curInfoID == index then
		...
		if self.fishDatas[index].max >= goldLenRange[2][1] then
			xyd.setUISpriteAsync(iconMax, nil, "activity_fishing_icon_gold")
		elseif self.fishDatas[index].max >= silverLenRange[2][1] then
