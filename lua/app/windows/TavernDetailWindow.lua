-- chunkname: @../../../Product/Bundles/Android/src/app/windows/TavernDetailWindow.lua
...
function TavernDetailWindow:selectHero(partner_id, notMove, posBefore)
	if self:checkSelect(partner_id) then
		...
		params.message = __("PUB_START_MISSION_6")
	...
	if not emptyRootData then
		...
		params.alertType = xyd.AlertType.TIPS
		params.message = __("PUB_MISSION_FULL_HERO")
...
function TavernDetailWindow:updateConditions()
	...
	for _, condition in ipairs(self.tempConditions_) do
		...
		if condition.star and maxStar >= condition.star then
			bingoImg.gameObject:SetActive(true)
...
function TavernDetailWindow:changeStartBtn()
	if not self:checkCanStart() then
		self.btnStart_.color = Color.New2(2560137471)
	else
		self.btnStart_.color = Color.New2(4294967295)
	end
end
...
function TavernDetailWindow:onClickBtnFastStart(_, noNeedTips)
	...
	if needNum > #partners and not noNeedTips then
		...
		params.alertType = xyd.AlertType.TIPS
		params.message = __("PUB_MISSION_TIPS")
		...
		if not tavernModel:checkHeroIsSelect(partnerID) then
			...
			if partner then
				...
				for _, data in ipairs(self.tempConditions_) do
					if not hasNumberInList(tmpConditions, data.conditionID) and (data.group and data.group == partner:getGroup() or data.job and data.job == partner:getJob() or data.star and star >= data.star) then
