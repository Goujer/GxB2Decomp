-- chunkname: @../../../Product/Bundles/Android/src/app/windows/VipWindow.lua
...
function VipWindow:setVIPValue()
	...
	if vip >= xyd.models.backpack:getMaxVipLev() then
...
function VipWindow:setPurchaseItem()
	...
	for i = 1, #ids do
		...
		if rank and data and isNotSubscrition and not isNeedReview then
			...
			if isDayilyGift and (data.end_time or 0) >= xyd.getServerTime() then
...
function VipWindow:setContent(vip)
	if vip <= 0 or vip > xyd.models.backpack:getMaxVipLev() then
		return
	end
...
function VipWindow:onScrollEnd(event)
	...
	if event.localX - self.scrollX < -20 then
		if self.currentBenefitVip >= xyd.models.backpack:getMaxVipLev() then
			return
		end
...
function RechargeItem:setMonthCard()
	...
	if self.endTime and self.endTime > xyd.getServerTime() then
