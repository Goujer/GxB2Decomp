-- chunkname: @../../../Product/Bundles/Android/src/app/windows/FriendBossSwitchBossLevelWindow.lua
...
function FriendBossSwitchBossLevelWindow:packBossLevelParams(id)
	local bossLevelParams = {
		...
		id = 0,
		isUnLock = function()
			if tonumber(self.unlockedMaxBossLevel) >= tonumber(id) then
				return true
			else
				return false
			end
	...
	bossLevelParams.id = id
	bossLevelParams.power = xyd.tables.friendBossTable:getNumber(id, "power")
	bossLevelParams.battleId = xyd.tables.friendBossTable:getNumber(id, "battle_id")
	bossLevelParams.killAward = xyd.tables.friendBossTable:getString(id, "kill_award")
	bossLevelParams.avatarId = xyd.tables.friendBossTable:getNumber(id, "avatar_id")
...
function BossLevelItem:fillBossLevelSign(data)
	...
	xyd.setUISpriteAsync(self.bossLevelGroup, nil, "quiz_diff_" .. tostring(bossLevel), nil, nil)
end
