-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityMonthlyHikeSkillWindow.lua
...
function SkillIcon:updateLine()
	...
	elseif followIDs[1] and followIDs[1] > 0 and not followIDs[2] then
		...
		if #limits <= 1 then
			...
			if self.id_ == limits[1][1] and self:getSkillLev() >= limits[1][2] then
			...
			if pos[2] > xyd.tables.activityMonthlySkillTable:getPos(self.id_)[2] then
				fangxiang = -1
			end

			if self.id_ == limits[1][1] and self:getSkillLev() >= limits[1][2] or self.id_ == limits[2][1] and self:getSkillLev() >= limits[2][2] then
...
function SkillIcon:updateBtn()
	...
	if self.addLev == 0 then
		...
		self.leftBtn.color = Color.New2(4294967295)
	...
	if maxLev <= self:getSkillLev() then
		...
		self.rightBtn.color = Color.New2(4294967295)
