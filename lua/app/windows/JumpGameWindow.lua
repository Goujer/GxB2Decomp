-- chunkname: @../../../Product/Bundles/Android/src/app/windows/JumpGameWindow.lua
...
function JumpGameWindow:CreateStage()
	...
	if self.stageTypePool[1] == 6 then
		...
		if self.nowEmpty > xyd.tables.miscTable:getNumber("nsshaft_max_empty_line", "value") then
	...
	elseif #self.stages == stageLimit then
		...
		for j = 1, stageLimit do
			if stageLimitX >= self.stages[j].transform.localPosition.x then
				flag = true
			end
		end
		...
		for j = 1, stageLimit do
			if stageLimitX >= math.abs(self.stages[#self.stages - stageLimit + j].transform.localPosition.x - topX) then
				flag = true
			end
		end
	...
	if self.stageTypePool[1] == 1 then
		stage:GetComponent(typeof(CollisionDetecter)).collisionEnterCall = function()
			if self.model.transform.localPosition.y - 78 > stage.transform.localPosition.y then
...
function JumpGameWindow:register()
	...
	UIEventListener.Get(self.touchField).onPress = function(go, isPressed)
		if isPressed == true then
			if xyd.mouseToLocalPos(self.window_:NodeByName("groupAction")).x > self.model.transform.localPosition.x then
				self.pressSpeed = 0.7
				self.model.velocity = Vector3(self.pressSpeed + self.stageMoveSpeed, self.model.velocity.y, 0)
			else
				self.pressSpeed = -0.7
