-- chunkname: @../../../Product/Bundles/Android/src/app/windows/GuildWindow.lua
...
function GuildWindow:onTouchTranlation()
	...
	if not self.showTrans then
		...
		self.inTrans = true
		self.guildMsg = {}
		self.guildMsg.inTransl = true
		self.guildMsg.originalContent = data.announcement or ""
		self.guildMsg.content = xyd.models.acDFA:preTraslation(data.announcement or "")
	...
	local function sort_(a, b)
		...
		if tonumber(a.job) == tonumber(b.job) then
			if tonumber(a.is_online) == tonumber(b.is_online) then
				result = tonumber(a.last_time) > tonumber(b.last_time)
			else
				result = tonumber(a.is_online) > tonumber(b.is_online)
			end
		else
			result = tonumber(a.job) > tonumber(b.job)
		end
...
function GuildWindow:updateUpIcon()
	if xyd.models.activity:isResidentReturnAddTime() then
		...
		xyd.setUISpriteAsync(self.btnCheckIn_upIcon.gameObject:GetComponent(typeof(UISprite)), nil, "common_tips_up_" .. return_multiple, nil, nil, nil)
