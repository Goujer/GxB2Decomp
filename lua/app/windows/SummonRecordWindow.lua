-- chunkname: @../../../Product/Bundles/Android/src/app/windows/SummonRecordWindow.lua
...
function SummonRecordWindow:onDragMoving()
	if self.scrollView then
		...
		if math.abs(self.scrollView.transform.localPosition.y - mainOffset) > scrollThresholdY then
...
function SummonRecordWindow:onSummonLog(event)
	...
	for i = 1, #list do
		...
		table.sort(partners, function(a, b)
			return xyd.tables.partnerTable:getStar(a) > xyd.tables.partnerTable:getStar(b)
		end)
		table.insert(collection, {
			type = type,
			times = times,
...
function SummonRecordWindowItem:onDragMoving()
	if self.scroller and self.parent.scrollView then
		...
		if math.abs(self.scroller.transform.localPosition.x - itemOffset) > scrollThresholdX then
...
function SummonRecordWindowItem:updateInfo()
	...
	if isStarry then
		...
		xyd.setUISpriteAsync(self.summonTypeIcon, nil, costIcon, nil, nil, true)
		...
		if type == SUMMON_TYPE.NORMAL then
			xyd.setUISpriteAsync(self.summonTypeIcon, nil, "summon_record_normal_icon", nil, nil, true)
		elseif type == SUMMON_TYPE.HIGH then
			xyd.setUISpriteAsync(self.summonTypeIcon, nil, "summon_record_high_icon", nil, nil, true)
		...
		elseif type == SUMMON_TYPE.FRIENDSHIP then
			xyd.setUISpriteAsync(self.summonTypeIcon, nil, "summon_record_friendship_icon", nil, nil, true)
		elseif type == SUMMON_TYPE.GUARANTEE then
			xyd.setUISpriteAsync(self.summonTypeIcon, nil, "summon_record_guarantee_icon", nil, nil, true)
		else
			xyd.setUISpriteAsync(self.summonTypeIcon, nil, "summon_record_ticket_icon", nil, nil, true)
		end
