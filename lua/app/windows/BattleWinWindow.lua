-- chunkname: @../../../Product/Bundles/Android/src/app/windows/BattleWinWindow.lua
...
function BattleWinWindow:ctor(name, params)
	...
	self.reportTypeList = {
		...
		[16] = xyd.BattleType.SOUL_LAND_REPORT
	}
...
function BattleWinWindow:initLayout()
	...
	elseif self.battleType == xyd.BattleType.GALAXY_TRIP_SPECIAL_BOSS_BATTLE then
		...
		if items and #items > 0 then
			...
			self.labelDamage1.effectColor = Color.New2(4294967295)
...
function BattleWinWindow:updateScroePart()
	...
	if nextId > 0 and selectInfo.select and tonumber(selectInfo.select) ~= 0 then
		...
		self.labelTime.color = Color.New2(4292346111)
...
function BattleWinWindow:updateGuildNewWarPart()
	...
	if not self.params_.is_last then
		...
		xyd.setUISpriteAsync(leftPoint, nil, "battle_round_num_" .. winTime, nil, nil, true)
		xyd.setUISpriteAsync(rightPoint, nil, "battle_round_num_" .. lostTime, nil, nil, true)
...
function BattleWinWindow:initNextBtn(str, callback)
	...
	xyd.setUISpriteAsync(self.nextBtn_sprite, nil, "blue_btn_65_65", nil, nil)

	self.nextBtn_button_label.text = __(str)
	self.nextBtn_button_label.color = Color.New2(4294967295)
...
function BattleWinWindow:initTower()
	...
	if maxTowerNum > self.stageId and not isRecord then
		local function callback()
...
function BattleWinWindow:initGuildCompetition()
	...
	if self.battleParams.type == 1 then
		...
		self.labelDamage1.effectStyle = UILabel.Effect.Outline8
		self.labelDamage1.fontSize = 24
		self.labelDamage2.color = Color.New2(3613720831)
		self.labelDamage2.effectColor = Color.New2(4294967295)
...
function BattleWinWindow:initEntranceTest()
	...
	self.labelDamage2.color = Color.New2(4182721023)
	self.labelDamage2.effectColor = Color.New2(4294967295)
	self.labelDamage2.effectStyle = UILabel.Effect.Outline8
	...
	elseif self.battleParams.isFirstPass or self.battleParams.isShowNewHarm then
		...
		if self.battleParams.isFirstPass then
			xyd.setUISpriteAsync(newImgUISprite, nil, "activity_entrance_bg_sctg_" .. xyd.Global.lang, nil, nil, true)
		elseif self.battleParams.isShowNewHarm then
			xyd.setUISpriteAsync(newImgUISprite, nil, "activity_entrance_bg_xjl_" .. xyd.Global.lang, nil, nil, true)
		end
	end
end
...
function BattleWinWindow:completeActivityProgress()
	...
	for i = 1, maxLev do
		if self.battleParams.total_harm >= RewardTable:getDamage(i) then
			lev = i
		else
			break
		end
...
function BattleWinWindow:initDogMiniGame()
	...
	if self.params_.dogLevel >= allLevelIds[#allLevelIds] then
