-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityEnconterStoryFightWindow.lua
...
function FightItem:setInfo(params)
	...
	if xyd.getServerTime() < activity_open_time + xyd.DAY_TIME * self.info_.open_time then
		...
		if stageNow < 0 or stageNow > self.info_.id then
			self.redPoint:SetActive(false)
		else
...
function ActivityEnconterStoryFightWindow:getChallengeInfo(id)
	...
	params.id = id
	params.battle_id = BattleTable:getBattleID(id)
	params.initial_partner = BattleTable:getOptionalPartners(id)
	params.awards = BattleTable:getAwards(id)
	params.open_time = BattleTable:getOpenDay(id)
	params.text = {}
