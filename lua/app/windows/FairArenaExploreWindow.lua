-- chunkname: @../../../Product/Bundles/Android/src/app/windows/FairArenaExploreWindow.lua
...
function FairArenaExploreWindow:initUIComponent()
	xyd.setUISpriteAsync(self.textImg_, nil, "fair_arena_text01_" .. xyd.Global.lang, nil, nil, true)
...
function FairArenaExploreWindow:onClickSelect(index)
	...
	if self.selectType == 1 then
		scale = pos[3] < 7 and 0.8981481481481481 or 0
...
function FairArenaExploreWindow:onBackpack()
	...
	if isCanShowFirstEquipRedPoint and isCanShowFirstEquipRedPoint == "2" and (not timeStamp or timeStamp < self.activityData:startTime() or timeStamp > self.activityData:getEndTime()) then
...
function FairArenaExploreWindow:onBattle()
	...
	elseif self.data.is_fail > 0 and self.data.fail_times > 0 and self.data.explore_stage < 11 then
...
function FairArenaExploreWindow:onClickShovel(index)
	...
	elseif self.data.explore_stage < 11 then
...
function FairArenaExploreWindow:checkIsEnd()
	if self.activityData:getEndTime() - xyd.getServerTime() > xyd.TimePeriod.DAY_TIME then
		return false
	end
