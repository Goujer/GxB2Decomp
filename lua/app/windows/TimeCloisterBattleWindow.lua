-- chunkname: @../../../Product/Bundles/Android/src/app/windows/TimeCloisterBattleWindow.lua
...
function TimeCloisterBattleWindow:onGetBattleResult(event)
	...
	if data.battle_result and data.battle_result.is_win == 1 then
		if self.cloister >= xyd.TimeCloisterMissionType.THREE and self.maxStage % 11 == 10 then
...
function StageItem:updateInfo()
	...
	if next_id ~= -1 then
		xyd.setUISpriteAsync(self.btnPreview_UISprite, nil, "attr_exclamation", nil, nil, nil)
	else
		xyd.setUISpriteAsync(self.btnPreview_UISprite, nil, "btn_award", nil, nil, nil)
	end
end
