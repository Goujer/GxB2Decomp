-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityWindow.lua
...
function ActivityWindow:ctor(name, params)
	...
	if not self.select or params.activity_type and ActivityTable:getType(self.select) and ActivityTable:getType(self.select) == 0 then
		-- block empty
	else
...
function ActivityWindow:initTopGroup()
	self.windowTop = WindowTop.new(self.window_, self.name_, nil, nil, self.closeBtnCallback)
...
function ActivityWindow:setNormalDisplay()
	...
	local sortedIDs = table.sortedKeys(activityList, function(a, b)
		...
		if ActivityTable:getRank(b) ~= ActivityTable:getRank(a) then
			return ActivityTable:getRank(a) > ActivityTable:getRank(b)
		else
			return b < a
		end
	end)
...
function ActivityWindow:setActivityDisplay()
	...
	local selectTitle, selectCurId, selectAllLength
...
function ActivityWindow:isComplete(id)
	if id == xyd.ActivityID.ACTIVITY_EQUIP_LEVEL_UP then
		...
		for i = 1, #ids do
			...
			if limit > tonumber(buyTimes[i]) then
				return false
			end
		end
	...
	if id == xyd.ActivityID.ACTIVITY_CHRISTMAS_SIGN_UP then
		...
		if data:getAward(data:getCurDateID()) ~= nil then
			if data:getCurGiftBag() ~= nil and data:getCurGiftBag() ~= 0 then
				if data.detail.charges == nil or data.detail.charges[data:getGiftBagIndex(data:getCurGiftBag())].buy_times >= data.detail.charges[data:getGiftBagIndex(data:getCurGiftBag())].limit_times then
					return true
				else
					return false
				end
...
ActivityWindow.ContentClass = {
	...
	[xyd.ActivityID.ACTIVITY_GALAXY_TRIP_MISSION2] = function()
		return require("app.windows.activity.ActivityGalaxyTripMission")
	end,
...
function ActivityTitleItem:initUIComponent()
	...
	elseif self.id == xyd.ActivityID.ACTIVITY_VOTE then
		...
		if self.id >= xyd.ActivityID.ACTIVITY_EXP_EMERGENCY and self.id <= xyd.ActivityID.NEW_FIVE_STAR_GIFT or self.id == xyd.ActivityID.SUMMON_WELFARE or self.id == xyd.ActivityID.SEVEN_STAR_GIFT or self.id == xyd.ActivityID.NINE_STAR_GIFT1 or self.id == xyd.ActivityID.NINE_STAR_GIFT2 or self.id >= xyd.ActivityID.MAGIC_DUST_PUSH_GIFTBGA and self.id <= xyd.ActivityID.ACADEMY_ASSESSMENT_PUSH_GIFTBAG or self.id == xyd.ActivityID.ACTIVITY_OPTIONAL_SUPPLY or self.id == xyd.ActivityID.ACTIVITY_OPTIONAL_SUPPLY_SUPER then
