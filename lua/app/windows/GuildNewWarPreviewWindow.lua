-- chunkname: @../../../Product/Bundles/Android/src/app/windows/GuildNewWarPreviewWindow.lua
...
function GuildNewWarPreviewWindow:registerEvent()
	...
	UIEventListener.Get(self.btnAim.gameObject).onClick = function()
		if xyd.models.guild.guildJob == xyd.GUILD_JOB.LEADER or xyd.models.guild.guildJob == xyd.GUILD_JOB.VICE_LEADER then
			-- block empty
		else
...
function GuildNewWarPreviewWindow:layout()
	...
	if self.isSelf then
		...
		self.labelSelf.effectColor = Color.New2(3088991487)
	...
	xyd.setUISpriteAsync(self.imgNode, nil, spriteName, nil, nil, true)
...
function GuildNewWarPreviewWindow:updateNodeInfoGroup()
	...
	if not self.isSelf and (self.curPeriod == xyd.GuildNewWarPeroid.FIGHTING1 or self.curPeriod == xyd.GuildNewWarPeroid.FIGHTING2) then
		-- block empty
	else
		show = false
	end
...
function GuildNewWarPreviewItem:updateInfo(index)
	...
	elseif self.parent.isSelf then
		...
		self.labelTitle.effectColor = Color.New2(3091292415)
	end
