-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ExSkillGradeUpWindow.lua
...
function ExSkillGradeUpWindow:initPreview()
	...
	for i = 1, 2 do
		...
		if costs[i][2] > xyd.models.backpack:getItemNumByID(costs[i][1]) then
			self["priceLabel" .. i].color = Color.New2(3422556671)
		else
...
function ExSkillGradeUpWindow:register()
	...
	UIEventListener.Get(self.activeBtn).onClick = handler(self, function()
		...
		for i = 1, 2 do
			if costs and costs[i][2] > xyd.models.backpack:getItemNumByID(costs[i][1]) then
