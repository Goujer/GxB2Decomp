-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivitySpfarmMapWindow.lua
		...
		if i == 3 then
			local function rightUpItemFun()
				if self:getIsMySelf() then
					if self.activityData:getTypeDefLimitNum() >= self.activityData:getTypeDefMaxNum() then
	...
	if not self:getIsMySelf() then
		...
		for i in pairs(self.gridArr) do
			if self.gridArr[i]:getState() == PLACE_STATE.BUILD then
				...
				if not buildRobInfo or not buildRobInfo.partners or not (#buildRobInfo.partners > 0) or self:getCheckBuildEnemyIsAllDepth(buildRobInfo) then
					-- block empty
				else
					local borderArr = self:getBorderIds(i)
...
function ActivitySpfarmMapWindow:onAward(event)
	...
	elseif type == xyd.ActivitySpfarmType.START_ROB then
		...
		self:waitForTime(1, function()
			...
			if (not viewingEndFiveMinsTips or viewingEndFiveMinsTips and (viewingEndFiveMinsTips < self.activityData:startTime() or viewingEndFiveMinsTips >= self.activityData:getEndTime())) and xyd.getServerTime() >= self.activityData:getEndTime() - self.activityData:getViewTimeSec() - 300 and xyd.getServerTime() < self.activityData:getEndTime() - self.activityData:getViewTimeSec() then
...
function MapGridItem:onTouch()
	...
	elseif self.state == PLACE_STATE.BUILD then
		...
		elseif self.tipsCon and self.tipsCon.gameObject.activeSelf then
			...
			if #self.activityData:getSlotsRob() >= self.activityData:getTypeBuildLimitNum(xyd.ActivitySpfarmPlicyType.ROOT_NUM) then
			...
			if not timeStamp or timeStamp < self.activityData:startTime() or timeStamp > self.activityData:getEndTime() then
				local params = {
...
function MapGridItem:updateState(state)
	...
	elseif state == PLACE_STATE.DOOR then
		...
		for i = #famousLevelArr, 1, -1 do
			if famousNum >= famousLevelArr[i] then
