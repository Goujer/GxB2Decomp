-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityRecallLotteryAwakeWindow.lua
...
function ActivityRecallLotteryAwakeWindow:updateAwards()
	...
	for i = 1, 3 do
		if self.activityData.detail.gets[i] ~= 0 then
			...
			if self.activityData.detail.point >= xyd.tables.activityVampireGrowTable:getPoint(i) then
				if i == 3 then
...
function ActivityRecallLotteryAwakeWindow:register()
	...
	for i = 1, 3 do
		UIEventListener.Get(self["award" .. i].gameObject).onClick = function()
			if self.activityData.detail.gets[i] ~= 0 then
				return
			elseif self.activityData.detail.point >= xyd.tables.activityVampireGrowTable:getPoint(i) then
				xyd.models.activity:reqAwardWithParams(xyd.ActivityID.ACTIVITY_RECALL_LOTTERY, cjson.encode({
					type = 3,
					id = i
				}))
