-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityBlindBoxSelectWindow.lua
	...
	for i = 1, #itemData do
		...
		UIEventListener.Get(icon:getGameObject()).onClick = function()
			if icon:getChoose() then
				...
				if self.selectedNum >= self.maxSelectNum then
					return
				end
...
function ActivityBlindBoxSelectWindow:layout()
	...
	for i = 2, 5 do
		...
		params[1] = self.activityData.detail_.round
		params[2] = i
