-- chunkname: @../../../Product/Bundles/Android/src/app/windows/TavernMultimissionWindow.lua
	...
	for _, list in pairs(self.missionListByStar) do
		table.sort(list, function(a, b)
			...
			if missionA.award[1] ~= missionB.award[1] then
				return missionA.award[1] > missionB.award[1]
			elseif missionA.award[2] ~= missionB.award[2] then
				return missionA.award[2] > missionB.award[2]
			else
...
function TavernMultimissionWindow:getContentList()
	...
	if self.doingOrDoneMissions >= maxMissions then
		return
	end
	...
	for _, star in ipairs(choosenStars) do
		...
		if missionIDs and #missionIDs > 0 then
			for _, missionID in ipairs(missionIDs) do
				...
				if self.doingOrDoneMissions + willDoing >= maxMissions then
					return
				end
			end
		end
...
function CompleteItem:updateInfo()
	...
	xyd.setUISpriteAsync(self.starBg, nil, "pub_mission_star" .. star, nil, nil)
