-- chunkname: @../../../Product/Bundles/Android/src/app/windows/BattleFormationWindow.lua
			...
			if win then
				xyd.WindowManager.get():closeWindow("group_buff_detail_window", function()
					XYDCo.WaitForTime(1, function()
						...
						params.buffID = self.info_.buffId
						params.type = self.info_.type_
						params.contenty = self.info_.contenty
						params.group7Num = self.info_.group7Num
		...
		if self.partnerId_ < 0 then
			if not self.friendHelpIcon then
				...
				if friendHelpBattleType == xyd.BattleType.TIME_CLOISTER_EXTRA or friendHelpBattleType == xyd.BattleType.TIME_CLOISTER_BATTLE then
					xyd.setUISpriteAsync(self.friendHelpIconUISprite, nil, "friend_boss_assist", nil, nil, nil)
				...
				elseif friendHelpBattleType == xyd.BattleType.GUILD_COMPETITION then
					xyd.setUISpriteAsync(self.friendHelpIconUISprite, nil, "guild_competition_bg_gh", nil, nil, nil)
...
function FormationItem:onClick()
	...
	if self.win_ ~= nil and not self.isSelected == true and self.win_.selectedNum >= self.win_.defaultMaxNum then
		isItemChoose = false
	end
	...
	if self.battleType ~= xyd.BattleType.ACADEMY_ASSESSMENT then
		...
		if self.battleType == xyd.BattleType.ENTRANCE_TEST then
			...
			if entranceTestSaveFormationTime then
				...
				if entranceTestSaveFormationTime < activityData:startTime() or entranceTestSaveFormationTime >= activityData:getEndTime() then
					self.localPartnerList = {}
					self.nowPartnerList = {}
				end
			end
				...
				if win then
					xyd.WindowManager.get():closeWindow("group_buff_detail_window", function()
						XYDCo.WaitForTime(1, function()
							...
							params.buffID = 16
							params.type = xyd.GroupBuffIconType.SPORTS
							params.contenty = 223
...
function BattleFormationWindow:arenaBattle(partnerParams)
	...
	if needCheck then
		...
		if self.power and self.power / power > tonumber(numSave) then
...
function BattleFormationWindow:campaignBattle(partnerParams)
	...
	xyd.closeWindow("campaign_window", nil, nil, true)
end
...
function BattleFormationWindow:gameAssistantArenaBattle(partnerParams)
	...
	presetData.arenaBattleFormationInfo.pet_id = self.pet
	presetData.arenaBattleFormationInfo.partners = partnerParams
	presetData.arenaBattleFormationInfo.power = self.power
...
function BattleFormationWindow:gameAssistantGuildBattle(partnerParams)
	...
	presetData.guildBattleFormationInfo.pet_id = self.pet
	presetData.guildBattleFormationInfo.partners = partnerParams
	presetData.guildBattleFormationInfo.power = self.power
...
function BattleFormationWindow:enterFairyTaleBattle(partnerParams)
	...
	for _, info in ipairs(partnerParams) do
		...
		fightPatner.partner_id = info.partner_id
		fightPatner.pos = info.pos
		fightPatner.is_npc = info.is_npc
...
function BattleFormationWindow:showPartnerDetail(partnerInfo)
	...
	if params.skin_id and showTime and showTime > xyd.getServerTime() then
		params.skin_id = nil
	end
...
function BattleFormationWindow:saveLocalformation(formation)
	if self.battleType and xyd.arrayIndexOf(self.showFormationTeamTypeArr, self.battleType) > 0 and self.isEditorQuickTeam then
		...
		dbParams.key = self.battleType .. "_quick"
		dbParams.value = self.chooseQuickTeam_
...
function BattleFormationWindow:iosTestChangeUI()
	...
	self.labelFront.color = Color.New2(4294967295)
	self.labelBack.color = Color.New2(4294967295)
end
