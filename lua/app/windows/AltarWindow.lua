-- chunkname: @../../../Product/Bundles/Android/src/app/windows/AltarWindow.lua
	...
	local params = {
		...
		callback = function()
			...
			if self.partner_:isLockFlag() == true then
				...
				elseif xyd.checkGalaxyFormation(self.partner_) then
...
function AltarWindow:initChoosePart()
	...
	for i = 1, 5 do
		...
		xyd.setUISpriteAsync(starIcon, nil, "star0" .. i, nil, nil)
		...
		if xyd.isIosTest() then
			xyd.setUISpriteAsync(starIcon, nil, "star0" .. i .. "_ios_test", nil, nil)
	...
	for i = 1, 6 do
		...
		xyd.setUISpriteAsync(iconGroup, nil, "img_group" .. i, nil, nil)

		if xyd.isIosTest() then
			xyd.setUISpriteAsync(iconGroup, nil, "img_group" .. i .. "_ios_test", nil, nil)
		end
...
function AltarWindow:initChoosePart_back()
	...
	for i = 1, 10 do
		...
		elseif i <= 9 then
			xyd.setUISpriteAsync(starIcon, nil, "star_red_" .. i, nil, nil)
		elseif i == 10 then
			xyd.setUISpriteAsync(starIcon, nil, "star_orange", nil, nil)
		end
	...
	if not iconChoose then
		...
		if not emptyRoot then
			...
			if CheckOldExplorePartner then
				...
				end, nil, nil, nil, __("BATTLE_FORMATION_WIN"))
			else
				addHeroIcon()
			end
		end
...
function AltarWindow:smartPutIn()
	...
	while #self.selPartnerIDs_ < MaxSelected and #self.selPartnerIDs_ < #sortPartners["0_0"] - 1 do
		...
		if not self:isPartnerLock(partner) and not xyd.tables.partnerTable:checkPuppetPartner(partner:getTableID()) then
			self:selectHero(partnerID, nil, nil, true)
		end
...
function AltarWindow:sendDisassembleRequest()
	...
	if not self:has4StarPartner() then
		...
		params.alertType = xyd.AlertType.YES_NO
		params.message = __("ALTAR_DECOMPOSE_TIP")

		function params.callback(yes)
			if yes then
				...
				if decomposeNum + star6Num > tonumber(xyd.tables.miscTable:getVal("altar_6up_day_limit")) then
...
function AltarWindow:iosTestChangeUI()
	...
	winTrans:ComponentByName("aniPos/btnPos/btnAutoSmart/labelDesc", typeof(UILabel)).color = Color.New2(4294967295)
	winTrans:ComponentByName("aniPos/btnPos/btnAutoSmart/labelDesc", typeof(UILabel)).effectStyle = UILabel.Effect.None
	winTrans:ComponentByName("aniPos/btnPos/btnSmart/labelDesc", typeof(UILabel)).color = Color.New2(4294967295)
	winTrans:ComponentByName("aniPos/btnPos/btnSmart/labelDesc", typeof(UILabel)).effectStyle = UILabel.Effect.None
	winTrans:ComponentByName("aniPos/heroGroup/labelDesc", typeof(UILabel)).color = Color.New2(4294967295)
	winTrans:ComponentByName("aniPos/heroGroup/labelNum", typeof(UILabel)).color = Color.New2(4294967295)
	winTrans:ComponentByName("aniPos/heroGroup/chooseBtn/btnTitle", typeof(UILabel)).color = Color.New2(4294967295)
