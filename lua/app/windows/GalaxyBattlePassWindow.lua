-- chunkname: @../../../Product/Bundles/Android/src/app/windows/GalaxyBattlePassWindow.lua
...
function GalaxyBattlePassWindow:onClickTab(index)
	...
	self["labelTab" .. self.curTabIndex].color = Color.New2(2846672383)
	...
	self["labelTab" .. self.curTabIndex].color = Color.New2(4294967295)
...
function GalaxyBattlePassWindow:initData()
	...
	for i = 1, #ids do
		...
		table.insert(self.data1, {
			...
			isCompleted = self.activityData:getTotalEnergy() >= awardTalbe:getPointLimit(id),
	...
	for i = 1, #ids do
		...
		elseif totalEnergy < needEnergy then
			if totalEnergy > awardTalbe:getPointLimit(id - 1) then
...
function GalaxyBattlePassWindow:initListPositon()
	self:waitForFrame(5, function()
		...
		for i = #ids, 1, -1 do
			...
			if self.activityData:getTotalEnergy() >= awardTalbe:getPointLimit(id) and (not self.activityData:getFreeAwardAwarded(id) or not self.activityData:getPaidAwardAwarded(id) and self.activityData:IfBuyGiftBag() == true) then
				moveIndex = id
				flag = true
			end
		end
...
function GalaxyBattlePassWindow:GetTask()
	...
	for i = 1, #ids do
		...
		if self.activityData:getTaskCompleteValue(id) >= missionTalbe:getComplete(id) and not self.activityData:getTaskAwarded(id) then
