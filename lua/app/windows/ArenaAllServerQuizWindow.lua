-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ArenaAllServerQuizWindow.lua
...
function ArenaAllServerQuizItem1:layout()
	...
	if round > 6 or not pickPlayers or not pickPlayers[1] then
		...
		xyd.setUISpriteAsync(self.imgAwardTips_, nil, source, nil, nil, true)
		self:initCost()
		self:initPlayer()
		self:updateBtn()
...
function ArenaAllServerQuizRecord:updateInfo()
	...
	self.labelMana_.color = isWin and Color.New2(227172351) or Color.New2(3865139711)
...
function ArenaAllServerQuizItem3:initList()
	...
	elseif totalNum > 0 then
		...
		self.labelMana_.color = Color.New2(3865139711)
	end
...
function ArenaAllServerQuizRecord2:updateInfo()
	...
	if info then
		...
		self.labelMana_.text = manaStr
		self.labelMana_.color = xyd.checkCondition(isWin and isWin == 1, Color.New2(227172351), Color.New2(3865139711))
	else
