-- chunkname: @../../../Product/Bundles/Android/src/app/windows/HouseHero.lua
...
function HouseHero:specialIdle(callback)
	if self.houseIdleCount_ > self.houseIdleNum_ then
		self.houseIdleCount_ = 1
...
function HouseHero:changeTouchNodePos(type)
	...
	if type == xyd.HouseItemInteractType.SLEEP then
		if self:getInfo().is_flip == 1 then
			local var_38_0 = -1
		else
			local direct = 1
		end
...
function HouseHero:playMoveAction(isNewUpdateZorder)
	...
	if direct.timeCount >= self.maxTimeCount then
...
function HouseHero:playDialogMove(params)
	...
	if params.index >= params.endIndex then
		params.is_end = true
