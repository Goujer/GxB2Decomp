-- chunkname: @../../../Product/Bundles/Android/src/app/windows/SoulEquip2StrengthenWindow.lua
...
function SoulEquip2StrengthenWindow:ctor(name, params)
	...
	self.newExData = {}
	self.newExData.ex_attr_ids = self.equip:getReplacesIDs()
	self.newExData.ex_factor = self.equip:getReplacesFactors()
end
...
function SoulEquip2StrengthenWindow:register()
	...
	UIEventListener.Get(self.btnExchange).onClick = function()
		...
		xyd.models.slot:reqChangeExBuff2(self.equipID, function(ex_ids, ex_factors)
			self.newExData = {}
			self.newExData.ex_attr_ids = ex_ids
			self.newExData.ex_factor = ex_factors
...
function SoulEquip2StrengthenWindow:updateProgressGroup()
	...
	for i = 1, 99999 do
		if maxLev > self.fakeLev and self.fakeExp >= xyd.tables.expSoulEquip2Table:getCost(self.fakeLev + 1) then
...
function SoulEquip2StrengthenWindow:updateContent2()
	...
	for i = 1, 4 do
		...
		if attr and attr[2] and attr[2] > 0 then
			state = nextQlt <= qlt and "soulEquip1LevelUp" or "soulEquipChange"
	...
	if qlt < nextQlt then
		...
		for i = 1, 2 do
			if xyd.models.backpack:getItemNumByID(cost[i][1]) < cost[i][2] then
				self["labelRes" .. i .. "NeedNum"].color = Color.New2(3422556671)
			else
...
function SoulEquip2StrengthenWindow:updateContent3()
	...
	for i = 1, 2 do
		if xyd.models.backpack:getItemNumByID(cost[i][1]) < cost[i][2] then
			self["labelRes3" .. i .. "NeedNum"].color = Color.New2(3422556671)
		else
...
function SoulEquip2StrengthenItem:onClickIcon()
	...
	elseif self.parent.chooseMaterailEquipHelpArr[self.equip:getSoulEquipID()] then
		...
		if self.parent.fakeLev >= self.parent.equip:getMaxLevel() then
		...
		if self.equip:getOwnerPartnerID() and self.equip:getOwnerPartnerID() > 0 then
			...
			if owner then
				...
				end, __("YES"), false, nil, nil, nil, nil, nil, nil)
			else
				self.parent.chooseMaterailEquipHelpArr[self.equip:getSoulEquipID()] = true
