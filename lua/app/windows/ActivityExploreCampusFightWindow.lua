-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityExploreCampusFightWindow.lua
			...
			for k in pairs(curSeasonBuffs) do
				if detail_[(self.params_.levelId - 1) % seasonIdsLength + 1][i] == curSeasonBuffs[k] then
					...
					if floor_id < 9 and self.index == BUFFS_TYPE.HARD_POINT then
						isLock = true
					elseif lockState and lockState == 1 and needPoint > tonumber(xyd.models.oldSchool:getAllInfo().max_score) then
						isLock = true
					end
...
function ActivityExploreCampusFightWindow:initUIComponent()
	...
	xyd.setUISpriteAsync(self.partnerTeamIndex, nil, "arena_3v3_t" .. self.params_.levelIndex, nil, nil)
...
function ActivityExploreCampusFightWindow:choiceBuff(buff_id)
	...
	if allHeight > self.explainScroller_uiPanel.height then
