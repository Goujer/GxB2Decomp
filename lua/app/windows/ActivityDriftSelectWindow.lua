-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityDriftSelectWindow.lua
...
function ActivityDriftSelectWindow:register()
	...
	UIEventListener.Get(self.btn).onClick = handler(self, function()
		...
		if self.isDouble and self.select1 <= 0 and self.select2 <= 0 or self.select <= 0 then
			-- block empty
		else
