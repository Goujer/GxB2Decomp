-- chunkname: @../../../Product/Bundles/Android/src/app/windows/TimeCloisterMainWindow.lua
...
function TimeCloisterMainWindow:clickTurntable()
	...
	if x > 0.5 then
		delta = pos.y > 0 and 2 or 1
	elseif x > -0.5 then
		delta = pos.y > 0 and 3 or 0
	else
		delta = pos.y > 0 and 4 or 5
	end
...
function TimeCloisterMainWindow:checkGetNextInfo(info)
	...
	if checkNextId <= checkMaxNum and proNum <= info.progress and self.cloisterInfo[self.curCloister + 1] and self.cloisterInfo[self.curCloister + 1].state == xyd.CloisterState.LOCK then
