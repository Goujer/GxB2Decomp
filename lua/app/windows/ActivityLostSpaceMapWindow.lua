-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityLostSpaceMapWindow.lua
...
function ActivityLostSpaceMapWindow:registerEvent()
	...
	UIEventListener.Get(self.finalAwardBtn.gameObject).onClick = handler(self, function()
		...
		if self.activityData.detail.stage_id >= 30 and allLength >= self.activityData.detail.stage_id then
			param.isShowMark = true
		end
...
function ActivityLostSpaceMapWindow:setSkillCompleteNum(id)
	...
	if self.skillEffectNum >= self.skillEffectMax then
		self:openUnLockGrid()
...
function MapGridItem:updateStateSecondFloorGrid(state)
	...
	xyd.setUISpriteAsync(self.cardItemBg, nil, "activity_lost_space_gz_pt", nil, nil, true)

	if state == PLACE_STATE.GREY then
		self.cardItemUp.gameObject:Y(11)
		xyd.setUISpriteAsync(self.cardItemUp, nil, "activity_lost_space_gz_yc", nil, nil, true)
	...
	elseif state == PLACE_STATE.COMMON then
		xyd.setUISpriteAsync(self.cardItemUp, nil, "activity_lost_space_gz_ktc", nil, nil, true)
	...
	elseif state == PLACE_STATE.EVENT then
		...
		if type(content) == "string" and #content > 0 then
			...
			if events[1] == "e" then
				...
				if eventId == xyd.ActivityLostSpaceEventType.EXIT then
					xyd.setUISpriteAsync(self.cardItemUp, nil, "activity_lost_space_gz_xyg", nil, nil, true)
					xyd.setUISpriteAsync(self.cardItemBg, nil, "activity_lost_space_gz_dian", nil, nil, true)
					self.cardItemUp.gameObject:Y(19)
				elseif eventId == xyd.ActivityLostSpaceEventType.TREASURE_ENTER then
					xyd.setUISpriteAsync(self.cardItemUp, nil, "activity_lost_space_gz_bjd", nil, nil, true)
					xyd.setUISpriteAsync(self.cardItemBg, nil, "activity_lost_space_gz_dian", nil, nil, true)
					self.cardItemUp.gameObject:Y(19)
				elseif eventId == xyd.ActivityLostSpaceEventType.TREASURE_PART then
					xyd.setUISpriteAsync(self.cardItemUp, nil, "activity_lost_space_gz_bjzb", nil, nil, true)
					xyd.setUISpriteAsync(self.cardItemBg, nil, "activity_lost_space_gz_dian", nil, nil, true)
					self.cardItemUp.gameObject:Y(19)
				end
			end
