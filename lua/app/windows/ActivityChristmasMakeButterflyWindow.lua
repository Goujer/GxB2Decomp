-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityChristmasMakeButterflyWindow.lua
...
function ActivityChristmasMakeButterflyWindow:initChoosePart()
	...
	for i = 1, 3 do
		...
		xyd.setUISpriteAsync(starIcon, nil, "star0" .. i + 2, nil, nil)
...
function ActivityChristmasMakeButterflyWindow:updateOptionalList(key)
	...
	for _, partnerID in ipairs(partnerData) do
		...
		if partnerInfo.star <= 2 or partnerInfo.star > 5 then
			-- block empty
		elseif partnerInfo.lev ~= 1 then
			-- block empty
		elseif xyd.tables.partnerTable:checkPuppetPartner(partnerInfo:getTableID()) then
			-- block empty
		else
	...
	local function confirmCallBack(num)
		if num > 0 then
			...
			for _, partnerID in ipairs(self.partnerDatas) do
				if selectNum >= slotModel:getPartnerNum() then
					break
				end
				...
				elseif num <= selectNum or selectNum >= maxNum then
					break
				end
			end
...
function ActivityChristmasMakeButterflyWindow:getMaxSmartNum()
	...
	if #self.partnerDatas >= slotModel:getPartnerNum() then
