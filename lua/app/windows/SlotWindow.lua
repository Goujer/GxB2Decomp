-- chunkname: @../../../Product/Bundles/Android/src/app/windows/SlotWindow.lua
...
function SlotPartnerCard:init()
	UIEventListener.Get(self.go).onClick = handler(self, function()
		...
		is_guide = table_id ~= nil and true or false
...
function SlotWindow:onItemChange(event)
	...
	flags[xyd.BackpackShowType.DEBRIS] = false
...
function SlotWindow:addSlotSpace()
	...
	if buyTime >= tonumber(mt:getVal("herobag_buy_limit")) then
		...
		function callback(flag)
			if flag then
				if cost > xyd.models.backpack:getCrystal() then
	...
	table.sort(data, function(a, b)
		...
		if aType == bType then
			...
			elseif aType == xyd.ItemType.HERO_DEBRIS then
				...
				aVal = xyd.checkCondition(aCost[2] > a.itemNum, 0, 10000)
				bVal = xyd.checkCondition(bCost[2] > b.itemNum, 0, 10000)
				...
				aVal = aVal + xyd.checkCondition(aCost[1] > bCost[1], 10, 0)
				bVal = bVal + xyd.checkCondition(bCost[1] > aCost[1], 10, 0)
				aVal = aVal + xyd.checkCondition(a.itemID > b.itemID, 1, 0)
				bVal = bVal + xyd.checkCondition(b.itemID > a.itemID, 1, 0)
