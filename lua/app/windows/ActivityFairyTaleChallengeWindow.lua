-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityFairyTaleChallengeWindow.lua
...
function ActivityFairyTaleChallengeWindow:initNav()
	...
	local labelStates = {
		chosen = {
			...
			effectColor = Color.New2(2606316799)
		},
		unchosen = {
...
function ActivityFairyTaleChallengeWindow:refreshMissionPart(keepPosition)
	...
	if self:isCompleteMission() then
		...
		for i = 1, #self.nowMissionList_ do
			...
			params.id = self.nowMissionList_[i]
...
function ActivityFairyTaleChallengeWindow:refreshBuffShowList(keepPosition)
	...
	for i = 1, #buffIds do
		...
		params.buffId = buffIds[i]
		params.type_ = xyd.GroupBuffIconType.FAIRY_TALE
		params.isAct = self:checkisAct(buffIds[i])
...
function FairyTaleMissionItem:setInfo(params)
	...
	self.missionProgressLabel_.text = xyd.checkCondition(self.value_ > self.completeValue_, self.completeValue_, self.value_) .. "/" .. self.completeValue_
...
function ChooseBuffItem:setInfo(id)
	...
	for index, skillId in ipairs(self.chooseIds_) do
		...
		local params = {
			...
			is_lock = self.id_ > self.parent_.lev_,
			level = self.id_,
