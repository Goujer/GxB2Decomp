-- chunkname: @../../../Product/Bundles/Android/src/app/windows/GuildDiningHall.lua
...
function GuildDiningHall:updateUpgradeOrder()
	...
	if self.orderList then
		for _, order in pairs(self.orderList) do
			if maxLevel > order.order_lv and order.start_time == 0 and maxLevel > self.upgradeLevel then
				canUpgrade = true
...
function GuildDiningHall:updateDonateRes()
	...
	if donate[1][2] > xyd.models.backpack:getMana() then
		self.labelCostRes1.color = Color.New2(3422556671)
	...
	if donate[2][2] > xyd.models.backpack:getMana() then
		self.labelCostRes10.color = Color.New2(3422556671)
...
function GuildDiningHall:upgradeAllOrder(upgradeLevel)
	...
	for i = 1, #self.diningHallItems do
		...
		if upgradeLevel > item.data.order_lv and item.data.start_time == 0 then
...
function GuildDiningHallItem:initUIComponent()
	...
	if #self.startsItem_ > self.data.order_lv then
