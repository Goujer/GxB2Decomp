-- chunkname: @../../../Product/Bundles/Android/src/app/windows/HouseMap.lua
...
function HouseMap:getFreeGrid(size, col, row, info, errorNode, centerPos)
	...
	for i = 1, #tmpGrids do
		...
		if errorNode[tostring(tmpGrid.row) .. "|" .. tostring(tmpGrid.col)] or tmpGrid.row < 0 or maxRow <= tmpGrid.row or tmpGrid.col < 0 or maxCol <= tmpGrid.col then
			-- block empty
...
function HouseMap:checkHeroPosValid(info, offX, offY)
	...
	if coordX < 0 or coordX >= self.mapCol or coordY < 0 or coordY >= self.mapRow then
		return result
...
function HouseMap:getInvalidGrids(info)
	local invalidGrids = {}
	local gridNum, hashCode, parentItem
	local colorGrids = {}
	local isValid = true
	...
	for row = info.coord_y, endy do
		for col = info.coord_x, endx do
			...
			if col < 0 or col >= self.mapCol or row < 0 or row >= self.mapRow then
				result = {
					flag = false
				}
			else
...
function HouseMap:getItemTopologyParent(key)
	...
	for deltaX = 0, attrs.length - 1 do
		for deltaY = 0, attrs.width - 1 do
			local coordX, coordY = attrs.coord_x + deltaX, attrs.coord_y + deltaY
			local index = coordY * self.mapCol + coordX
...
function HouseMap:updateTimeAction()
	for i = 1, #self.timeAction_ do
		...
		if action.count > action.delay_frame then
