-- chunkname: @../../../Product/Bundles/Android/src/app/windows/BatchShenXueWindow.lua
...
function BatchShenXueWindow:sortByTableID()
	...
	table.sort(partnerIDs, function(a, b)
		...
		if aStar == 5 and (not aTenStarTableID or not (aTenStarTableID > 0)) then
			aIsFeiTai5 = true
		end

		if bStar == 5 and (not bTenStarTableID or not (bTenStarTableID > 0)) then
			bIsFeiTai5 = true
		end
	...
	for i = 1, #self.material_detail do
		...
		function m_detail.callback()
			...
			function params.confirmCallback()
				...
				if #m_detail.partners >= m_detail.needNum then
					icon:setOrigin()
