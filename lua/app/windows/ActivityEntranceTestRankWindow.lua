-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityEntranceTestRankWindow.lua
...
function ActivityEntranceTestRankWindow:initRankList()
	...
	if level >= xyd.EntranceTestLevelType.R1 and level <= xyd.EntranceTestLevelType.R3 then
		self.rankUpLevelIcon.gameObject:SetActive(true)
		xyd.setUISpriteAsync(self.rankUpLevelIcon, nil, "entrance_test_level_" .. level + 1, nil, nil, true)
...
function EntranceRankItem:layout()
	if tonumber(self.rank_) ~= nil and tonumber(self.rank_) <= 3 and self.point_ and self.point_ >= 0 then
		xyd.setUISpriteAsync(self.imgRankIcon, nil, "rank_icon0" .. self.rank_, nil, nil)
	...
	if self.group_ == 0 then
		...
		xyd.setUISpriteAsync(self.groupImg, nil, "img_group" .. self.group_, nil, nil)
	end
...
function ArenaAwardItem:setInfo(id, colName, table, notShowSpecial)
	...
	if not notShowSpecial and tonumber(info.rank) ~= nil and tonumber(info.rank) <= 3 then
		...
		xyd.setUISpriteAsync(self.imgRank, nil, "rank_icon0" .. info.rank, nil, nil)
		self.labelRank:SetActive(false)
	else
