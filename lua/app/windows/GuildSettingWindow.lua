-- chunkname: @../../../Product/Bundles/Android/src/app/windows/GuildSettingWindow.lua
...
function GuildSettingWindow:initUIComponent()
	...
	if xyd.models.guild.base_info.announcement and xyd.models.guild.base_info.announcement ~= "" then
		...
		self.editableText.color = Color.New2(4294967295)
	else
		self.editableText.text = __("GUILD_TEXT61")
		self.editableText.color = Color.New2(3385711103)
	end
...
function GuildSettingWindow:guildDissolveOrCancel()
	...
	if data.dissolve_time and data.dissolve_time > 0 then
		...
		if newGuildWarData then
			local endTime, curPeriod

			curPeriod, endTime = newGuildWarData:getCurPeriod()

			if curPeriod ~= xyd.GuildNewWarPeroid.BEGIN_RELAX and curPeriod ~= xyd.GuildNewWarPeroid.END_RELAX and (curPeriod ~= xyd.GuildNewWarPeroid.NORMAL_RELAX or not (endTime - xyd.getServerTime() > 7260)) then
