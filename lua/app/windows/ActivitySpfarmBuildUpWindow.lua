-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivitySpfarmBuildUpWindow.lua
...
function ActivitySpfarmBuildUpWindow:registerEvent()
	...
	UIEventListener.Get(self.collectionBtn.gameObject).onClick = function()
		...
		if famousLimt > self.activityData:getFamousNum() then
	...
	UIEventListener.Get(self.defendConBg.gameObject).onClick = handler(self, function()
		...
		if (not self.buildInfo.partners or self.buildInfo.partners and #self.buildInfo.partners == 0) and self.activityData:getTypeDefMyNum() >= self.activityData:getTypeDefLimitNum() then
...
function ActivitySpfarmBuildUpWindow:checkNowLevShow()
	...
	if maxLev <= self.showLev then
		...
		if self.showLev > self.buildInfo.lv then
