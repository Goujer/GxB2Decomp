-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ExploreAdventureWindow.lua
...
function ExploreAdventureWindow:getUIComponent()
	...
	for i = 1, 3 do
		...
		self.labelItemList[i].labelName = labelItem:ComponentByName("labelName", typeof(UILabel))
		self.labelItemList[i].labelNum = labelItem:ComponentByName("labelNum", typeof(UILabel))
	end
...
function ExploreAdventureWindow:setBtn()
	...
	elseif self.data.event_id == 0 then
		...
		if self.adventureCost[2] > xyd.models.backpack:getItemNumByID(self.adventureCost[1]) then
			self.labelCost.color = Color.New2(3981269247)
		else
		...
		if self.data.lv >= self.travelQuickLimit and not self.onAutoAdven then
...
function ExploreAdventureWindow:setGiftBoxContent()
	...
	for i = 1, self.slotNum do
		...
		self.giftBoxItemList[i]:setInfo({
			...
			boxIndex = i,
			isNew = i > self.lastBoxNum and boxID ~= 0
		})
	end
...
function ExploreAdventureWindow:setOpenBoxBtn()
	for i = 1, self.slotNum do
		if self.giftBoxItemList[i].id > 0 and self.giftBoxItemList[i].isFree then
			xyd.setUISpriteAsync(self.imgOpenBoxBtn, nil, "btn_open_box_free", nil, nil, true)

			return
		end
	end

	xyd.setUISpriteAsync(self.imgOpenBoxBtn, nil, "btn_open_box", nil, nil, true)
end
...
function ExploreAdventureWindow:dealAdventureEvent()
	if not self.isplayAnimation then
		...
		if evtType == EventType.BATTLE then
			...
			if next(cost) ~= nil and tonumber(cost[2]) > xyd.models.backpack:getItemNumByID(tonumber(cost[1])) then
				xyd.showToast(__("NOT_ENOUGH", xyd.tables.itemTable:getName(tonumber(cost[1]))))
		...
		if self.autoAdvenIsOpen then
			if not xyd.db.formation:getValue(xyd.BattleType.EXPLORE_ADVENTURE) then
				xyd.WindowManager.get():openWindow("adventure_battle_formation_window", {
					...
					showSkip = self.data.lv >= self.travelQuickLimit,
...
function ExploreAdventureWindow:onLevelUp(event)
	...
	if self.data.lv >= self.travelQuickLimit and self.data.event_id == 0 then
		self.autoAdven:SetActive(true)
		self.btnSetup:SetActive(true)
		self.btnFormation:SetActive(true)
		self.btnOpenBox:SetActive(true)
...
function BoxFormationItem:setInfo(info)
	...
	if self.id == 0 then
		...
		xyd.setUISpriteAsync(self.boxImg, nil, "icon_bx_" .. self.id, nil, nil, true)
		...
		if duration > 0 then
			...
			self.timeLabelCount_:setInfo({
				duration = duration,
				callback = function()
					xyd.setUISpriteAsync(self.boxImg, nil, "icon_bx_" .. self.id .. "_2", nil, nil, true)
