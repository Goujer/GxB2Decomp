-- chunkname: @../../../Product/Bundles/Android/src/app/windows/DressShowShopWindow.lua
...
function DressShowShop1Item:register()
	UIEventListener.Get(self.btnBuy).onClick = function()
		...
		if self.data.cost[2] > xyd.models.backpack:getItemNumByID(self.data.cost[1]) then
...
function DressShowShop1Item:setInfo(params)
	...
	if data.limitTime - data.buyTime <= 0 or data.awakeTime >= data.maxTime then
...
function DressShowShop2Item:register()
	UIEventListener.Get(self.btnBuy).onClick = function()
		if self.data.point > self.parent.parent.activityData:getHistoryTotalScore() then
...
function DressShowShop2Item:setInfo(params)
	...
	self.shadow:SetActive(self.data.point > self.parent.parent.activityData:getHistoryTotalScore())
