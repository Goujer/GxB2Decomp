-- chunkname: @../../../Product/Bundles/Android/src/app/windows/PetTrainingWindow.lua
...
function PetTrainingWindow:initPetIcon()
	...
	for i = 1, #ids do
		...
		if petLev >= xyd.tables.miscTable:getNumber("pet_training_boss_level", "value") and (self.petTrainingModel:getPetBattleTimes() == nil or self.petTrainingModel:getPetBattleTimes()[math.floor(petID / 100)] == nil or xyd.tables.miscTable:getNumber("pet_training_pet_energy", "value") - self.petTrainingModel:getPetBattleTimes()[math.floor(petID / 100)] > 0) then
			maxIndex = i
	...
	for i = 1, #ids do
		...
		if pet.lev > 0 then
			...
			if i == maxIndex and pet.lev >= xyd.tables.miscTable:getNumber("pet_training_boss_level", "value") then
				icon:setChosen(true)
...
function PetTrainingWindow:registerEvent()
	...
	UIEventListener.Get(self.fightBtn).onClick = function()
		...
		if xyd.tables.miscTable:getNumber("pet_training_boss_limit", "value") - self.petTrainingModel:getBattleTimes() > 0 then
			if self.petTrainingModel:getPetBattleTimes() and self.petTrainingModel:getPetBattleTimes()[math.floor(self.chosenPet.petID / 100)] and self.petTrainingModel:getPetBattleTimes()[math.floor(self.chosenPet.petID / 100)] >= xyd.tables.miscTable:getNumber("pet_training_pet_energy", "value") then
	...
	UIEventListener.Get(self.strengthBtn).onClick = function()
		if self.petTrainingModel:getBuyTimeTimes() >= xyd.tables.miscTable:getNumber("pet_training_energy_buy_limit", "value") then
...
function PetTrainingWindow:refreshTrainingAward()
	...
	for _, extraData in ipairs(levExtraDatas) do
		...
		if self.allPetLev >= tonumber(lev) then
...
function PetTrainingWindow:onFight()
	...
	sequence:InsertCallback(0.4166666666666667, function()
		...
		if self.petTrainingModel:getBossHp() >= self.lastHp then
