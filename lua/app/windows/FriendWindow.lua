-- chunkname: @../../../Product/Bundles/Android/src/app/windows/FriendWindow.lua
...
function FriendListItem:updateBtn()
	if FriendModel:checkIsSend(self.data_.player_id) then
		xyd.setUISpriteAsync(self.btnSendLove, nil, "friend_icon_love4", nil, nil)
		self.btnSendLoveMask_.gameObject:SetActive(true)
	else
		xyd.setUISpriteAsync(self.btnSendLove, nil, "friend_icon_love3", nil, nil)
	...
	if status == 1 then
		...
		xyd.setUISpriteAsync(self.btnGetLove, nil, "friend_icon_love1", nil, nil)
	...
	elseif status == 2 then
		...
		xyd.setUISpriteAsync(self.btnGetLove, nil, "friend_icon_love2", nil, nil)
	else
		if self.effectGetLove_ then
			self.effectGetLove_:stop()
			self.effectGetLove_:SetActive(false)
...
function FriendAssistant:layout()
	xyd.setUITextureAsync(self.imgMap_, "Textures/friend_web/friend_boss_map", nil, nil)
...
function FriendAssistant:update(data)
	...
	local function callback()
		...
		if bossID > 0 then
			...
			self.btnSearchLabel_.color = Color.New2(4294967295)
			...
			if duration > 0 then
				...
				function params.callback()
					...
					self.btnSearch_.color = Color.New2(4294967295)
				end
...
function FriendAssistant2:initBossHeadIcon()
	...
	xyd.setUISpriteAsync(self.friendBossIcon, nil, "friend_team_boss_head" .. tostring(bossLevel), nil, nil)
end
...
function FriendList:sortFriend()
	table.sort(self.friendList_, function(a, b)
		...
		aVal = aVal + a.player_id > b.player_id and 100 or 0
		bVal = bVal + b.player_id > a.player_id and 100 or 0
...
function FriendWindow:updateSelect()
	for i = 1, 4 do
		...
		if i == self.curSelect_ then
			if i == 3 then
				if not xyd.models.friend:isFullFriends() and #xyd.models.friend:getRequestList() >= tonumber(xyd.tables.miscTable:getVal("friend_apply_limit")) then
...
function FriendWindow:onGetRecommendList()
	if self.items_[2] then
		self.items_[2]:update(nil, nil, true)
	end
end
