-- chunkname: @../../../Product/Bundles/Android/src/app/windows/PetDetailWindow.lua
...
function PetDetailWindow:checkGuide(lev)
	...
	if not res and lev >= tonumber(xyd.tables.miscTable:getVal("pet_exskill_open_level")) then
...
function PetPasSkill:levUpTouch()
	if self:checkCanLevUp() then
		...
		if curLev >= self:getCurMaxLev() or not self:checkLevUpEnough(curLev) then
			self:reqLevUp()
