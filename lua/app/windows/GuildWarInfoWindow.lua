-- chunkname: @../../../Product/Bundles/Android/src/app/windows/GuildWarInfoWindow.lua
...
function GuildWarInfoWindow:register()
	...
	UIEventListener.Get(self.btnAward_.gameObject).onClick = function()
		...
		if not (tonumber(self.model_:getInfo().is_signed) > 0) then
...
function GuildWarInfoWindow:onClickFormation()
	...
	if tonumber(self.model_:getInfo().is_signed) > 0 then
		...
		if moment > self.model_.MOMENT.BEFORE_FINAL then
...
function GuildWarInfoWindow:onClickApply()
	...
	if self.model_:judgeMoment() > self.model_.MOMENT.RANK_MATCH then
		xyd.showToast(__("GUILD_WAR_RANK_MATCH_END"))
	...
	if not self.tab then
		...
		self.tab:setBrforeChangeFuc(function(index, currentIndex)
			if index == 2 then
				...
				if moment < self.model_.MOMENT.BEFORE_FINAL and not (self.model_:getInfo().is_signed > 0) then
					xyd.showToast(__("NOT_SIGNED"))
...
function GuildWarInfoWindow:setChildrenColorGrey(go)
	...
	for i = 1, go.transform.childCount do
		...
		if label then
			widget.color = Color.New2(4294967295)
		end
...
function GuildWarFormation:initUI()
	...
	for i = 1, 6 do
		...
		heroIcon.root = goTrans:NodeByName("groupPartner/HeroIcon" .. i).gameObject
		heroIcon.cover = goTrans:NodeByName("groupPartner/HeroIcon" .. i .. "/cover").gameObject
...
function GuildWarMatchOther:onClickMatch()
	...
	local params = {
		...
		callback = function()
			...
			self.btnMatchLabel_.color = Color.New2(4294967295)
...
function GuildWarAllFormationSmall:initUI()
	...
	xyd.setUISpriteAsync(self.e_image, nil, "guild_war_bg06", nil, nil)
end
...
function GuildWarAllFormationSmall:setInfo(params)
	...
	for idx, teamInfo in ipairs(self.data_.team_infos.teams) do
		...
		tempInfo.index = idx
		tempInfo.isHide = isHide
		tempInfo.teamInfo = teamInfo
...
function GuildWarFormationFive:initUI()
	...
	for i = 1, 6 do
		...
		heroIcon.root = go:NodeByName("groupPartner/HeroIcon" .. i).gameObject
		heroIcon.cover = go:NodeByName("groupPartner/HeroIcon" .. i .. "/cover").gameObject
...
function GuildWarBeforeFinal:initUI()
	...
	xyd.setUISpriteAsync(self.e_image, nil, "guild_war_bg04", nil, nil)
end
...
function GuildWarFinalItem:initUI()
	...
	xyd.setUISpriteAsync(self.e_image1, nil, "guild_war_deco2", nil, nil)

	self.e_image2 = go:ComponentByName("e:image/e:image2", typeof(UISprite))

	xyd.setUISpriteAsync(self.e_image2, nil, "guild_war_deco2", nil, nil)
