-- chunkname: @../../../Product/Bundles/Android/src/app/windows/MonthCardPushWindow.lua
...
function SubscriptionPushItem:initUI()
	...
	self.timeLabel.color = Color.New2(4294967295)
	self.timeLabel.effectColor = Color.New2(2570024703)
...
function FundationPushItem:initUI()
	...
	self.descLabel.color = Color.New2(3790263551)
...
function FirstRechargePushItem:initUI()
	...
	self.descLabel.color = Color.New2(4125090559)
	self.descLabel.effectColor = Color.New2(2235715071)
...
function SubscriptionPushSingleItem:setTextManaSub()
	...
	self.labelText4.text = __("TERMS_SERVICE")
	self.labelText5.text = __("PRIVACY_POLICY")
	self.btnPurchase.text = tostring(__("WEEKCARD_BTN_LABEL")) .. " " .. tostring(xyd.tabels.giftBagTextTable:getCurrency(giftBagID)) .. " " .. tostring(xyd.tables.giftBagTextTable:getCharge(giftBagID))
...
function SubscriptionPushSingleItem:calRetTimeSub()
	...
	if time > 0 then
		if data.cur_giftbag_id == xyd.GIFTBAG_ID.WEEK_SUBSCRIPTION or data.cur_giftbag_id == xyd.GIFTBAG_ID.WEEK_SUBSCRIPTION_AND or data.cur_giftbag_id == xyd.GIFTBAG_ID.WEEK_SUBSCRIPTION_FREE or data.cur_giftbag_id == xyd.GIFTBAG_ID.WEEK_SUBSCRIPTION_AND_FREE then
			self.groupLeft:SetLocalPosition(227, 247, 0)
...
function SubscriptionPushSingleItem:setBtnStateSub()
	...
	xyd.setUISpriteAsync(self.btnWeek, nil, "mana_week_card_btn01", nil, nil)

	if data.end_time > xyd.getServerTime() then
	...
	if not UNITY_IOS then
		...
		if isFree == true then
			xyd.setUISpriteAsync(self.btnWeek, nil, "green_btn_192_67", nil, nil)

			self.btnWeek_button_label.color = Color.New2(4294967295)
			self.btnWeek_button_label.effectColor = Color.New2(560209151)
			self.btnWeek_button_label.text = freeText
		else
			xyd.setUISpriteAsync(self.btnWeek, nil, "mana_week_card_btn01", nil, nil)

			self.btnWeek_button_label.color = Color.New2(3224980479)
			self.btnWeek_button_label.effectColor = Color.New2(4294967295)
...
function SubscriptionPushSingleItem:setBtnStateManaSub()
	...
	if data.end_time > xyd:getServerTime() then
		self.btnPurchase.text = __("ALREADY_SUBSCRIBE")
