-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityVoteMissionWindow.lua
...
function ActivityVoteMissionItem:updateBtnStatus(first)
	...
	if cur_mission_award ~= 0 then
		...
		label_display.effectColor = Color.New2(4294967295)
...
function ActivityVoteMissionWindow:getAllAward()
	for i = 1, #self.item_list_ do
		if self.activity_data_.detail.mission_awarded[i] == 0 and self.activity_data_.detail.mission_count[i] >= self.table_:getComplete(i) then
			local params = {
				type = 2,
				mission_id = i
			}
