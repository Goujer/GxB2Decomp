-- chunkname: @../../../Product/Bundles/Android/src/app/windows/MarketWindow.lua
...
function MarketItem:onClickBtnBuy()
	...
	if hasNum >= self.cost_[2] then
		...
		params.message = __("NOT_ENOUGH", xyd.tables.itemTable:getName(self.cost_[1]))
...
function MarketWindow:initItemsOnpage(pageNum)
	...
	for idx = 1, MAXSLOT do
		...
		if self.donnotShowIndex and realIndex >= self.donnotShowIndex then
...
function MarketWindow:setPageIcon()
	for idx, dotIcon in ipairs(self.dotIconList_) do
		if idx ~= self.page_ then
			xyd.setUISpriteAsync(dotIcon, nil, "market_dot_bg1", nil, nil)
		else
			xyd.setUISpriteAsync(dotIcon, nil, "market_dot_bg2", nil, nil)
		end
