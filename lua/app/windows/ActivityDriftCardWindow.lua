-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityDriftCardWindow.lua
...
function ActivityDriftCardWindow:layoutItem(touchIndex)
	...
	xyd.setUISpriteAsync(self["icon" .. touchIndex], nil, icon, nil, nil, true)
	...
	if not flag and ranRes2 >= self.buff then
		ranRes2 = ranRes2 + 1
	end
		...
		for i = 1, 3 do
			if i ~= touchIndex then
				if ranRes1 then
					...
					xyd.setUISpriteAsync(self["icon" .. i], nil, icon, nil, nil, true)
