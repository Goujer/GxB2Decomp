-- chunkname: @../../../Product/Bundles/Android/src/app/windows/MainWindow.lua
...
local LARGE_BTN_LABEL_STROKE_COLOR = {
	[xyd.ActivityID.ICE_SECRET] = Color.New2(3341074943),
	[xyd.ActivityID.RETURN] = Color.New2(563252991),
	[xyd.ActivityID.FAIRY_TALE] = Color.New2(3514841599),
	[xyd.ActivityID.ENTRANCE_TEST] = Color.New2(1984660735),
	[xyd.ActivityID.SPORTS] = Color.New2(1905404671),
	[xyd.ActivityID.TIME_LIMIT_CALL] = Color.New2(2387223039),
	[xyd.ActivityID.ACTIVITY_YEAR_FUND] = Color.New2(3613720831),
	[xyd.ActivityID.ACTIVITY_NEWBEE_FUND] = Color.New2(3613720831),
...
local LARGE_BTN_LABEL_TEXT_COLOR = {
	[xyd.ActivityID.TIME_LIMIT_CALL] = Color.New2(4227377407)
}
...
function MainBottomBtn:checkTips()
	...
	if wishData and wishData:getEndTime() > xyd.getServerTime() then
		flag = true
		img = xyd.checkCondition(xyd.Global.lang ~= "ja_jp", "wish_up_tips", "wish_up_tips_ja_jp")
	elseif summonGiftBagData and summonGiftBagData:getEndTime() > xyd.getServerTime() then
		flag = true
...
function MainBottomBtn:change(status)
	if status == 0 then
		...
		self.btnLabel_.effectColor = Color.New2(4294967295)
		...
		self.btnLabel_.color = Color.New2(4294967295)
...
function MainUIActivityItem:ctor(goItem, itemdata, scrollerPanel)
	...
	self.defaultTextColor = Color.New2(4294967295)
...
function MainUIActivityItem:updateInfo(itemdata)
	...
	if xyd.isH5() then
		LARGE_BTN_LABEL_STROKE_COLOR[self.id] = Color.New2(2387223039)
		LARGE_BTN_LABEL_TEXT_COLOR[self.id] = Color.New2(4227377407)
		self.defaultTextColor = Color.New2(4227377407)
	end
...
function MainWindow:CheckScrollerUpdateLargeActBtn()
	...
	if isNewInit == false then
		for i in pairs(self.largeBtnArr) do
			...
			if actData and actData:getEndTime() - xyd.getServerTime() > 0 and actData:isOpen() then
				-- block empty
			else
				isNewInit = true
...
function MainWindow:onClickBtnSummon()
	...
	if wnd then
		...
		flag = (not xyd.WindowManager.get():getWindow("activity_window") and not xyd.WindowManager.get():getWindow("campaign_window") or false) and true
...
function MainWindow:updateLargeScrollerBtnRedPoint()
	local iceRedPoint, returnBackPlayerRedPoint, sportsRedPoint, yearFundRedPoint
...
function MainWindow:onWindowClose(event)
	...
	if activityData and activityData:getEndTime() > xyd.getServerTime() and activityData:isOpen() then
