-- chunkname: @../../../Product/Bundles/Android/src/app/windows/sanxiao_window/MapWindow.lua
...
function MapWindow:_checkShowQuestTip()
	...
	for idStr, _ in pairs(curQuestsView) do
		...
		if quest.state == xyd.QuestConstants.ACTIVE and questInfo.star <= SelfInfo.get():get_star() and quest.id ~= xyd.FINAL_QUEST or quest.state == xyd.QuestConstants.STARTED and SelfInfo.get():getTime() >= quest.endTime or quest.state >= xyd.QuestConstants.COMPLETE then
