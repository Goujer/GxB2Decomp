-- chunkname: @../../../Product/Bundles/Android/src/app/windows/sanxiao_window/GameWindow.lua
...
function GameWindow:iPhoneXBottom()
	...
	do
		local ____TS_obj, ____TS_index = self.setting_btn, "y"

		____TS_obj[____TS_index] = ____TS_obj[____TS_index] - changeVal
	end

	do
		local ____TS_obj, ____TS_index = self.group_sound_bg, "y"

		____TS_obj[____TS_index] = ____TS_obj[____TS_index] - changeVal
	end

	do
		local ____TS_obj, ____TS_index = self.group_sound_effect, "y"

		____TS_obj[____TS_index] = ____TS_obj[____TS_index] - changeVal
	end

	do
		local ____TS_obj, ____TS_index = self.group_question, "y"

		____TS_obj[____TS_index] = ____TS_obj[____TS_index] - changeVal
	end

	do
		local ____TS_obj, ____TS_index = self.group_exit, "y"

		____TS_obj[____TS_index] = ____TS_obj[____TS_index] - changeVal
	end

	do
		local ____TS_obj, ____TS_index = self.bg_shezhi, "y"

		____TS_obj[____TS_index] = ____TS_obj[____TS_index] - changeVal
	end

	local ____TS_obj, ____TS_index = self.group_bottom, "bottom"
...
function GameWindow:onOpDownNormal(e)
	...
	if e.localX > self.BOARD_MAX_WIDTH then
		return
	end

	if e.localY > self.BOARD_MAX_HEIGHT then
		return
	end
...
function GameWindow:_clearItemMask()
	...
	while i < #self.group_items do
		...
		self.labelItemNum[i + 1].color = Color.New2(4278054911)
		i = i + 1
	end
