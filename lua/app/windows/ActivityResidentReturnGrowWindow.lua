-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityResidentReturnGrowWindow.lua
...
function ActivityResidentReturnGrowWindow:layout()
	self:initTop()
	xyd.setUISpriteAsync(self.logoImg, nil, "resident_return_grow_logo_" .. xyd.Global.lang, nil, nil)
...
function ShowItem:initBaseInfo(index)
	...
	xyd.setUISpriteAsync(self.showUpIcon, nil, "common_tips_up_" .. multiple, nil, nil)
...
function ShowItem:initItem(index)
	UIEventListener.Get(self.showItemBg.gameObject).onClick = handler(self, function()
		xyd.goWay(xyd.tables.activityReturn2AddTable:getGetwayId(index), nil, nil, function()
			return
		end)
	end)
end
