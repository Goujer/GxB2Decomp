-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ProphetChoosePartnerWindow.lua
...
function ProphetChoosePartnerWindow:setGroupData()
	...
	for _, partner_id in ipairs(sortedPartners[tostring(xyd.partnerSortType.LEV) .. "_0"]) do
		...
		if not p or p:getGroup() >= xyd.PartnerGroup.EMO or p:getStar() ~= 4 and p:getStar() ~= 5 then
			-- block empty
		else
