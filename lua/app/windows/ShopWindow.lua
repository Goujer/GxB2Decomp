-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ShopWindow.lua
...
function MarketItem:setInfo(params)
	...
	xyd.setUISpriteAsync(self.btnIcon_, nil, costIcon, nil, nil)
	...
	if self.shopType == xyd.ShopType.SHOP_HERO and self.unchange then
		if self.end_time > xyd.getServerTime() then
...
function MarketItem:onClickBtnBuy()
	...
	elseif self.parent_.shopType_ == xyd.ShopType.SHOP_ASSESSMENT_ACADEMY then
		...
		if xyd.tables.shopSchoolPracticeTable:checkShowChoose(self.index_) then
			...
			params.message = __("CONFIRM_BUY")
			params.alertType = xyd.AlertType.YES_NO

			function params.callback(confirmBuy)
				if confirmBuy then
...
function ShopWindow:setFreshBtn(isFree)
	...
	xyd.setUISpriteAsync(costIcon, nil, xyd.tables.itemTable:getIcon(cost[1]), nil, nil)
	...
	UIEventListener.Get(btnRefresh.gameObject).onClick = function()
		if not self.shopInfo_ or not xyd.tables.shopConfigTable:isRefresh(self.shopType_) then
			...
			if hasNum < cost[2] then
				...
				params.message = __("NOT_ENOUGH", xyd.tables.itemTable:getName(cost[1]))
			...
			function params.callback(yes_no)
				if yes_no then
					self.shopModel_:refreshShop(self.shopType_)
				end
			end
...
function ShopWindow:goNextPage(changePageNum, buyDrag)
	if buyDrag and (changePageNum + self.page_ <= 0 or self.page_ + changePageNum > self.pageNum_) then
		return
	end
	...
	elseif self.page_ > self.pageNum_ then
		self.page_ = 1
	end
...
function ShopWindow:setPageIcon()
	for idx, dotIcon in ipairs(self.dotIconList_) do
		if idx ~= self.page_ then
			xyd.setUISpriteAsync(dotIcon, nil, "emotbtn1", nil, nil, true)

			dotIcon.width = 16
			dotIcon.height = 16
		else
			xyd.setUISpriteAsync(dotIcon, nil, "market_dot_bg2", nil, nil, true)
