-- chunkname: @../../../Product/Bundles/Android/src/app/windows/PetTrainingDetail1Window.lua
...
function PetTrainingDetail1Window:layout()
	...
	for k, extraData in ipairs(levExtraDatas) do
		...
		if pointK == 0 and self.allLev >= tonumber(lev) and (not nextExtraData or self.allLev < nextExtraData[1]) then
			pointK = k
			isGreen = true
		end
