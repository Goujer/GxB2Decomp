-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityFairyTaleStoryWindow.lua
...
function ActivityFairyTaleStoryWindow:awardBack(event)
	...
	xyd.itemFloat(itemsArr, nil, nil, 6500)
...
function ActivityFairyTaleStoryWindow:refreshStoryList()
	...
	for _, id in ipairs(storyIds) do
		...
		params.id = id
		params.is_awarded = self:checkHasAward(id)
...
function fairyStoryItem:update(_, _, info)
	...
	xyd.setUISpriteAsync(self.storyImg_, nil, imgName, nil, nil)
	...
	self.hasLock_ = self.unLockValue_ > self.parent_.unlockValue
