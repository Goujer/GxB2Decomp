-- chunkname: @../../../Product/Bundles/Android/src/app/windows/SummonDropProbabilityWindow.lua

local BaseWindow = import(".BaseWindow")
local SummonDropProbabilityWindow = class("SummonDropProbabilityWindow", BaseWindow)
...
function SummonDropProbabilityWindow:ctor(name, params)
	...
	self.labelSelect_gamble = {
		0,
		1,
		1
	}

	local data = xyd.models.activity:getActivity(xyd.ActivityID.NEW_SUMMON_GIFTBAG)

	if data then
		self.box_id_[1] = 10004
	end

	self.box_id_ = self.box_id_
	self.prefix = "SUMMON_DROP_NAV"
	self.title = __("DROP_PROBABILITY_WINDOW_TITLE")
	self.type = 1

	if params then
		self.box_id_ = params.box_id_
		self.prefix = params.prefix
		self.title = params.title
		self.type = params.type + 1
	end
end

function SummonDropProbabilityWindow:getUIComponent()
	...
	if self.params then
		...
		self.nav1:ComponentByName("navLabel", typeof(UILabel)).width = 300
		self.nav2:ComponentByName("navLabel", typeof(UILabel)).width = 300
	end
end

function SummonDropProbabilityWindow:initWindow()
	SummonDropProbabilityWindow.super.initWindow(self)
	self:getUIComponent()
	self:initLayOut()
	self:register()
...
function SummonDropProbabilityWindow:initLayOut()
	...
	for i = 1, 3 do
		...
		if self.params then
			navLabel.text = __(prefix .. tostring(self.labelSelect_gamble[i]))
		else
...
function SummonDropProbabilityWindow:updateLayout(i)
	self.cur_select_ = i
...
function SummonDropProbabilityWindow:updateTopButton()
	for i = 1, 3 do
		local nav = self["nav" .. tostring(i)]
...
function SummonDropProbabilityWindow:iosTestChangeUI()
...
return SummonDropProbabilityWindow
