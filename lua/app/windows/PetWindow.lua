-- chunkname: @../../../Product/Bundles/Android/src/app/windows/PetWindow.lua
...
function PetWindow:initDataGroup()
	...
	table.sort(ids, function(a, b)
		...
		local scoreA = petA:getGrade() > petB:getGrade() and 1000 or 0
		local scoreB = petB:getGrade() > petA:getGrade() and 1000 or 0

		scoreA = scoreA + (petA:getLevel() > 0 and 10000 or 0)
		scoreB = scoreB + (petB:getLevel() > 0 and 10000 or 0)
		scoreA = scoreA + (petA:getLevel() > petB:getLevel() and 100 or 0)
		scoreB = scoreB + (petB:getLevel() > petA:getLevel() and 100 or 0)
	...
	for i = 1, #ids do
		...
		if time and time > xyd.getServerTime() then
			-- block empty
		else
			table.insert(data, {
				pos = i,
...
function PetListItem:update(index, realIndex, info)
	...
	if pet:getLevel() <= 0 then
		...
		self.button_label.color = Color.New2(4278124287)
		...
		self.button_num.color = Color.New2(4278124287)
...
function PetListItem:effectClip()
	if self.isFirstEffectClip == nil and self.db and self.petWindow and not tolua.isnull(self.petWindow.window_) then
		...
		if self.petWindow and self.petWindow.scroller and self.petWindow.scroller.gameObject ~= nil and self.go and self.go.gameObject ~= nil and self.petWindow.fistPetItem_y then
			-- block empty
		else
...
function PetListItem:detailTouch()
	...
	xyd.WindowManager.get():closeWindow("pet_window", nil, nil, true)
end
