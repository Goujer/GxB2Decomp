-- chunkname: @../../../Product/Bundles/Android/src/app/windows/HeroChallengeDetailWindow.lua
...
function HeroChallengeDetailItem:createItem()
	...
	if info and info.base_info and info.base_info.fight_max_stage >= self.id then
		icon:setChoose(true)
	end
end
...
function HeroChallengeDetailItem:checkLock()
	...
	if currentStage == -1 or currentStage >= self.id then
		self:unlock()
	else
		self:lock()
	end
