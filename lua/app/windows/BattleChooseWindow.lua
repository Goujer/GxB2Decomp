-- chunkname: @../../../Product/Bundles/Android/src/app/windows/BattleChooseWindow.lua
...
function BattleChooseWindow:layoutPVE()
	...
	if xyd.models.academyAssessment:checkFunctionOpen() then
		...
		if startTime <= xyd.getServerTime() and xyd.getServerTime() - startTime < xyd.TimePeriod.DAY_TIME then
			...
			self.labelTimeAcademy.color = Color.New2(2986279167)
			self.labelTimeAcademy.effectColor = Color.New2(977490687)
		elseif tmpTime > xyd.getServerTime() and xyd.getServerTime() >= tmpTime - xyd.TimePeriod.DAY_TIME then
			self.labelTimeAcademy.text = __("SCHOOL_PRACTICE_FINISH")
			self.labelTimeAcademy.color = Color.New2(4291856383)
			self.labelTimeAcademy.effectColor = Color.New2(977490687)
	...
	xyd.setUISpriteAsync(self.imgShrineHurdleText, nil, "shrine_hurdle_text_" .. xyd.Global.lang, nil, nil, true)
	self:onDragMoving()
...
function BattleChooseWindow:layoutPVP()
	...
	if xyd.models.arenaAllServerNew:checkFunctionOpen() then
		...
		if openTime > xyd.getServerTime() then
			...
			if xyd.models.arenaAllServerNew:checkOpen() then
				...
				self.timeAllServer.color = Color.New2(4275079167)
			elseif startTime > xyd.getServerTime() and xyd.getServerTime() >= startTime - xyd.DAY_TIME then
...
function BattleChooseWindow:registerEvent()
	...
	UIEventListener.Get(self.soulLand).onClick = function()
		if xyd.models.soulLand:isBeforStartTime() then
			...
			if startTime > xyd.getServerTime() then
...
function BattleChooseWindow:updateOpenBlock(g, funID)
	if funID and not xyd.tables.functionTable:getOpenValue(funID) then
		local openLev = 0
	end
end
	...
	if xyd.checkFunctionOpen(xyd.FunctionID.ARENA, true) then
		...
		elseif xyd.models.arena:getIsOld() ~= nil then
			...
			if newArenaStartTimeLeft > 0 and newArenaStartTimeLeft < xyd.models.arena:getNewSeasonOpenTime() then
				...
				isHasTime = true
			elseif xyd.models.arena:getDDL() > xyd.getServerTime() then
				self.groupArenaTime:SetActive(true)
				self:updateArenaLabel(true)
