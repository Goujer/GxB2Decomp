-- chunkname: @../../../Product/Bundles/Android/src/app/windows/GuildTerritoryWindow.lua
...
function GuildTerritoryWindow:onTouchButton(type)
	...
	elseif type == 3 then
		...
		if lev > xyd.models.guild.level then
...
function GuildTerritoryWindow:setShopModel()
	...
	if activityData and xyd.models.guild.saleLev >= xyd.tables.miscTable:getNumber("guild_shop_open_limit", "value") and activityData:isOpen() and xyd.models.guild.saleLev ~= 0 then
...
function GuildTerritoryWindow:updateGuildCompetitionTime()
	if xyd.models.guild:getGuildCompetitionInfo() then
		...
		if timeData.type == 1 then
			...
			self.guildCompetitionTimeText.color = Color.New2(3526711295)
