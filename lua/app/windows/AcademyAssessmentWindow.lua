-- chunkname: @../../../Product/Bundles/Android/src/app/windows/AcademyAssessmentWindow.lua
...
function AcademyAssessmentExtraItem:ctor(go, parent)
	...
	if xyd.getServerTime() >= tonumber(time) then
	...
	UIEventListener.Get(self.btnFight_.gameObject).onClick = function()
		...
		if currentStageID ~= -1 and currentStageID == self.id_ and skipStage <= historyMaxStage and currentStage < n then
			local params = {
				...
				noCallback = function()
					if currentStageID == -1 or currentStageID > self.id_ then
						self:onSweep()
...
function AcademyAssessmentExtraItem:initItem()
	...
	if currentStage == -1 or currentStage > self.id_ then
		self:unLock()

		self.btnFightLabel.text = __("FRIEND_SWEEP")
		rewards = self.fortTable_:getSweepReward(self.id_)

		xyd.setUISpriteAsync(self.itemBgImg_, nil, "academy_assessment_award_bg", nil, nil)
		xyd.setUISpriteAsync(self.btnFightIcon, nil, "icon_63_small", nil, nil)
	...
	elseif currentStage == self.id_ then
		...
		xyd.setUISpriteAsync(self.itemBgImg_, nil, "academy_assessment_award_bg_first", nil, nil)
		xyd.setUISpriteAsync(self.btnFightIcon, nil, "icon_64_small", nil, nil)
		...
		xyd.setUISpriteAsync(self.itemBgImg_, nil, "academy_assessment_award_bg_lock", nil, nil)
		xyd.setUISpriteAsync(self.btnFightIcon, nil, "icon_64_small", nil, nil)
...
function AcademyAssessmentItem:initItem()
	...
	xyd.setUISpriteAsync(self.imgDone, nil, "academy_assessment_done_" .. xyd.Global.lang, nil, nil, true)
...
function AcademyAssessmentWindow:initWindow()
	...
	if xyd.models.academyAssessment:getIsNewSeason() == false then
		xyd.setUISpriteAsync(self.titleBgLeft, nil, "academy_assessment_title_bg", nil, nil)
		xyd.setUISpriteAsync(self.titleBgRight, nil, "academy_assessment_title_bg", nil, nil)
	...
	if switchTime and switchTime < xyd.getServerTime() and not switchTimeFlag then
		...
		xyd.alert(xyd.AlertType.CONFIRM, __("SCHOOL_PRACTISE_100STAGE_TIP"), callback, __("CONFIRM"), nil, nil, nil, nil, callback)
	end
end
...
function AcademyAssessmentWindow:registerEvent()
	...
	UIEventListener.Get(self.helpBtn_.gameObject).onClick = function()
		...
		elseif xyd.models.academyAssessment.seasonId >= tonumber(xyd.tables.miscTable:getNumber("school_practise_new_help_tips", "value")) then
			xyd.WindowManager:get():openWindow("help_window", {
