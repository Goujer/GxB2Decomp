-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ArcticExpeditionBuyWindow.lua
...
function ArcticExpeditionBuyWindow:initAwardCost()
	...
	local function addCallback()
		...
		if maxCanBuy < self.purchaseNum then
			if priceArr[2] * self.purchaseNum > priceArr[2] < xyd.models.backpack:getItemNumByID(priceArr[1]) then
...
function ArcticExpeditionBuyWindow:updateCostLayout()
	...
	if needMoney > xyd.models.backpack:getItemNumByID(xyd.ItemID.CRYSTAL) then
		self.labelTotal.color = Color.New2(4278190335)
	else
