-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityLimitCultivateWindow.lua
...
function ActivityLimitCultivateWindow:initUIComponent()
	xyd.setUISpriteAsync(self.imgText, nil, "logo_zp_" .. xyd.Global.lang, nil, nil, true)
	...
	for index, id in ipairs(awardIds) do
		...
		if self.activityData.detail.times_awarded[index] == 0 then
			...
			if self.activityData.detail.times >= xyd.tables.activityLimitCultivateAwardTable:getPoint(index) then
...
function ActivityLimitCultivateWindow:register()
	...
	for i = 1, #self.awardItems_ do
		UIEventListener.Get(self["awardItemNewRoot" .. i]).onClick = function()
			if self.activityData.detail.times >= xyd.tables.activityLimitCultivateAwardTable:getPoint(i) and self.activityData.detail.times_awarded[i] == 0 then
				local data = require("cjson").encode({
					type = 2,
					id = i
				})
