-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ExploreTrainingRoomWindow.lua
...
function ExploreTrainingRoomWindow:resizeChooseGroup()
	...
	if sHeight > xyd.Global.maxHeight then
...
function ExploreTrainingRoomWindow:updateContent(index)
	...
	if curLev < trainingTable:getLvMax(self.index) then
		...
		self.groupLabel2[3].color = Color.New2(2795939583)
	...
	for i = 1, 2 do
		...
		if data[2] > xyd.models.backpack:getItemNumByID(data[1]) then
			self["labelCost" .. i].color = Color.New2(3981269247)
			flag = false
		else
	...
	for i = 1, 5 do
		if i ~= self.index and minLev > self.data[i].level then
...
function ExploreTrainingRoomWindow:onItemChange(event)
	...
	for _, item in ipairs(items) do
		...
		elseif item.item_id == xyd.ItemID.MANA then
			...
			if data[2] > xyd.models.backpack:getItemNumByID(data[1]) then
				self.labelCost1.color = Color.New2(3981269247)
			else
