-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityEntranceTestWindow.lua
...
function ActivityEntranceTestWindow:layout()
	...
	if openTime and openTime > self.activityData:startTime() then
	...
	for i = 1, self.activityData:getPveMaxStage() do
		xyd.setUISpriteAsync(self["pvebtnTextImg" .. i], nil, "activity_entrance_ysz_" .. i .. "_" .. xyd.Global.lang, nil, nil, true)
		...
		if pveBtnRedPointClickTime then
			...
			if pveBtnRedPointClickTime < self.activityData:startTime() or pveBtnRedPointClickTime >= self.activityData:getEndTime() then
				self["pveBtnRedPoint" .. i]:SetActive(true)
			end
		end
	end
