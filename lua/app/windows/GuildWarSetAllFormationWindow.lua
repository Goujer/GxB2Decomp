-- chunkname: @../../../Product/Bundles/Android/src/app/windows/GuildWarSetAllFormationWindow.lua
...
function GuildWarSetAllFormationWindow:onClickSave()
	...
	if xyd.models.guildWar:judgeMoment() > xyd.models.guildWar.MOMENT.BEFORE_FINAL then
...
function GuildWarFormationSetItem:onClickShowHide(force)
	...
	if not self.isHide_ then
		if self.parent_.hideNum >= tonumber(xyd.tables.miscTable:getVal("guild_war_battle_hide_num")) then
...
function GuildWarFormationSetItem:onClickSelect()
	...
	if not self.isMember_ then
		if self.parent_.selectedNum >= tonumber(xyd.tables.miscTable:getVal("guild_war_battle_max_num")) then
