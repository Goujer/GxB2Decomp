-- chunkname: @../../../Product/Bundles/Android/src/app/windows/DatesWindow.lua
...
function DatesWindow:initData()
	...
	if self.partner:getLovePoint() >= MiscTable:getNumber("love_point_max_base", "value") and data and PartnerAchievementTable:getLastID(data.table_id) == 0 and data.is_complete and data.is_reward == 0 and (data.table_id > 35 or data.table_id < 26) then
		self:partnerStory()
	end
end
		...
		if id and tonumber(id) > 0 then
			...
			if soundID and soundID ~= "" and tonumber(dialog) ~= -1 then
				...
				elseif type_ == xyd.PartnerToSoundTableKey.love_point_sound then
					...
					if maxLovePoint and maxLovePoint >= unLockPoint[lovePointIndex] then
						table.insert(data, params)
					end
				else
					table.insert(data, params)
...
function DatesWindow:onSendGifts(event)
	...
	if self.partner:getLovePoint() >= MiscTable:getNumber("love_point_max_base", "value") or index1 > -1 then
		self:partnerStory()
	end
			...
			local function callback2()
				...
				if i == 5 then
					...
					local function callback3()
						...
						if self.partner:getLovePoint() >= max then
							for t = 1, 5 do
...
function DatesWindow:updateRedMark2()
	...
	if lastIndex < 4 then
		...
		for i = lastIndex + 1, #limits do
			if maxLovePoint >= limits[i] and dataIDs[i] then
				isRed2 = true
...
function DatesWindow:setPledgeLayout()
	if not self.partner:isVowed() and self.partner:getLovePoint() >= MiscTable:getNumber("love_point_max_base", "value") then
