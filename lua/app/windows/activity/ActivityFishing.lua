-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityFishing.lua
...
function ActivityFishing:initUIComponent()
	...
	xyd.setUISpriteAsync(self.imgText, nil, "activity_fishing_text_" .. xyd.Global.lang, nil, nil, true)
...
function ActivityFishing:register()
	...
	self:registerEvent(xyd.event.GO_FISHING, function(event)
		...
		if event.data.need_id and tostring(event.data.need_id) ~= "" and tostring(math.floor(xyd.tables.activityFishSurpriseTable:getLength(event.data.id)[1] * 100)) <= tostring(math.floor(event.data.len * 100 + 0.1)) and tostring(math.floor(xyd.tables.activityFishSurpriseTable:getLength(event.data.id)[2] * 100)) >= tostring(math.floor(event.data.len * 100 + 0.1)) and self.activityData.detail.fishs[event.data.id].surprise == nil then
			lenSurprise = true
		...
		if event.data.len >= goldLenRange[2][1] or event.data.len <= goldLenRange[1][2] then
			if event.data.len >= goldLenRange[2][1] and self.activityData.detail.fishs[event.data.id].get_max == nil then
				goldSurprise = true
			...
			local sequence = self:getSequence(function()
				...
				local function closeCallback()
							...
							if lenSurprise == true or goldSurprise == true and not self.autoTime then
								...
								if not self.autoTime or self.todayFirstTime then
									...
									if goldSurprise == true and self.autoTime then
										...
										xyd.alert(xyd.AlertType.CONFIRM, __("ACTIVITY_FISH_GOLDEN_SURPRISE", xyd.tables.activityFishingTextTable:getName(event.data.id)), alertCallback, __("ACTIVITY_FISH_SURPRISE_BUTTON"), nil, nil, __("ACTIVITY_FISH_SURPRISE_TITLE"), nil, alertCallback)
					...
					if lenSurprise == true then
						...
						xyd.alert(xyd.AlertType.CONFIRM, __("ACTIVITY_FISH_LENGTH_SURPRISE", string.format("%.2f", event.data.len), xyd.tables.activityFishingTextTable:getName(event.data.id)), callback, __("ACTIVITY_FISH_SURPRISE_BUTTON"), nil, nil, __("ACTIVITY_FISH_SURPRISE_TITLE"), nil, callback)
					end
					...
					if goldSurprise == true and not self.autoTime then
						...
						xyd.alert(xyd.AlertType.CONFIRM, __("ACTIVITY_FISH_GOLDEN_SURPRISE", xyd.tables.activityFishingTextTable:getName(event.data.id)), callback, __("ACTIVITY_FISH_SURPRISE_BUTTON"), nil, nil, __("ACTIVITY_FISH_SURPRISE_TITLE"), nil, callback)
					end
				end
	...
	UIEventListener.Get(self.btnAward).onClick = function()
		...
		for i = 1, #ids do
			...
			info.id = ids[i]
			info.max_value = xyd.tables.activityFishingAwardTable:getComplete(ids[i])
