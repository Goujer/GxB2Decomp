-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityKeyboard.lua
	...
	UIEventListener.Get(self.touchField).onClick = function()
		...
		if self.curBottomIndex == self.bigAwardIndex and #self.lastBigAwards > 0 then
			...
			self.itemIconNoteEffects[realIndex]:play("texiao01", 1, 1, function()
				...
				xyd.itemFloat(self.awards, nil, nil, 5000)
...
function ActivityKeyboard:playTopAwardEffect(num)
	...
	self:waitForTime(t, function()
		...
		self:waitForTime(0.03, function()
			...
			if self.curTopIndex > self.activityData:getCurLayerItemsNum() then
				self.curTopIndex = 1
			end
...
function ActivityKeyboard:chooseLayer(layer)
	...
	xyd.alertYesNo(__("ACTIVITY_KEYBOARD_TEXT1"), callback, __("YES"), false, nil, nil, nil, nil, nil, nil)
end
...
function ActivityKeyboardItem:update(index, info)
	...
	if self.data.layer == self.parent.maxLayer then
		...
		xyd.setUISpriteAsync(self.bg, nil, "activity_keyboard_bg_yp_" .. self.data.layer % 2, nil, nil, true)
	...
	xyd.setUISpriteAsync(self.IndexImg, nil, "activity_keyboard_bg_" .. self.data.layer, nil, nil, true)
