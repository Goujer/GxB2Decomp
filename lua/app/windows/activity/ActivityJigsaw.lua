-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityJigsaw.lua
...
function ActivityJigsaw:setCountDown()
	if self.activityData:getUpdateTime() > xyd.getServerTime() then
		...
		params.duration = self.activityData:getUpdateTime() - xyd.getServerTime()
...
function ActivityJigsaw:updateProgress()
	...
	for i = 1, maxNum do
		...
		if id then
			...
			source = tar_cnt <= tonumber(cur_cnt) and (id <= 3 and "activity_jigsaw_icon01_1" or id <= 4 and "activity_jigsaw_icon02_1" or "activity_jigsaw_open_icon") or id <= 3 and "activity_jigsaw_icon01_0" or id <= 4 and "activity_jigsaw_icon02_0" or "trial_icon04"
