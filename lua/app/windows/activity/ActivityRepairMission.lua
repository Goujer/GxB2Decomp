-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityRepairMission.lua
...
function ActivityRepairMission:euiComplete()
	xyd.setUISpriteAsync(self.log, nil, "activity_repair_mission_logo_" .. xyd.Global.lang, nil, nil, true)
	self:setText()
	self:setItem()
...
function FundItem:initItem(itemdata)
	...
	if self.progressBar_.value == 1 then
		xyd.setUISpriteAsync(self.progressImg, nil, "activity_bar_thumb_2", nil, nil, true)
	else
		xyd.setUISpriteAsync(self.progressImg, nil, "activity_bar_thumb", nil, nil, true)
	end
	...
	if self.getWayId_ and tonumber(self.getWayId_) > 0 then
		local function onClick()
			xyd.goWay(self.getWayId_, nil, nil, function()
