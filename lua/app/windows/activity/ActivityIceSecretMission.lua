-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityIceSecretMission.lua
...
local ActivityIceSecretMission = class("ActivityIceSecretMission", ActivityContent)
local CommonTabBar = require("app.common.ui.CommonTabBar")
local CountDown = import("app.components.CountDown")
local ActivityIceSecretMissionItem = class("ActivityIceSecretMissionItem", import("app.components.CopyComponent"))

function ActivityIceSecretMission:ctor(parentGO, params)
	ActivityContent.ctor(self, parentGO, params)

	self.missionTable = xyd.tables.activityIceSecretMissionTable
	self.missionTextTable = xyd.tables.activityIceSecretMissionTextTable
...
function ActivityIceSecretMission:onActivityByID(event)
...
function ActivityIceSecretMission:getPrefabPath()
	return "Prefabs/Windows/activity/activity_ice_secret_mission"
end

function ActivityIceSecretMission:createChildren()
	self:getUIComponent()
...
function ActivityIceSecretMission:onRefresh(event)
...
function ActivityIceSecretMission:getUIComponent()
	local go = self.go

	self.activity_ice_secret_mission_item = go:NodeByName("activity_ice_secret_mission_item").gameObject
	self.allGroup = go:NodeByName("e:Group").gameObject
	self.partnerGroup = self.allGroup:ComponentByName("partnerGroup", typeof(UITexture))
	...
	self.timeLabel1 = self.contentGroup:ComponentByName("e:Group/timeTextLabel/timeLabel1", typeof(UILabel))
	self.timeLabel2 = self.contentGroup:ComponentByName("e:Group/timeTextLabel/timeLabel2", typeof(UILabel))
...
function ActivityIceSecretMission:initNav()
	local index = 2
	local labelText = {}
	local labelStates = {
		chosen = {
			color = Color.New2(4294967295),
			effectColor = Color.New2(1631156479)
		},
		unchosen = {
			color = Color.New2(1347575039),
...
function ActivityIceSecretMission:updateNav(i)
	...
	if self.curMissionType == MissionType.DAILY then
		...
		if dayEndtime > 0 and dayEndtime > xyd.TimePeriod.DAY_TIME then
			...
			self.timeTextLabel:X(-60)
			self.timeLabel1:SetActive(true)
			self.timeLabel2:SetActive(false)
			self.timeLabel1:X(self.timeTextLabel.width / 2 + 40)
		else
		...
		if self.isTimeWeekOver == false then
			self.timeLabel1:SetActive(false)
		...
		if self.round < 2 then
			...
			self.timeLabel2:X(self.timeTextLabel.width / 2 + 40)
		else
			self.timeLabel1:SetActive(false)
			self.timeLabel2:SetActive(false)
			self.timeTextLabel:X(0)
...
function ActivityIceSecretMission:initTextAndImage()
	xyd.setUITextureAsync(self.logoImg, "Textures/activity_web/ice_secret_mission/activity_ice_secret_mission_logo_text_" .. tostring(xyd.Global.lang))
end

function ActivityIceSecretMission:initCountDownAndRound()
	local startTime = self.activityData:startTime()
	local passedTotalTime = xyd.getServerTime() - startTime
	...
	if self.curMissionType == MissionType.DAILY then
		...
		if dayEndtime > 0 and dayEndtime > xyd.TimePeriod.DAY_TIME then
			...
			if xyd.Global.lang == "fr_fr" then
				self.timeLabel1:X(self.timeTextLabel.width / 2 + 25)
			end
		else
			self.timeLabel1:SetActive(false)
	...
	if self.activityData:getEndTime() > xyd.getServerTime() then
		...
		xyd.models.activity:updateRedMarkCount(xyd.ActivityID.ACTIVITY_ICE_SECRET_MISSION, function()
			xyd.db.misc:setValue({
				key = "activity_ice_secret_mission",
				value = dayRound
			})
		end)
...
function ActivityIceSecretMission:timeOverDaily()
	xyd.models.activity:reqActivityByID(self.id)
	self:initCountDownAndRound()
end

function ActivityIceSecretMission:timeOverWeek()
	self.isTimeWeekOver = true
end

function ActivityIceSecretMission:initEffect()
	local effectName = "zhanghe_pifu03_lihui01"

	NGUITools.DestroyChildren(self.partnerGroup.transform)

	self.summonEffect_ = xyd.Spine.new(self.partnerGroup.gameObject)

	self.summonEffect_:setInfo(effectName, function()
		self.summonEffect_:setRenderTarget(self.partnerGroup, 1)
		self.summonEffect_:play("animation", 0)
		self.summonEffect_:SetLocalPosition(4, -969, 0)
	end)

	if self.round % 3 == 0 then
		self.partnerGroup.transform.localEulerAngles = Vector3(0, 0, -15)
	else
		self.partnerGroup.transform.localEulerAngles = Vector3(0, 0, 0)
	end
end

function ActivityIceSecretMission:initMissionGroup()
	...
	for i in pairs(paramsNow) do
		if self.itemArr[i] == nil then
			local tmp = NGUITools.AddChild(self.missionGroup.gameObject, self.activity_ice_secret_mission_item.gameObject)
			local item = ActivityIceSecretMissionItem.new(tmp, paramsNow[i])
...
function ActivityIceSecretMission:switchMissionTypeRegister()
...
function ActivityIceSecretMission:switchMissionType()
...
function ActivityIceSecretMission:countDown2Zero()
...
function ActivityIceSecretMissionItem:ctor(goItem, params)
	self.goItem_ = goItem
...
function ActivityIceSecretMissionItem:getGo()
	return self.goItem_
end

function ActivityIceSecretMissionItem:createChildren()
	self:initIcon()
	self:initProgress()

	UIEventListener.Get(self.goItem_).onClick = handler(self, function()
		local getWayId = xyd.tables.activityIceSecretMissionTable:getGetway(self.id)
...
function ActivityIceSecretMissionItem:initIcon()
...
function ActivityIceSecretMissionItem:initProgress()
...
function ActivityIceSecretMissionItem:updateParams(params)
	self.id = params.id
	self.descLabel.text = params.desc
	self.completeNum = params.completeNum
	self.value = params.value
...
return ActivityIceSecretMission
