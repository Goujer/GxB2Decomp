-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityGoldfish.lua
...
function ActivityGoldfish:summon(num)
	...
	if num > xyd.models.backpack:getItemNumByID(xyd.ItemID.GOLDFISH_NET) then
...
function ActivityGoldfish:onGetAward(event)
	...
	if data.activity_id == xyd.ActivityID.ACTIVITY_GOLDFISH then
		...
		if type_id == 1 then
			...
			function params.buyCallback(_, _, is_cost2)
				if is_cost2 then
					self:summon(10)
				else
					self:summon(1)
