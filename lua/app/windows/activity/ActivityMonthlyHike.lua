-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityMonthlyHike.lua
...
function StageItem:setInfo(stageID)
	...
	if self.stageNow > 0 or self.stageNow <= 0 and chapterID <= 5 then
		...
		elseif self.stageID > self.stageNow then
...
function ActivityMonthlyHike:updateDotList()
	for i = 1, 6 do
		...
		if i ~= self.chapterID then
			xyd.setUISpriteAsync(dotImg, nil, "market_dot_bg1", nil, nil)
		else
			xyd.setUISpriteAsync(dotImg, nil, "market_dot_bg2", nil, nil)
		end
...
function ActivityMonthlyHike:playFakeBattleReport(stage_id, items)
	...
	params2.event_data = {}
	params2.battle_type = xyd.BattleType.WORLD_BOSS
...
function ActivityMonthlyHike:updateMap()
	...
	if lineID ~= 4 then
		...
		xyd.setUISpriteAsync(self.lineImg, nil, "monthly_stage_line_bg" .. lineID, nil, nil, true)
...
function ActivityMonthlyHike:getHasFinish()
	...
	if self:getStageNow() > 0 then
		if self.stageID < self:getStageNow() then
			isFinish = 1
		elseif self.stageID > self:getStageNow() then
			isFinish = -1
		end
	else
		isFinish = self.chapterID <= 5 and 1 or -2
	end
