-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityWine.lua
...
function ActivityWine:initUIComponent()
	xyd.setUISpriteAsync(self.logoImg, nil, "activity_wine_text_" .. xyd.Global.lang, nil, nil, nil)
...
function ActivityWine:onRegister()
	...
	UIEventListener.Get(self.downIcon.gameObject).onClick = handler(self, function()
		...
		for i in pairs(ids) do
			...
			data.id = ids[i]
			data.max_value = xyd.tables.activity3BirthdayDinnerPointTable:getPoint(ids[i]) / item_data[2]
