-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/NewTrialBattlepass.lua
...
function TrialBattlePassItem:update(_, _, info)
	...
	if expNow >= self.needExp_ then
		self.levLabel_.color = Color.New2(4293307391)
		self.levLabel_.effectColor = Color.New2(3311934868)
		...
		self.levLabel_.effectColor = Color.New2(4294569983)
...
function TrialBattlePassItem:refresh()
	self:update(nil, nil, self.info_)
end
...
function TrialBattlePassItem:updateItemList()
	...
	if self.parent_.activityData.detail.point >= self.needExp_ and self.isAwarded_ == 0 then
	...
	if self.parent_.activityData:checkBuy() and self.parent_.activityData.detail.point >= self.needExp_ and self.isExawarded_ == 0 then
