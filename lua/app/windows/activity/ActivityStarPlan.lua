-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityStarPlan.lua
...
function ActivityStarPlan:updateBoxItemImg()
	for i = 1, 7 do
		if self.targetIndex_ and self.targetIndex_ == i then
			...
			xyd.setUISpriteAsync(self["showItem" .. i], nil, pic, nil, nil, true)
		else
...
function ActivityStarPlan:showGetAni()
	...
	xyd.setUISpriteAsync(self.showAwardItem, nil, pic, nil, nil, true)
	xyd.setUISpriteAsync(self["showItem" .. centerIndex], nil, pic, nil, nil, true)
...
function ActivityStarPlan:buyBox(times)
	if times > xyd.models.backpack:getItemNumByID(454) then
