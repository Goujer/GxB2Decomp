-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityChristmasSale.lua
...
function ActivityChristmasSale:setText()
	...
	xyd.setUISpriteAsync(self.imgText, nil, "christmas_sale_logo_text_" .. xyd.Global.lang, nil, nil, true)
...
function ActivityChristmasSaleItem:ctor(go, parent, params)
	...
	self.parent = parent
	self.data = params
	self.isLock = ActivityChristmasSaleAwardsTable:getRequirement(self.data.id) > self.data.cost
...
function ActivityChristmasSaleItem:setBtn()
	if self.data.leftTimes == 0 then
		...
		xyd.setUISpriteAsync(self.button_icon, nil, xyd.tables.itemTable:getSmallIconNew(xyd.ItemID.DEER_CANDY), nil, nil, true)
	...
	elseif self.isLock then
		self.button_icon:SetActive(true)
		xyd.setUISpriteAsync(self.button_icon, nil, "awake_lock", nil, nil, true)
...
function ActivityChristmasSaleItem:setOpAward(opAwards)
	...
	for i = 1, self.opAwardsnum do
		if self.opAwards[i] then
			...
			self.opAwardsIcon[i]:setIcon(nil, nil)
		end
	end
...
function ActivityChristmasSaleItem:onExchange()
	...
	elseif self.opAwardsnum == 0 or self.opAwards and self.selectAwardsNum == self.opAwardsnum then
		if self.data.id == 1 then
			...
			local canBuy = xyd.models.backpack:getItemNumByID(xyd.ItemID.DEER_CANDY) >= tonumber(ActivityChristmasSaleAwardsTable:getCost(self.data.id)[2])
