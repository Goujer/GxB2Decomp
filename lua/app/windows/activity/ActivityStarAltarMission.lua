-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityStarAltarMission.lua
...
function StarAltarMissionItem:initUI()
	...
	UIEventListener.Get(self.bg.gameObject).onClick = handler(self, function()
		if self.data_.get_way and self.data_.get_way > 0 then
			xyd.goWay(self.data_.get_way, nil, nil, function()
...
function StarAltarMissionItem:setInfo(data)
	...
	if self.data_.is_completed >= self.data_.limit then
		...
		self.completeNum.color = Color.New2(2889360639)
...
function ActivityStarAltarMission:onClickNav(index)
	if self.chooseIndex_ == index then
		...
		for i = 1, 2 do
			...
			if i ~= index then
				self["navLabel" .. i].color = Color.New2(3907117055)
			else
...
function ActivityStarAltarMission:updateMissionList()
	...
	for i = 1, #missionIds do
		...
		params.id = tonumber(missionIds[i])
	...
	table.sort(missionData, function(a, b)
		...
		if a.is_completed >= a.limit then
			avalue = avalue + 1000
		end

		if b.is_completed >= b.limit then
			bvalue = bvalue + 1000
		end
...
function ActivityStarAltarMission:updateItemList()
	...
	for i = 1, #itemIDs do
		...
		params.id = i
		params.item = xyd.tables.activityStarAltarExchangeTable:getAward(i)
		params.cost = xyd.tables.activityStarAltarExchangeTable:getCost(i)
		params.limit = xyd.tables.activityStarAltarExchangeTable:getLimit(i)
		params.buy_times = self.activityData.detail.buy_times[i]
