-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityPromotionLadder2.lua
...
function ActivityPromotionLadder2:initUIComponent()
	xyd.setUISpriteAsync(self.textImg, nil, "activity_promotion_ladder_text_" .. xyd.Global.lang, nil, nil, true)
...
function ActivityPromotionLadder2:register()
	...
	UIEventListener.Get(self.btnPromote.gameObject).onClick = function()
		...
		if self.costNum > xyd.models.backpack:getItemNumByID(costItemID) then
...
function ActivityPromotionLadder2:getTargetTableID(index)
	...
	if self.materialPartner then
		...
		starIndex = star == 5 and 1 or star < 10 and 2 or 3
	end
