-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/LevelUpGiftBag.lua
...
function LevelUpGiftBag:initBtn()
	...
	if self.activityData.detail.charge.buy_times >= GiftBagTable:get():getBuyLimit(self.giftBagID) or self.activityData:getUpdateTime() <= xyd:getServerTime() then
...
function LevelUpGiftBag:onRecharge(evt)
	...
	if self.activityData.detail.charge.buy_times >= GiftBagTable:get():getBuyLimit(self.giftBagID) then
		self:setBtnState(false)
	end
end
...
function LevelUpGiftBag:returnCommonScreen()
	...
	local ____TS_obj, ____TS_index = self.mainGroup, "y"
...
function NewLevelUpGiftBag:initBtn()
	...
	if self.activityData.detail[self.type].charge.buy_times >= GiftBagTable:get():getBuyLimit(self.giftBagID) or self:getUpdateTime() <= xyd:getServerTime() then
		self:setBtnState(false)
	end
end
...
function NewLevelUpGiftBag:onRecharge(evt)
	...
	if self.activityData.detail[self.type].charge.buy_times >= GiftBagTable:get():getBuyLimit(self.giftBagID) then
		self:setBtnState(false)
	end
end
