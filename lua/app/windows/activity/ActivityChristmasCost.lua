-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityChristmasCost.lua
...
function ActivityChristmasCost:initUIComponent()
	xyd.setUISpriteAsync(self.textImg, nil, "christmas_cost_title_" .. xyd.Global.lang, nil, nil, true)
	self:setText()
	self:setItem()
	self:eventRegister()
...
function ActivityChristmasCost:setItem()
	...
	table.sort(awards, function(a, b)
		...
		if a.point <= math.fmod(a.curPoint, maxPoint) == (b.point <= math.fmod(b.curPoint, maxPoint)) then
			...
			return a.point > math.fmod(a.curPoint, maxPoint)
		end
	end)
...
function ActivityChristmasCostItem:initItem(itemdata)
	...
	for _, reward in pairs(itemdata.awards) do
		...
		if itemdata.curPoint >= itemdata.point then
			icon:setChoose(true)
		end
	end
end
