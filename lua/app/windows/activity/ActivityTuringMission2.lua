-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityTuringMission2.lua

local CountDown = import("app.components.CountDown")
local ActivityContent = import(".ActivityContent")
local ActivityTuringMission2 = class("ActivityTuringMission2", ActivityContent)
local ActivityTuringMission2Item = class("ActivityTuringMission2Item", import("app.components.CopyComponent"))
local ActivityTuringMissionTable = xyd.tables.activityTuringMissionTable

function ActivityTuringMission2:ctor(parentGO, params)
	self.currentState = xyd.Global.lang
	self.data = {}

	ActivityContent.ctor(self, parentGO, params)
	xyd.models.activity:reqActivityByID(xyd.ActivityID.ACTIVITY_TURING_MISSION2)
end

function ActivityTuringMission2:resizeToParent()
	ActivityTuringMission2.super.resizeToParent(self)

	local allHeight = self.go:GetComponent(typeof(UIWidget)).height
	local heightDis = allHeight - 874
end

function ActivityTuringMission2:getPrefabPath()
	return "Prefabs/Windows/activity/activity_turing_mission2"
end

function ActivityTuringMission2:initUI()
	self:getUIComponent()
	ActivityTuringMission2.super.initUI(self)
	xyd.setUISpriteAsync(self.textImg, nil, "activity_turing_mission2_logo_" .. xyd.Global.lang, nil, nil, true)
	self:setText()
...
function ActivityTuringMission2:getUIComponent()
	local go = self.go
...
function ActivityTuringMission2:setText()
	if xyd.Global.lang == "de_de" then
		self.textLabel.width = 520
	end

	self.endLabel.text = __("TEXT_END")
	self.textLabel.text = __("TURING_MISSION02_TEXT01")
end

function ActivityTuringMission2:setItem()
	...
	for i = 1, #data do
		...
		local item = ActivityTuringMission2Item.new(tmp, data[i], self)
	end
...
function ActivityTuringMission2:onRegister()
...
function ActivityTuringMission2:onWindowClose(event)
...
function ActivityTuringMission2:onActivityByID(event)
	local id = event.data.act_info.activity_id

	if id == xyd.ActivityID.ACTIVITY_TURING_MISSION2 then
		self:setItem()
	end
end

function ActivityTuringMission2Item:ctor(parentGo, params)
	...
	ActivityTuringMission2Item.super.ctor(self, parentGo)
end

function ActivityTuringMission2Item:initUI()
	ActivityTuringMission2Item.super.initUI(self)
	self:getUIComponent()
	self:initText()
	self:initItem()
	self:registEvent()
...
function ActivityTuringMission2Item:registEvent()
...
function ActivityTuringMission2Item:getUIComponent()
...
function ActivityTuringMission2Item:initText()
...
function ActivityTuringMission2Item:initItem()
