-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityTuringMission3.lua

local CountDown = import("app.components.CountDown")
local ActivityContent = import(".ActivityContent")
local ActivityTuringMission3 = class("ActivityTuringMission3", ActivityContent)
local ActivityTuringMission3Item = class("ActivityTuringMission3Item", import("app.components.CopyComponent"))
local ActivityTuringMissionTable = xyd.tables.activityTuringMissionTable

function ActivityTuringMission3:ctor(parentGO, params)
	self.currentState = xyd.Global.lang
	self.data = {}

	ActivityContent.ctor(self, parentGO, params)
	xyd.models.activity:reqActivityByID(xyd.ActivityID.ACTIVITY_TURING_MISSION3)
end

function ActivityTuringMission3:resizeToParent()
	ActivityTuringMission3.super.resizeToParent(self)

	local allHeight = self.go:GetComponent(typeof(UIWidget)).height
	local heightDis = allHeight - 874
end

function ActivityTuringMission3:getPrefabPath()
	return "Prefabs/Windows/activity/activity_turing_mission3"
end

function ActivityTuringMission3:initUI()
	self:getUIComponent()
	ActivityTuringMission3.super.initUI(self)
	xyd.setUISpriteAsync(self.textImg, nil, "activity_turing_mission3_logo_" .. xyd.Global.lang, nil, nil, true)
	self:setText()
...
function ActivityTuringMission3:getUIComponent()
	local go = self.go
...
function ActivityTuringMission3:setText()
	if xyd.Global.lang == "de_de" then
		self.textLabel.width = 520
	end

	self.endLabel.text = __("TEXT_END")
	self.textLabel.text = __("TURING_MISSION03_TEXT01")
end

function ActivityTuringMission3:setItem()
	...
	for i = 1, #ids do
		...
		if self.activityData.detail.tasks[id] >= ActivityTuringMissionTable:getLimit(id) then
			is_completed = true
		end
	...
	for i = 1, #data do
		...
		local item = ActivityTuringMission3Item.new(tmp, data[i], self)
	end
...
function ActivityTuringMission3:onRegister()
	...
	UIEventListener.Get(self.btnPlus).onClick = function()
		...
		if getWayId > 0 then
			xyd.goWay(getWayId, nil, nil, nil)
		end
	end
...
function ActivityTuringMission3:onWindowClose(event)
...
function ActivityTuringMission3:onActivityByID(event)
	local id = event.data.act_info.activity_id

	if id == xyd.ActivityID.ACTIVITY_TURING_MISSION3 then
		self:setItem()
	end
end

function ActivityTuringMission3Item:ctor(parentGo, params)
	...
	ActivityTuringMission3Item.super.ctor(self, parentGo)
end

function ActivityTuringMission3Item:initUI()
	ActivityTuringMission3Item.super.initUI(self)
	self:getUIComponent()
	self:initText()
	self:initItem()
	self:registEvent()
...
function ActivityTuringMission3Item:registEvent()
...
function ActivityTuringMission3Item:getUIComponent()
...
function ActivityTuringMission3Item:initText()
...
function ActivityTuringMission3Item:initItem()
