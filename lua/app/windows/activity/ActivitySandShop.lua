-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivitySandShop.lua
...
function ActivitySandShop:onClickBtnExplore()
	...
	local upFlag = content_height_[state] > content_height_[self.view_state_]
	local preholdValue = {}
...
function ActivitySandShopLayerItem:refresh()
	...
	if self.title then
		...
		for i = 1, #self.data do
			...
			if self.parent.activityData:getNowConditionValue() > xyd.tables.activitySandShopTable:getCondition(self.data[i]) then
				self["shadow" .. i]:SetActive(false)
			else
				self["shadow" .. i]:SetActive(true)
			end
