-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityLasso.lua
...
function ActivityLasso:initUIComponent()
	xyd.setUISpriteAsync(self.logoImg, nil, "activity_lasso_logo_" .. xyd.Global.lang, nil, nil, true)
...
function AwardItem:register()
	UIEventListener.Get(self.go).onClick = handler(self, function()
		...
		if #awards == 1 or round > self.info then
			return
		end
...
function AwardItem:updateLayout()
	...
	self.icon:setChoose(round > self.info)
