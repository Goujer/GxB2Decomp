-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ProphetSummonGiftBag.lua
...
function ProphetSummonGiftBag:euiComplete()
	xyd.setUISpriteAsync(self.textImg, nil, "prophet_summon_giftbag_text01_" .. xyd.Global.lang, nil, nil, true)
	self:setText()
	self:setItem()
	self:eventRegister()
...
function ProphetSummonGiftBag:setItem()
	...
	for i, v in pairs(ids) do
		...
		if self.activityData.detail.point >= ActivityTreeTable:getPoint(tonumber(id)) or self.activityData.detail.circle_times >= xyd.tables.activityTable:getRound(self.id)[2] then
			is_completed = true
		end
...
function ProphetSummonGiftBagItem:initItem(itemdata)
	...
	if itemdata.point > itemdata.limit_point or itemdata.isCompleted then
