-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityHalloween.lua
...
function ActivityHalloween:initUIComponent()
	...
	xyd.setUISpriteAsync(self.imgText, nil, "activity_halloween_text_" .. xyd.Global.lang, nil, nil, true)
...
function ActivityHalloween:updateRedPoint()
	...
	for i = 1, #awardIDs do
		if self.activityData.detail.times >= xyd.tables.activityHalloweenTrickAwardTable:getComplete(i) and self.activityData.detail.awards[i] == 0 then
			awardRed = true
		end
	end
	...
	for i = 1, #shopIDs do
		...
		if self.activityData.detail.buy_times[i] < xyd.tables.activityHalloweenShopTable:getLimit(i) and xyd.models.backpack:getItemNumByID(cost[1]) >= cost[2] then
			shopRed = true
		end
	end
...
function ActivityHalloween:updateProgressBar()
	...
	if self.activityData.detail.point >= self.progressBarMaxValue then
...
function ActivityHalloween:updateAwardPreview()
	...
	for i = 1, #ids do
		if self.activityData.detail.awards[i] == 0 then
			if self.activityData.detail.times >= xyd.tables.activityHalloweenTrickAwardTable:getComplete(i) then
...
function ActivityHalloween:register()
	...
	UIEventListener.Get(self.btnAward).onClick = function()
		...
		for i = 1, #ids do
			...
			info.id = ids[i]
			info.max_value = xyd.tables.activityHalloweenTrickAwardTable:getComplete(ids[i])
