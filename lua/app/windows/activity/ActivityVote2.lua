-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityVote2.lua
...
function ActivityVote2:initResGroup()
	...
	xyd.setUISpriteAsync(self.textImg2_, nil, "activity_vote_text01_" .. xyd.Global.lang, nil, nil, true)
...
function ActivityVote2:euiComplete()
	...
	xyd.setUISpriteAsync(self.textImg_, nil, "activity_vote_text01_" .. xyd.Global.lang, nil, nil, true)
	self:updateRedPoint()
...
function ActivityVote2:updateRedPoint()
	...
	for i = 1, #mission_count do
		if mission_count[i] >= xyd.tables.activityWeddingVote2MissionTable:getComplete(i) and (not mission_awarded[i] or mission_awarded[i] == 0) then
...
function ActivityVote2:onTouchSortType(type, need_event)
	...
	if type == SortType.VOTE_COUNT then
		self.countSort_label.color = Color.New2(4294967295)
		...
		self.defaultSort_label.effectColor = Color.New2(4294967295)
	else
		self.defaultSort_label.color = Color.New2(4294967295)
		...
		self.countSort_label.effectColor = Color.New2(4294967295)
	end
		...
		if list and #list > 0 then
			...
			elseif type == SortType.VOTE_COUNT then
				table.sort(list, function(a, b)
					if a and b then
						return a.count_fun(a.id) > b.count_fun(b.id)
					else
						return false
					end
				end)
