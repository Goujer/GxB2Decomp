-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityEquipLevelAntique.lua
...
function ActivityEquipLevelAntique:initContent2()
	...
	for _, id in ipairs(ids) do
		...
		if timeStamp and timeStamp > 0 and timeStamp > xyd.getServerTime() then
			-- block empty
		else
...
function LevupItem:setMainAntique(itemID)
	...
	if itemID ~= 0 then
		...
		xyd.setUISpriteAsync(self.imgIcon, nil, source, nil, nil, true)
...
function LevupItem:setLabelCostColor()
	if self.cost[2] > xyd.models.backpack:getItemNumByID(self.cost[1]) then
		self.labelCost.color = Color.New2(3828429311)
	else
