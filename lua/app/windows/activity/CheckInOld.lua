-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/CheckInOld.lua
...
function CheckInOld:setBtnState()
	...
	if self.activityData.detail.online_days <= self.activityData.detail.count then
		...
		self.btnCheckIn_label.effectColor = Color.New2(4294967040)
		imgPath = "check_in_white_btn"

		self.btnCheckInMask:SetActive(true)
	else
		self.btnCheckIn_label.color = Color.New2(4294967295)
...
function CheckInItem:setState()
	...
	self.icon:setMask(detail.count >= self.index)
	self.imgReward:SetActive(detail.count >= self.index)
	...
	if self.index > detail.count and self.index <= detail.online_days then
