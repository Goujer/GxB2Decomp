-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/WishingPoolGiftBag.lua
...
function WishingPoolGiftBag:onGetAward(event)
	...
	if type_ == 1 then
		...
		if self.activityData.detail.point >= xyd.tables.activityGambleTable:getLastPoint() and self.activityData.detail.circle_times < xyd.tables.activityTable:getRound(xyd.ActivityID.WISHING_POOL_GIFTBAG)[1] then
...
function WishingPoolGiftBag:setItem()
	...
	for i in ipairs(ids) do
		...
		if point >= table_instance:getPoint(id) then
			is_completed = true
		end
...
function WishingPoolGiftbagItem:initItem(itemdata)
	...
	local isComplete = itemdata.point >= itemdata.max_point

	for i, reward in pairs(itemdata.awarded) do
		if #itemdata.awarded >= 5 then
			scaleNum = (i == 1 or i == 2) and 0.6 or 0.7
		end
