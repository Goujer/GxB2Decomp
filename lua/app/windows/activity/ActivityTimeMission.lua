-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityTimeMission.lua
...
function ActivityTimeMission:getUIComponent()
	...
	xyd.setUISpriteAsync(self.logo, nil, "activity_time_mission_logo_" .. xyd.Global.lang, nil, nil)
	...
	for i = 1, 7 do
		...
		if i > math.ceil(timeDis / 86400) then
			self["tab_mask_" .. i].gameObject:SetActive(true)
		else
			self["tab_mask_" .. i].gameObject:SetActive(false)
		end

		xyd.setUISpriteAsync(self["tab_chosen_" .. i], nil, chosen_img, nil, nil)
		xyd.setUISpriteAsync(self["tab_unchosen_" .. i], nil, unchosen_img, nil, nil)
		xyd.setUISpriteAsync(self["tab_mask_" .. i], nil, maskImg, nil, nil)
	end
...
function ActivityTimeMission:onTouchGo()
	...
	if goWay and goWay > 0 then
		xyd.goWay(goWay, nil, nil, function()
			xyd.models.activity:reqActivityByID(xyd.ActivityID.ACTIVITY_TIME_MISSION)
		end)
	end
end
...
function ActivityTimeMissionItem:initUI()
	...
	xyd.setUISpriteAsync(self.awardImg_, nil, "mission_awarded_" .. xyd.Global.lang, nil, nil)
