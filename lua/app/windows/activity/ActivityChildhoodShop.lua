-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityChildhoodShop.lua
...
function ActivityChildhoodShop:resizeToParent()
	...
	self:resizePosY(self.bottomBg, -877, -930)
	self:resizePosY(self.exchangeInterface, -372, -402)
	self:resizePosY(self.specialItem, -741, -792)
...
function ActivityChildhoodShop:initUIComponent()
	...
	xyd.setUISpriteAsync(self.logo, nil, "childhood_shop_logo_" .. xyd.Global.lang, nil, nil, true)
...
function ActivityChildhoodShop:updateSpecialItem(isInit)
	...
	if self:checkAllExchangeLineComplete() and limitTime > self.activityData.detail.buy_times[self.specialExchangeID] then
...
function ActivityChildhoodShop:register()
	...
	UIEventListener.Get(self.specialItemBtn).onClick = function()
		...
		if self.specialExchangeCost[2] > xyd.models.backpack:getItemNumByID(self.specialExchangeCost[1]) then
