-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityWeekMission.lua
...
function ActivityWeekMission:initTopGroup()
	xyd.setUISpriteAsync(self.textImg, nil, "activity_week_mission_text_" .. xyd.Global.lang, nil, nil, true)
	...
	for i = 1, #ids do
		...
		UIEventListener.Get(self.giftItemList[i].giftItem).onClick = function()
			...
			if nowPoint >= point and pointAwarded[i] == 0 then
...
function ActivityWeekMissionItem:updateInfo()
	...
	if not self.data.isDiamondMission then
		...
		if self.data.is_awarded == 1 then
			...
			self.buttonLabel.color = Color.New2(4294967295)
		...
		elseif self.data.isCompleted == 1 then
			...
			self.buttonLabel.effectColor = Color.New2(4294967295)
