-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityBeachSummer.lua
...
function ActivityBeachSummer:checkFinger()
	...
	if hasClick and tonumber(hasClick) == 1 then
		-- block empty
	else
...
function ActivityBeachSummerMissionItem:initProgress()
	if self.value >= self.complete_value_ then
...
function ActivityBeachIslandItem:initUI()
	...
	if self.info_.stageNow < self.index_ and self.info_.stageNow > 0 then
		...
		for i = 1, 3 do
			if self["point" .. i] and (self.info_.stageNow > self.index_ or self.info_.stageNow < 0) then
