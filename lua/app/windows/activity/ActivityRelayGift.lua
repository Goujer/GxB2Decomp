-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityRelayGift.lua

local ActivityRelayGift = class("ActivityRelayGift", import(".ActivityContent"))
...
function ActivityRelayGift:ctor(parentGO, params)
	ActivityRelayGift.super.ctor(self, parentGO, params)
end

function ActivityRelayGift:getPrefabPath()
	return "Prefabs/Windows/activity/activity_relay_gift"
end

function ActivityRelayGift:resizeToParent()
	ActivityRelayGift.super.resizeToParent(self)

	local allHeight = self.go:GetComponent(typeof(UIWidget)).height
	local heightDis = allHeight - 874
end

function ActivityRelayGift:initUI()
	self.activityData = xyd.models.activity:getActivity(xyd.ActivityID.ACTIVITY_RELAY_GIFT)
	self.id = xyd.ActivityID.ACTIVITY_RELAY_GIFT
	self.tempGiftImgIndex = 1

	self:getUIComponent()
	ActivityRelayGift.super.initUI(self)
	self:initUIComponent()
	self:register()
end

function ActivityRelayGift:getUIComponent()
	self.trans = self.go
...
function ActivityRelayGift:register()
	self:registerEvent(xyd.event.GET_ACTIVITY_AWARD, function(event)
		...
		if data.activity_id == xyd.ActivityID.ACTIVITY_RELAY_GIFT then
			local awards = xyd.tables.activityRelayGiftTable:getAwards(detail.awarded_id)
			local items = {}
...
function ActivityRelayGift:initUIComponent()
	CountDown.new(self.timeLabel_, {
		duration = self.activityData:getEndTime() - xyd.getServerTime()
	})
	xyd.setUISpriteAsync(self.imgLogo, nil, "activity_relay_gift_logo_" .. xyd.Global.lang)
...
function ActivityRelayGift:initData()
...
function ActivityRelayGiftItem:setInfo(params)
	...
	self.labelTitle.text = __("ACTIVITY_RELAY_GIFT_TEXT03", params.index)

	if self.tableID <= #freeGiftIDs then
		self.isFree = true
		self.awards = xyd.tables.activityRelayGiftTable:getAwards(self.tableID)
		...
		if charge.buy_times >= charge.limit_times then
			self.state = 1
		elseif giftIDs[self.parent.curIndex] ~= self.tableID then
			self.state = 3
		else
	...
	if self.state == 1 then
		...
		xyd.setUISpriteAsync(self.imgGun, nil, "activity_relay_gift_icon_q_1", nil, nil, true)
		xyd.setUISpriteAsync(self.imgGift, nil, "activity_relay_gift_icon_lb_" .. self.parent.tempGiftImgIndex .. "_a", nil, nil, true)
	...
	elseif self.state == 2 then
		function self.parent.playAni2_()
			...
			self.parent.sequence2_:AppendCallback(function()
				xyd.setUISpriteAsync(self.imgGun, nil, "activity_relay_gift_icon_q_2", nil, nil, true)
		...
		xyd.setUISpriteAsync(self.imgGun, nil, "activity_relay_gift_icon_q_1", nil, nil, true)
		xyd.setUISpriteAsync(self.imgGift, nil, "activity_relay_gift_icon_lb_" .. self.parent.tempGiftImgIndex .. "_b", nil, nil, true)
		xyd.applyOrigin(self.btnPurchase:GetComponent(typeof(UISprite)))
		self.label:ApplyOrigin()
		xyd.setTouchEnable(self.btnPurchase, true)
...
function ActivityRelayGiftItem:buyFreeGiftbag()
	xyd.models.activity:reqAwardWithParams(xyd.ActivityID.ACTIVITY_RELAY_GIFT, json.encode({}))
end

return ActivityRelayGift
