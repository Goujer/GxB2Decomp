-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityExchange.lua
...
function ActivityExchangeItem:registerEvent()
	UIEventListener.Get(self.btn).onClick = function()
		local enough1 = xyd.models.backpack:getItemNumByID(self.cost[1][1]) >= self.cost[1][2]
		local enough2 = xyd.models.backpack:getItemNumByID(self.cost[2][1]) >= self.cost[2][2]
		...
		local params = {
			...
			limitNum = self.limit - self.buyTimes,
			eventType = xyd.event.GET_ACTIVITY_AWARD,
			maxCanBuy = math.min(math.floor(xyd.models.backpack:getItemNumByID(self.cost[1][1]) / self.cost[1][2]), math.floor(xyd.models.backpack:getItemNumByID(self.cost[2][1]) / self.cost[2][2])),
...
function ActivityExchangeItem:updateInfo()
	...
	for i = 1, 3 do
		...
		if label then
			...
			if num1 < num2 then
				label.color = Color.New2(4127195391)
			else
				label.color = Color.New2(3414179839)
			end
		end
	end
end
