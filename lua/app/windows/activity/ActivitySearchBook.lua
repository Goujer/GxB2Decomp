-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivitySearchBook.lua
...
function ActivitySearchBook:initUI()
	...
	xyd.setUISpriteAsync(self.logo, nil, "activity_search_book_logo_" .. tostring(xyd.Global.lang), nil, nil, true)
...
function ActivitySearchBook:setItem()
	...
	for _, id in pairs(ids) do
		...
		if self:getPoint(id) >= xyd.tables.activitySearchBookTable:getCompleteValue(id) then
			is_completed = true
		end
...
function ActivitySearchBookItem:updateInfo()
	...
	if self.getWayId_ and tonumber(self.getWayId_) > 0 then
		local function onClick()
			xyd.goWay(self.getWayId_, nil, nil, nil)
		end
