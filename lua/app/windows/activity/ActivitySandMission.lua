-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivitySandMission.lua
...
function ActivitySandMission:initMissionGroup()
	...
	table.sort(paramsNow, function(a, b)
		local aComplete = a.value >= a.completeNum
		local bComplete = b.value >= b.completeNum
...
function ActivitySandMissionItem:createChildren()
	...
	UIEventListener.Get(self.goItem_).onClick = handler(self, function()
		...
		if getWayId > 0 then
			xyd.goWay(getWayId, nil, nil, function()
...
function ActivitySandMissionItem:initIcon()
	...
	for _, v in ipairs(self.items) do
		...
		local fontColor = self.value >= self.completeNum and xyd.FONT_COLOR.LABEL_STYLE1_DARK or xyd.FONT_COLOR.LABEL_STYLE1_LIGHT
