-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityHalloweenPumpkinField.lua
...
function ActivityHalloweenPumpkinField:getUIComponent()
	...
	xyd.setUISpriteAsync(self.textImg, nil, "halloween_name_" .. tostring(xyd.Global.lang), nil, nil, true)
	...
	xyd.setUISpriteAsync(self.textImg2, nil, "halloween_name_" .. tostring(xyd.Global.lang), nil, nil, true)
...
function ActivityHalloweenPumpkinField:updateTenItem(isPlayEffect, dataValue)
	for i = 1, 10 do
		...
		if not self.tenItemIDs[i] or self.tenItemIDs[i] and self.tenItemIDs[i] ~= awardsId or awardsId == 0 then
			...
			xyd.setUISpriteAsync(self["pumpkinBtn" .. i], nil, imgName, nil, nil, true)
		...
		if isPlayEffect and isPlayEffect == true then
			if not self.tenItemIDs[i] and awardsId ~= 0 or awardsId ~= 0 and self.tenItemIDs[i] and self.tenItemIDs[i] ~= awardsId then
				...
				self:playEffect(function()
					xyd.itemFloat(dataValue.items, nil, nil, 6500)
					...
					xyd.setUISpriteAsync(self["pumpkinBtn" .. i], nil, "halloween_pumpkin_cut", nil, nil, true)
