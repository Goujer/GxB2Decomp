-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityJackPotMachine.lua
...
function ActivityJackPotMachine:updateRedPoint()
	...
	for i = 1, #freeIDs do
		if xyd.tables.activityJackpotExchangeTable:getLimit(freeIDs[i]) > self.activityData.detail.buy_times[freeIDs[i]] and not self.activityData.giftbagRed then
...
function ActivityJackPotMachine:register()
	...
	UIEventListener.Get(self.btnAward.gameObject).onClick = function()
		...
		for i = 1, #ids do
			...
			info.id = ids[i]
			info.max_value = xyd.tables.activityJackpotAwardTable:getComplete(ids[i])
	...
	UIEventListener.Get(self.btnStart.gameObject).onClick = function()
		if self.type == MACHINE_TYPE.SENIOR and self.activityData.detail.charges[self.privilegeCardGiftIndex].buy_times == 0 and xyd.tables.miscTable:getNumber("activity_jackpot_unlock", "value") > self.activityData.detail.jackpot_times then
