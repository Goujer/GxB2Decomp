-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityEquipLevelUp.lua
...
function ActivityEquipLevelUp:initContent_1()
	xyd.setUISpriteAsync(self.textImg, nil, "equip_level_up_logo_" .. xyd.Global.lang, nil, nil, true)
	...
	if self.activityData:getEndTime() > xyd.getServerTime() then
		if xyd.Global.lang == "fr_fr" then
			...
			self.leftTime.effectColor = Color.New2(4294967295)
			self.endLabel.color = Color.New2(2986279167)
...
function ActivityEquipLevelUp:changeStoneNum()
	...
	if self.chosenGroup == 0 or self.chosenStar == 0 then
		...
		if costNum < tonumber(cost[1][2]) then
			self.labelNum.color = Color.New2(3422556671)
			self.labelNum.effectColor = Color.New2(4294967295)
		else
			self.labelNum.color = Color.New2(4294967295)
...
function ActivityEquipLevelUp:canLevelUp()
	...
	for i = 1, #suit do
		...
		if cost > xyd.models.backpack:getItemNumByID(data[1]) then
			return 1
		end
	end
...
function ActivityEquipLevelUp:getBaseItemInfo()
	...
	for i = 1, #self.base_equip_info do
		...
		for j = 1, 4 do
			...
			tempdata2.itemID = self.base_equip_info[i][j]
...
function ActivityEquipLevelUp:changeItemNum(label, hasNum, needNum)
	...
	if hasNum < needNum then
		label.color = Color.New2(3422556671)
...
function ActivityEquipLevelUp:updateContent()
	...
	table.sort(self.idx, function(a, b)
		return a.leftTime == b.leftTime and a.id < b.id or a.leftTime > b.leftTime
	end)
...
function ActivityEquipLevelUp:updateCostGroupOfExchange()
	...
	if self.selectedSuit_exchange_base ~= 0 and self.selectedSuit_exchange_base ~= nil then
		...
		if hasNum < costNum then
			self.labelNum_exchange.color = Color.New2(3422556671)
			self.labelNum_exchange.effectColor = Color.New2(4294967295)
		else
...
function ActivityEquipLevelUp:updateBasetGroupOfExchange()
	...
	for i = 1, 4 do
		...
		if hasNum < needNum then
			self.baseItemList_exchange[i].labelNum_.color = Color.New2(3422556671)
			self.baseItemList_exchange[i].labelNum_.effectStyle = UILabel.Effect.Outline
			self.baseItemList_exchange[i].labelNum_.effectColor = Color.New2(4294967295)
		else
			self.baseItemList_exchange[i].labelNum_.color = Color.New2(4294967295)
			self.baseItemList_exchange[i].labelNum_.effectStyle = UILabel.Effect.Outline
...
function ActivityEquipLevelUpItem:initUI()
	...
	for i = 1, #beforeData do
		...
		if hasNum < data[2] then
			icon.labelNum_.color = Color.New2(3422556671)
			icon.labelNum_.effectStyle = UILabel.Effect.Outline
			icon.labelNum_.effectColor = Color.New2(4294967295)
		else
			icon.labelNum_.color = Color.New2(4294967295)
		end
	...
	for i = 1, #costData do
		...
		self["labelCost" .. i].color = data[2] <= xyd.models.backpack:getItemNumByID(data[1]) and Color.New2(960513791) or Color.New2(3422556671)
	end
