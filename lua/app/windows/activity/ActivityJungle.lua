-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityJungle.lua
...
function ActivityJungle:initShopData()
	...
	for i = 1, #ids do
		local id = i
		local isCompleted = buyTimes[i] >= xyd.tables.ActivityJungleShopTable:getLimit(id)
...
function ActivityJungle:initExploreData()
	for i = 1, 3 do
		...
		for j = 1, #ids do
			...
			if area == i then
				if point > self["maxPointArea" .. i] then
...
function ActivityJungle:updateContent(index)
	...
	if index == 1 then
		self.navLabel1.color = Color.New2(47244640255)
		self.navLabel1.effectColor = Color.New2(1012112383)
		self.navLabel2.color = Color.New2(960513791)
		self.navLabel2.effectColor = Color.New2(4294967295)
		...
		self.navLabel1.effectColor = Color.New2(4294967295)
		self.navLabel2.color = Color.New2(47244640255)
		...
		UIEventListener.Get(self["processBtn" .. i]).onClick = function()
			...
			for j in pairs(ids) do
				if xyd.tables.ActivityJungleAwardsTable:getArea(j) == i then
					...
					data.id = j
					data.max_value = xyd.tables.ActivityJungleAwardsTable:getPoint(j)
...
function ActivityJungleItem:updateInfo()
	...
	for i = 1, #self.cost do
		...
		if xyd.models.backpack:getItemNumByID(data[1]) < data[2] then
			groupCost:ComponentByName("costLabel", typeof(UILabel)).color = Color.New2(4280030207)
		else
...
function ActivityJungleItem:onExchange()
	...
	if select_max_num > Mathf.Floor(costItemNum / select_multiple) then
