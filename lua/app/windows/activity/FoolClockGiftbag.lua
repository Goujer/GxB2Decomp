-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/FoolClockGiftbag.lua
...
function FoolClockGiftbag:initUIComponent()
	xyd.setUISpriteAsync(self.textImg, nil, "fool_clock_giftbag_text_" .. xyd.Global.lang, nil, nil, true)
	...
	for i = #giftbagIds, 1, -1 do
		...
		if giftbagData and next(giftbagData) then
			...
			if giftbagCharge.buy_times >= giftbagCharge.limit_times then
...
function FoolClockGiftbagItem:initUI()
	...
	if self.itemType == 1 then
		...
		self.purchaseBtnLabel.color = Color.New2(4294967295)
...
function FoolClockGiftbagItem:setInfo(params)
	...
	if self.buyTimes >= self.limitTImes then
		isBuyout = true
...
function FoolClockGiftbagItem:register()
	...
	UIEventListener.Get(self.purchaseBtn).onClick = handler(self, function()
		...
		elseif self.chooseIndex == 0 then
