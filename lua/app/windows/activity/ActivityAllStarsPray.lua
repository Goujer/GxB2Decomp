-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityAllStarsPray.lua
...
function ActivityAllStarsPray:updateGroupProgress()
	...
	canGetAward = xyd.models.backpack:getItemNumByID(self.groupFragmentID) >= self.activityData.needNumsOfGroupFragment[self.nowSelectGroup]
...
function ActivityAllStarsPray:onUsePrayItem(event)
	...
	dump(otherItems, "a")
	xyd.itemFloat(otherItems, nil, nil, 6003)

	if omniFragment ~= nil then
		if xyd.models.backpack:getItemNumByID(self.omniFragmentID) >= self.activityData.needOmniFragmentNum and self.activityData.noShowTipWindow == nil then
			local params = {
				jumpToMode = true,
				showDesc = true,
				items = {
...
function ActivityAllStarsPray:initBtns()
	...
	xyd.setUISpriteAsync(self.useWishCoinBtn, nil, "blue_btn70_70", nil, nil)
	...
	xyd.setUISpriteAsync(self.useGemBtn, nil, "blue_btn70_70", nil, nil)
	...
	UIEventListener.Get(self.useGemBtn.gameObject).onClick = handler(self, function()
		...
		if xyd.models.backpack:getItemNumByID(self.activityData.groupFragmentIDs[self.nowSelectGroup]) >= self.activityData.needNumsOfGroupFragment[self.nowSelectGroup] then
	...
	UIEventListener.Get(self.taskAwardBtn).onClick = handler(self, function()
		...
		for j in pairs(ids) do
			...
			data.id = j
			data.max_value = xyd.tables.activityPrayAwardTable:getComplete(j)
...
function ActivityAllStarsPray:updateNowPage(page)
	...
	if self.nowSelectGroup == 0 then
		...
		canGetAward = self.activityData.detail.benches[self.nowSelectGroup].point >= self.activityData.needNumsOfGroupFragment[self.nowSelectGroup]
...
function ActivityAllStarsPrayGroupItem:updateMask()
	...
	canGetAward = xyd.models.backpack:getItemNumByID(self.parent.activityData.groupFragmentIDs[self.groupId_]) >= self.parent.activityData.needNumsOfGroupFragment[self.groupId_]
