-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityChristmasExchangeDummy.lua
...
function ExchangeItemClass:confirmCallback(optionalList, materialList)
	...
	if #self.materialList >= 1 then
		...
		self.inputNumLabel_.color = Color.New2(3360838399)
...
function ExchangeItemClass:layout()
	if self.cost[2] > xyd.models.backpack:getItemNumByID(xyd.ItemID.DUMMY_BUTTERFLY) then
		self.costButterNum_.color = Color.New2(3360838399)
...
function BuyItemClass:onClickBuy()
	...
	elseif not isBatch or isBatch ~= 1 then
		...
		local params = {
			...
			limitNum = math.floor(xyd.models.backpack:getItemNumByID(self.cost[1]) / self.cost[2]),
