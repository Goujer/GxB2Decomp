-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityExploreOldCampusPVE.lua
...
function ActivityExploreOldCampusPVE:getUIComponent()
	...
	xyd.setUISpriteAsync(self.textLabelImg, nil, "old_building_desc_bg", nil, nil)
	xyd.setUISpriteAsync(self.imgBg1, nil, "old_building_score_bg", nil, nil)
	...
	self:registerEvent(xyd.event.OLD_BUILDING_ACTIVITY_SET_TEAMS, function(event)
		...
		for i in pairs(infoItems) do
			if infoItems[i].data and infoItems[i].data.floorId == data.floor_id and infoItems[i].data.floorId == self.activityData:getDefUpdataInfo().floor_id then
				...
				infoItems[i]:update(nil, nil, infoItems[i].data)
...
function ActivityExploreOldCampusPVE:checkFloorLock(backFloorId)
	...
	if floorIndex > self.activityData.detail.area_infos[self.area_id].max_index then
...
function CampusItem:ctor(goItem, areaId, parent)
	...
	xyd.setUISpriteAsync(self.imgBg_, nil, "old_building_area_bg_" .. self.areaId, nil, nil)
	xyd.setUISpriteAsync(self.locakImg_, nil, "lock", nil, nil)
...
function SecondSmallItem:ctor(goItem, levelId, parent, cur_score, levelIndex)
	...
	xyd.setUISpriteAsync(self.imgBg, nil, "9gongge16", nil, nil)
