-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityArtifactExchange.lua
...
function ActivityArtifactExchange:initUIComponent()
	xyd.setUISpriteAsync(self.imgText, nil, "activity_artifact_exchange_text_" .. xyd.Global.lang, nil, nil, true)
...
function ActivityArtifactExchange:updateContent()
	...
	for i = 1, self.navNum do
		...
		if i == self.curNav then
			xyd.setUISpriteAsync(self.navItems[i], nil, "market_dot_bg2", nil, nil, true)

			self.navItems[i].width = 20
			self.navItems[i].height = 20
		else
			xyd.setUISpriteAsync(self.navItems[i], nil, "emotbtn1", nil, nil, true)
