-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivitySandSearch.lua
...
function SandItem:setInfo(index, card_id)
	...
	if not card_id or card_id <= 0 then
		xyd.setUISpriteAsync(self.itemImg, nil, "activity_sand_search_item_icon1", nil, nil, true)
...
function SandItem:playTypeAni(card_id, total_ids, stepNum, from_type)
	...
	if type == 1 then
		self.effectNode2:SetActive(true)
		xyd.setUISpriteAsync(self.typeImg, nil, "activity_sand_search_type_" .. card_id, nil, nil, true)
...
function ActivitySandSearch:playCardAni(total_ids, callback, mapInfo)
	...
	if stage_id > self.stage_id then
...
function ActivitySandSearch:initProgress()
	...
	for i = 3, 1, -1 do
		...
		if point >= pointStage[i] then
			...
			self["awardLabel" .. i].color = Color.New2(2956140543)

			self["awardSelect" .. i]:SetActive(false)

			if pointStage[i - 1] and point >= pointStage[i - 1] then
