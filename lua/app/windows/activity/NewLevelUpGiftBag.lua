-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/NewLevelUpGiftBag.lua
...
function NewLevelUpGiftBag:initBtn()
	...
	if self.activityData.detail[self.type].charge.buy_times >= xyd.tables.giftBagTable:getBuyLimit(self.giftBagID) or self:getUpdateTime() <= xyd.getServerTime() then
...
function NewLevelUpGiftBag:setText()
	...
	xyd.setUISpriteAsync(self.imgText01, nil, "level_up_giftbag_text01_" .. tostring(xyd.Global.lang), nil, nil, true)
end
...
function NewLevelUpGiftBag:setBtnState(can)
	if not can then
		...
		self.buyBtnLabel.color = Color.New2(2975269119)
...
function NewLevelUpGiftBag:onRecharge(event)
	...
	if self.activityData.detail[self.type].charge.buy_times >= xyd.tables.giftBagTable:getBuyLimit(self.giftBagID) then
		self:setBtnState(false)
	end
end
