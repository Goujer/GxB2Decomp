-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/LevelFund.lua
...
function LevelFund:layout()
	...
	xyd.setUISpriteAsync(self.imgTitle_, nil, "level_fund_text_" .. tostring(xyd.Global.lang), nil, nil, true)
	xyd.setUISpriteAsync(self.imgBg_, nil, "level_fund_bg", nil, nil, true)

	self.labelVip_.text = "+" .. tostring(tostring(xyd.tables.giftBagTable:getVipExp(self.giftBagId_))) .. " VIP EXP"

	if self.activityData.detail.charges[1].buy_times >= self.activityData.detail.charges[1].limit_times then
