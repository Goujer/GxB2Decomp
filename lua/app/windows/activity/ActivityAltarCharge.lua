-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityAltarCharge.lua
...
function BottleItem:layout()
	...
	xyd.setUISpriteAsync(self.bottleItemImg, nil, "star_bottle_big_" .. self.index_ .. "_1", nil, nil, true)
	xyd.setUISpriteAsync(self.bottleItemValueImg, nil, "star_bottle_big_" .. self.index_ .. "_2", nil, nil, true)
	xyd.setUISpriteAsync(self.bottleItemUpImg, nil, "star_bottle_big_" .. self.index_ .. "_3", nil, nil, true)
	...
	elseif nowStage > self.index_ then
		...
		elseif progressValue >= 0.25 and progressValue < 0.5 then
			...
			xyd.setUISpriteAsync(self.bottleItemShowImg, nil, "star_bottle_big_" .. self.index_ .. "_4", nil, nil, true)
		...
		elseif progressValue >= 0.5 and progressValue < 1 then
			...
			self.bottleItemShowImg.gameObject:SetActive(true)
			xyd.setUISpriteAsync(self.bottleItemShowImg, nil, "star_bottle_big_" .. self.index_ .. "_5", nil, nil, true)
...
function ActivityAltarCharge:layout()
	...
	for i = 1, 4 do
		if i < nowStage then
			xyd.setUISpriteAsync(self["stageLine" .. i], nil, "activity_star_altar_charge_xyp_line2", nil, nil, true)
		else
			xyd.setUISpriteAsync(self["stageLine" .. i], nil, "activity_star_altar_charge_xyp_line1", nil, nil, true)
		end
	end
end
...
function ActivityAltarCharge:updateAwardList()
	...
	for index, award in ipairs(awards) do
		...
		self.awardItemList_[index]:setChoose(nowStage > self.cur_center_ or (value - preNum) / (nowNum - preNum) >= 1 and nowStage == self.cur_center_)
	end
