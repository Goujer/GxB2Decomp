-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/MonthBeginningGiftBag.lua
...
function MonthBeginningGiftBag:setTextures()
	...
	xyd.setUISpriteAsync(self.imgText02, nil, "activity_month_gift_time_bg", nil, nil)
...
function MonthBeginningGiftBag:initTime()
	if self.activityData:getUpdateTime() > xyd.getServerTime() then
		if xyd.Global.lang == "fr_fr" then
			self.labelTime.color = Color.New2(4294901503)
			self.labelText01.color = Color.New2(2986279167)
...
function MonthBeginningGiftBag:getItemInfos()
	...
	for i in pairs(crystalCharge) do
		...
		if crystalCharge[i] >= xyd.tables.activityMonthGiftTable:getLimit(i) then
...
function MonthBeginningGiftBag:onFree(event)
	...
	xyd.itemFloat(items, nil, nil, self.scroller_Panel.depth + 1)
