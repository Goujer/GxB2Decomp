-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityLafuliCastle.lua
...
function ActivityLafuliCastle:initText()
	xyd.setUISpriteAsync(self.imgText, nil, "activity_lafuli_castle_text_" .. xyd.Global.lang, nil, nil, true)
...
function ActivityLafuliCastle:updateBackground()
	...
	for i = 1, #ids do
		...
		if point > self.activityData.detail.points[type] then
			self[object]:SetActive(false)
		elseif not self.notFirstInit then
			self[object]:SetActive(true)

			self["curStage" .. type] = i
		elseif not self["curStage" .. type] or i > self["curStage" .. type] then
			self["curStage" .. type] = i
...
function ActivityLafuliCastle:updateTasks()
	...
	self.taskNumofType1, self.taskNumofType2 = 0, 0
	self.curTaskIDofType1, self.curTaskIDofType2 = 0, 0
...
function ActivityLafuliCastle:updatePartnerAward()
	...
	if self.activityData.detail.energy >= partnerAwardEnergy then
