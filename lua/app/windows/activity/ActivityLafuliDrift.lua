-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityLafuliDrift.lua
...
function ActivityLafuliDrift:layout()
	...
	self.autoLabel.effectColor = Color.New2(4294967295)
	...
	for i = 1, #ids do
		...
		elseif type == 3 then
			...
			if xyd.tables.activityLafuliDriftTable:getBgID(id) and xyd.tables.activityLafuliDriftTable:getBgID(id) == 4 then
				...
				xyd.setUISpriteAsync(bg2, nil, "lafuli_drift_bg" .. xyd.tables.activityLafuliDriftTable:getBgID(id), nil, nil, true)
				...
				xyd.setUISpriteAsync(bg, nil, xyd.tables.activityLafuliDriftTable:getIcon(id), nil, nil, true)
				bg:Y(12)
			else
...
function ActivityLafuliDrift:moveResult()
	...
	elseif type == 5 then
		...
		self.modelEffect:play("cry", 0, 1)
		xyd.alertTips(__("ACTIVITY_LAFULI_DRIFT_WATER"), nil, nil, nil, nil, nil, -10)
	end
...
function ActivityLafuliDrift:registEvent()
	...
	for i = 1, #self.items do
		UIEventListener.Get(self.items[i]).onClick = handler(self, function()
			...
			elseif award[1] == xyd.ItemID.CORAL_BRANCH then
				NGUITools.DestroyChildren(self.tipsIcon.transform)
				xyd.setUISpriteAsync(self.tipsIcon, nil, "activity_lafuli__sh" .. self.lvs[ids[i]], nil, nil, true)
			else
				NGUITools.DestroyChildren(self.tipsIcon.transform)
				...
				elseif iconName == "lafuli_drift_slow" then
					...
					xyd.setUISpriteAsync(self.tipsIcon, nil, iconName, nil, nil, true)
				end
			end
	...
	UIEventListener.Get(self.selectBtn).onClick = handler(self, function()
		...
		if self.isDouble and (self.select1 <= 0 or self.select2 <= 0) then
			-- block empty
		elseif not self.isDouble and self.select <= 0 then
			-- block empty
		else
			self.onGoing = true
...
function ActivityLafuliDrift:startAutoPlay(num)
	...
	self.autoLabel.color = Color.New2(4278124287)
	self.autoLabel.effectColor = Color.New2(2604482047)
