-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityTreasure.lua
...
function ActivityTreasure:initUIComponent()
	xyd.setUISpriteAsync(self.logoImg, nil, "activity_treasure_" .. xyd.Global.lang, nil, nil, nil)
	...
	table.sort(up_all_ids, function(a, b)
		if nums[a] >= xyd.tables.activity3BirthdayGambleTable:getNum(a) and nums[b] >= xyd.tables.activity3BirthdayGambleTable:getNum(b) then
			return a < b
		else
			if nums[a] >= xyd.tables.activity3BirthdayGambleTable:getNum(a) then
				return false
			end

			if nums[b] >= xyd.tables.activity3BirthdayGambleTable:getNum(b) then
				return true
			else
				return a < b
			end
	...
	for i = 1, math.ceil(#up_all_ids / 5) do
		...
		for j = for_left, for_right do
			...
			small_data.id = up_all_ids[j]
			small_data.items = xyd.tables.activity3BirthdayGambleTable:getAwards(up_all_ids[j])
			small_data.num = xyd.tables.activity3BirthdayGambleTable:getNum(up_all_ids[j])
	...
	for i = 1, math.ceil(#down_all_ids / 5) do
		...
		for j = for_left, for_right do
			...
			small_data.id = down_all_ids[j]
			small_data.items = xyd.tables.activity3BirthdayGambleTable:getAwards(down_all_ids[j])
			small_data.num = xyd.tables.activity3BirthdayGambleTable:getNum(down_all_ids[j])
...
function ActivityTreasure:openShowAllWindow(type)
	...
	for j = 1, #all_ids do
		...
		small_data.items = xyd.tables.activity3BirthdayGambleTable:getAwards(all_ids[j])
		small_data.num = xyd.tables.activity3BirthdayGambleTable:getNum(all_ids[j])
...
function LongItem:initUI()
	...
	elseif self.type == 2 then
		...
		xyd.setUISpriteAsync(self.imgBg, nil, "activity_treasure_bg3", nil, nil, nil)
	end
...
function SmallItem:initInfo()
	...
	if self.info.cur_num == 0 then
		self.hasNumLabel.color = Color.New2(3422556671)
	end
end
