-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/StageGiftBag.lua
...
function StageGiftBag:euiComplete()
	...
	if self.activityData.detail.charge.buy_times >= self.limit or self.activityData:getUpdateTime() <= xyd:getServerTime() then
...
function StageGiftBag:onRecharge(evt)
	...
	if self.activityData.detail.charge.buy_times >= GiftBagTable:get():getBuyLimit(self.activityData.detail.charge.table_id) then
		self:setBtnState(false)
	end
end
...
function StageGiftBag:returnCommonScreen()
	...
	local ____TS_obj, ____TS_index = self.contentGroup, "y"
...
function NewStageGiftBag:onRecharge(evt)
	...
	if self.activityData.detail[self.type].charge.buy_times >= GiftBagTable:get():getBuyLimit(self.activityData.detail[self.type].charge.table_id) then
		self:setBtnState(false)
	end
end
