-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/NewYearNewSignIn.lua
...
function NewYearNewSignIn:onGetAward(event)
	...
	if awardid ~= -1 then
		...
		xyd.itemFloat(items, nil, nil, 6500)
	end
end
...
function NewYearNewSignInItem:updateLayout(type)
	if type and type == "award_back" then
		if self.id_ > self.parent.activityData.detail.day then
			return -1
		end
