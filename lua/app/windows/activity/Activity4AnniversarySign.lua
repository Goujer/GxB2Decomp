-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/Activity4AnniversarySign.lua
...
function Activity4AnniversarySign:initData()
	...
	for i, giftbagID in ipairs(self.giftbagIDs) do
		...
		if self.signTimes >= self.giftbagOpenLimit[giftbagID] then
			self.curTab = i
		end
	end
end
...
function Activity4AnniversarySign:initUIComponent()
	xyd.setUISpriteAsync(self.logo, nil, "activity_4anniversary_sign_text_" .. xyd.Global.lang, nil, nil, true)
...
function Activity4AnniversarySign:updateGiftbag()
	for i = 1, 4 do
		if self.signTimes >= self.giftbagOpenLimit[self.giftbagIDs[i]] then
		...
		if i == self.curTab then
			xyd.setUISpriteAsync(self["tabBg" .. i], nil, "activity_4anniversary_sign_tab_chosen", nil, nil, true)

			self["tabLabel" .. i].color = Color.New2(2642897407)
		else
			xyd.setUISpriteAsync(self["tabBg" .. i], nil, "activity_4anniversary_sign_tab_unchosen", nil, nil, true)

			self["tabLabel" .. i].color = Color.New2(4126272511)
		end
	end
...
function Activity4AnniversarySignItem:ctor(go, parent, id)
	...
	elseif self.id == self.parent.day then
		...
		self.labelSign.color = Color.New2(3947917311)
	...
	UIEventListener.Get(self.go).onClick = function()
		if self.awarded == 1 or self.id > self.parent.day then
			return
		end
