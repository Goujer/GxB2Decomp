-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityEaster2022.lua
		...
		xyd.openWindow("common_activity_task_window", {
			...
			clickBgCallback = function(id)
				...
				if wayID and wayID > 0 then
					xyd.goWay(wayID, nil, nil, function()
...
function ActivityEaster2022:playRoundAnimation1(row, cols, playTime, callback)
	...
	for j = 1, 6 do
		...
		if needTime > 0 then
			...
			if needTime > self.doingMaxTimeRow then
	...
	if type == 1 then
		...
		for i = 1, 4 do
			...
			if self.specialPosOffset[i] == 0 then
				...
				elseif newSpecialPosArr[i] - 1 >= self.tempRecord[i] then
