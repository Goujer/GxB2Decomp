-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityRepairGiftBag.lua
...
function FundItem:setBtn()
	...
	if self.info.isCrystal ~= nil then
		...
		xyd.setUISpriteAsync(self.btnPurchaseImg, nil, "activity_repair_giftbag_btn_l", nil, nil)
	...
	if self.info.isFree == nil and self.info.isCrystal == nil then
		xyd.setUISpriteAsync(self.btnPurchaseImg, nil, "activity_repair_giftbag_btn_h", nil, nil)
...
function ActivityRepairGiftBag:setTextures()
	xyd.setUISpriteAsync(self.logo_, nil, "activity_repair_giftbag_logo_" .. xyd.Global.lang, nil, nil, true)
...
function ActivityRepairGiftBag:getItemInfos()
	...
	local isCrystalCan = limit > tonumber(data)
	local item1 = {
		isShowVipExp = false,
		isCrystal = true,
