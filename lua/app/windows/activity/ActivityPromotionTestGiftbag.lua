-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityPromotionTestGiftbag.lua

local ActivityContent = import(".ActivityContent")
local ActivityPromotionTestGiftbag = class("ActivityPromotionTestGiftbag", ActivityContent)
local ActivityPromotionTestGiftbagItem = class("ActivityPromotionTestGiftbagItem", import("app.components.CopyComponent"))
local FixedWrapContent = import("app.common.ui.FixedWrapContent")
local CountDown = import("app.components.CountDown")

function ActivityPromotionTestGiftbag:ctor(parentGO, params)
	ActivityContent.ctor(self, parentGO, params)
end

function ActivityPromotionTestGiftbag:getPrefabPath()
	return "Prefabs/Windows/activity/activity_promotion_test_giftbag"
end

function ActivityPromotionTestGiftbag:initUI()
	self:getUIComponent()
	ActivityPromotionTestGiftbag.super.initUI(self)
...
function ActivityPromotionTestGiftbag:getUIComponent()
	local go = self.go
...
function ActivityPromotionTestGiftbag:initWrapContent()
	self.wrapContent = FixedWrapContent.new(self.scroller, self.wrapContentCon, self.common_giftbag_item, ActivityPromotionTestGiftbagItem, self)
end

function ActivityPromotionTestGiftbag:initUIComponent()
	self:setText()
	self:setItems()
	self:setTextures()
	self:initTime()
...
function ActivityPromotionTestGiftbag:initTime()
...
function ActivityPromotionTestGiftbag:timeOver()
	self.labelTime.text = "00:00:00"
end

function ActivityPromotionTestGiftbag:onRegisterEvent()
	self:registerEvent(xyd.event.RECHARGE, handler(self, self.onRecharge))
end

function ActivityPromotionTestGiftbag:setTextures()
	if xyd.Global.lang ~= "en_en" then
		xyd.setUISpriteAsync(self.imgText01, nil, "value_giftbag_text01_" .. xyd.Global.lang, nil, nil, true)
	end

	xyd.setUISpriteAsync(self.imgText02, nil, "value_giftbag_text02_" .. xyd.Global.lang, nil, nil, true)
	xyd.setUISpriteAsync(self.imgBg, nil, "value_giftbag_bg01", nil, nil, true)
...
function ActivityPromotionTestGiftbag:setText()
	self.labelText01.text = __("TEXT_END")
end

function ActivityPromotionTestGiftbag:getItemInfos()
	local cantBuy = {}
	local t = {}
	...
	for i = 1, #datas do
		...
		if data.buy_times >= xyd.tables.giftBagTable:getBuyLimit(id) then
...
function ActivityPromotionTestGiftbag:setItems()
...
function ActivityPromotionTestGiftbag:onRecharge(event)
...
function ActivityPromotionTestGiftbag:updateFreeList(data)
...
function ActivityPromotionTestGiftbag:updateCrystalList(data)
	local giftBagID = data.giftbag_id
	local t = self:getItemInfos()
	local isGoOn = false
...
function ActivityPromotionTestGiftbag:getScrollView()
	return self.scroller
end

function ActivityPromotionTestGiftbagItem:ctor(go, parent)
	self.go = go
...
function ActivityPromotionTestGiftbagItem:getGiftBagID()
	return self.giftBagID
end

function ActivityPromotionTestGiftbagItem:update(index, info)
	if not info then
...
function ActivityPromotionTestGiftbagItem:getUIComponent()
	local go = self.go
...
function ActivityPromotionTestGiftbagItem:initUIComponent()
...
function ActivityPromotionTestGiftbagItem:setIcon()
...
function ActivityPromotionTestGiftbagItem:setBtn()
	...
	if self.info.isCrystal ~= nil then
		...
		xyd.setUISpriteAsync(self.prop_icon, nil, "icon_" .. self.info.cost[1], nil, nil)
	...
	if self.info.isFree == nil and self.info.isCrystal == nil then
		...
		xyd.setDarkenBtnBehavior(self.btnPurchase, self, function()
			local specialGiftbagIDs = xyd.tables.miscTable:split2Cost("activity_promotion_test_giftbag", "value", "|")
			local tipFlag = false

			print(self.info.giftBagID)
			print(specialGiftbagIDs[2])

			if self.info.giftBagID == specialGiftbagIDs[2] then
				local charges = self.parent.activityData.detail.charges

				for i = 1, #charges do
					if charges[i].table_id == specialGiftbagIDs[1] and charges[i].buy_times < xyd.tables.giftBagTable:getBuyLimit(charges[i].table_id) then
						tipFlag = true
					end
				end
			end
			...
			if tipFlag then
				xyd.alert(xyd.AlertType.YES_NO, __("ACTIVITY_PROMOTION_TEST_TEXT05"), function(yes)
					if yes then
		...
		if self.data.buy_times >= xyd.tables.giftBagTable:getBuyLimit(self.giftBagID) then
			self.btnPurchase:GetComponent(typeof(UnityEngine.BoxCollider)).enabled = false
...
function ActivityPromotionTestGiftbagItem:updateFreeBtn()
...
function ActivityPromotionTestGiftbagItem:updateCrystalBtn()
	if self ~= nil and self.info ~= nil and self.info.isCrystal ~= nil and self.btnPurchase then
		...
		xyd.itemFloat(items, nil, nil, self.info.parentScroller:GetComponent(typeof(UIPanel)).depth + 1)

		if tonumber(self.info.data) >= tonumber(self.info.limit) then
			self.btnPurchase:GetComponent(typeof(UnityEngine.BoxCollider)).enabled = false
...
return ActivityPromotionTestGiftbag
