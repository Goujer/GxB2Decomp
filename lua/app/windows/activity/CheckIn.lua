-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/CheckIn.lua
...
function CheckIn:setVip()
	...
	if xyd.tables.vipTable:judgeLoginDouble(vip) then
		...
		while #nums > 0 do
			...
			xyd.setUISpriteAsync(sprite, nil, "player_vip_num_" .. num, nil, nil, true)
		end
	else
...
function CheckInPopUpItem:ctor(go, checkIn, params)
	...
	if self.realDay > self.gotDays and self.realDay <= self.onlineDays then
		self.canGet = true
	else
		self.canGet = false
	end
