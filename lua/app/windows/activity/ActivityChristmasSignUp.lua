-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityChristmasSignUp.lua
...
function ActivityChristmasSignUp:initUI()
	self.fg3_imgs = {
		[12] = "activity_christmas_sign_in_item_fg3_2",
		[6] = "activity_christmas_sign_in_item_fg3_1",
		[18] = "activity_christmas_sign_in_item_fg3_3",
		[24] = "activity_christmas_sign_in_item_fg3_4"
	}
...
function ActivityChristmasSignUp:initUIComponent()
	xyd.setUISpriteAsync(self.titleImg, nil, "activity_christmas_sign_in_logo_" .. xyd.Global.lang, nil, nil, true)
...
function ActivityChristmasSignUp:initData()
	...
	if self.datas[self.curDateID].type >= 1 then
		xyd.setUISpriteAsync(self.curDateItemFg1, nil, "activity_christmas_sign_in_item_fg1_" .. self.datas[self.curDateID].id % 2, nil, nil, true)
	end

	if self.datas[self.curDateID].type >= 2 then
		xyd.setUISpriteAsync(self.curDateItemFg2, nil, "activity_christmas_sign_in_item_fg2_" .. self.datas[self.curDateID].id % 2, nil, nil, true)
	end

	if self.datas[self.curDateID].type >= 3 then
		xyd.setUISpriteAsync(self.curDateItemFg3, nil, self.fg3_imgs[self.curDateID], nil, nil, true)
	end
	...
	if self.datas[self.curDateID].award == nil then
		...
		UIEventListener.Get(self.curDateItemClickMask).onClick = function()
			...
			if self.activityData:getCurDateID() > self.curDateID then
...
function ActivityChristmasSignUp:initGiftbag()
	...
	if self.activityData.detail.charges[self.activityData:getGiftBagIndex(self.giftBagID)].buy_times >= self.activityData.detail.charges[self.activityData:getGiftBagIndex(self.giftBagID)].limit_times then
...
function ActivityChristmasSignUpItem:setInfo(params)
	...
	if self.type >= 1 then
		xyd.setUISpriteAsync(self.fg1, nil, "activity_christmas_sign_in_item_fg1_" .. self.id % 2, nil, nil, true)
	end

	if self.type >= 2 then
		xyd.setUISpriteAsync(self.fg2, nil, "activity_christmas_sign_in_item_fg2_" .. self.id % 2, nil, nil, true)
	end

	if self.type >= 3 then
		xyd.setUISpriteAsync(self.fg3, nil, self.fg3_imgs[self.id], nil, nil, true)
	end
	...
	if self.id == self.curDateID then
		UIEventListener.Get(self.clickMask).onClick = function()
			if xyd.getServerTime() >= self.parent.activityData:getEndTime() then
				xyd.alertTips(__("ACTIVITY_END_YET"))

				return
			end

			if self.parent.activityData:getCurDateID() > self.curDateID then
				xyd.alertTips(__("ACTIVITY_CHRISTMAS_SIGN_UP_TEXT02"))
				self:initData()
