-- chunkname: @../../../Product/Bundles/Android/src/app/windows/activity/ActivityBlindBoxMission.lua
...
function ActivityBlindBoxMission:setTexture()
	xyd.setUISpriteAsync(self.logo, nil, "activity_blind_box_mission_logo_" .. xyd.Global.lang, nil, nil, true)
end
...
function FundItem:initItem(itemInfo)
	...
	if self.progressBar.value == 1 then
		xyd.setUISpriteAsync(self.progressImg, nil, "jindu_xinxi_1", nil, nil, true)

		local Color1 = Color.New2(75665919)

		self.progressDesc.effectColor = Color1
	else
		xyd.setUISpriteAsync(self.progressImg, nil, "jindu_xinxi_2", nil, nil, true)

		local Color1 = Color.New2(3077511935)
	...
	if itemInfo.jumpWay and tonumber(itemInfo.jumpWay) > 0 then
		local function onClick()
			xyd.goWay(itemInfo.jumpWay, nil, nil, function()
				return
			end)
		end
