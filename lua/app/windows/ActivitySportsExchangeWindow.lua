-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivitySportsExchangeWindow.lua
...
function ActivitySportsExchangeWindow:onGetAward(event)
	...
	xyd.itemFloat(items, nil, nil, 6000)
...
function ActivitySportsAchieveItem:updateAchieve()
	if self.achievement_info.achieve_id == 0 or self.achievement_info.achieve_id > self.achieveID_ then
		self.btnAward_:SetActive(false)
		self.imgAwarded_.gameObject:SetActive(true)
	elseif self.achievement_info.value >= self.info_.complete_value and self.achievement_info.achieve_id == self.achieveID_ then
...
function ActivitySportsMissionItem:updateMission()
	...
	if self.info_.value >= self.completeNum then
		-- block empty
	else
		showBtnGo = true
	end
	...
	if self.info_.is_awarded == 1 then
		...
		self.btnGoLabel_.effectColor = Color.New2(4294967295)
	end
