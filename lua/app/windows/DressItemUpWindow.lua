-- chunkname: @../../../Product/Bundles/Android/src/app/windows/DressItemUpWindow.lua
...
function DressItemUpWindow:setShowMaxIntemIndex(idIndex)
	if not self.showMaxItemInex or self.showMaxItemInex and idIndex > self.showMaxItemInex then
...
function DressItemUpWindow:updateCost()
	...
	for i, item in pairs(self.general_cost_arr) do
		xyd.setUISpriteAsync(self["costIcon" .. i], nil, xyd.tables.itemTable:getIcon(item[1]), nil, nil, nil)

		self["labelCostRes" .. i].text = tostring(xyd.getRoughDisplayNumber(item[2]))

		if item[2] > xyd.models.backpack:getItemNumByID(item[1]) then
			self["labelCostRes" .. i].color = Color.New2(3422556671)
		else
...
function DressItemUpWindow:updateShowItem()
	...
	if curRgihtIndex >= self.showMaxItemInex then
...
function DressItemUpWindow:updateFragment()
	...
	for i in pairs(self.fragment_cost_arr) do
		...
		if params.choice_num > params.item_num then
...
function DressItemUpWindow:onTouchBtn()
	...
	for i in pairs(cost_items) do
		if cost_items[i][2] > xyd.models.backpack:getItemNumByID(cost_items[i][1]) then
...
function FreeItem:updateNumText(leftNum)
	...
	if leftNum < self.item_num then
		self.numText.color = Color.New2(4278124287)
	else
		self.numText.color = Color.New2(2986279167)
	end
end
