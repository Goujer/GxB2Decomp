-- chunkname: @../../../Product/Bundles/Android/src/app/windows/SummonWindow.lua
...
function SummonWindow:checkSummonGiftBagOpen()
	...
	if summonGiftData and summonGiftData:getEndTime() > xyd.getServerTime() then
		return true
	else
		return false
	end
...
function SummonWindow:playOpenAnimation(callback)
	...
	if wishData and wishData:getEndTime() > xyd.getServerTime() and self.middleConType == MIDDLETYPE.WISH and xyd.GuideController.get():isGuideComplete() then
...
function SummonWindow:onLimitSummonTen()
	...
	if limitTenScrollNum >= cost[2] then
...
function SummonWindow:initWishGroup(page)
	...
	if xyd.models.activity:getActivity(xyd.ActivityID.WISH_CAPSULE):getUpdateTime() > xyd.getServerTime() then
