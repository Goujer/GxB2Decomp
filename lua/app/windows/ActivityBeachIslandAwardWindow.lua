-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityBeachIslandAwardWindow.lua
...
function ActivityBeachIslandAwardWindow:initItemList()
	...
	table.sort(itemInfos, function(a, b)
		...
		valueA = valueA + xyd.checkCondition(a.star >= a.complete_point, -10, 0)
		valueB = valueB + xyd.checkCondition(b.star >= b.complete_point, -10, 0)
...
function BeachStarAwardItem:layout()
	...
	if self.info_.is_awarded and tonumber(self.info_.is_awarded) == 1 then
		...
		xyd.setUISpriteAsync(self.awardImg_, nil, "mission_awarded_" .. xyd.Global.lang, nil, nil, true)
	else
		...
		if complete_point <= self.info_.star then
			...
			self.awardBtnLabel_.color = Color.New2(4294967295)
			...
			self.awardBtnLabel_.effectColor = Color.New2(4294967295)
		end
	end
end
...
function BeachStarAwardItem:onClickAward()
	...
	if self.info_.star >= self.info_.complete_point then
