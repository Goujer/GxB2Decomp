-- chunkname: @../../../Product/Bundles/Android/src/app/windows/ActivityResidentReturnMainWindow.lua
...
function ActivityResidentReturnMainWindow:layout()
	xyd.setUISpriteAsync(self.logoImg, nil, "resident_return_main_logo_" .. xyd.Global.lang, nil, nil)
...
function ActivityResidentReturnMainWindow:registerEvent()
	for i = 1, 5 do
		UIEventListener.Get(self["clickCon" .. i].gameObject).onClick = handler(self, function()
			...
			if returnData and returnData:getReturnStartTime() ~= -1 and returnData:getReturnEndTime() ~= -1 and xyd.getServerTime() >= returnData:getReturnStartTime() and xyd.getServerTime() < returnData:getReturnStartTime() + xyd.tables.miscTable:getNumber("activity_return2_time" .. i, "value") then
				-- block empty
			else
