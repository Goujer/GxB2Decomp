-- chunkname: @../../../Product/Bundles/Android/src/app/windows/BackpackWindow.lua
	...
	if type_ == xyd.ItemType.ARTIFACT_DEBRIS or type_ == xyd.ItemType.DRESS_DEBRIS then
		...
		if type_ == xyd.ItemType.SOUL_EQUIP1 or type_ == xyd.ItemType.SOUL_EQUIP2_POS1 or type_ == xyd.ItemType.SOUL_EQUIP2_POS2 or type_ == xyd.ItemType.SOUL_EQUIP2_POS3 or type_ == xyd.ItemType.SOUL_EQUIP2_POS4 then
			...
			function param.callback()
				...
				function params1.upArrowCallback()
		...
		if overItem and (not alertTime or not xyd.isSameDay(alertTime, xyd.getServerTime())) then
			...
			for i = 1, #itemList do
				self:waitForTime(0.1 + 2 * (i - 1), function()
					xyd.alertTips(__("BAG_NUM_LIMIT_TEXT2", xyd.tables.itemTable:getName(itemList[i])), nil, nil, nil, nil, nil, nil, nil, nil, nil, 2)
				end)
			end
...
function BackpackWindow:onUseOptionalGiftBox(event)
	if event.data ~= nil then
		...
		elseif itemData.item_id and (xyd.tables.itemTable:getType(itemData.item_id) == xyd.ItemType.SOUL_EQUIP1 or xyd.tables.itemTable:getType(itemData.item_id) == xyd.ItemType.SOUL_EQUIP2_POS1 or xyd.tables.itemTable:getType(itemData.item_id) == xyd.ItemType.SOUL_EQUIP2_POS2 or xyd.tables.itemTable:getType(itemData.item_id) == xyd.ItemType.SOUL_EQUIP2_POS3 or xyd.tables.itemTable:getType(itemData.item_id) == xyd.ItemType.SOUL_EQUIP2_POS4) then
			-- block empty
		else
			xyd.alertItems(item)
		end
...
function BackpackWindow:onItemChange(event)
	...
	flags[xyd.BackpackShowType.EQUIP] = false
	flags[xyd.BackpackShowType.ITEM] = false
	flags[xyd.BackpackShowType.ARTIFACT] = false
	flags[xyd.BackpackShowType.CONSUMABLES] = false
...
function BackpackWindow:sortItem(data)
	table.sort(data, function(a, b)
		...
		if bQlt < aQlt then
			return true
		elseif aQlt == bQlt and a.itemID > b.itemID then
			return true
		else
			return false
		end
