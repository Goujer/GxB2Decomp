-- chunkname: @../../../Product/Bundles/Android/src/app/windows/OldSchoolShopWindow.lua
...
function OldSchoolShopWindow:onGetShopInfo(evt)
	...
	tabText[1] = __("OLD_SCHOOL_SHOP_TEXT1")
	tabText[2] = __("OLD_SCHOOL_SHOP_TEXT2")
...
function OldSchoolShopGroupItem:setInfo(params)
	...
	if params.group == 1 then
		...
		isLock = point > tonumber(xyd.models.oldSchool:getAllInfo().score)
		...
		if hisScores then
			for _, score in pairs(hisScores) do
				if tonumber(score) >= params.point then
					hasNum = hasNum + 1
				end
			end
		end

		if curScores >= params.point then
			hasNum = hasNum + 1
		end
...
function OldSchoolShopItem:onTouch()
	if self.limit > 0 and self.buyTimes_ and self.buyTimes_ >= self.limit then
		return
	end
	...
	if self.limit - self.buyTimes_ == 1 then
		...
		local params = {
			...
			titleWords = __("ITEM_BUY_WINDOW", xyd.tables.itemTable:getName(self.item_[1])),
			limitNum = self.limit - self.buyTimes_,
			eventType = xyd.event.OLD_BUILDING_GET_SHOP_AWARD
		}
...
function OldSchoolShopItem:updateShaddow()
	...
	if self.group == 1 then
		...
		if hisScores then
			for _, score in pairs(hisScores) do
				if tonumber(score) >= self.needPoint_ then
					hasNum = hasNum + 1
				end
			end
		end

		if curScores >= self.needPoint_ then
			hasNum = hasNum + 1
		end
