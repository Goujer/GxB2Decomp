-- chunkname: @../../../Product/Bundles/Android/src/app/windows/GambleWindow.lua
...
function GambleWindow:updateLayout()
	...
	if self.curWindowType_ == windowType.NORMAL then
		...
		xyd.setUISpriteAsync(self.imgBg_, nil, "gb_normal_bg_2", nil, nil)
		...
		xyd.setUISpriteAsync(self.imgMidBg_, nil, "gb_normal_mid_bg", nil, nil)
		xyd.setUISpriteAsync(self.tenIcon_, nil, "icon_35", nil, nil)
		xyd.setUISpriteAsync(self.oneIcon_, nil, "icon_35", nil, nil)
		xyd.setUISpriteAsync(self.btnRecords_, nil, "gb_normal_records", nil, nil)
		...
		xyd.setUISpriteAsync(self.imgBg_, nil, "gb_senior_bg_2", nil, nil)
		xyd.setUITextureAsync(self.windowBg_, "Textures/scenes_web/gb_senior_bg", function()
			self.imgBg_.gameObject:SetActive(false)
		end, false)
		xyd.setUISpriteAsync(self.imgMidBg_, nil, "gb_senior_mid_bg", nil, nil)
		xyd.setUISpriteAsync(self.tenIcon_, nil, "icon_29", nil, nil)
		xyd.setUISpriteAsync(self.oneIcon_, nil, "icon_29", nil, nil)
		xyd.setUISpriteAsync(self.btnRecords_, nil, "gb_senior_records", nil, nil)
...
function GambleWindow:initIcon(go, idx, itemData)
	...
	if itemData.buy_limit ~= 0 and itemData.buy_times >= itemData.buy_limit then
...
function GambleWindow:initButton()
	...
	if not isFree then
		...
		xyd.setUISpriteAsync(self.refreshBtnCostIcon_, nil, "icon_" .. cost[1], nil, nil)
...
function GambleWindow:oneTouch()
	...
	if xyd.models.backpack:getItemNumByID(cost1[1]) >= cost1[2] then
...
function GambleWindow:tenTouch()
	...
	elseif xyd.models.backpack:getItemNumByID(cost10[1]) >= cost10[2] then
		local index
		...
		if self.ifShowBtnGo and self.curWindowType_ == xyd.GambleWindowType.NORMAL and self.sideType > 1 then
			...
			if selfLev >= needLev[tmpIndex] or selfVip >= needVip[tmpIndex] then
				index = tmpIndex
			end
		end
...
function GambleWindow:initNavState()
	...
	if self.curWindowType_ == windowType.NORMAL then
		xyd.setUISpriteAsync(self.navNormal_, nil, "nav_btn_blue_left", nil, nil)

		self.navNormalLabel_.color = Color.New2(4294967295)
		...
		if self:checkSeniorEnable() then
			xyd.setUISpriteAsync(self.navSenior_, nil, "nav_btn_white_right", nil, nil)
			self.navSeniorLabel_:X(0)

			self.navSeniorLabel_.effectStyle = UILabel.Effect.None
		else
			xyd.setUISpriteAsync(self.navSenior_, nil, "nav_btn_grey_right", nil, nil)

			self.navSeniorLabel_.effectStyle = UILabel.Effect.Outline
			self.navSeniorLabel_.effectColor = Color.New2(4294967295)
		...
		self.navSeniorLabel_.color = Color.New2(4294967295)
		...
		xyd.setUISpriteAsync(self.navNormal_, nil, "nav_btn_white_left", nil, nil)
		xyd.setUISpriteAsync(self.navSenior_, nil, "nav_btn_blue_right", nil, nil)
	end
end
...
function GambleWindow:checkSeniorEnable()
	...
	if selfVip >= vip[1] or openValue <= selfLev then
		return true
	else
		return false
	end
