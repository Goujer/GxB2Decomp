-- chunkname: @../../../Product/Bundles/Android/src/app/models/TimeCloisterModel.lua
...
function TimeCloisterModel:getBattleEnergyAndTime()
	...
	if real >= energyMax[2] then
		return real, 0
	elseif real + delta >= energyMax[2] then
...
function TimeCloisterModel:checkBattleRedStateAfterTwo(cloister)
	...
	if cloisterInfo[afterTwoCloister] == nil or xyd.tables.timeCloisterTable:getLockType(afterTwoCloister) == -1 or cloisterInfo[afterTwoCloister].state == xyd.CloisterState.UN_OPEN then
		-- block empty
