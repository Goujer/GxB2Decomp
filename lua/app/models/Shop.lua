-- chunkname: @../../../Product/Bundles/Android/src/app/models/Shop.lua
...
function Shop:ctor()
	...
	self.redPoint_ = {}
	self.TableClase_ = {}
	self.TableClase_[xyd.ShopType.SHOP_HERO] = xyd.tables.shopHeroTable
...
function Shop:onRefreshShopInfo(event)
	...
	if not self:judgeHasUnchangeItem(shopType) then
		...
		while i <= #refresh_item do
			if shopType == xyd.ShopType.SHOP_HERO and xyd.tables.shopHeroTable:checkUnchangeByItemID(refresh_item[i].item[1]) then
				-- block empty
			else
