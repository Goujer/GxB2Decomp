-- chunkname: @../../../Product/Bundles/Android/src/app/models/Map.lua
...
function MapsModel:onMapsInfo(event)
	...
	mapInfo.map_type = params.map_type

	local updateMainActivityEnter = false

	if self:getMapInfo(xyd.MapType.CAMPAIGN) and self:getMapInfo(xyd.MapType.CAMPAIGN).max_stage and params.max_stage and params.max_stage > self:getMapInfo(xyd.MapType.CAMPAIGN).max_stage then
		...
		for id in pairs(partners) do
			if xyd.tableContains(selectedList, tonumber(id)) then
				if needNum > #selects then
					...
					if partners[id]:getPower() > selects[1]:getPower() then
...
function MapsModel:onMapRank(event)
	...
	infos.rank = params.rank
	infos.score = params.score
	infos.map_type = params.map_type
	infos.list = {}
...
function MapsModel:onSoulLandMapRank(event)
	...
	infos.rank = params.rank
	infos.score = params.score
	infos.map_type = xyd.MapType.SOUL_LAND
	infos.list = {}
...
function MapsModel:onFriendRank(event)
	...
	for i = 1, #params.list do
		data.list[i] = {}
		data.list[i].mapType = xyd.MapType.FRIEND_RANK
		data.list[i].rank = i
		data.list[i].score = params.list[i].score
		data.list[i].player_name = params.list[i].player_name
		data.list[i].avatar_id = params.list[i].avatar_id
		data.list[i].avatar_frame_id = params.list[i].avatar_frame_id
		data.list[i].lev = params.list[i].lev
		data.list[i].player_id = params.list[i].player_id
	end
