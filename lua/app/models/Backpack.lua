-- chunkname: @../../../Product/Bundles/Android/src/app/models/Backpack.lua
...
function Backpack:updateItems(items)
	...
	for i = 1, #items do
		...
		if ItemTable:getQuality(item.item_id) == xyd.QualityColor.GREEN and ItemTable:getType(item.item_id) >= xyd.ItemType.WEAPON and ItemTable:getType(item.item_id) <= xyd.ItemType.SHOES then
...
function Backpack:checkCollectionShopRed(isSetting)
	...
	if hasLev and xyd.getServerTime() > tonumber(mondayTime) then
		timeRed = true
	end
...
function Backpack:getVipLev()
	...
	if vipExp > self:getMaxVipExp() then
...
function Backpack:checkCanCompose(type)
	...
	for i in pairs(datas) do
		...
		if not canCompose then
			...
			if cost and cost[2] and itemNum >= cost[2] then
				canCompose = true
	...
	for i in pairs(datas) do
		...
		if ItemTable:isShowInBag(itemID) then
			...
			elseif itemType == xyd.ItemType.HERO_DEBRIS then
				...
				if qlt >= 5 and itemNum >= cost[2] then
					canCompose = true
				end
			end
		end
