-- chunkname: @../../../Product/Bundles/Android/src/app/models/Shrine.lua
...
function Shrine:sortAchievement()
	...
	table.sort(self.achievements, function(a, b)
		...
		local not_completed_a = aValue > a.value and 1 or 0
		local not_completed_b = bValue > b.value and 1 or 0
...
function Shrine:updateChimeRedPoint()
	...
	for chimeID, info in pairs(self.chimeInfo) do
		...
		if info.lev < 0 and xyd.models.backpack:getItemNumByID(unlockCost[1]) >= unlockCost[2] then
			flag = true
