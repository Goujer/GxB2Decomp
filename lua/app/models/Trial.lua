-- chunkname: @../../../Product/Bundles/Android/src/app/models/Trial.lua
...
function Trial:checkClose()
	...
	if xyd.getServerTime() >= tonumber(timeOpen) then
		return false
	end

	if xyd.getServerTime() >= tonumber(time1) then
		return true
	elseif xyd.getServerTime() >= tonumber(time2) and (not self.data_.is_open or self.data_.is_open == 0) then
		return true
	end
...
function Trial:checkIsOpen()
	if self.data_.is_open == 1 and xyd.getServerTime() < self.data_.end_time then
		return true
	elseif self.data_.is_open == 0 and xyd.getServerTime() >= self.data_.start_time then
		return true
	end
