-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/GuildNewWarData.lua
...
function GuildNewWarData:getCurPeriod()
	...
	elseif timePass2 >= xyd.DAY_TIME and timePass2 < xyd.DAY_TIME + 3600 then
	...
	function callback()
		...
		if self.leftFightTime > 0 and nowBraveHP > 0 then
			XYDCo.WaitForTime(0.3, function()
				self:reqFight(nodeID, flagID, nil, nil, callback)
...
function GuildNewWarData:getVsTotalData()
	...
	data.curSeason = self:getCurSeason()
	data.baseInfo[1] = {
...
function GuildNewWarData:getNodeDetailData(nodeIndex)
	...
	data.baseInfo = {}
	data.list = self.mapInfo[nodeIndex].flagInfos
...
function GuildNewWarData:canJoinGuildNewWar()
	...
	if guildLevel >= helpArr[1] and memberNum >= helpArr[3] then
		return true
	end
