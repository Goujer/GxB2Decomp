-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/ActivityEntranceTestData.lua
...
function ActivityEntranceTestData:updateCharterData()
	...
	for i = 1, #ids do
		...
		if xyd.tables.activityWarmupArenaTaskAwardTable:getPlotID(id) ~= 0 and self:getTotalEnergy() >= xyd.tables.activityWarmupArenaTaskAwardTable:getEnergy(id) then
			starNum = starNum + 1
...
function ActivityEntranceTestData:getCurrentPlotID()
	...
	for i = 1, #ids do
		...
		if xyd.tables.activityWarmupArenaTaskAwardTable:getPlotID(id) ~= 0 and self:getTotalEnergy() >= xyd.tables.activityWarmupArenaTaskAwardTable:getEnergy(id) then
			...
			if self:getCharterStar() > self:getCharterState() then
...
function ActivityEntranceTestData:getCharterStar()
	if xyd.db.misc:getValue("entrance_test_charter_star") == nil or type(json.decode(xyd.db.misc:getValue("entrance_test_charter_star"))) == "number" then
		...
		if timeStamp == nil or timeStamp < self.start_time or timeStamp > self.end_time then
			local starinfo = {
				starNum = 0,
				timeStamp = 0
			}
...
function ActivityEntranceTestData:battleFinish(isWin)
	...
	for i = 1, #ids do
		...
		if rank >= xyd.tables.activityWarmupArenaTaskTable:getRank(id) then
...
function ActivityEntranceTestData:checkRedMaskOfTaskAward()
	...
	for i = 1, #ids do
		...
		if self:getTotalEnergy() >= xyd.tables.activityWarmupArenaTaskAwardTable:getEnergy(id) and (self.detail.mission_info.awards[id] == 0 or self.detail.mission_info.ex_awards[id] == 0 and self:IfBuyGiftBag() == true) then
			red = true
		end
	end
...
function ActivityEntranceTestData:checkRedMaskOfTask()
	...
	for i = 1, #ids do
		...
		if self.detail.mission_info.mission_values[id] + self.fakeComplete[id] >= tonumber(xyd.tables.activityWarmupArenaTaskTable:getComplete(id)) and self.detail.mission_info.awards[id] == 0 then
			...
			for j = 1, #ids1 do
				...
				if self:getTotalEnergy() >= xyd.tables.activityWarmupArenaTaskAwardTable:getEnergy(id1) and (self.detail.mission_info.awards[id1] == 0 or self.detail.mission_info.ex_awards[id1] == 0 and self:IfBuyGiftBag() == true) then
					red = true
				end
			end
		end
...
function ActivityEntranceTestData:addBossHarm(bossId, harm)
	...
	if harm > self.detail.harms[bossId] then
	...
	if self.detail.total_harms[bossId] then
		...
		if bossTotalScore > self.detail.total_harms[bossId] and bossTotalScore <= self.detail.total_harms[bossId] + harm then
			isFirstPass = true
		end

		if bossTotalScore > self.detail.total_harms[bossId] then
