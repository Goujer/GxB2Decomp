-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/Activity4BirthdayMusicData.lua
...
function Activity4BirthdayMusicData:getRedMarkState()
	...
	if not clickShowViewBtnTime then
		...
		if clickShowViewBtnTime < self:startTime() or clickShowViewBtnTime > self:getEndTime() then
			xyd.models.redMark:setMark(xyd.RedMarkType.ACTIVITY_4BIRTHDAY_MUSIC, true)
	...
	if self.checkBackpackItemNum >= self.checkItemNeedNum then
		xyd.models.redMark:setMark(xyd.RedMarkType.ACTIVITY_4BIRTHDAY_MUSIC, true)
...
function Activity4BirthdayMusicData:getChoiceAwards()
	...
	if dataGetChoiceYesAwards then
		...
		if not dataGetChoiceYesAwards.saveTime then
			...
			if saveTime < self:startTime() or saveTime > self:getEndTime() then
				dataGetChoiceYesAwards = nil
			end
		end
		...
		if dataGetChoiceYesAwards and dataGetChoiceYesAwards.choiceIds then
			...
			if not (num >= 10) or self.detail.awards and #self.detail.awards ~= 0 then
				-- block empty
			else
				dataGetChoiceYesAwards = nil
			end
	...
	if not dataGetChoiceYesAwards then
		-- block empty
	else
...
function Activity4BirthdayMusicData:updateEmptyData()
	...
	self.dataGetChoiceYesAwards.choiceIds = {}
	...
	if self.detail.awards and #self.detail.awards ~= 0 then
		...
		for i, infos in pairs(self.dataGetChoiceYesAwards.choiceIds) do
			for j, info in pairs(infos) do
				if not (info.sort > 0) or not (info.index > 0) or not info.isGet or info.isGet ~= 1 or checkIsSame[tostring(i) .. info.sort .. info.index] == 1 then
					-- block empty
				else
			...
			for j, info in pairs(infos) do
				if info.sort > 0 and info.index > 0 then
					if checkIsSame[tostring(i) .. info.sort .. info.index] == 1 then
						if info.isGet and info.isGet == 1 then
							-- block empty
						else
							info.sort = 0
							info.index = 0
