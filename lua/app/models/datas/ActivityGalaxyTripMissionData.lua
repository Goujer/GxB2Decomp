-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/ActivityGalaxyTripMissionData.lua
...
function ActivityGalaxyTripMissionData:checkRedMaskOfAward()
	...
	for i = 1, #ids do
		...
		if self:getTotalEnergy() >= awardTalbe:getPointLimit(id) and (not self:getFreeAwardAwarded(id) or not self:getPaidAwardAwarded(id) and self:IfBuyGiftBag() == true) then
			red = true
		end
	end
...
function ActivityGalaxyTripMissionData:onGetAward(event)
	...
	for i = 1, #ids do
		...
		if self:getTotalEnergy() >= awardTalbe:getPointLimit(id) then
