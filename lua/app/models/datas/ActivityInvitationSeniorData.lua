-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/ActivityInvitationSeniorData.lua
...
function ActivityInvitationSeniorData:checkRedPoint()
	if self:getState() ~= xyd.ActivityInvitationSeniorState.NEW or self:getInviter() and self:getInviter() ~= 0 then
		-- block empty
	else
	...
	if self:getState() == xyd.ActivityInvitationSeniorState.OLD then
		...
		for i, id in pairs(ids) do
			...
			if not (inviter_completes >= params[1]) or inviter_awarded[id] and inviter_awarded[id] == 1 then
				-- block empty
			else
				xyd.models.redMark:setMark(xyd.RedMarkType.ACTIVITY_INVITATION_SENIOR, true)
