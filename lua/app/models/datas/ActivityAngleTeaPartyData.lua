-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/ActivityAngleTeaPartyData.lua
...
function ActivityAngleTeaPartyData:checkRedPoint_cup(id)
	...
	if (self.detail.point >= self.needPoints[id] or self.detail.charges[1].buy_times > 0) and id > self:getNowCharterState() then
		return true
	end
...
function ActivityAngleTeaPartyData:checkRedPoint_award(id)
	...
	if (self.detail.point >= self.needPoints[id] or self.detail.charges[1].buy_times > 0) and self.detail.awarded[id] ~= 1 and id <= self:getNowCharterState() then
		return true
	end
...
function ActivityAngleTeaPartyData:checkRedPoint_goto()
	...
	if self.detail.point >= self.needPoints[4] or self.detail.charges[1].buy_times > 0 then
		return false
	end
