-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/ActivityJackpotLotteryData.lua
...
function ActivityJackpotLotteryData:onAward(data)
	...
	elseif detail.award_type ~= 3 then
		...
		if detail.items and #detail.items > 0 then
			...
			num = detail.num == 1 and 1 or num > 10 and 10 or num
...
function ActivityJackpotLotteryData:getRedMarkState()
	...
	if xyd.models.backpack:getItemNumByID(upCostOne[1]) >= upCostOne[2] or xyd.models.backpack:getItemNumByID(downCostOne[1]) >= downCostOne[2] then
		self.defRedMark = true
	end
