-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/ActivityFreeBuyData.lua
...
function ActivityFreeBuyData:isOpen()
	...
	if not hasBuy and xyd.getServerTime() > self:getShowEndTime() then
		return false
	end
	...
	if hasBuy and self:checkHasAllawarded() and xyd.getServerTime() > self:getShowEndTime() then
		return false
	end
...
function ActivityFreeBuyData:checkCanBuy(index)
	if xyd.getServerTime() > self:getShowEndTime() then
		return false
	end
