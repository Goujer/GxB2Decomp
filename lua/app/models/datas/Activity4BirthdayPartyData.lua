-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/Activity4BirthdayPartyData.lua
...
function Activity4BirthdayPartyData:checkRedMarkOfPartyBtnUse(type)
	...
	if self:getPartyPoint(type) < self:getPartyNeedPoint(type) and xyd.models.backpack:getItemNumByID(cost[1]) >= cost[2] then
		return true
	end
...
function Activity4BirthdayPartyData:checkRedMarkOfPartyBuy()
	if self:checkPartySpecialAwardHaveGot() then
		...
		for i = 1, #ids do
			...
			if leftTime > 0 and xyd.models.backpack:getItemNumByID(cost[1]) >= cost[2] then
				return true
			end
		end
	end
