-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/ActivityJungleData.lua
...
function ActivityJungleData:getRedMarkState()
	...
	if xyd.models.backpack:getItemNumByID(255) > self.singleCost[2] then
		return true
	end
...
function ActivityJungleData:getExploreProgressRedMarkState(area)
	...
	for i in pairs(ids) do
		if self.detail.awarded[i] == 0 and area == xyd.tables.ActivityJungleAwardsTable:getArea(i) and self.detail.points[area] >= xyd.tables.ActivityJungleAwardsTable:getPoint(i) then
			return true
		end
	end
