-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/ActivityHalloweenData.lua
...
function ActivityHalloweenData:getRedMarkState()
	...
	if not self:isFunctionOnOpen() then
		...
		for i = 1, #awardIDs do
			if self.detail.times >= xyd.tables.activityHalloweenTrickAwardTable:getComplete(i) and self.detail.awards[i] == 0 then
				self.defRedMark = true
			end
		end
		...
		for i = 1, #shopIDs do
			...
			if self.detail.buy_times[i] < xyd.tables.activityHalloweenShopTable:getLimit(i) and xyd.models.backpack:getItemNumByID(cost[1]) >= cost[2] then
				self.defRedMark = true
			end
		end
	end
