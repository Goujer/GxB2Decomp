-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/AllStarsPrayData.lua
...
function AllStarsPrayData:getRedMarkState()
	...
	if xyd.models.backpack:getItemNumByID(self.WishCoinID) >= self.singleCost_wishCoin then
		return true
	end

	for i = 1, #self.gemIDs do
		if xyd.models.backpack:getItemNumByID(self.gemIDs[i]) >= self.singleCosts_gem[i] then
			return true
		end
	end
...
function AllStarsPrayData:checkRedPoint_gem(groupID)
	...
	if xyd.models.backpack:getItemNumByID(self.gemIDs[groupID]) >= self.singleCosts_gem[groupID] then
		return true
	end
...
function AllStarsPrayData:checkRedPoint_task()
	...
	for j in pairs(ids) do
		...
		data.cur_value = tonumber(self.detail.finish_times)

		if data.cur_value > data.max_value then
			data.cur_value = data.max_value
		end

		if self.detail.award_records[j] == 0 and self.detail.finish_times >= data.max_value then
			flag = true
		end
	end
