-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/ActivityFireworkData.lua
	...
	for i in pairs(xyd.tables.activityFireworkShopRankTable:getIDs()) do
		...
		if num <= costNum then
			for j, checkId in pairs(xyd.tables.activityFireworkShopTable:getRanksAward()[i]) do
				...
				if limit > self.detail.buy_times[checkId] and xyd.models.backpack:getItemNumByID(cost[1]) >= cost[2] then
					return true
				end
			end
		end
