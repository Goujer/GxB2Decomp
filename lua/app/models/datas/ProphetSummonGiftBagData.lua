-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/ProphetSummonGiftBagData.lua
...
function ProphetSummonGiftBagData:onProphet(event)
	if xyd.getServerTime() <= self:getEndTime() then
		...
		if self.detail_.point >= ActivityTable:getLaterPoint() and self.detail_.circle_times < xyd.tables.activityTable:getRound(self.id)[2] then
