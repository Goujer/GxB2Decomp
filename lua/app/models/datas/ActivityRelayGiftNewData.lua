-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/ActivityRelayGiftNewData.lua
...
function ActivityRelayGiftNewData:getCurIndex()
	for i = 1, #self.giftBagIDs do
		if self.giftBagIDs[i] <= #self.freeGiftbagIDs then
			if self.detail.awarded_id <= 0 or self.giftBagIDs[i] > self.detail.awarded_id then
				return i
			end
		else
