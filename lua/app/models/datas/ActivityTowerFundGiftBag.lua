-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/ActivityTowerFundGiftBag.lua
...
function ActivityTowerFundGiftBag:getRedMarkState()
	...
	elseif self:isFirstRedMark() then
		...
		if self:isHide() then
			self.defRedMark = false
		elseif not lastLevel or maxLevel > tonumber(lastLevel) then
			self.defRedMark = true
		else
			self.defRedMark = false
		end
		...
		if buyTimes >= #self.detail.charges then
			...
			if curTowerID > math.max(maxTowerID, lastViewTowerID) then
