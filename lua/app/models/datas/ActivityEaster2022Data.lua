-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/ActivityEaster2022Data.lua
...
function ActivityEaster2022Data:checkRedMarkOfRes()
	...
	if xyd.models.backpack:getItemNumByID(res1[1]) >= res1[2] then
		return true
	end

	local res2 = self:getResource2()

	if xyd.models.backpack:getItemNumByID(res2[1]) >= res2[2] then
		return true
	end
...
function ActivityEaster2022Data:checkRedMarkOfProgressAward()
	...
	for j in pairs(ids) do
		...
		data.id = j
		data.max_value = xyd.tables.activityEaster2022AwardsTable:getPoint(j)
		data.cur_value = self:getCurPoint()
