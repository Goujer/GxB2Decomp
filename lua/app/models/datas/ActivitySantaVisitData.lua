-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/ActivitySantaVisitData.lua
		...
		if data.activity_id == xyd.ActivityID.ACTIVITY_SANTA_VISIT then
			...
			if not detail.table_id then
				...
				for i = 1, #award_ids do
					...
					if maxLevel > self.detail.lvs[index] then
...
function ActivitySantaVisitData:getRedPointOfDraw()
	...
	if xyd.models.backpack:getItemNumByID(singleCost[1]) >= singleCost[2] then
		flag = true
	end
...
function ActivitySantaVisitData:getRedPointOfCanGetLevelAward()
	...
	for j in pairs(ids) do
		...
		data.max_value = xyd.tables.activityChristmasSocksLevelTable:getLevel(j)
		data.cur_value = self:getCurLevel()

		if self:getLevelAwardedData()[j] == 0 and data.cur_value >= data.max_value then
			flag = true
		end
	end
