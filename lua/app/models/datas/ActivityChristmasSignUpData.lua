-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/ActivityChristmasSignUpData.lua
...
function ActivityChristmasSignUpData:getRedMarkState()
	...
	if self:getAward(self:getCurDateID()) ~= nil then
		if self:getCurGiftBag() ~= nil and self:getCurGiftBag() ~= 0 then
			if self.detail.charges == nil or self.detail.charges[self:getGiftBagIndex(self:getCurGiftBag())].buy_times >= self.detail.charges[self:getGiftBagIndex(self:getCurGiftBag())].limit_times then
				return false
			else
