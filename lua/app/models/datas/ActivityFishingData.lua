-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/ActivityFishingData.lua
...
function ActivityFishingData:onFishing(event)
	...
	if self.detail.fishs[event.data.id].min > event.data.len or self.detail.fishs[event.data.id].min == 0 then
...
function ActivityFishingData:getAwardRedMark()
	...
	for i = 1, #ids do
		if self.detail.awards[i] == 0 and self.detail.point >= xyd.tables.activityFishingAwardTable:getComplete(ids[i]) then
			return true
		end
	end
