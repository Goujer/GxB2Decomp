-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/ActivityResidentReturnData.lua
...
function ActivityResidentReturnData:ctor(params)
	...
	if self:getReturnStartTime() ~= -1 and self:getReturnEndTime() ~= -1 and xyd.getServerTime() >= self:getReturnStartTime() and xyd.getServerTime() < self:getReturnStartTime() + xyd.tables.miscTable:getNumber("activity_return2_time1", "value") then
...
function ActivityResidentReturnData:getReturnSupportCanResitScore()
	...
	if self.detail.point + CanResitScore > ActivityResidentReturnRewardTable:getTotalPoint() then
...
function ActivityResidentReturnData:setRedMarkState(type)
	...
	if not type or type == xyd.RedMarkType.ACTIVITY_RESIDENT_RETURN_RED_1 then
		...
		if xyd.getServerTime() >= self:getReturnAlongEndTime(1) then
	...
	if not type or type == xyd.RedMarkType.ACTIVITY_RESIDENT_RETURN_RED_2 then
		...
		if xyd.getServerTime() >= self:getReturnAlongEndTime(2) then
	...
	if not type or type == xyd.RedMarkType.ACTIVITY_RESIDENT_RETURN_RED_5 then
		...
		if xyd.getServerTime() >= self:getReturnAlongEndTime(5) then
