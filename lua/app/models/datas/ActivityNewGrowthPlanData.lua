-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/ActivityNewGrowthPlanData.lua
...
function ActivityNewGrowthPlanData:getCanResitScore()
	if self.detail.point >= activityGrowthPlanTable:getTotalPoint() then
		return -1
	end
	...
	if self.detail.point + CanResitScore > activityGrowthPlanTable:getTotalPoint() then
...
function ActivityNewGrowthPlanData:checkRedPointOfGoto()
	if self.detail.point >= activityGrowthPlanTable:getTotalPoint() or self:checkRedPointOfChoosePartner() == true then
		return false
	end
