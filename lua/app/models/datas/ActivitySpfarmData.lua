-- chunkname: @../../../Product/Bundles/Android/src/app/models/datas/ActivitySpfarmData.lua
...
function ActivitySpfarmData:ctor(params)
	...
	if (not viewingEndFiveMinsTips or viewingEndFiveMinsTips and (viewingEndFiveMinsTips < self:startTime() or viewingEndFiveMinsTips >= self:getEndTime())) and xyd.getServerTime() < self:getEndTime() - self:getViewTimeSec() - 300 then
...
function ActivitySpfarmData:checkPartnerRobAllDie()
	...
	for _, info in ipairs(self.detail.partners_rob) do
		if info.status and info.status.hp and tonumber(info.status.hp) == 0 then
			-- block empty
		else
			return false
		end
...
function ActivitySpfarmData:checkCanAddPolicy()
	...
	for i = 1, 3 do
		...
		if (not level or level < 1) and xyd.models.backpack:getItemNumByID(cost[1]) >= cost[2] then
			canAdd = true
...
function ActivitySpfarmData:isGridAllEmpty()
	...
	for i = 2, #robBuilds do
		...
		if not outCome or not (#outCome > 0) or robBuilds[i].is_rob and robBuilds[i].is_rob == 1 then
			-- block empty
		else
			return false
		end
