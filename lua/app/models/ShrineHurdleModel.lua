-- chunkname: @../../../Product/Bundles/Android/src/app/models/ShrineHurdleModel.lua
...
function ShrineHurdleModel:onSelectRt(event)
	...
	if self.partnerInfos_ and #self.partnerInfos_ > 0 then
		...
		table.sort(self.powerPartnerList_, function(a, b)
			...
			return partnerInfoA.power > partnerInfoB.power
		end)
	end
...
function ShrineHurdleModel:getGuidePartnerList()
	...
	for j = 1, #partners do
		...
		if partner and partner:getStar() >= 10 then
			...
			partnerInfo.status = {}
			partnerInfo.status.hp = 100
			partnerInfo.status.mp = 0
...
function ShrineHurdleModel:fakeBattle(type, partnerParams, pet)
	...
	params2.event_data = {}
	params2.battle_type = xyd.BattleType.SHRINE_HURDLE
