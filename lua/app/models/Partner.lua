-- chunkname: @../../../Product/Bundles/Android/src/app/models/Partner.lua
...
function Partner:populate(params)
	...
	if params.isUpdateAttrs and params.isUpdateAttrs == false or params.noUpdateAttrs then
		-- block empty
	else
		self:updateAttrs()
	end
...
function Partner:updateAttrs(params)
	...
	local fullStarOrigin = false

	if params and params.fullStarOrigin then
		fullStarOrigin = true
	elseif self.fullStarOrigin then
		fullStarOrigin = self.fullStarOrigin
	end
...
function Partner:getAwakeMaterial()
	if self.awake >= self:getMaxAwake() then
...
function Partner:getLockType()
	...
	for i = 1, xyd.PartnerFlag.LOCK_NUM do
		if not lockFlags[i] or lockFlags[i] == 0 then
			-- block empty
		else
			return i
		end
