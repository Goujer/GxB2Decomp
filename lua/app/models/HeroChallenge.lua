-- chunkname: @../../../Product/Bundles/Android/src/app/models/HeroChallenge.lua
...
function HeroChallenge:onPartnerChallengeChessGetInfo(event)
	self.data_chess = {}
	self.data_chess.map_info = {}
	self.data_chess.map_list = {}
...
function HeroChallenge:checkNeedShowRed()
	...
	for id in pairs(newIds) do
		if xyd.arrayIndexOf(touchIds, tonumber(id)) < 0 then
			...
			if xyd.models.activity:getActivity(activity_id) then
				-- block empty
			else
				return true
			end
