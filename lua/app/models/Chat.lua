-- chunkname: @../../../Product/Bundles/Android/src/app/models/Chat.lua
...
function Chat:getGMMsgByEventData(data)
	...
	newData.talker_id = data.talker_id
	newData.talker_name = data.talker_name
	newData.created_time = data.created_time
	newData.id = data.id
	newData.is_talk_over = data.is_talk_over
	newData.content = data.content
	newData.player_id = data.player_id
	newData.is_gm_online = data.is_gm_online
	newData.msg_format = data.msg_format
	newData.type = xyd.MsgType.GM
	newData.hashCode = self:getMsgID()
	newData.is_filter = false
...
function Chat:addMsg(data)
	...
	if #msgs > xyd.MAX_CHAT_MSG_NUM then
		-- block empty
	end
...
function Chat:checkEmoLegal(str)
	if string.sub(str, 1, 8) == "#emotion" then
		...
		if num and tonumber(num) >= 1 and all >= tonumber(num) then
			return tonumber(num)
		end
	end
...
function Chat:checkGifLegal(str)
	if string.sub(str, 1, 4) == "#gif" then
		...
		if num and tonumber(num) >= 1 and all >= tonumber(num) and xyd.tables.emotionGifTable:getIsShow(tonumber(num)) then
			return tonumber(num)
		end
	end
...
function Chat:checkUpdateUpLoadTimes()
	...
	if oldTb.hour >= self.DailyUpdateHour then
