-- chunkname: @../../../Product/Bundles/Android/src/app/models/Slot.lua
...
function Slot:onSendGifts(event)
	...
	if not self.maxLovePoints[key] or data.love_point > self.maxLovePoints[key] then
...
function Slot:isRequireMaxLovePoint(tableID, partner)
	...
	if self.maxLovePoints[key] or self.maxLovePoints[key] == 0 then
		if partner.love_point > self.maxLovePoints[key] then
			return true
		end
...
function Slot:getMaxLovePointSuper(tableID)
	...
	if not point then
		local list = self.getListByTableID(self, tableID)
		local maxPoint = 0
...
function Slot:checkPromotablePartner()
	...
	for i = 1, partnerNum do
		...
		if lev < max_lev and mana > cost[xyd.ItemID.MANA] and exp > cost[xyd.ItemID.PARTNER_EXP] then
...
function Slot:refreshShenxueForgeStatus()
	...
	for group in pairs(self.forgeList) do
		...
		for i = #tableIDs, 1, -1 do
			if xyd.tables.partnerTable:getShowInForge(tableIDs[i]) > xyd.getServerTime() then
				table.remove(self.forgeList[group], i)
			end
		end
	end
	...
	if data.sets then
		for index, sets in pairs(data.sets) do
			...
			if set and set[1] and set[1] ~= "-1" then
				...
				combination.id = tonumber(index)
				combination.name = set[6]
				combination.equipIDs = {}
