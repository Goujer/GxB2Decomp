-- chunkname: @../../../Product/Bundles/Android/src/app/models/SelfPlayer.lua
...
function SelfPlayer:updateGbobalTimer()
	for i in ipairs(self.globalTimerCallBack) do
		if self.globalTimerCallBack ~= nil and self.globalTimerCallBack[i] ~= nil then
			...
			if self.globalTimerCallBack[i].time_yet >= self.globalTimerCallBack[i].timeDis then
				...
				if self.globalTimerCallBack[i] ~= nil and keyId == self.globalTimerCallBack[i].keyid then
					...
					if self.globalTimerCallBack[i].alltimes ~= -1 then
						...
						if self.globalTimerCallBack[i].already_Times >= self.globalTimerCallBack[i].alltimes then
...
function SelfPlayer:checkPlayerBaned()
	...
	if not self.banRoomInfo_.is_banned or self.banRoomInfo_.is_banned == 0 then
		return false
	elseif self.banRoomInfo_.ban_until_time == -1 or self.banRoomInfo_.ban_until_time > xyd.getServerTime() then
		return true
	end
