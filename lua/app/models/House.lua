-- chunkname: @../../../Product/Bundles/Android/src/app/models/House.lua
...
function House:isHangMaxTime()
	...
	if hangTime > 0 then
		...
		if trueMaxHangTime < serverTime then
			flag = true
		end
	end
...
function House:checkCanOpenFloor()
	...
	if not values[nextFloor] or comfortNum >= values[nextFloor] then
		return true
	end
