-- chunkname: @../../../Product/Bundles/Android/src/app/models/ArenaAllServerScore.lua
...
function ArenaAllServerScore:isInOpentime()
	...
	if serverTime >= self:getOpenTime() and serverTime <= self:getDDL() then
		return true
	else
		return false
	end
...
function ArenaAllServerScore:onAreanAsFight(event)
	if event and event.data then
		if event.data.is_win and tonumber(event.data.is_win) == -1 then
			-- block empty
		else
...
function ArenaAllServerScore:reqEnemyList()
	if xyd.getServerTime() - self:getRefreshTime() > xyd.HOUR or self.needGetList_ then
...
function ArenaAllServerScore:updateDefendRed()
	...
	if self.defFormation and self.defFormation.partners and #self.defFormation.partners > 0 and self:getDDL() - xyd.getServerTime() < xyd.DAY_TIME * 3 and self:getDDL() - xyd.getServerTime() > xyd.DAY_TIME then
