-- chunkname: @../../../Product/Bundles/Android/src/app/models/Activity.lua
...
function Activity:onActivityList(event)
	...
	if UNITY_EDITOR then
		...
		for i, v in pairs(a.activity_list) do
			if tonumber(v.activity_id) == 350 then
				dump(v, "350===========================")
			end
		end
	...
	for i, data in ipairs(list) do
		...
		if xyd.GuideController.get():isGuideComplete() or data.activity_id == xyd.ActivityID.CHECKIN or xyd.tables.activityTable:getType(data.activity_id) ~= xyd.EventType.LIMIT then
			if id == xyd.ActivityID.ACTIVITY_VOT or id == xyd.ActivityID.ACTIVITY_VOTE2 then
				if self.vote_req_time[id] == -1 or xyd.getServerTime() - 3600 > tonumber(self.vote_req_time[id]) then
...
function Activity:isNewBieCampOpen()
	...
	if xyd.models.selfPlayer:getCreatedTime() > tonumber(timeStamps[2]) or xyd.getServerTime() > tonumber(timeStamps[1]) then
		return false
	end
...
function Activity:isManaCardPurchased()
	...
	return not not activityData and not not activityData:isPurchased()
end
...
function Activity:onExchangeSchoolGiftBox(evt)
	...
	if data then
		...
		xyd.itemFloat(detail.items, nil, nil, 4005)
	end
end
...
function Activity:equipGacha(index)
	...
	if num > xyd.models.backpack:getItemNumByID(xyd.ItemID.EQUIP_GACHA) then
...
function Activity:getIceSecretTime()
	for _, data in pairs(self.activityList) do
		if data.id == xyd.ActivityID.ICE_SECRET then
			...
			if endTime and endTime > xyd.getServerTime() then
...
function Activity:isResidentReturnTimeIn()
	...
	if returnData and returnData:getReturnStartTime() ~= -1 and returnData:getReturnEndTime() ~= -1 and xyd.getServerTime() >= returnData:getReturnStartTime() and xyd.getServerTime() < returnData:getReturnEndTime() then
		return true
	end
...
function Activity:isResidentReturnAddTime()
	...
	if returnData and returnData:getReturnStartTime() ~= -1 and returnData:getReturnEndTime() ~= -1 and xyd.getServerTime() >= returnData:getReturnStartTime() and xyd.getServerTime() < returnData:getReturnStartTime() + xyd.tables.miscTable:getNumber("activity_return2_time1", "value") then
		return true
	end
