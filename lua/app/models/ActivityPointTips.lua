-- chunkname: @../../../Product/Bundles/Android/src/app/models/ActivityPointTips.lua
...
function ActivityPointTips:updateData8(event)
	...
	if event.data.function_id == xyd.FunctionID.ACTIVITY then
		...
		for i = 1, #ids do
			if id <= xyd.tables.achievementTypeTable:getEndAchievement(tonumber(ids[i])) and id >= xyd.tables.achievementTypeTable:getStartAchievement(tonumber(ids[i])) then
	...
	for aid, _ in pairs(self.data_1) do
		if aid == xyd.ActivityID.ACTIVITY_TREE_GROUP then
			for i = 1, #self.data_1[aid].point do
				if self.data_1[aid].point[i] >= self.data_1[aid].limits[i] then
		...
		elseif self.data_1[aid].circle < self.data_1[aid].limit_circle then
			for i = 1, #self.data_1[aid].limits do
				...
				if limit > self.data_1[aid].basePoint and limit <= self.data_1[aid].point then
					self:addTips(self.data_1[aid].table_id, aid, {
						limit,
						self.data_1[aid].circle + 1
					}, 1)

					if self.data_1[aid].point >= self.data_1[aid].round_point then
...
function ActivityPointTips:checkTips2()
	for aid, _ in pairs(self.data_2) do
		...
		for i = 1, #points do
			if points[i] >= limits[i] and awarded[i] == 0 then
