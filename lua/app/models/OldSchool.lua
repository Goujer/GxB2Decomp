-- chunkname: @../../../Product/Bundles/Android/src/app/models/OldSchool.lua
...
function OldSchool:getSelfScore()
	...
	fight_rank = not fight_rank and 0 or fight_rank + 1
...
function OldSchool:startTimeSend()
	...
	if self.allInfo == nil and firstStartTime > xyd.getServerTime() then
...
function OldSchool:updateAllInfo(isMustUpdate)
	if isMustUpdate == nil and self.allInfo == nil then
		...
		if xyd.models.towerMap.stage and xyd.models.towerMap.stage >= xyd.tables.miscTable:getNumber("old_building_floor_limit", "value") then
			isSend = true
		end
...
function OldSchool:checkOpenState(type)
	...
	if self.isTowerLevel_Open == false and (type == 2 or type == nil) and xyd.models.towerMap.stage and xyd.models.towerMap.stage > xyd.tables.miscTable:getNumber("old_building_floor_limit", "value") then
		self.isTowerLevel_Open = true
	...
	elseif self.allInfo then
		for i in pairs(self.allInfo.awards) do
			if self.allInfo.awards[i] == 0 then
				...
				if score >= self:getOldBuildingAward1Table():getPoint(i) then
...
function OldSchool:isCanOpen(isShowTips)
	...
	if self.allInfo == nil or self.allInfo and (self.allInfo.season_info.start_time > xyd.getServerTime() or xyd.getServerTime() >= self:getShowEndTime()) then
		if isShowTips and isShowTips == true then
...
function OldSchool:isCheckScoreCanGetAward()
	if self:getChallengeEndTime() > xyd.getServerTime() and xyd.getServerTime() > self:getStartTime() then
		for i in pairs(self.allInfo.awards) do
			if self.allInfo.awards[i] == 0 then
				local score = tonumber(self.allInfo.score)
...
function OldSchool:getFloorAwardBack(event)
	...
	xyd.itemFloat(dataInfo.items, nil, nil, 7000)
