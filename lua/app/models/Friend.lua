-- chunkname: @../../../Product/Bundles/Android/src/app/models/Friend.lua
...
function Friend:checkRedPoint(newData)
	...
	if #oldRequest ~= #newRequest and not (#newFriends >= tonumber(xyd.tables.miscTable:getVal("friend_max_num"))) then
...
function Friend:updateMaxCanJumpBattleLevel()
	...
	for i = 1, #self.selfWinCounts do
		if needNum <= self.selfWinCounts[i] and i > self.maxCanJumpBattle then
...
function Friend:canJumpBattle(bossLev)
	if tonumber(bossLev) > self.maxCanJumpBattle then
		return false
	else
		return true
	end
...
function Friend:setClassifiedFriendSharedPartner()
	...
	self.classifiedFriendSharedPartnerList["0"] = {}
