-- chunkname: @../../../Product/Bundles/Android/src/app/models/GameAssistant.lua
...
function GameAssistant:buyMidas(Index)
	...
	if Index == 2 and self:buyMidasTotalCost(self.presetData.midas.paid) > xyd.models.backpack:getItemNumByID(xyd.ItemID.CRYSTAL) then
		return false
	end
...
function GameAssistant:reqDailyQuizAward()
	...
	for i = 1, 3 do
		...
		if data and data.cur_quiz_id and data.cur_quiz_id > 0 and xyd.models.dailyQuiz:checkCanFight(data.cur_quiz_id) then
			...
			if data.fight_times < self.presetData.dailyQuiz.paid[i] and data.limit_times >= self.presetData.dailyQuiz.paid[i] then
...
function GameAssistant:reqBaseSummon(isFree)
	...
	is_base_free = not (baseInterval > nowTime - baseFreeTime)
...
function GameAssistant:reqSeniorSummon(isFree)
	...
	is_senior_free = not (seniorInterval > nowTime - seniorFreeTime)
...
function GameAssistant:reqFriendLove()
	...
	if giftNum < maxGiftNum and maxLove > xyd.models.friend:getLoveNum() and #getIDs > 0 then
...
function GameAssistant:reqArenaBattle()
	...
	if needCheck then
		...
		if battleInfo.power and battleInfo.power / power > tonumber(numSave) then
...
function GameAssistant:reqGamble(time)
	if time > xyd.models.backpack:getItemNumByID(xyd.ItemID.GAMBLE_NORMAL) then
		...
		elseif time == 1 then
			...
			index = selfVip >= needVip[2] and 2 or 3
...
function GameAssistant:completeAllChallengePet(limitTime)
	...
	for i = 1, #ids do
		...
		if tonumber(id) ~= 0 then
			...
			if limitLev <= lev and leftTili > 0 and leftChallengTime > 0 and limitTime > self.petChallengeTime then
				for i = 1, math.min(leftTili, leftChallengTime) do
					if limitTime > self.petChallengeTime then
...
function GameAssistant:buyChallengeTiliPet()
	...
	if costNum > xyd.models.backpack:getItemNumByID(xyd.ItemID.CRYSTAL) then
		return false
	end
...
function GameAssistant:jungeIfCanDoTab1()
	...
	if self.presetData.midas.paid > 0 and self.presetData.midas.paid > self.todayHaveDoneData.midas.paid and self.presetData.midas.paid > xyd.models.midas.buy_times then
	...
	for i = 1, 3 do
		...
		if self.presetData.dailyQuiz.paid[i] > 0 and cur_quiz_id and cur_quiz_id > 0 and self.presetData.dailyQuiz.paid[i] > data.limit_times then
	...
	if self.presetData.summon.normal == 0 and nowTime - baseFreeTime >= xyd.tables.summonTable:getFreeTimeInterval(xyd.SummonType.BASE_FREE) then
	...
	if self.presetData.summon.senior == 0 and nowTime - seniorFreeTime >= xyd.tables.summonTable:getFreeTimeInterval(xyd.SummonType.SENIOR_FREE) then
	...
	if self.presetData.arena ~= true or self.todayHaveDoneData.arena ~= false or xyd.models.arena:getIsSettlementing() then
		-- block empty
...
function GameAssistant:jungeIfCanDoTab2()
	...
	if fort > 0 and self.presetData.academyAssessment.paid > 0 and self.presetData.academyAssessment.paid > self.todayHaveDoneData.academyAssessment.paid and self:getMaxBuyTicketAcademyAssessment() > 0 then
...
function GameAssistant:jungeIfCanDoTab3()
	...
	if self.presetData.explore.bread > self:getBuyTimeBreadExplore() then
...
function GameAssistant:jungeIfCanDoTab4()
	...
	if not (xyd.models.guild.guildID > 0) or not (xyd.models.guild.level >= self.limitLevs[3]) then
		return flag
	end
	...
	if self.presetData.guild.gym ~= true or self.todayHaveDoneData.guild.gym ~= false or not (xyd.models.guild.guildID > 0) or not (leftTime > 0) or os.date("!*t", xyd.getServerTime()).wday == 6 and os.date("!*t", xyd.getServerTime()).hour == 0 then
		-- block empty
	else
